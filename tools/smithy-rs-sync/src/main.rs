...
fn sync_with_latest(smithy_rs: &Path, aws_sdk: &Path, branch: &str) -> Result<()> {
    ...
    println!("Successfully synced {} commits", commit_revs.len());
    ...
    // set_last_synced_commit(aws_sdk, last_synced_commit).context(context).unwrap();
...
fn _set_last_synced_commit(repo_path: &Path, oid: Oid) -> Result<()> {
    let path = repo_path.join(COMMIT_HASH_FILENAME);
    let mut file = OpenOptions::new()
        .write(true)
        .truncate(true)
...
fn checkout_branch_to_sync_to(aws_sdk_repo: &Repository, aws_sdk_branch: &str) -> Result<()> {
    ...
    // TODO don't need to fetch in CI, delete this
    aws_sdk_repo
        .find_remote("origin")?
