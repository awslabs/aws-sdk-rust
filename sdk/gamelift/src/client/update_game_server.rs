...
impl super::Client {
    ...
    ///   - [`utilization_status(GameServerUtilizationStatus)`](crate::operation::update_game_server::builders::UpdateGameServerFluentBuilder::utilization_status) / [`set_utilization_status(Option<GameServerUtilizationStatus>)`](crate::operation::update_game_server::builders::UpdateGameServerFluentBuilder::set_utilization_status): <p>Indicates if the game server is available or is currently hosting gameplay. You can update a game server status from <code>AVAILABLE</code> to <code>UTILIZED</code>, but you can't change a the status from <code>UTILIZED</code> to <code>AVAILABLE</code>.</p>
