            ...
            let mut tmp =
                 {
                    ...
                    let _ = response;
                    output = crate::protocol_serde::shape_tagging_failed_exception::de_tagging_failed_exception_json_err(response.body().as_ref(), output).map_err(crate::error::TagResourceError::unhandled)?;
                    let output = output.meta(generic);
                    output.build()
                }
            ;
