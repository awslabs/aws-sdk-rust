...
///     EncryptionStatus::KmsKeyInaccessible => { /* ... */ },
...
/// match outputformat {
///     OutputFormat::IonBinary => { /* ... */ },
///     OutputFormat::IonText => { /* ... */ },
///     OutputFormat::Json => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
...
/// match errorcause {
///     ErrorCause::IamPermissionRevoked => { /* ... */ },
///     ErrorCause::KinesisStreamNotFound => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `errorcause` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `ErrorCause::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `ErrorCause::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `ErrorCause::NewFeature` is defined.
