            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "ActionThreshold" => {
                            builder = builder.set_action_threshold(crate::protocol_serde::shape_action_threshold::de_action_threshold(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "Definition" => {
                            builder = builder.set_definition(crate::protocol_serde::shape_definition::de_definition(tokens, _value, depth + 1)?);
                        }
                        "ExecutionRoleArn" => {
                        ...
                        "ApprovalModel" => {
                            builder = builder.set_approval_model(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::ApprovalModel::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "Status" => {
