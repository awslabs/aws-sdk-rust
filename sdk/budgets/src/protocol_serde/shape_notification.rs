...
pub(crate) fn de_notification<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::Notification>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "NotificationState" => {
                                builder = builder.set_notification_state(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::NotificationState::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
            ...
            Ok(Some(crate::serde_util::notification_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
