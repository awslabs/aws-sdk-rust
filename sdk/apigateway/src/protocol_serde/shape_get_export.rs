...
pub fn de_get_export_http_response(
    ...
    Ok({
        ...
        output = output.set_body(crate::protocol_serde::shape_get_export_output::de_body_payload(_response_body)?);
        output = output.set_content_disposition(
            crate::protocol_serde::shape_get_export_output::de_content_disposition_header(_response_headers).map_err(|_| {
                crate::operation::get_export::GetExportError::unhandled("Failed to parse contentDisposition from header `Content-Disposition")
            })?,
        );
