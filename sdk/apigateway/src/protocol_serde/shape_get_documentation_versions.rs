...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::get_documentation_versions::GetDocumentationVersionsError::unhandled)?;
    generic_builder = ::aws_http::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::get_documentation_versions::GetDocumentationVersionsError::unhandled(
                generic,
            ))
        }
    };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_get_documentation_versions_http_response_with_props(
    _response_status: u16,
    _response_headers: &::http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_get_documentation_versions::de_get_documentation_versions(_response_body, output)
            .map_err(crate::operation::get_documentation_versions::GetDocumentationVersionsError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "item" => {
                    builder =
                        builder.set_items(crate::protocol_serde::shape_list_of_documentation_version::de_list_of_documentation_version(tokens)?);
                }
                "position" => {
