// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct DeleteTrainingDatasetInput  {
    /// <p>The Amazon Resource Name (ARN) of the training dataset that you want to delete.</p>
    pub training_dataset_arn: ::std::option::Option<::std::string::String>,
}
impl  DeleteTrainingDatasetInput  {
    /// <p>The Amazon Resource Name (ARN) of the training dataset that you want to delete.</p>
    pub fn training_dataset_arn(&self) -> ::std::option::Option<& str> {
        self.training_dataset_arn.as_deref()
    }
}
impl DeleteTrainingDatasetInput {
    /// Creates a new builder-style object to manufacture [`DeleteTrainingDatasetInput`](crate::operation::delete_training_dataset::DeleteTrainingDatasetInput).
    pub fn builder() -> crate::operation::delete_training_dataset::builders::DeleteTrainingDatasetInputBuilder {
        crate::operation::delete_training_dataset::builders::DeleteTrainingDatasetInputBuilder::default()
    }
}

/// A builder for [`DeleteTrainingDatasetInput`](crate::operation::delete_training_dataset::DeleteTrainingDatasetInput).
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
pub struct DeleteTrainingDatasetInputBuilder {
    pub(crate) training_dataset_arn: ::std::option::Option<::std::string::String>,
}
impl DeleteTrainingDatasetInputBuilder {
    /// <p>The Amazon Resource Name (ARN) of the training dataset that you want to delete.</p>
    /// This field is required.
    pub fn training_dataset_arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.training_dataset_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the training dataset that you want to delete.</p>
    pub fn set_training_dataset_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.training_dataset_arn = input; self
    }
    /// <p>The Amazon Resource Name (ARN) of the training dataset that you want to delete.</p>
    pub fn get_training_dataset_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.training_dataset_arn
    }
    /// Consumes the builder and constructs a [`DeleteTrainingDatasetInput`](crate::operation::delete_training_dataset::DeleteTrainingDatasetInput).
    pub fn build(self) -> ::std::result::Result<crate::operation::delete_training_dataset::DeleteTrainingDatasetInput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(
            crate::operation::delete_training_dataset::DeleteTrainingDatasetInput {
                training_dataset_arn: self.training_dataset_arn
                ,
            }
        )
    }
}

