            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "outboundMode" => {
                            builder =
                                builder.set_outbound_mode(crate::protocol_serde::shape_sms_outbound_mode::de_sms_outbound_mode(tokens, _value)?);
                        }
                        "defaultOutboundConfig" => {
                            builder = builder.set_default_outbound_config(crate::protocol_serde::shape_sms_outbound_config::de_sms_outbound_config(
                                tokens, _value,
                            )?);
                        }
