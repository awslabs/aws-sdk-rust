...
> {
    Ok({
        ...
        output = output.set_location(
            crate::protocol_serde::shape_initiate_multipart_upload_output::de_location_header(
                response.headers(),
            )
            .map_err(|_| {
                crate::error::InitiateMultipartUploadError::unhandled(
                    "Failed to parse location from header `Location",
                )
            })?,
        );
        output = output.set_upload_id(
            crate::protocol_serde::shape_initiate_multipart_upload_output::de_upload_id_header(
                response.headers(),
            )
            .map_err(|_| {
                crate::error::InitiateMultipartUploadError::unhandled(
