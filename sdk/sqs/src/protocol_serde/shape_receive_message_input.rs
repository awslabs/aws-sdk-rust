...
pub fn ser_receive_message_input_input(
    ...
    if let Some(var_8) = &input.max_number_of_messages {
        object.key("MaxNumberOfMessages").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_8).into()));
    }
    if let Some(var_9) = &input.visibility_timeout {
        object.key("VisibilityTimeout").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_9).into()));
    }
    if let Some(var_10) = &input.wait_time_seconds {
        object.key("WaitTimeSeconds").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_10).into()));
    }
