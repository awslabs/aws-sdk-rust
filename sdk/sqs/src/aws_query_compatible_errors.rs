...
fn aws_query_compatible_error_from_header(headers: &HeaderMap<HeaderValue>) -> Result<Option<&str>, ToStrError> {
    headers.get(X_AMZN_QUERY_ERROR).map(|v| v.to_str()).transpose()
}
...
#[cfg(test)]
mod test {
    use crate::aws_query_compatible_errors::{aws_query_compatible_error_from_header, parse_aws_query_compatible_error, X_AMZN_QUERY_ERROR};
