...
if (*use_fips) == (false) {
                                #[allow(unused_variables)]
if let Some(kvs_arn) = kvs_arn { #[allow(unused_variables)]
if let Some(parsed_arn) = crate::endpoint_lib::arn::parse_arn(kvs_arn, _diagnostic_collector) { if (parsed_arn.service()) == ("cloudfront") {
                                if (parsed_arn.region()) == ("") {
                                #[allow(unused_variables)]
if let Some(arn_type) = parsed_arn.resource_id()
...
if let Some(partition_result) = partition_resolver.resolve_partition(region, _diagnostic_collector) { if (partition_result.name()) == (parsed_arn.partition()) {
                                #[allow(unused_variables)]
...
.build()); }
return Err(::aws_smithy_http::endpoint::ResolveEndpointError::message("Provided endpoint is not a valid URL"
.to_string())); }
...
return Err(::aws_smithy_http::endpoint::ResolveEndpointError::message("No resource type found in the KVS ARN. Resource type must be `key-value-store`."
.to_string())); }
return Err(::aws_smithy_http::endpoint::ResolveEndpointError::message("No resource type found in the KVS ARN. Resource type must be `key-value-store`."
.to_string()));
                            }
...
out.push('`');
out })); }
return Err(::aws_smithy_http::endpoint::ResolveEndpointError::message("KVS ARN must be a valid ARN"
.to_string())); }
return Err(::aws_smithy_http::endpoint::ResolveEndpointError::message("KVS ARN must be provided to use this service"
.to_string()));
                            }
return Err(::aws_smithy_http::endpoint::ResolveEndpointError::message("Invalid Configuration: FIPS is not supported with CloudFront-KeyValueStore."
.to_string()));
                }
