// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
impl super::Client {
    /// Constructs a fluent builder for the [`GetOpenIdToken`](crate::operation::get_open_id_token::builders::GetOpenIdTokenFluentBuilder) operation.
                            ///
                            /// - The fluent builder is configurable:
    ///   - [`identity_id(impl Into<String>)`](crate::operation::get_open_id_token::builders::GetOpenIdTokenFluentBuilder::identity_id) / [`set_identity_id(Option<String>)`](crate::operation::get_open_id_token::builders::GetOpenIdTokenFluentBuilder::set_identity_id):<br>required: **true**<br><p>A unique identifier in the format REGION:GUID.</p><br>
    ///   - [`logins(impl Into<String>, impl Into<String>)`](crate::operation::get_open_id_token::builders::GetOpenIdTokenFluentBuilder::logins) / [`set_logins(Option<HashMap::<String, String>>)`](crate::operation::get_open_id_token::builders::GetOpenIdTokenFluentBuilder::set_logins):<br>required: **false**<br><p>A set of optional name-value pairs that map provider names to provider tokens. When using graph.facebook.com and www.amazon.com, supply the access_token returned from the provider's authflow. For accounts.google.com, an Amazon Cognito user pool provider, or any other OpenID Connect provider, always include the <code>id_token</code>.</p><br>
                            /// - On success, responds with [`GetOpenIdTokenOutput`](crate::operation::get_open_id_token::GetOpenIdTokenOutput) with field(s):
    ///   - [`identity_id(Option<String>)`](crate::operation::get_open_id_token::GetOpenIdTokenOutput::identity_id): <p>A unique identifier in the format REGION:GUID. Note that the IdentityId returned may not match the one passed on input.</p>
    ///   - [`token(Option<String>)`](crate::operation::get_open_id_token::GetOpenIdTokenOutput::token): <p>An OpenID token, valid for 10 minutes.</p>
                            /// - On failure, responds with [`SdkError<GetOpenIdTokenError>`](crate::operation::get_open_id_token::GetOpenIdTokenError)
    pub fn get_open_id_token(&self) -> crate::operation::get_open_id_token::builders::GetOpenIdTokenFluentBuilder {
                                crate::operation::get_open_id_token::builders::GetOpenIdTokenFluentBuilder::new(self.handle.clone())
                            }
}

