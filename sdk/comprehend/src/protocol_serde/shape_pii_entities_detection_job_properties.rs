...
pub(crate) fn de_pii_entities_detection_job_properties<'a, I>(
    tokens: &mut ::std::iter::Peekable<I>,
    _value: &'a [u8],
) -> ::std::result::Result<Option<crate::types::PiiEntitiesDetectionJobProperties>, ::aws_smithy_json::deserialize::error::DeserializeError>
where
            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "OutputDataConfig" => {
                            builder = builder.set_output_data_config(crate::protocol_serde::shape_pii_output_data_config::de_pii_output_data_config(
                                tokens, _value,
                            )?);
                        }
                        "RedactionConfig" => {
                            builder =
                                builder.set_redaction_config(crate::protocol_serde::shape_redaction_config::de_redaction_config(tokens, _value)?);
                        }
                        "LanguageCode" => {
                        ...
                        "Mode" => {
                            builder = builder.set_mode(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::PiiEntitiesDetectionMode::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
