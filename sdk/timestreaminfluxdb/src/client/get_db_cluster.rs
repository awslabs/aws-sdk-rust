...
impl super::Client {
    ...
    ///   - [`effective_db_parameter_group_identifier(Option<String>)`](crate::operation::get_db_cluster::GetDbClusterOutput::effective_db_parameter_group_identifier): <p>The ID of the DB parameter group actually applied to your DB cluster. When the service applies optimized defaults, it creates a service-managed DB parameter group and this field reflects that group, while dbParameterGroupIdentifier reflects the customer-provided DB parameter group. When no service-managed DB parameter group is applied, this value matches dbParameterGroupIdentifier.</p>
