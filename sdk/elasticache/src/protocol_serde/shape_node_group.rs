...
pub fn de_node_group(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::NodeGroup, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("NodeGroupMembers") /* NodeGroupMembers com.amazonaws.elasticache#NodeGroup$NodeGroupMembers */ =>  {
                let var_6 =
                    Some(
                        crate::protocol_serde::shape_node_group_member_list::de_node_group_member_list(&mut tag, depth + 1)
                        ?
                    )
                ;
