...
pub fn ser_server_side_encryption_by_default(input: &crate::types::ServerSideEncryptionByDefault, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
     {
        let mut inner_writer = scope.start_el("SSEAlgorithm").finish();
        inner_writer.data(
            input.sse_algorithm.as_str()
        );
    }
...
pub fn de_server_side_encryption_by_default(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::ServerSideEncryptionByDefault, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    Ok(crate::serde_util::server_side_encryption_by_default_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
