...
pub fn ser_s3_location(input: &crate::types::S3Location, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
     {
        let mut inner_writer = scope.start_el("BucketName").finish();
        inner_writer.data(
            input.bucket_name.as_str()
        );
    }
     {
