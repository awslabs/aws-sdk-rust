...
pub fn de_encryption_type_mismatch_xml_err(
...
) -> Result<crate::types::error::builders::EncryptionTypeMismatchBuilder, ::aws_smithy_xml::decode::XmlDecodeError> {
    if inp.is_empty() {
        return Ok(builder);
    }
    ...
    while let Some(mut tag) = error_decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("Message") /* Message com.amazonaws.s3#EncryptionTypeMismatch$Message */ =>  {
                let var_1 =
                    Some(
