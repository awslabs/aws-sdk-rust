...
pub fn de_restore_object_http_error(response: &http::Response<bytes::Bytes>) -> std::result::Result<crate::output::RestoreObjectOutput, crate::error::RestoreObjectError> {
    #[allow(unused_mut)]
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(response).map_err(crate::error::RestoreObjectError::unhandled)?;
    ...
    Err(match error_code {
        "ObjectAlreadyInActiveTierError" => crate::error::RestoreObjectError::ObjectAlreadyInActiveTierError({
            #[allow(unused_mut)]
            let mut tmp =
                 {
                    #[allow(unused_mut)]
                    let mut output = crate::error::object_already_in_active_tier_error::Builder::default();
                    let _ = response;
                    output = crate::protocol_serde::shape_object_already_in_active_tier_error::de_object_already_in_active_tier_error_xml_err(response.body().as_ref(), output).map_err(crate::error::RestoreObjectError::unhandled)?;
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_restore_object_http_response(response: &http::Response<bytes::Bytes>) -> std::result::Result<crate::output::RestoreObjectOutput, crate::error::RestoreObjectError> {
    Ok({
        #[allow(unused_mut)]
        ...
        output = output.set_request_charged(
            crate::protocol_serde::shape_restore_object_output::de_request_charged_header(response.headers())
                                    .map_err(|_|crate::error::RestoreObjectError::unhandled("Failed to parse RequestCharged from header `x-amz-request-charged"))?
        );
