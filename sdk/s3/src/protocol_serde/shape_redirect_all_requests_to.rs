...
pub fn de_redirect_all_requests_to(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::RedirectAllRequestsTo, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    Ok(crate::serde_util::redirect_all_requests_to_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}

pub fn ser_redirect_all_requests_to(input: &crate::types::RedirectAllRequestsTo, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
     {
        let mut inner_writer = scope.start_el("HostName").finish();
        inner_writer.data(
            input.host_name.as_str()
        );
    }
