...
impl ListObjectsV2Input {
    /// Consumes the builder and constructs an Operation<[`ListObjectsV2`](crate::operation::list_objects_v2::ListObjectsV2)>
    #[allow(unused_mut)]
    #[allow(clippy::let_and_return)]
    #[allow(clippy::needless_borrow)]
    pub async fn make_operation(&self, _config: &crate::config::Config) -> std::result::Result<aws_smithy_http::operation::Operation<crate::operation::list_objects_v2::ListObjectsV2, aws_http::retry::AwsResponseRetryClassifier>, aws_smithy_http::operation::error::BuildError> {
        ...
        let mut request = {
            fn uri_base(_input: &crate::operation::list_objects_v2::ListObjectsV2Input, output: &mut String) -> std::result::Result<(), aws_smithy_http::operation::error::BuildError> {
                write!(output, "/").expect("formatting should succeed");
                Ok(())
            }
            fn uri_query(_input: &crate::operation::list_objects_v2::ListObjectsV2Input, mut output: &mut String) -> Result<(), aws_smithy_http::operation::error::BuildError> {
                let mut query = aws_smithy_http::query::Writer::new(&mut output);
                query.push_kv("list-type", "2");
                if let Some(inner_1) = &_input.delimiter {
                     {
                        query.push_kv("delimiter", &aws_smithy_http::query::fmt_string(&inner_1));
                    }
                }
                if let Some(inner_2) = &_input.encoding_type {
                     {
                        query.push_kv("encoding-type", &aws_smithy_http::query::fmt_string(&inner_2));
                    }
                }
                if _input.max_keys != 0 {
                    query.push_kv("max-keys", aws_smithy_types::primitive::Encoder::from(_input.max_keys).encode());
                }
                ...
                if let Some(inner_4) = &_input.continuation_token {
                     {
                        query.push_kv("continuation-token", &aws_smithy_http::query::fmt_string(&inner_4));
                    }
                }
                if _input.fetch_owner {
                    query.push_kv("fetch-owner", aws_smithy_types::primitive::Encoder::from(_input.fetch_owner).encode());
                }
        ...
        let op = aws_smithy_http::operation::Operation::new(request, crate::operation::list_objects_v2::ListObjectsV2::new())
                            .with_metadata(aws_smithy_http::operation::Metadata::new("ListObjectsV2", "s3"));
...
impl aws_smithy_http::response::ParseStrictResponse for ListObjectsV2 {
                type Output = std::result::Result<crate::operation::list_objects_v2::ListObjectsV2Output, crate::operation::list_objects_v2::ListObjectsV2Error>;
