            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "or" => {
                            builder = builder.set_or(crate::protocol_serde::shape_expressions::de_expressions(tokens, _value, depth + 1)?);
                        }
                        "and" => {
                            builder = builder.set_and(crate::protocol_serde::shape_expressions::de_expressions(tokens, _value, depth + 1)?);
                        }
                        "not" => {
                            builder =
