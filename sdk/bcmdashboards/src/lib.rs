...
//! offered by AWS Billing and Cost Management Dashboards. The return value of each of these methods is a "fluent builder",
...
/// Client for calling AWS Billing and Cost Management Dashboards.
...
/// let config = aws_sdk_bcmdashboards::config::Builder::from(&sdk_config)
/// # /*
...
/// For example, the [`CreateDashboard`](crate::operation::create_dashboard) operation has
/// a [`Client::create_dashboard`], function which returns a builder for that operation.
/// The fluent builder ultimately has a `send()` function that returns an async future that
/// returns a result, as illustrated below:
///
/// ```rust,ignore
/// let result = client.create_dashboard()
///     .name("example")
///     .send()
///     .await;
/// ```
...
/// Configuration for AWS Billing and Cost Management Dashboards.
pub mod config;
