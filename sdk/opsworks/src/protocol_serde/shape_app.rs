            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "AppSource" => {
                            builder = builder.set_app_source(crate::protocol_serde::shape_source::de_source(tokens)?);
                        }
                        "Domains" => {
                            builder = builder.set_domains(crate::protocol_serde::shape_strings::de_strings(tokens)?);
                        }
                        "EnableSsl" => {
                            builder = builder.set_enable_ssl(::aws_smithy_json::deserialize::token::expect_bool_or_null(tokens.next())?);
                        }
                        "SslConfiguration" => {
                            builder = builder.set_ssl_configuration(crate::protocol_serde::shape_ssl_configuration::de_ssl_configuration(tokens)?);
                        }
                        "Attributes" => {
                            builder = builder.set_attributes(crate::protocol_serde::shape_app_attributes::de_app_attributes(tokens)?);
                        }
                        "CreatedAt" => {
