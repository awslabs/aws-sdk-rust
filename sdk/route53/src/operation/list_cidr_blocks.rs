...
impl ListCidrBlocksInput {
    /// Consumes the builder and constructs an Operation<[`ListCidrBlocks`](crate::operation::list_cidr_blocks::ListCidrBlocks)>
    #[allow(unused_mut)]
    #[allow(clippy::let_and_return)]
    #[allow(clippy::needless_borrow)]
    pub async fn make_operation(&self, _config: &crate::config::Config) -> std::result::Result<aws_smithy_http::operation::Operation<crate::operation::list_cidr_blocks::ListCidrBlocks, aws_http::retry::AwsResponseRetryClassifier>, aws_smithy_http::operation::error::BuildError> {
        ...
        let mut request = {
            fn uri_base(_input: &crate::operation::list_cidr_blocks::ListCidrBlocksInput, output: &mut String) -> std::result::Result<(), aws_smithy_http::operation::error::BuildError> {
                let input_1 = &_input.collection_id;
                let input_1 = input_1.as_ref().ok_or_else(|| aws_smithy_http::operation::error::BuildError::missing_field("collection_id", "cannot be empty or unset"))?;
                let collection_id = aws_smithy_http::label::fmt_string(input_1, aws_smithy_http::label::EncodingStrategy::Default);
                if collection_id.is_empty() {
                                return Err(aws_smithy_http::operation::error::BuildError::missing_field("collection_id", "cannot be empty or unset"))
                            }
                write!(output, "/2013-04-01/cidrcollection/{CollectionId}/cidrblocks", CollectionId = collection_id).expect("formatting should succeed");
                Ok(())
            }
            fn uri_query(_input: &crate::operation::list_cidr_blocks::ListCidrBlocksInput, mut output: &mut String) -> Result<(), aws_smithy_http::operation::error::BuildError> {
                let mut query = aws_smithy_http::query::Writer::new(&mut output);
                if let Some(inner_2) = &_input.location_name {
                     {
        ...
        let op = aws_smithy_http::operation::Operation::new(request, crate::operation::list_cidr_blocks::ListCidrBlocks::new())
                            .with_metadata(aws_smithy_http::operation::Metadata::new("ListCidrBlocks", "route53"));
...
impl aws_smithy_http::response::ParseStrictResponse for ListCidrBlocks {
                type Output = std::result::Result<crate::operation::list_cidr_blocks::ListCidrBlocksOutput, crate::operation::list_cidr_blocks::ListCidrBlocksError>;
                fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
                     tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
                     if !response.status().is_success() && response.status().as_u16() != 200 {
                        crate::protocol_serde::shape_list_cidr_blocks::de_list_cidr_blocks_http_error(response)
                     } else {
                        crate::protocol_serde::shape_list_cidr_blocks::de_list_cidr_blocks_http_response(response)
                     }
                }
            }
