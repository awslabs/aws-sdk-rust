...
pub fn de_list_cidr_blocks_http_response(
    ...
    Ok({
        ...
        output = crate::protocol_serde::shape_list_cidr_blocks::de_list_cidr_blocks(_response_body, output)
            .map_err(crate::operation::list_cidr_blocks::ListCidrBlocksError::unhandled)?;
...
pub fn de_list_cidr_blocks(
    ...
    if !start_el.matches("ListCidrBlocksResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected ListCidrBlocksResponse but got {start_el:?}. This is likely a bug in the SDK."
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("CidrBlocks") /* CidrBlocks com.amazonaws.route53.synthetic#ListCidrBlocksOutput$CidrBlocks */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_cidr_block_summaries::de_cidr_block_summaries(&mut tag, depth + 1)
                        ?
                    )
                ;
