...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_resource_record_sets::ListResourceRecordSetsError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_resource_record_sets::de_list_resource_record_sets(_response_body, output)
            .map_err(crate::operation::list_resource_record_sets::ListResourceRecordSetsError::unhandled)?;
        output._set_request_id(::aws_types::request_id::RequestId::request_id(_response_headers).map(str::to_string));
        crate::serde_util::list_resource_record_sets_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_resource_record_sets::ListResourceRecordSetsError::unhandled)?
    })
}
...
> {
    ...
    if !start_el.matches("ListResourceRecordSetsResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected ListResourceRecordSetsResponse but got {start_el:?}. This is likely a bug in the SDK."
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("ResourceRecordSets") /* ResourceRecordSets com.amazonaws.route53.synthetic#ListResourceRecordSetsOutput$ResourceRecordSets */ =>  {
                let var_5 =
                    Some(
                        crate::protocol_serde::shape_resource_record_sets::de_resource_record_sets(&mut tag, depth + 1)
                        ?
                    )
                ;
