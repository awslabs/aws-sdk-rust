...
) -> std::result::Result<crate::operation::list_cidr_collections::builders::ListCidrCollectionsOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError>
{
    ...
    if !start_el.matches("ListCidrCollectionsResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected ListCidrCollectionsResponse but got {start_el:?}. This is likely a bug in the SDK."
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("CidrCollections") /* CidrCollections com.amazonaws.route53.synthetic#ListCidrCollectionsOutput$CidrCollections */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_collection_summaries::de_collection_summaries(&mut tag, depth + 1)
                        ?
                    )
                ;
