...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_traffic_policy_versions::ListTrafficPolicyVersionsError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_traffic_policy_versions::de_list_traffic_policy_versions(_response_body, output)
            .map_err(crate::operation::list_traffic_policy_versions::ListTrafficPolicyVersionsError::unhandled)?;
        output._set_request_id(::aws_types::request_id::RequestId::request_id(_response_headers).map(str::to_string));
        crate::serde_util::list_traffic_policy_versions_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_traffic_policy_versions::ListTrafficPolicyVersionsError::unhandled)?
    })
}
...
> {
    ...
    if !start_el.matches("ListTrafficPolicyVersionsResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected ListTrafficPolicyVersionsResponse but got {start_el:?}. This is likely a bug in the SDK."
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("TrafficPolicies") /* TrafficPolicies com.amazonaws.route53.synthetic#ListTrafficPolicyVersionsOutput$TrafficPolicies */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_traffic_policies::de_traffic_policies(&mut tag, depth + 1)
                        ?
                    )
                ;
