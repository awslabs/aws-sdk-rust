...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::update_hosted_zone_comment::UpdateHostedZoneCommentError::unhandled(
                generic,
            ))
        }
    };
...
pub fn de_update_hosted_zone_comment(
    ...
    if !start_el.matches("UpdateHostedZoneCommentResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected UpdateHostedZoneCommentResponse but got {:?}. This is likely a bug in the SDK.",
            start_el
        )));
    }
