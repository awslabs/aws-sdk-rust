...
> {
    Ok({
        ...
        crate::serde_util::get_health_check_last_failure_reason_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::get_health_check_last_failure_reason::GetHealthCheckLastFailureReasonError::unhandled)?
    })
}
...
> {
    ...
    if !start_el.matches("GetHealthCheckLastFailureReasonResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected GetHealthCheckLastFailureReasonResponse but got {start_el:?}. This is likely a bug in the SDK."
        )));
    }
