...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_get_hosted_zone_limit::de_get_hosted_zone_limit(_response_body, output)
            .map_err(crate::operation::get_hosted_zone_limit::GetHostedZoneLimitError::unhandled)?;
...
) -> std::result::Result<crate::operation::get_hosted_zone_limit::builders::GetHostedZoneLimitOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError>
{
    ...
    if !start_el.matches("GetHostedZoneLimitResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected GetHostedZoneLimitResponse but got {start_el:?}. This is likely a bug in the SDK."
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("Limit") /* Limit com.amazonaws.route53.synthetic#GetHostedZoneLimitOutput$Limit */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_hosted_zone_limit::de_hosted_zone_limit(&mut tag, depth + 1)
                        ?
                    )
                ;
