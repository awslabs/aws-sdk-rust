...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_traffic_policies::de_list_traffic_policies(_response_body, output)
            .map_err(crate::operation::list_traffic_policies::ListTrafficPoliciesError::unhandled)?;
        output._set_request_id(::aws_types::request_id::RequestId::request_id(_response_headers).map(str::to_string));
        crate::serde_util::list_traffic_policies_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_traffic_policies::ListTrafficPoliciesError::unhandled)?
    })
}
...
) -> std::result::Result<crate::operation::list_traffic_policies::builders::ListTrafficPoliciesOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError>
{
    ...
    if !start_el.matches("ListTrafficPoliciesResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected ListTrafficPoliciesResponse but got {start_el:?}. This is likely a bug in the SDK."
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("TrafficPolicySummaries") /* TrafficPolicySummaries com.amazonaws.route53.synthetic#ListTrafficPoliciesOutput$TrafficPolicySummaries */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_traffic_policy_summaries::de_traffic_policy_summaries(&mut tag, depth + 1)
                        ?
                    )
                ;
