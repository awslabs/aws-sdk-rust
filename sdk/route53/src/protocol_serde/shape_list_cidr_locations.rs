...
pub fn de_list_cidr_locations(
...
) -> std::result::Result<crate::operation::list_cidr_locations::builders::ListCidrLocationsOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    if !start_el.matches("ListCidrLocationsResponse") {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "encountered invalid XML root: expected ListCidrLocationsResponse but got {start_el:?}. This is likely a bug in the SDK."
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("CidrLocations") /* CidrLocations com.amazonaws.route53.synthetic#ListCidrLocationsOutput$CidrLocations */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_location_summaries::de_location_summaries(&mut tag, depth + 1)
                        ?
                    )
                ;
