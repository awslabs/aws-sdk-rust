...
pub fn de_contributor_attributes(
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("entry") => {
                crate::protocol_serde::shape_contributor_attributes::de_contributor_attributes_entry(&mut tag, &mut out)?;
            }
            _ => {}
        }
    }
...
pub fn de_contributor_attributes_entry(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("key") /* key com.amazonaws.cloudwatch#ContributorAttributes$key */ =>  {
                k = Some(
            ...
            ,
            s if s.matches("value") /* value com.amazonaws.cloudwatch#ContributorAttributes$value */ =>  {
                v = Some(
                    Result::<::std::string::String, ::aws_smithy_xml::decode::XmlDecodeError>::Ok(
                        ::aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                        .into()
