...
pub fn de_datapoint_value_map(decoder: &mut aws_smithy_xml::decode::ScopedDecoder) -> Result<std::collections::HashMap<std::string::String, f64>, aws_smithy_xml::decode::XmlDecodeError> {
    let mut out = std::collections::HashMap::new();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("entry") =>  {
                crate::protocol_serde::shape_datapoint_value_map::de_datapoint_value_map_entry(&mut tag, &mut out)?;
            }
            _ => {}
        }
    }
...
pub fn de_datapoint_value_map_entry(decoder: &mut aws_smithy_xml::decode::ScopedDecoder, out: &mut std::collections::HashMap<std::string::String, f64>) -> Result<(), aws_smithy_xml::decode::XmlDecodeError> {
