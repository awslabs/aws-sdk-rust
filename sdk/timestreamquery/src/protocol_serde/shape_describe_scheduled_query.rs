...
pub fn de_describe_scheduled_query_http_error(
...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::describe_scheduled_query::DescribeScheduledQueryError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_scheduled_query::de_describe_scheduled_query(_response_body, output)
            .map_err(crate::operation::describe_scheduled_query::DescribeScheduledQueryError::unhandled)?;
...
pub(crate) fn de_describe_scheduled_query(
    value: &[u8],
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "ScheduledQuery" => {
                    builder = builder.set_scheduled_query(crate::protocol_serde::shape_scheduled_query_description::de_scheduled_query_description(
                        tokens,
                    )?);
                }
