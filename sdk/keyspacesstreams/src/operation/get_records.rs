...
impl GetRecords {
    ...
    > {
        ...
        ::aws_smithy_runtime::client::orchestrator::invoke_with_stop_point("KeyspacesStreams", "GetRecords", input, runtime_plugins, stop_point)
            // Create a parent span for the entire operation. Includes a random, internal-only,
            // seven-digit ID for the operation orchestration so that it can be correlated in the logs.
            .instrument(::tracing::debug_span!(
                "KeyspacesStreams.GetRecords",
...
impl GetRecordsError {
    ...
    /// Returns `true` if the error kind is `GetRecordsError::InternalServerException`.
    ...
    /// Returns `true` if the error kind is `GetRecordsError::ThrottlingException`.
    pub fn is_throttling_exception(&self) -> bool {
        matches!(self, Self::ThrottlingException(_))
    }
    /// Returns `true` if the error kind is `GetRecordsError::ValidationException`.
