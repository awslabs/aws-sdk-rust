// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_update_q_app_input_input(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::operation::update_q_app::UpdateQAppInput,
) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.app_definition {
        #[allow(unused_mut)]
        let mut object_2 = object.key("appDefinition").start_object();
        crate::protocol_serde::shape_app_definition_input::ser_app_definition_input(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.app_id {
        object.key("appId").string(var_3.as_str());
    }
    if let Some(var_4) = &input.description {
        object.key("description").string(var_4.as_str());
    }
    if let Some(var_5) = &input.title {
        object.key("title").string(var_5.as_str());
    }
    Ok(())
}
