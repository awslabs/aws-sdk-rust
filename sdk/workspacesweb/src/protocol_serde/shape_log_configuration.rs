...
pub fn ser_log_configuration(
    ...
    if let Some(var_1) = &input.s3 {
        ...
        crate::protocol_serde::shape_s3_log_configuration::ser_s3_log_configuration(&mut object_2, var_1)?;
        object_2.finish();
    }
    Ok(())
}
            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "s3" => {
                            builder = builder.set_s3(crate::protocol_serde::shape_s3_log_configuration::de_s3_log_configuration(tokens)?);
                        }
