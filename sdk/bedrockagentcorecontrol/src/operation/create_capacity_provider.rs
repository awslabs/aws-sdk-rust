...
impl CreateCapacityProvider {
    ...
    > {
        ...
        .instrument(::tracing::debug_span!(
            "Bedrock AgentCore Control.CreateCapacityProvider",
    ...
    pub(crate) fn operation_runtime_plugins(
        ...
        runtime_plugins = runtime_plugins.with_operation_plugin(crate::client_idempotency_token::IdempotencyTokenRuntimePlugin::new(
            |token_provider, input| {
                let input: &mut crate::operation::create_capacity_provider::CreateCapacityProviderInput = input.downcast_mut().expect("correct type");
...
impl ::aws_smithy_runtime_api::client::ser_de::DeserializeResponse for CreateCapacityProviderResponseDeserializer {
    fn deserialize_nonstreaming_with_config(
        ...
        let parse_result = if !success && status != 202 || force_error {
            crate::protocol_serde::shape_create_capacity_provider::de_create_capacity_provider_http_error(status, headers, body, _cfg)
        } else {
            crate::protocol_serde::shape_create_capacity_provider::de_create_capacity_provider_http_response(status, headers, body, _cfg)
        };
...
impl CreateCapacityProviderError {
    ...
    /// Returns `true` if the error kind is `CreateCapacityProviderError::AccessDeniedException`.
    pub fn is_access_denied_exception(&self) -> bool {
        matches!(self, Self::AccessDeniedException(_))
    }
    /// Returns `true` if the error kind is `CreateCapacityProviderError::ConflictException`.
    pub fn is_conflict_exception(&self) -> bool {
        matches!(self, Self::ConflictException(_))
    }
    /// Returns `true` if the error kind is `CreateCapacityProviderError::InternalServerException`.
    pub fn is_internal_server_exception(&self) -> bool {
        matches!(self, Self::InternalServerException(_))
    }
    /// Returns `true` if the error kind is `CreateCapacityProviderError::ResourceNotFoundException`.
    pub fn is_resource_not_found_exception(&self) -> bool {
        matches!(self, Self::ResourceNotFoundException(_))
    }
    /// Returns `true` if the error kind is `CreateCapacityProviderError::RetryableConflictException`.
    pub fn is_retryable_conflict_exception(&self) -> bool {
        matches!(self, Self::RetryableConflictException(_))
    }
    /// Returns `true` if the error kind is `CreateCapacityProviderError::ServiceQuotaExceededException`.
    pub fn is_service_quota_exceeded_exception(&self) -> bool {
        matches!(self, Self::ServiceQuotaExceededException(_))
    }
    /// Returns `true` if the error kind is `CreateCapacityProviderError::ThrottlingException`.
    pub fn is_throttling_exception(&self) -> bool {
        matches!(self, Self::ThrottlingException(_))
    }
    /// Returns `true` if the error kind is `CreateCapacityProviderError::ValidationException`.
