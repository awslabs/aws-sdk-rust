...
impl EvaluatorConfig {
    ...
    pub fn as_llm_as_a_judge(&self) -> ::std::result::Result<&crate::types::LlmAsAJudgeEvaluatorConfig, &Self> {
        if let EvaluatorConfig::LlmAsAJudge(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`LlmAsAJudge`](crate::types::EvaluatorConfig::LlmAsAJudge).
    pub fn is_llm_as_a_judge(&self) -> bool {
        self.as_llm_as_a_judge().is_ok()
    }
