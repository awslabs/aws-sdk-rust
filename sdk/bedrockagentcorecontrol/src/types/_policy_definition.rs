...
impl PolicyDefinition {
    ...
    /// Tries to convert the enum instance into [`Policy`](crate::types::PolicyDefinition::Policy), extracting the inner [`PolicyStatement`](crate::types::PolicyStatement).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_policy(&self) -> ::std::result::Result<&crate::types::PolicyStatement, &Self> {
        if let PolicyDefinition::Policy(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`Policy`](crate::types::PolicyDefinition::Policy).
    pub fn is_policy(&self) -> bool {
        self.as_policy().is_ok()
    }
