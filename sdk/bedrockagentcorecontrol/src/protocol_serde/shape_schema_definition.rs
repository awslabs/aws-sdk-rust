            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "properties" => {
                            builder = builder.set_properties(crate::protocol_serde::shape_schema_properties::de_schema_properties(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "required" => {
                            builder = builder.set_required(crate::protocol_serde::shape_required_properties::de_required_properties(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "items" => {
                            builder = builder.set_items(
                                crate::protocol_serde::shape_schema_definition::de_schema_definition(tokens, _value, depth + 1)?.map(Box::new),
                            );
                        }
                        "description" => {
