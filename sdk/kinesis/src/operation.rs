...
impl aws_smithy_http::response::ParseStrictResponse for AddTagsToStream {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_add_tags_to_stream::de_add_tags_to_stream_http_error(
                response,
            )
        } else {
...
impl aws_smithy_http::response::ParseStrictResponse for ListStreamConsumers {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_list_stream_consumers::de_list_stream_consumers_http_error(
                response,
            )
        } else {
...
impl aws_smithy_http::response::ParseStrictResponse for UpdateShardCount {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_update_shard_count::de_update_shard_count_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_update_shard_count::de_update_shard_count_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for UpdateStreamMode {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_update_stream_mode::de_update_stream_mode_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_update_stream_mode::de_update_stream_mode_http_response(
                response,
            )
        }
    }
