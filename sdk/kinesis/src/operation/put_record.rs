...
impl ::aws_smithy_runtime_api::client::interceptors::Intercept for PutRecordTelemetryInputCaptureInterceptor {
    ...
    fn read_before_execution(
        ...
        if requested.should_capture("PartitionKey") {
            if let ::std::option::Option::Some(value) = input.partition_key.as_deref() {
                captured.insert("PartitionKey", value);
            }
        }
        if requested.should_capture("ExplicitHashKey") {
            if let ::std::option::Option::Some(value) = input.explicit_hash_key.as_deref() {
                captured.insert("ExplicitHashKey", value);
            }
        }
        if requested.should_capture("SequenceNumberForOrdering") {
            if let ::std::option::Option::Some(value) = input.sequence_number_for_ordering.as_deref() {
                captured.insert("SequenceNumberForOrdering", value);
            }
        }
