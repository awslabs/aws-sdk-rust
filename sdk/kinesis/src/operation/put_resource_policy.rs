...
.set_resource_arn(Some(_input.resource_arn.clone().filter(|f|!AsRef::<str>::as_ref(f).trim().is_empty()).ok_or_else(||::aws_smithy_types::error::operation::BuildError::missing_field("resource_arn", "A required field was not set"))?))
                        .build()
