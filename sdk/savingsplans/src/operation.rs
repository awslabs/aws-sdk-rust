...
impl aws_smithy_http::response::ParseStrictResponse for CreateSavingsPlan {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_create_savings_plan::de_create_savings_plan_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_create_savings_plan::de_create_savings_plan_http_response(
                response,
            )
        }
    }
