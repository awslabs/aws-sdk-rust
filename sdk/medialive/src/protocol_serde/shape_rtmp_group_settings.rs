...
pub fn ser_rtmp_group_settings(
    ...
    if let Some(var_6) = &input.cache_length {
        object.key("cacheLength").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_6).into()));
    }
    ...
    if let Some(var_9) = &input.restart_delay {
        object.key("restartDelay").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_9).into()));
    }
                        ...
                        match key.to_unescaped()?.as_ref() {
                            "adMarkers" => {
                                builder = builder.set_ad_markers(
                                    crate::protocol_serde::shape_list_of_rtmp_ad_markers::de_list_of_rtmp_ad_markers(tokens)?
                                );
                            }
                            ...
                            "includeFillerNalUnits" => {
                                builder = builder.set_include_filler_nal_units(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::IncludeFillerNalUnits::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
