    ...
    pub async fn make_operation(&self, _config: &crate::config::Config) -> std::result::Result<aws_smithy_http::operation::Operation<crate::operation::list_reservations::ListReservations, aws_http::retry::AwsResponseRetryClassifier>, aws_smithy_http::operation::error::BuildError> {
        ...
        let mut request = {
            ...
            fn uri_query(_input: &crate::operation::list_reservations::ListReservationsInput, mut output: &mut String) -> Result<(), aws_smithy_http::operation::error::BuildError> {
                ...
                if let Some(inner_3) = &_input.maximum_bitrate {
                     {
                        query.push_kv("maximumBitrate", &aws_smithy_http::query::fmt_string(&inner_3));
                    }
                }
                if let Some(inner_4) = &_input.maximum_framerate {
                     {
                        query.push_kv("maximumFramerate", &aws_smithy_http::query::fmt_string(&inner_4));
                    }
                }
                ...
                if let Some(inner_6) = &_input.resolution {
                     {
                        query.push_kv("resolution", &aws_smithy_http::query::fmt_string(&inner_6));
                    }
                }
                if let Some(inner_7) = &_input.resource_type {
                     {
                        query.push_kv("resourceType", &aws_smithy_http::query::fmt_string(&inner_7));
                    }
                }
                if let Some(inner_8) = &_input.special_feature {
                     {
                        query.push_kv("specialFeature", &aws_smithy_http::query::fmt_string(&inner_8));
                    }
                }
                if let Some(inner_9) = &_input.video_quality {
                     {
                        query.push_kv("videoQuality", &aws_smithy_http::query::fmt_string(&inner_9));
                    }
                }
                Ok(())
            }
