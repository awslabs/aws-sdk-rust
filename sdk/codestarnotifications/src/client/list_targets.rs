...
impl super::Client {
    ...
    ///   - [`filters(ListTargetsFilter)`](crate::operation::list_targets::builders::ListTargetsFluentBuilder::filters) / [`set_filters(Option<Vec<ListTargetsFilter>>)`](crate::operation::list_targets::builders::ListTargetsFluentBuilder::set_filters): <p>The filters to use to return information by service or resource type. Valid filters include target type, target address, and target status.</p> <note>   <p>A filter with the same name can appear more than once when used with OR statements. Filters with different names should be applied with AND statements.</p>  </note>
