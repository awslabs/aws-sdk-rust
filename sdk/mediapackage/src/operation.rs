...
impl aws_smithy_http::response::ParseStrictResponse for CreateHarvestJob {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_create_harvest_job::de_create_harvest_job_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_create_harvest_job::de_create_harvest_job_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for DescribeHarvestJob {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_describe_harvest_job::de_describe_harvest_job_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_describe_harvest_job::de_describe_harvest_job_http_response(
                response,
            )
        }
    }
