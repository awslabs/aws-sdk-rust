...
pub fn ser_update_origin_endpoint_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::operation::update_origin_endpoint::UpdateOriginEndpointInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    ...
    if let Some(var_14) = &input.startover_window_seconds {
        object.key("startoverWindowSeconds").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_14).into()));
    }
    if let Some(var_15) = &input.time_delay_seconds {
        object.key("timeDelaySeconds").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_15).into()));
    }
