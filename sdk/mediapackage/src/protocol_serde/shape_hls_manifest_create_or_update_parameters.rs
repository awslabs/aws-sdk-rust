...
pub fn ser_hls_manifest_create_or_update_parameters(
    ...
    if let Some(var_10) = &input.playlist_window_seconds {
        object.key("playlistWindowSeconds").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_10).into()));
    }
    if let Some(var_11) = &input.program_date_time_interval_seconds {
        object.key("programDateTimeIntervalSeconds").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_11).into()));
    }
    Ok(())
}
