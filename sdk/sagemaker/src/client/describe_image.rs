// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
impl super::Client {
    /// Constructs a fluent builder for the [`DescribeImage`](crate::operation::describe_image::builders::DescribeImageFluentBuilder) operation.
    ///
    /// - The fluent builder is configurable:
    ///   - [`image_name(impl Into<String>)`](crate::operation::describe_image::builders::DescribeImageFluentBuilder::image_name) / [`set_image_name(Option<String>)`](crate::operation::describe_image::builders::DescribeImageFluentBuilder::set_image_name):<br>required: **true**<br><p>The name of the image to describe.</p><br>
    /// - On success, responds with [`DescribeImageOutput`](crate::operation::describe_image::DescribeImageOutput) with field(s):
    ///   - [`creation_time(Option<DateTime>)`](crate::operation::describe_image::DescribeImageOutput::creation_time): <p>When the image was created.</p>
    ///   - [`description(Option<String>)`](crate::operation::describe_image::DescribeImageOutput::description): <p>The description of the image.</p>
    ///   - [`display_name(Option<String>)`](crate::operation::describe_image::DescribeImageOutput::display_name): <p>The name of the image as displayed.</p>
    ///   - [`failure_reason(Option<String>)`](crate::operation::describe_image::DescribeImageOutput::failure_reason): <p>When a create, update, or delete operation fails, the reason for the failure.</p>
    ///   - [`image_arn(Option<String>)`](crate::operation::describe_image::DescribeImageOutput::image_arn): <p>The ARN of the image.</p>
    ///   - [`image_name(Option<String>)`](crate::operation::describe_image::DescribeImageOutput::image_name): <p>The name of the image.</p>
    ///   - [`image_status(Option<ImageStatus>)`](crate::operation::describe_image::DescribeImageOutput::image_status): <p>The status of the image.</p>
    ///   - [`last_modified_time(Option<DateTime>)`](crate::operation::describe_image::DescribeImageOutput::last_modified_time): <p>When the image was last modified.</p>
    ///   - [`role_arn(Option<String>)`](crate::operation::describe_image::DescribeImageOutput::role_arn): <p>The ARN of the IAM role that enables Amazon SageMaker AI to perform tasks on your behalf.</p>
    /// - On failure, responds with [`SdkError<DescribeImageError>`](crate::operation::describe_image::DescribeImageError)
    pub fn describe_image(&self) -> crate::operation::describe_image::builders::DescribeImageFluentBuilder {
        crate::operation::describe_image::builders::DescribeImageFluentBuilder::new(self.handle.clone())
    }
}
