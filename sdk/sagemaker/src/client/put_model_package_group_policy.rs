// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
impl super::Client {
    /// Constructs a fluent builder for the [`PutModelPackageGroupPolicy`](crate::operation::put_model_package_group_policy::builders::PutModelPackageGroupPolicyFluentBuilder) operation.
    ///
    /// - The fluent builder is configurable:
    ///   - [`model_package_group_name(impl ::std::convert::Into<String>)`](crate::operation::put_model_package_group_policy::builders::PutModelPackageGroupPolicyFluentBuilder::model_package_group_name) / [`set_model_package_group_name(Option<String>)`](crate::operation::put_model_package_group_policy::builders::PutModelPackageGroupPolicyFluentBuilder::set_model_package_group_name): <p>The name of the model group to add a resource policy to.</p>
    ///   - [`resource_policy(impl ::std::convert::Into<String>)`](crate::operation::put_model_package_group_policy::builders::PutModelPackageGroupPolicyFluentBuilder::resource_policy) / [`set_resource_policy(Option<String>)`](crate::operation::put_model_package_group_policy::builders::PutModelPackageGroupPolicyFluentBuilder::set_resource_policy): <p>The resource policy for the model group.</p>
    /// - On success, responds with [`PutModelPackageGroupPolicyOutput`](crate::operation::put_model_package_group_policy::PutModelPackageGroupPolicyOutput) with field(s):
    ///   - [`model_package_group_arn(Option<String>)`](crate::operation::put_model_package_group_policy::PutModelPackageGroupPolicyOutput::model_package_group_arn): <p>The Amazon Resource Name (ARN) of the model package group.</p>
    /// - On failure, responds with [`SdkError<PutModelPackageGroupPolicyError>`](crate::operation::put_model_package_group_policy::PutModelPackageGroupPolicyError)
    pub fn put_model_package_group_policy(
        &self,
    ) -> crate::operation::put_model_package_group_policy::builders::PutModelPackageGroupPolicyFluentBuilder {
        crate::operation::put_model_package_group_policy::builders::PutModelPackageGroupPolicyFluentBuilder::new(self.handle.clone())
    }
}
