// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_delete_artifact_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::DeleteArtifactInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.artifact_arn {
        object.key("ArtifactArn").string(var_1.as_str());
    }
    if let Some(var_2) = &input.source {
        #[allow(unused_mut)]
        let mut object_3 = object.key("Source").start_object();
        crate::protocol_serde::shape_artifact_source::ser_artifact_source(&mut object_3, var_2)?;
        object_3.finish();
    }
    Ok(())
}

