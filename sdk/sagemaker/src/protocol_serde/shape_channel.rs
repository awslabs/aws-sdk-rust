                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "RecordWrapperType" => {
                            builder = builder.set_record_wrapper_type(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::RecordWrapper::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "InputMode" => {
                        ...
                        "ShuffleConfig" => {
                            builder = builder.set_shuffle_config(crate::protocol_serde::shape_shuffle_config::de_shuffle_config(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
