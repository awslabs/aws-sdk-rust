    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "Results" => {
                    builder = builder.set_results(crate::protocol_serde::shape_search_results_list::de_search_results_list(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
                ...
                "TotalHits" => {
                    builder = builder.set_total_hits(crate::protocol_serde::shape_total_hits::de_total_hits(tokens, _value, depth + 1)?);
                }
