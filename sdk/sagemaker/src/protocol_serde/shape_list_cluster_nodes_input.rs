// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_list_cluster_nodes_input_input(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::operation::list_cluster_nodes::ListClusterNodesInput,
) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.cluster_name {
        object.key("ClusterName").string(var_1.as_str());
    }
    if let Some(var_2) = &input.creation_time_after {
        object
            .key("CreationTimeAfter")
            .date_time(var_2, ::aws_smithy_types::date_time::Format::EpochSeconds)?;
    }
    if let Some(var_3) = &input.creation_time_before {
        object
            .key("CreationTimeBefore")
            .date_time(var_3, ::aws_smithy_types::date_time::Format::EpochSeconds)?;
    }
    if let Some(var_4) = &input.instance_group_name_contains {
        object.key("InstanceGroupNameContains").string(var_4.as_str());
    }
    if let Some(var_5) = &input.max_results {
        object.key("MaxResults").number(
            #[allow(clippy::useless_conversion)]
            ::aws_smithy_types::Number::NegInt((*var_5).into()),
        );
    }
    if let Some(var_6) = &input.next_token {
        object.key("NextToken").string(var_6.as_str());
    }
    if let Some(var_7) = &input.sort_by {
        object.key("SortBy").string(var_7.as_str());
    }
    if let Some(var_8) = &input.sort_order {
        object.key("SortOrder").string(var_8.as_str());
    }
    if let Some(var_9) = &input.include_node_logical_ids {
        object.key("IncludeNodeLogicalIds").boolean(*var_9);
    }
    Ok(())
}
