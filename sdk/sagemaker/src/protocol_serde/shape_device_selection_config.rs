            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "DeviceSubsetType" => {
                            builder = builder.set_device_subset_type(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::DeviceSubsetType::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "Percentage" => {
                        ...
                        "DeviceNames" => {
                            builder =
                                builder.set_device_names(crate::protocol_serde::shape_device_names::de_device_names(tokens, _value, depth + 1)?);
                        }
                        "DeviceNameContains" => {
