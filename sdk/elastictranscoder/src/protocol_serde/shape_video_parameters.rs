            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "CodecOptions" => {
                            builder =
                                builder.set_codec_options(crate::protocol_serde::shape_codec_options::de_codec_options(tokens, _value, depth + 1)?);
                        }
                        "KeyframesMaxDist" => {
                        ...
                        "Watermarks" => {
                            builder = builder.set_watermarks(crate::protocol_serde::shape_preset_watermarks::de_preset_watermarks(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
