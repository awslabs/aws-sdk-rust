...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_profiling_groups::de_list_profiling_groups(_response_body, output)
            .map_err(crate::operation::list_profiling_groups::ListProfilingGroupsError::unhandled)?;
        output._set_request_id(::aws_types::request_id::RequestId::request_id(_response_headers).map(str::to_string));
        crate::serde_util::list_profiling_groups_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_profiling_groups::ListProfilingGroupsError::unhandled)?
    })
}
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "profilingGroupNames" => {
                    builder =
                        builder.set_profiling_group_names(crate::protocol_serde::shape_profiling_group_names::de_profiling_group_names(tokens)?);
                }
                "profilingGroups" => {
                    builder = builder
                        .set_profiling_groups(crate::protocol_serde::shape_profiling_group_descriptions::de_profiling_group_descriptions(tokens)?);
                }
