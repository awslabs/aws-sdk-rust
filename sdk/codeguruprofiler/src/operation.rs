...
impl aws_smithy_http::response::ParseStrictResponse for BatchGetFrameMetricData {
                ...
                fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
                     tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
                     if !response.status().is_success() && response.status().as_u16() != 200 {
                        crate::protocol_serde::shape_batch_get_frame_metric_data::de_batch_get_frame_metric_data_http_error(response)
                     } else {
                        crate::protocol_serde::shape_batch_get_frame_metric_data::de_batch_get_frame_metric_data_http_response(response)
                     }
                }
            }
...
impl aws_smithy_http::response::ParseStrictResponse for PostAgentProfile {
                ...
                fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
                     tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
                     if !response.status().is_success() && response.status().as_u16() != 204 {
                        crate::protocol_serde::shape_post_agent_profile::de_post_agent_profile_http_error(response)
                     } else {
                        crate::protocol_serde::shape_post_agent_profile::de_post_agent_profile_http_response(response)
                     }
                }
            }
