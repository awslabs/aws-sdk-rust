...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::get_signed_bluinsights_url::GetSignedBluinsightsUrlError::unhandled(
                generic,
            ))
        }
    };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_get_signed_bluinsights_url_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        crate::serde_util::get_signed_bluinsights_url_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::get_signed_bluinsights_url::GetSignedBluinsightsUrlError::unhandled)?
    })
}
