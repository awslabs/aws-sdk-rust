...
impl aws_smithy_http::response::ParseStrictResponse for GetDataSetDetails {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_get_data_set_details::de_get_data_set_details_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_get_data_set_details::de_get_data_set_details_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for ListEngineVersions {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_list_engine_versions::de_list_engine_versions_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_list_engine_versions::de_list_engine_versions_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for UpdateEnvironment {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_update_environment::de_update_environment_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_update_environment::de_update_environment_http_response(
                response,
            )
        }
    }
