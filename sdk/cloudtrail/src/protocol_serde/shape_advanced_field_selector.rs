            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "Equals" => {
                            builder = builder.set_equals(crate::protocol_serde::shape_operator::de_operator(tokens, _value, depth + 1)?);
                        }
                        "StartsWith" => {
                            builder = builder.set_starts_with(crate::protocol_serde::shape_operator::de_operator(tokens, _value, depth + 1)?);
                        }
                        "EndsWith" => {
                            builder = builder.set_ends_with(crate::protocol_serde::shape_operator::de_operator(tokens, _value, depth + 1)?);
                        }
                        "NotEquals" => {
                            builder = builder.set_not_equals(crate::protocol_serde::shape_operator::de_operator(tokens, _value, depth + 1)?);
                        }
                        "NotStartsWith" => {
                            builder = builder.set_not_starts_with(crate::protocol_serde::shape_operator::de_operator(tokens, _value, depth + 1)?);
                        }
                        "NotEndsWith" => {
                            builder = builder.set_not_ends_with(crate::protocol_serde::shape_operator::de_operator(tokens, _value, depth + 1)?);
                        }
