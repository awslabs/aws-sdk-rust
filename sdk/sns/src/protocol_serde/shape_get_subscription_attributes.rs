...
> {
    ...
    if !(start_el.matches("GetSubscriptionAttributesResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected GetSubscriptionAttributesResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("GetSubscriptionAttributesResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected GetSubscriptionAttributesResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("Attributes") /* Attributes com.amazonaws.sns.synthetic#GetSubscriptionAttributesOutput$Attributes */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_subscription_attributes_map::de_subscription_attributes_map(&mut tag, depth + 1)
                        ?
                    )
                ;
