...
pub fn ser_create_scheduled_action_input_input(
    ...
    if let Some(var_7) = &input.namespace_name {
        object.key("namespaceName").string(var_7.as_str());
    }
    if let Some(var_8) = &input.enabled {
        object.key("enabled").boolean(*var_8);
    }
    if let Some(var_9) = &input.scheduled_action_description {
        object.key("scheduledActionDescription").string(var_9.as_str());
    }
    if let Some(var_10) = &input.start_time {
        object.key("startTime").date_time(var_10, ::aws_smithy_types::date_time::Format::EpochSeconds)?;
    }
    if let Some(var_11) = &input.end_time {
        object.key("endTime").date_time(var_11, ::aws_smithy_types::date_time::Format::EpochSeconds)?;
    }
    Ok(())
}
