...
impl super::Client {
    ...
    ///   - [`data_destination_configs(Option<Vec::<DataDestinationConfig>>)`](crate::operation::get_campaign::GetCampaignOutput::data_destination_configs): <p>The destination where the campaign sends data. You can send data to an MQTT topic, or store it in Amazon S3 or Amazon Timestream.</p> <p>MQTT is the publish/subscribe messaging protocol used by Amazon Web Services IoT to communicate with your devices.</p> <p>Amazon S3 optimizes the cost of data storage and provides additional mechanisms to use vehicle data, such as data lakes, centralized data storage, data processing pipelines, and analytics.</p> <p>You can use Amazon Timestream to access and analyze time series data, and Timestream to query vehicle data so that you can identify trends and patterns.</p>
    ///   - [`data_partitions(Option<Vec::<DataPartition>>)`](crate::operation::get_campaign::GetCampaignOutput::data_partitions): <p>The data partitions associated with the signals collected from the vehicle.</p>
    ///   - [`signals_to_fetch(Option<Vec::<SignalFetchInformation>>)`](crate::operation::get_campaign::GetCampaignOutput::signals_to_fetch): <p>Information about a list of signals to fetch data from.</p>
