...
pub fn ser_sensor(
    ...
    if let Some(var_6) = &input.min {
        object.key("min").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::Float((*var_6).into()));
    }
    if let Some(var_7) = &input.max {
        object.key("max").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::Float((*var_7).into()));
    }
    ...
    if let Some(var_10) = &input.struct_fully_qualified_name {
        object.key("structFullyQualifiedName").string(var_10.as_str());
    }
    Ok(())
}
...
pub(crate) fn de_sensor<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::Sensor>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
            ...
            Ok(Some(crate::serde_util::sensor_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
