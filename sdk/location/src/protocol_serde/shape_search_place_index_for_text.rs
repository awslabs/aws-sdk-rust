...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::search_place_index_for_text::SearchPlaceIndexForTextError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        crate::serde_util::search_place_index_for_text_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::search_place_index_for_text::SearchPlaceIndexForTextError::unhandled)?
    })
}
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "Summary" => {
                    builder = builder.set_summary(
                        crate::protocol_serde::shape_search_place_index_for_text_summary::de_search_place_index_for_text_summary(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
