    ...
    pub(crate) fn operation_runtime_plugins(
        ...
        runtime_plugins = runtime_plugins.with_operation_plugin(crate::client_idempotency_token::IdempotencyTokenRuntimePlugin::new(
            |token_provider, input| {
                let input: &mut crate::operation::create_rule::CreateRuleInput = input.downcast_mut().expect("correct type");
...
impl ::aws_smithy_runtime_api::client::interceptors::Intercept for CreateRuleTelemetryInputCaptureInterceptor {
    ...
    fn read_before_execution(
        ...
        if requested.should_capture("ruleDescription") {
            if let ::std::option::Option::Some(value) = input.rule_description.as_deref() {
                captured.insert("ruleDescription", value);
            }
        }
...
impl CreateRuleError {
    ...
    /// Returns `true` if the error kind is `CreateRuleError::ConflictException`.
    ...
    /// Returns `true` if the error kind is `CreateRuleError::ServiceUnavailableException`.
    pub fn is_service_unavailable_exception(&self) -> bool {
        matches!(self, Self::ServiceUnavailableException(_))
    }
    /// Returns `true` if the error kind is `CreateRuleError::TagPolicyViolationException`.
