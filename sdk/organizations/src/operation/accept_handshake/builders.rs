...
/// <p>When a handshake is accepted, Organizations logs membership events in CloudTrail, available only in the management account's event history. If the account was standalone and joined a new organization, an <code>AccountJoinedOrganization</code> event is logged with <code>joinedMethod:INVITED</code> and <code>joinedTime</code> fields. If the account departed one organization and joined another, both an <code>AccountDepartedOrganization</code> event with <code>departureMethod:LEFT</code> and <code>departureTime</code> and an <code>AccountJoinedOrganization</code> event with <code>joinedMethod:INVITED</code> and <code>joinedTime</code> are logged in their respective management accounts.</p>
/// <p>When a billing transfer (<code>TRANSFER_RESPONSIBILITY</code>) handshake is accepted, Organizations publishes a <code>ResponsibilityTransferAccepted</code> service event to CloudTrail. Each affected account receives this event, including upstream participants such as distributors in a chained transfer. For an example log entry, see <a href="https://docs.aws.amazon.com/organizations/latest/userguide/orgs_cloudtrail-integration.html#Log-entries-accept-responsibility-transfer">Example log entries: AcceptResponsibilityTransfer</a> in the <i>Organizations User Guide</i>.</p>
