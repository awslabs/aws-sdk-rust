...
impl crate::operation::remove_account_from_organization::builders::RemoveAccountFromOrganizationInputBuilder {
...
/// <p>When an account is removed from an organization, Organizations logs a membership event in CloudTrail. The event is an <code>AccountDepartedOrganization</code> event with <code>departureMethod:REMOVED</code> and <code>departureTime</code>. This event is available only in the management account's event history.</p><important>
/// <ul>
/// <li>
...
impl RemoveAccountFromOrganizationFluentBuilder {
    /// Creates a new `RemoveAccountFromOrganizationFluentBuilder`.
