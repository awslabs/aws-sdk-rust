...
impl aws_smithy_http::response::ParseStrictResponse for ExportSchema {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !success && status != 200 {
            crate::protocol_serde::shape_export_schema::de_export_schema_http_error(
                status, headers, body,
            )
        } else {
            crate::protocol_serde::shape_export_schema::de_export_schema_http_response_with_props(
                status, headers, body,
            )
        }
    }
