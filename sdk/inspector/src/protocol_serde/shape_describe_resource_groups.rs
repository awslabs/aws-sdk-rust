...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::describe_resource_groups::DescribeResourceGroupsError::unhandled(
                generic,
            ))
        }
    };
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "resourceGroups" => {
                    builder = builder.set_resource_groups(crate::protocol_serde::shape_resource_group_list::de_resource_group_list(tokens)?);
                }
                "failedItems" => {
