...
pub fn ser_get_exclusions_preview_input(
    ...
    crate::protocol_serde::shape_get_exclusions_preview_input::ser_get_exclusions_preview_input(
        &mut object,
        input,
    )?;
    object.finish();
...
> {
    ...
    let error_code = match generic.code() {
        ...
        None => {
            return Err(
                crate::operation::get_exclusions_preview::GetExclusionsPreviewError::unhandled(
                    generic,
                ),
            )
        }
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_get_exclusions_preview_http_response_with_props(
    _response_status: u16,
    _response_headers: &http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_get_exclusions_preview::de_get_exclusions_preview(
            _response_body,
            output,
        )
        .map_err(crate::operation::get_exclusions_preview::GetExclusionsPreviewError::unhandled)?;
                ...
                match key.to_unescaped()?.as_ref() {
                    "previewStatus" => {
                        builder = builder.set_preview_status(
                            ...
                            .map(|s| {
                                s.to_unescaped()
                                    .map(|u| crate::types::PreviewStatus::from(u.as_ref()))
                            })
                            .transpose()?,
                        );
                    }
