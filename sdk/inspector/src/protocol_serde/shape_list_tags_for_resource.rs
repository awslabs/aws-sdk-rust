    ...
    Err(match error_code {
        ...
        "NoSuchEntityException" => crate::error::ListTagsForResourceError::NoSuchEntityException({
            ...
            let mut tmp = {
                ...
                let _ = response;
                output = crate::protocol_serde::shape_no_such_entity_exception::de_no_such_entity_exception_json_err(response.body().as_ref(), output).map_err(crate::error::ListTagsForResourceError::unhandled)?;
                let output = output.meta(generic);
                output.build()
            };
            if tmp.message.is_none() {
