            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "provisioned" => {
                            builder = builder.set_provisioned(crate::protocol_serde::shape_provisioned::de_provisioned(tokens, _value, depth + 1)?);
                        }
                        "serverless" => {
                            builder = builder.set_serverless(crate::protocol_serde::shape_serverless::de_serverless(tokens, _value, depth + 1)?);
                        }
