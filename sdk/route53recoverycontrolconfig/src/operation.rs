...
impl aws_smithy_http::response::ParseStrictResponse for CreateSafetyRule {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_create_safety_rule::de_create_safety_rule_http_error(
                response,
            )
        } else {
...
impl aws_smithy_http::response::ParseStrictResponse for DeleteControlPanel {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_delete_control_panel::de_delete_control_panel_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_delete_control_panel::de_delete_control_panel_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for DescribeSafetyRule {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_describe_safety_rule::de_describe_safety_rule_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_describe_safety_rule::de_describe_safety_rule_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for ListControlPanels {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_list_control_panels::de_list_control_panels_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_list_control_panels::de_list_control_panels_http_response(
                response,
            )
        }
    }
