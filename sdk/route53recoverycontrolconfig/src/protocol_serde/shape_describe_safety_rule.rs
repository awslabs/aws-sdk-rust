    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "AssertionRule" => {
                    builder = builder.set_assertion_rule(crate::protocol_serde::shape_assertion_rule::de_assertion_rule(tokens, _value, depth + 1)?);
                }
                "GatingRule" => {
                    builder = builder.set_gating_rule(crate::protocol_serde::shape_gating_rule::de_gating_rule(tokens, _value, depth + 1)?);
                }
