// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_update_test_grid_project_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::UpdateTestGridProjectInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.project_arn {
        object.key("projectArn").string(var_1.as_str());
    }
    if let Some(var_2) = &input.name {
        object.key("name").string(var_2.as_str());
    }
    if let Some(var_3) = &input.description {
        object.key("description").string(var_3.as_str());
    }
    if let Some(var_4) = &input.vpc_config {
        #[allow(unused_mut)]
        let mut object_5 = object.key("vpcConfig").start_object();
        crate::protocol_serde::shape_test_grid_vpc_config::ser_test_grid_vpc_config(&mut object_5, var_4)?;
        object_5.finish();
    }
    Ok(())
}

