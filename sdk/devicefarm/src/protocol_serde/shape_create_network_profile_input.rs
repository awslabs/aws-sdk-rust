...
pub fn ser_create_network_profile_input_input(
    ...
    if let Some(var_11) = &input.uplink_loss_percent {
        object.key("uplinkLossPercent").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_11).into()));
    }
    if let Some(var_12) = &input.downlink_loss_percent {
        object.key("downlinkLossPercent").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_12).into()));
    }
    Ok(())
}
