...
/// <p>Defines the automation configuration for building, testing, and distributing images. A pipeline references the resources that its builds use, such as the recipe and infrastructure configuration. It also holds the settings that control its builds, such as the schedule and custom workflows.</p>
#[non_exhaustive]
...
pub struct ImagePipeline {
    ...
    /// <li>
    /// <p>If the pipeline execution fails, Image Builder increments the number of consecutive failures. If the failure count reaches the limit defined in the <code>AutoDisablePolicy</code>, Image Builder disables the pipeline.</p></li>
    /// </ul>
