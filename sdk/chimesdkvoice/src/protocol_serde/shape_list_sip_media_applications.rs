...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::list_sip_media_applications::ListSipMediaApplicationsError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_sip_media_applications::ListSipMediaApplicationsError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_sip_media_applications::de_list_sip_media_applications(_response_body, output)
            .map_err(crate::operation::list_sip_media_applications::ListSipMediaApplicationsError::unhandled)?;
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "SipMediaApplications" => {
                    builder = builder.set_sip_media_applications(
                        crate::protocol_serde::shape_sip_media_application_list::de_sip_media_application_list(tokens, _value, depth + 1)?,
                    );
                }
