                        ...
                        match key.to_unescaped()?.as_ref() {
                            "dashManifests" => {
                                builder = builder.set_dash_manifests(
                                    crate::protocol_serde::shape_list_of_dash_manifest::de_list_of_dash_manifest(tokens)?
                                );
                            }
                            "encryption" => {
...
pub fn ser_dash_package(
    ...
    if let Some(var_12) = &input.segment_duration_seconds {
        object.key("segmentDurationSeconds").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_12).into()));
    }
