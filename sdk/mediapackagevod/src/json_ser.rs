// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn serialize_structure_configure_logs_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ConfigureLogsInput,
) {
    if let Some(var_1) = &input.egress_access_logs {
        let mut object_2 = object.key("egressAccessLogs").start_object();
        crate::json_ser::serialize_structure_egress_access_logs(&mut object_2, var_1);
        object_2.finish();
    }
}

pub fn serialize_structure_create_asset_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::CreateAssetInput,
) {
    if let Some(var_3) = &input.id {
        object.key("id").string(var_3);
    }
    if let Some(var_4) = &input.packaging_group_id {
        object.key("packagingGroupId").string(var_4);
    }
    if let Some(var_5) = &input.resource_id {
        object.key("resourceId").string(var_5);
    }
    if let Some(var_6) = &input.source_arn {
        object.key("sourceArn").string(var_6);
    }
    if let Some(var_7) = &input.source_role_arn {
        object.key("sourceRoleArn").string(var_7);
    }
    if let Some(var_8) = &input.tags {
        let mut object_9 = object.key("tags").start_object();
        for (key_10, value_11) in var_8 {
            {
                object_9.key(key_10).string(value_11);
            }
        }
        object_9.finish();
    }
}

pub fn serialize_structure_create_packaging_configuration_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::CreatePackagingConfigurationInput,
) {
    if let Some(var_12) = &input.cmaf_package {
        let mut object_13 = object.key("cmafPackage").start_object();
        crate::json_ser::serialize_structure_cmaf_package(&mut object_13, var_12);
        object_13.finish();
    }
    if let Some(var_14) = &input.dash_package {
        let mut object_15 = object.key("dashPackage").start_object();
        crate::json_ser::serialize_structure_dash_package(&mut object_15, var_14);
        object_15.finish();
    }
    if let Some(var_16) = &input.hls_package {
        let mut object_17 = object.key("hlsPackage").start_object();
        crate::json_ser::serialize_structure_hls_package(&mut object_17, var_16);
        object_17.finish();
    }
    if let Some(var_18) = &input.id {
        object.key("id").string(var_18);
    }
    if let Some(var_19) = &input.mss_package {
        let mut object_20 = object.key("mssPackage").start_object();
        crate::json_ser::serialize_structure_mss_package(&mut object_20, var_19);
        object_20.finish();
    }
    if let Some(var_21) = &input.packaging_group_id {
        object.key("packagingGroupId").string(var_21);
    }
    if let Some(var_22) = &input.tags {
        let mut object_23 = object.key("tags").start_object();
        for (key_24, value_25) in var_22 {
            {
                object_23.key(key_24).string(value_25);
            }
        }
        object_23.finish();
    }
}

pub fn serialize_structure_create_packaging_group_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::CreatePackagingGroupInput,
) {
    if let Some(var_26) = &input.authorization {
        let mut object_27 = object.key("authorization").start_object();
        crate::json_ser::serialize_structure_authorization(&mut object_27, var_26);
        object_27.finish();
    }
    if let Some(var_28) = &input.egress_access_logs {
        let mut object_29 = object.key("egressAccessLogs").start_object();
        crate::json_ser::serialize_structure_egress_access_logs(&mut object_29, var_28);
        object_29.finish();
    }
    if let Some(var_30) = &input.id {
        object.key("id").string(var_30);
    }
    if let Some(var_31) = &input.tags {
        let mut object_32 = object.key("tags").start_object();
        for (key_33, value_34) in var_31 {
            {
                object_32.key(key_33).string(value_34);
            }
        }
        object_32.finish();
    }
}

pub fn serialize_structure_tag_resource_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::TagResourceInput,
) {
    if let Some(var_35) = &input.tags {
        let mut object_36 = object.key("tags").start_object();
        for (key_37, value_38) in var_35 {
            {
                object_36.key(key_37).string(value_38);
            }
        }
        object_36.finish();
    }
}

pub fn serialize_structure_update_packaging_group_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::UpdatePackagingGroupInput,
) {
    if let Some(var_39) = &input.authorization {
        let mut object_40 = object.key("authorization").start_object();
        crate::json_ser::serialize_structure_authorization(&mut object_40, var_39);
        object_40.finish();
    }
}

pub fn serialize_structure_egress_access_logs(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::EgressAccessLogs,
) {
    if let Some(var_41) = &input.log_group_name {
        object.key("logGroupName").string(var_41);
    }
}

pub fn serialize_structure_cmaf_package(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::CmafPackage,
) {
    if let Some(var_42) = &input.encryption {
        let mut object_43 = object.key("encryption").start_object();
        crate::json_ser::serialize_structure_cmaf_encryption(&mut object_43, var_42);
        object_43.finish();
    }
    if let Some(var_44) = &input.hls_manifests {
        let mut array_45 = object.key("hlsManifests").start_array();
        for item_46 in var_44 {
            {
                let mut object_47 = array_45.value().start_object();
                crate::json_ser::serialize_structure_hls_manifest(&mut object_47, item_46);
                object_47.finish();
            }
        }
        array_45.finish();
    }
    if input.include_encoder_configuration_in_segments {
        object
            .key("includeEncoderConfigurationInSegments")
            .boolean(input.include_encoder_configuration_in_segments);
    }
    if input.segment_duration_seconds != 0 {
        object.key("segmentDurationSeconds").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((input.segment_duration_seconds).into()),
        );
    }
}

pub fn serialize_structure_dash_package(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::DashPackage,
) {
    if let Some(var_48) = &input.dash_manifests {
        let mut array_49 = object.key("dashManifests").start_array();
        for item_50 in var_48 {
            {
                let mut object_51 = array_49.value().start_object();
                crate::json_ser::serialize_structure_dash_manifest(&mut object_51, item_50);
                object_51.finish();
            }
        }
        array_49.finish();
    }
    if let Some(var_52) = &input.encryption {
        let mut object_53 = object.key("encryption").start_object();
        crate::json_ser::serialize_structure_dash_encryption(&mut object_53, var_52);
        object_53.finish();
    }
    if input.include_encoder_configuration_in_segments {
        object
            .key("includeEncoderConfigurationInSegments")
            .boolean(input.include_encoder_configuration_in_segments);
    }
    if let Some(var_54) = &input.period_triggers {
        let mut array_55 = object.key("periodTriggers").start_array();
        for item_56 in var_54 {
            {
                array_55.value().string(item_56.as_str());
            }
        }
        array_55.finish();
    }
    if input.segment_duration_seconds != 0 {
        object.key("segmentDurationSeconds").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((input.segment_duration_seconds).into()),
        );
    }
    if let Some(var_57) = &input.segment_template_format {
        object.key("segmentTemplateFormat").string(var_57.as_str());
    }
}

pub fn serialize_structure_hls_package(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::HlsPackage,
) {
    if let Some(var_58) = &input.encryption {
        let mut object_59 = object.key("encryption").start_object();
        crate::json_ser::serialize_structure_hls_encryption(&mut object_59, var_58);
        object_59.finish();
    }
    if let Some(var_60) = &input.hls_manifests {
        let mut array_61 = object.key("hlsManifests").start_array();
        for item_62 in var_60 {
            {
                let mut object_63 = array_61.value().start_object();
                crate::json_ser::serialize_structure_hls_manifest(&mut object_63, item_62);
                object_63.finish();
            }
        }
        array_61.finish();
    }
    if input.segment_duration_seconds != 0 {
        object.key("segmentDurationSeconds").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((input.segment_duration_seconds).into()),
        );
    }
    if input.use_audio_rendition_group {
        object
            .key("useAudioRenditionGroup")
            .boolean(input.use_audio_rendition_group);
    }
}

pub fn serialize_structure_mss_package(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::MssPackage,
) {
    if let Some(var_64) = &input.encryption {
        let mut object_65 = object.key("encryption").start_object();
        crate::json_ser::serialize_structure_mss_encryption(&mut object_65, var_64);
        object_65.finish();
    }
    if let Some(var_66) = &input.mss_manifests {
        let mut array_67 = object.key("mssManifests").start_array();
        for item_68 in var_66 {
            {
                let mut object_69 = array_67.value().start_object();
                crate::json_ser::serialize_structure_mss_manifest(&mut object_69, item_68);
                object_69.finish();
            }
        }
        array_67.finish();
    }
    if input.segment_duration_seconds != 0 {
        object.key("segmentDurationSeconds").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((input.segment_duration_seconds).into()),
        );
    }
}

pub fn serialize_structure_authorization(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::Authorization,
) {
    if let Some(var_70) = &input.cdn_identifier_secret {
        object.key("cdnIdentifierSecret").string(var_70);
    }
    if let Some(var_71) = &input.secrets_role_arn {
        object.key("secretsRoleArn").string(var_71);
    }
}

pub fn serialize_structure_cmaf_encryption(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::CmafEncryption,
) {
    if let Some(var_72) = &input.constant_initialization_vector {
        object.key("constantInitializationVector").string(var_72);
    }
    if let Some(var_73) = &input.speke_key_provider {
        let mut object_74 = object.key("spekeKeyProvider").start_object();
        crate::json_ser::serialize_structure_speke_key_provider(&mut object_74, var_73);
        object_74.finish();
    }
}

pub fn serialize_structure_hls_manifest(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::HlsManifest,
) {
    if let Some(var_75) = &input.ad_markers {
        object.key("adMarkers").string(var_75.as_str());
    }
    if input.include_iframe_only_stream {
        object
            .key("includeIframeOnlyStream")
            .boolean(input.include_iframe_only_stream);
    }
    if let Some(var_76) = &input.manifest_name {
        object.key("manifestName").string(var_76);
    }
    if input.program_date_time_interval_seconds != 0 {
        object.key("programDateTimeIntervalSeconds").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((input.program_date_time_interval_seconds).into()),
        );
    }
    if input.repeat_ext_x_key {
        object.key("repeatExtXKey").boolean(input.repeat_ext_x_key);
    }
    if let Some(var_77) = &input.stream_selection {
        let mut object_78 = object.key("streamSelection").start_object();
        crate::json_ser::serialize_structure_stream_selection(&mut object_78, var_77);
        object_78.finish();
    }
}

pub fn serialize_structure_dash_manifest(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::DashManifest,
) {
    if let Some(var_79) = &input.manifest_layout {
        object.key("manifestLayout").string(var_79.as_str());
    }
    if let Some(var_80) = &input.manifest_name {
        object.key("manifestName").string(var_80);
    }
    if input.min_buffer_time_seconds != 0 {
        object.key("minBufferTimeSeconds").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((input.min_buffer_time_seconds).into()),
        );
    }
    if let Some(var_81) = &input.profile {
        object.key("profile").string(var_81.as_str());
    }
    if let Some(var_82) = &input.stream_selection {
        let mut object_83 = object.key("streamSelection").start_object();
        crate::json_ser::serialize_structure_stream_selection(&mut object_83, var_82);
        object_83.finish();
    }
}

pub fn serialize_structure_dash_encryption(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::DashEncryption,
) {
    if let Some(var_84) = &input.speke_key_provider {
        let mut object_85 = object.key("spekeKeyProvider").start_object();
        crate::json_ser::serialize_structure_speke_key_provider(&mut object_85, var_84);
        object_85.finish();
    }
}

pub fn serialize_structure_hls_encryption(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::HlsEncryption,
) {
    if let Some(var_86) = &input.constant_initialization_vector {
        object.key("constantInitializationVector").string(var_86);
    }
    if let Some(var_87) = &input.encryption_method {
        object.key("encryptionMethod").string(var_87.as_str());
    }
    if let Some(var_88) = &input.speke_key_provider {
        let mut object_89 = object.key("spekeKeyProvider").start_object();
        crate::json_ser::serialize_structure_speke_key_provider(&mut object_89, var_88);
        object_89.finish();
    }
}

pub fn serialize_structure_mss_encryption(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::MssEncryption,
) {
    if let Some(var_90) = &input.speke_key_provider {
        let mut object_91 = object.key("spekeKeyProvider").start_object();
        crate::json_ser::serialize_structure_speke_key_provider(&mut object_91, var_90);
        object_91.finish();
    }
}

pub fn serialize_structure_mss_manifest(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::MssManifest,
) {
    if let Some(var_92) = &input.manifest_name {
        object.key("manifestName").string(var_92);
    }
    if let Some(var_93) = &input.stream_selection {
        let mut object_94 = object.key("streamSelection").start_object();
        crate::json_ser::serialize_structure_stream_selection(&mut object_94, var_93);
        object_94.finish();
    }
}

pub fn serialize_structure_speke_key_provider(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::SpekeKeyProvider,
) {
    if let Some(var_95) = &input.role_arn {
        object.key("roleArn").string(var_95);
    }
    if let Some(var_96) = &input.system_ids {
        let mut array_97 = object.key("systemIds").start_array();
        for item_98 in var_96 {
            {
                array_97.value().string(item_98);
            }
        }
        array_97.finish();
    }
    if let Some(var_99) = &input.url {
        object.key("url").string(var_99);
    }
}

pub fn serialize_structure_stream_selection(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::StreamSelection,
) {
    if input.max_video_bits_per_second != 0 {
        object.key("maxVideoBitsPerSecond").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((input.max_video_bits_per_second).into()),
        );
    }
    if input.min_video_bits_per_second != 0 {
        object.key("minVideoBitsPerSecond").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((input.min_video_bits_per_second).into()),
        );
    }
    if let Some(var_100) = &input.stream_order {
        object.key("streamOrder").string(var_100.as_str());
    }
}
