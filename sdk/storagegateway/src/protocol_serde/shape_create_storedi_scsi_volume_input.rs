// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_create_storedi_scsi_volume_input_input(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::operation::create_storedi_scsi_volume::CreateStorediScsiVolumeInput,
) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.gateway_arn {
        object.key("GatewayARN").string(var_1.as_str());
    }
    if let Some(var_2) = &input.disk_id {
        object.key("DiskId").string(var_2.as_str());
    }
    if let Some(var_3) = &input.snapshot_id {
        object.key("SnapshotId").string(var_3.as_str());
    }
    if let Some(var_4) = &input.preserve_existing_data {
        object.key("PreserveExistingData").boolean(*var_4);
    }
    if let Some(var_5) = &input.target_name {
        object.key("TargetName").string(var_5.as_str());
    }
    if let Some(var_6) = &input.network_interface_id {
        object.key("NetworkInterfaceId").string(var_6.as_str());
    }
    if let Some(var_7) = &input.kms_encrypted {
        object.key("KMSEncrypted").boolean(*var_7);
    }
    if let Some(var_8) = &input.kms_key {
        object.key("KMSKey").string(var_8.as_str());
    }
    if let Some(var_9) = &input.tags {
        let mut array_10 = object.key("Tags").start_array();
        for item_11 in var_9 {
            {
                #[allow(unused_mut)]
                let mut object_12 = array_10.value().start_object();
                crate::protocol_serde::shape_tag::ser_tag(&mut object_12, item_11)?;
                object_12.finish();
            }
        }
        array_10.finish();
    }
    Ok(())
}
