...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_db_cluster_endpoints::DescribeDBClusterEndpointsError::unhandled)?;
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_db_cluster_endpoints::de_describe_db_cluster_endpoints(_response_body, output)
            .map_err(crate::operation::describe_db_cluster_endpoints::DescribeDBClusterEndpointsError::unhandled)?;
...
pub fn de_describe_db_cluster_endpoints(
    inp: &[u8],
    mut builder: crate::operation::describe_db_cluster_endpoints::builders::DescribeDbClusterEndpointsOutputBuilder,
) -> std::result::Result<
    crate::operation::describe_db_cluster_endpoints::builders::DescribeDbClusterEndpointsOutputBuilder,
...
> {
    ...
    if !(start_el.matches("DescribeDBClusterEndpointsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeDBClusterEndpointsResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DescribeDBClusterEndpointsResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DescribeDBClusterEndpointsResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("DBClusterEndpoints") /* DBClusterEndpoints com.amazonaws.rds.synthetic#DescribeDBClusterEndpointsOutput$DBClusterEndpoints */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_db_cluster_endpoint_list::de_db_cluster_endpoint_list(&mut tag, depth + 1)
                        ?
                    )
                ;
