...
> {
    ...
    if !(start_el.matches("DescribeDBProxyTargetsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeDBProxyTargetsResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DescribeDBProxyTargetsResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DescribeDBProxyTargetsResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("Targets") /* Targets com.amazonaws.rds.synthetic#DescribeDBProxyTargetsOutput$Targets */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_target_list::de_target_list(&mut tag, depth + 1)
                        ?
                    )
                ;
