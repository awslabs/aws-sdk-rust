...
> {
    ...
    Err(match error_code {
        ...
        "VpcEncryptionControlViolationException" => {
            crate::operation::restore_db_instance_from_s3::RestoreDBInstanceFromS3Error::VpcEncryptionControlViolationException({
                #[allow(unused_mut)]
                let mut tmp = {
                    #[allow(unused_mut)]
                    let mut output = crate::types::error::builders::VpcEncryptionControlViolationExceptionBuilder::default();
                    output = crate::protocol_serde::shape_vpc_encryption_control_violation_exception::de_vpc_encryption_control_violation_exception_xml_err(_response_body, output).map_err(crate::operation::restore_db_instance_from_s3::RestoreDBInstanceFromS3Error::unhandled)?;
                    let output = output.meta(generic);
                    output.build()
                };
                if tmp.message.is_none() {
...
> {
    ...
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("RestoreDBInstanceFromS3Result")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected RestoreDBInstanceFromS3Result got {start_el:?}"
            )));
        }
