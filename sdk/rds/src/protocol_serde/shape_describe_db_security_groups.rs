...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_db_security_groups::DescribeDBSecurityGroupsError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::describe_db_security_groups::DescribeDBSecurityGroupsError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_db_security_groups::de_describe_db_security_groups(_response_body, output)
            .map_err(crate::operation::describe_db_security_groups::DescribeDBSecurityGroupsError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("DescribeDBSecurityGroupsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeDBSecurityGroupsResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DescribeDBSecurityGroupsResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DescribeDBSecurityGroupsResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("DBSecurityGroups") /* DBSecurityGroups com.amazonaws.rds.synthetic#DescribeDBSecurityGroupsOutput$DBSecurityGroups */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_db_security_groups::de_db_security_groups(&mut tag, depth + 1)
                        ?
                    )
                ;
