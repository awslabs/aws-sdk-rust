...
pub fn ser_modify_db_cluster_input_input(input: &crate::operation::modify_db_cluster::ModifyDbClusterInput) -> Result<aws_smithy_http::body::SdkBody, aws_smithy_http::operation::error::SerializationError> {
    ...
    #[allow(unused_mut)]
    let mut scope_80 = writer.prefix("EngineMode");
    if let Some(var_81) = &input.engine_mode {
        scope_80.string(var_81);
    }
    #[allow(unused_mut)]
    let mut scope_82 = writer.prefix("AllowEngineModeChange");
    if input.allow_engine_mode_change {
        scope_82.boolean(input.allow_engine_mode_change);
    }
    writer.finish();
