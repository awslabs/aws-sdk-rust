    ...
    Err(match error_code {
        ...
        "DomainNotFoundFault" => crate::error::CreateDBClusterError::DomainNotFoundFault({
            ...
            let mut tmp = {
                ...
                let _ = response;
                output = crate::protocol_serde::shape_domain_not_found_fault::de_domain_not_found_fault_xml_err(response.body().as_ref(), output).map_err(crate::error::CreateDBClusterError::unhandled)?;
                let output = output.meta(generic);
                output.build()
            };
            if tmp.message.is_none() {
