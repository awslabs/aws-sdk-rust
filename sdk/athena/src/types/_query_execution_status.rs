...
pub struct QueryExecutionStatus {
    ...
    /// <p>For queries that experience certain transient errors, the state transitions from <code>RUNNING</code> back to <code>QUEUED</code>. The <code>FAILED</code> state is always terminal with no automatic retry.</p>
    /// </note>
