...
pub(crate) fn de_vector_enrichment_job_input_config<'a, I>(
    ...
    _value: &'a [u8],
    depth: u32,
) -> ::std::result::Result<Option<crate::types::VectorEnrichmentJobInputConfig>, ::aws_smithy_json::deserialize::error::DeserializeError>
where
                        ...
                        "DocumentType" => {
                            builder = builder.set_document_type(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::VectorEnrichmentJobDocumentType::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "DataSourceConfig" => {
                            builder = builder.set_data_source_config(
                                    crate::protocol_serde::shape_vector_enrichment_job_data_source_config_input::de_vector_enrichment_job_data_source_config_input(tokens, _value, depth + 1)?
                                );
                        }
