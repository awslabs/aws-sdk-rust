// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_list_vector_enrichment_jobs_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::ListVectorEnrichmentJobsInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.max_results {
        object.key("MaxResults").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_1).into()));
    }
    if let Some(var_2) = &input.next_token {
        object.key("NextToken").string(var_2.as_str());
    }
    if let Some(var_3) = &input.sort_by {
        object.key("SortBy").string(var_3.as_str());
    }
    if let Some(var_4) = &input.sort_order {
        object.key("SortOrder").string(var_4.as_str());
    }
    if let Some(var_5) = &input.status_equals {
        object.key("StatusEquals").string(var_5.as_str());
    }
    Ok(())
}

