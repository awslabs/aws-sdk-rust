// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_canary_code_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::model::CanaryCodeInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.s3_bucket {
        object.key("S3Bucket").string(var_1.as_str());
    }
    if let Some(var_2) = &input.s3_key {
        object.key("S3Key").string(var_2.as_str());
    }
    if let Some(var_3) = &input.s3_version {
        object.key("S3Version").string(var_3.as_str());
    }
    if let Some(var_4) = &input.zip_file {
        object.key("ZipFile").string_unchecked(&aws_smithy_types::base64::encode(var_4));
    }
    if let Some(var_5) = &input.handler {
        object.key("Handler").string(var_5.as_str());
    }
    Ok(())
}

