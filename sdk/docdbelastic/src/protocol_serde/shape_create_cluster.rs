            ...
            let mut tmp = {
                ...
                output = output.set_retry_after_seconds(
                    crate::protocol_serde::shape_throttling_exception::de_retry_after_seconds_header(_response_headers).map_err(|_| {
                        crate::operation::create_cluster::CreateClusterError::unhandled("Failed to parse retryAfterSeconds from header `Retry-After")
                    })?,
                );
                let output = output.meta(generic);
...
pub fn de_create_cluster_http_response(
    ...
    Ok({
        ...
        crate::serde_util::create_cluster_output_correct_errors(output).build()
    })
}
