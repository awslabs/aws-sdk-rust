// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_write_request(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::model::WriteRequest) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.put_request {
        #[allow(unused_mut)]
        let mut object_2 = object.key("PutRequest").start_object();
        crate::protocol_serde::shape_put_request::ser_put_request(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.delete_request {
        #[allow(unused_mut)]
        let mut object_4 = object.key("DeleteRequest").start_object();
        crate::protocol_serde::shape_delete_request::ser_delete_request(&mut object_4, var_3)?;
        object_4.finish();
    }
    Ok(())
}

pub(crate) fn de_write_request<'a, I>(tokens: &mut std::iter::Peekable<I>) -> Result<Option<crate::model::WriteRequest>, aws_smithy_json::deserialize::error::DeserializeError>
                    where I: Iterator<Item = Result<aws_smithy_json::deserialize::Token<'a>, aws_smithy_json::deserialize::error::DeserializeError>> {
    match tokens.next().transpose()? {
        Some(aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        Some(aws_smithy_json::deserialize::Token::StartObject { .. }) => {
            #[allow(unused_mut)]
            let mut builder = crate::model::write_request::Builder::default();
            loop {
                match tokens.next().transpose()? {
                    Some(aws_smithy_json::deserialize::Token::EndObject { .. }) => break,
                                            Some(aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                        match key.to_unescaped()?.as_ref() {
                            "PutRequest" => {
                                builder = builder.set_put_request(
                                    crate::protocol_serde::shape_put_request::de_put_request(tokens)?
                                );
                            }
                            "DeleteRequest" => {
                                builder = builder.set_delete_request(
                                    crate::protocol_serde::shape_delete_request::de_delete_request(tokens)?
                                );
                            }
                            _ => aws_smithy_json::deserialize::token::skip_value(tokens)?
                        }
                    }
                    other => return Err(aws_smithy_json::deserialize::error::DeserializeError::custom(format!("expected object key or end object, found: {:?}", other)))
                }
            }
            Ok(Some(builder.build()))
        }
        _ => {
            Err(aws_smithy_json::deserialize::error::DeserializeError::custom("expected start object or null"))
        }
    }
}

