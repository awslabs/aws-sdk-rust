...
impl ::aws_smithy_runtime_api::client::ser_de::DeserializeResponse for BatchExecuteStatementResponseDeserializer {
    fn deserialize_nonstreaming_with_config(
        ...
        let parse_result = if !success && status != 200 || force_error {
            crate::protocol_serde::shape_batch_execute_statement::de_batch_execute_statement_http_error(status, headers, body, _cfg)
        } else {
            crate::protocol_serde::shape_batch_execute_statement::de_batch_execute_statement_http_response(status, headers, body, _cfg)
        };
...
impl BatchExecuteStatementError {
    ...
    /// Returns `true` if the error kind is `BatchExecuteStatementError::ThrottlingException`.
