...
/// <p>Hourly granularity is only available for EC2-Instances (Elastic Compute Cloud) resource-level data. All other resource-level data is available at daily granularity.</p><note>
...
impl GetCostAndUsageWithResourcesFluentBuilder {
    /// Creates a new `GetCostAndUsageWithResourcesFluentBuilder`.
