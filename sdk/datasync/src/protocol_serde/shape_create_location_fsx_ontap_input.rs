// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_create_location_fsx_ontap_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::CreateLocationFsxOntapInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.protocol {
        #[allow(unused_mut)]
        let mut object_2 = object.key("Protocol").start_object();
        crate::protocol_serde::shape_fsx_protocol::ser_fsx_protocol(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.security_group_arns {
        let mut array_4 = object.key("SecurityGroupArns").start_array();
        for item_5 in var_3 {
             {
                array_4.value().string(item_5.as_str());
            }
        }
        array_4.finish();
    }
    if let Some(var_6) = &input.storage_virtual_machine_arn {
        object.key("StorageVirtualMachineArn").string(var_6.as_str());
    }
    if let Some(var_7) = &input.subdirectory {
        object.key("Subdirectory").string(var_7.as_str());
    }
    if let Some(var_8) = &input.tags {
        let mut array_9 = object.key("Tags").start_array();
        for item_10 in var_8 {
             {
                #[allow(unused_mut)]
                let mut object_11 = array_9.value().start_object();
                crate::protocol_serde::shape_tag_list_entry::ser_tag_list_entry(&mut object_11, item_10)?;
                object_11.finish();
            }
        }
        array_9.finish();
    }
    Ok(())
}

