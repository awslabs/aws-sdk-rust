...
impl super::Client {
    ...
    ///   - [`filters(LocationFilter)`](crate::operation::list_locations::builders::ListLocationsFluentBuilder::filters) / [`set_filters(Option<Vec<LocationFilter>>)`](crate::operation::list_locations::builders::ListLocationsFluentBuilder::set_filters): <p>You can use API filters to narrow down the list of resources returned by <code>ListLocations</code>. For example, to retrieve all tasks on a specific source location, you can use <code>ListLocations</code> with filter name <code>LocationType S3</code> and <code>Operator Equals</code>.</p>
