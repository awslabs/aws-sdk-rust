...
impl super::Client {
    ...
    ///   - [`filters(TaskFilter)`](crate::operation::list_tasks::builders::ListTasksFluentBuilder::filters) / [`set_filters(Option<Vec<TaskFilter>>)`](crate::operation::list_tasks::builders::ListTasksFluentBuilder::set_filters): <p>You can use API filters to narrow down the list of resources returned by <code>ListTasks</code>. For example, to retrieve all tasks on a specific source location, you can use <code>ListTasks</code> with filter name <code>LocationId</code> and <code>Operator Equals</code> with the ARN for the location.</p>
