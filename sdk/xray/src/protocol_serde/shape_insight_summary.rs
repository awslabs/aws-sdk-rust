            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "RootCauseServiceId" => {
                            builder = builder.set_root_cause_service_id(crate::protocol_serde::shape_service_id::de_service_id(tokens)?);
                        }
                        "Categories" => {
                            builder = builder.set_categories(crate::protocol_serde::shape_insight_category_list::de_insight_category_list(tokens)?);
                        }
                        "State" => {
                            builder = builder.set_state(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::InsightState::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "StartTime" => {
                        ...
                        "TopAnomalousServices" => {
                            builder = builder
                                .set_top_anomalous_services(crate::protocol_serde::shape_anomalous_service_list::de_anomalous_service_list(tokens)?);
                        }
                        "LastUpdateTime" => {
