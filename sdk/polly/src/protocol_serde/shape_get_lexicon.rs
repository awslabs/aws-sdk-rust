    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "Lexicon" => {
                    builder = builder.set_lexicon(crate::protocol_serde::shape_lexicon::de_lexicon(tokens, _value, depth + 1)?);
                }
                "LexiconAttributes" => {
                    builder = builder.set_lexicon_attributes(crate::protocol_serde::shape_lexicon_attributes::de_lexicon_attributes(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
