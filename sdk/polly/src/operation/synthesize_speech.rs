...
impl aws_smithy_http::response::ParseHttpResponse for SynthesizeSpeech {
                ...
                fn parse_unloaded(&self, response: &mut aws_smithy_http::operation::Response) -> Option<Self::Output> {
                    ...
                    Some(crate::protocol_serde::shape_synthesize_speech::de_synthesize_speech_http_response_(response))
                }
                fn parse_loaded(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
                    // if streaming, we only hit this case if its an error
                    crate::protocol_serde::shape_synthesize_speech::de_synthesize_speech_http_error(response.status().as_u16(), response.headers(), response.body().as_ref())
                }
            }
