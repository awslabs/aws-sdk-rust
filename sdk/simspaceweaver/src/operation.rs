...
impl aws_smithy_http::response::ParseStrictResponse for DeleteSimulation {
                ...
                fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
                     tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
                     if !response.status().is_success() && response.status().as_u16() != 200 {
                        crate::protocol_serde::shape_delete_simulation::de_delete_simulation_http_error(response)
                     } else {
                        crate::protocol_serde::shape_delete_simulation::de_delete_simulation_http_response(response)
                     }
                }
            }
...
impl aws_smithy_http::response::ParseStrictResponse for StartSimulation {
                ...
                fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
                     ...
                     if !response.status().is_success() && response.status().as_u16() != 200 {
                        ...
                        crate::protocol_serde::shape_start_simulation::de_start_simulation_http_response(response)
                     }
                }
            }
