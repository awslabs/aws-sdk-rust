    ...
    if let Some(inner_11) = &input.request_headers {
        for (k, v) in inner_11 {
                            ...
                            let header_name = http::header::HeaderName::from_str(&format!("{}{}", "x-amzn-dataexchange-header-", &k)).map_err(|err| {
                                aws_smithy_http::operation::BuildError::InvalidField { field: "request_headers", details: format!("`{}` cannot be used as a header name: {}", k, err)}
                            })?;
...
pub fn deser_prefix_header_send_api_asset_send_api_asset_output_response_headers(header_map: &http::HeaderMap) -> std::result::Result<std::option::Option<std::collections::HashMap<std::string::String, std::string::String>>, aws_smithy_http::header::ParseError> {
