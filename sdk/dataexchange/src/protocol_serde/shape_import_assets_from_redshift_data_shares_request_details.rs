// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_import_assets_from_redshift_data_shares_request_details(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::model::ImportAssetsFromRedshiftDataSharesRequestDetails) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.asset_sources {
        let mut array_2 = object.key("AssetSources").start_array();
        for item_3 in var_1 {
             {
                #[allow(unused_mut)]
                let mut object_4 = array_2.value().start_object();
                crate::protocol_serde::shape_redshift_data_share_asset_source_entry::ser_redshift_data_share_asset_source_entry(&mut object_4, item_3)?;
                object_4.finish();
            }
        }
        array_2.finish();
    }
    if let Some(var_5) = &input.data_set_id {
        object.key("DataSetId").string(var_5.as_str());
    }
    if let Some(var_6) = &input.revision_id {
        object.key("RevisionId").string(var_6.as_str());
    }
    Ok(())
}

