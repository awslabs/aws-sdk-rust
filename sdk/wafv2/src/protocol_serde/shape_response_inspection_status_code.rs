                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "SuccessCodes" => {
                            builder = builder.set_success_codes(
                                    crate::protocol_serde::shape_response_inspection_status_code_success_codes::de_response_inspection_status_code_success_codes(tokens, _value, depth + 1)?
                                );
                        }
                        "FailureCodes" => {
                            builder = builder.set_failure_codes(
                                    crate::protocol_serde::shape_response_inspection_status_code_failure_codes::de_response_inspection_status_code_failure_codes(tokens, _value, depth + 1)?
                                );
                        }
