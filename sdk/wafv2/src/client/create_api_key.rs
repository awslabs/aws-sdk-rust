...
impl super::Client {
    ...
    ///   - [`token_domains(impl Into<String>)`](crate::operation::create_api_key::builders::CreateAPIKeyFluentBuilder::token_domains) / [`set_token_domains(Option<Vec<String>>)`](crate::operation::create_api_key::builders::CreateAPIKeyFluentBuilder::set_token_domains): <p>The client application domains that you want to use this API key for. </p>  <p>Example JSON: <code>"TokenDomains": ["abc.com", "store.abc.com"]</code> </p>  <p>Public suffixes aren't allowed. For example, you can't use <code>usa.gov</code> or <code>co.uk</code> as token domains.</p>
