...
impl aws_smithy_http::response::ParseStrictResponse for GetAccountAlias {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_get_account_alias::de_get_account_alias_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_get_account_alias::de_get_account_alias_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for PutAccountAlias {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_put_account_alias::de_put_account_alias_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_put_account_alias::de_put_account_alias_http_response(
                response,
            )
        }
    }
