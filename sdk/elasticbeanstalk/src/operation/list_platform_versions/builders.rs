...
impl crate::operation::list_platform_versions::builders::ListPlatformVersionsInputBuilder {
...
///
/// <p>Lists the platform versions available for your account in an Amazon Web Services Region. Provides summary information about each platform version. Compare to <code>DescribePlatformVersion</code>, which provides full details about a single platform version.</p>
/// <p>This action only returns information about platform versions that the calling principle has IAM permissions to access. For example, consider a case where a user only has permission to access one of ten platform versions. When the user calls the <i>ListPlatformVersions</i> action, the response will only include the one platform version that the user has permission to access instead of all ten platform versions. If the user doesn’t have access to any of the platform versions an empty result is returned.</p><note>
...
impl ListPlatformVersionsFluentBuilder {
    /// Creates a new `ListPlatformVersionsFluentBuilder`.
