...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_compose_environments::de_compose_environments(_response_body, output)
            .map_err(crate::operation::compose_environments::ComposeEnvironmentsError::unhandled)?;
...
) -> std::result::Result<crate::operation::compose_environments::builders::ComposeEnvironmentsOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError>
{
    ...
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("ComposeEnvironmentsResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected ComposeEnvironmentsResult got {start_el:?}"
            )));
        }
