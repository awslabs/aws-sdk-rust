...
pub fn de_application_resource_lifecycle_config(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::ApplicationResourceLifecycleConfig, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("VersionLifecycleConfig") /* VersionLifecycleConfig com.amazonaws.elasticbeanstalk#ApplicationResourceLifecycleConfig$VersionLifecycleConfig */ =>  {
                let var_6 =
                    Some(
                        crate::protocol_serde::shape_application_version_lifecycle_config::de_application_version_lifecycle_config(&mut tag, depth + 1)
                        ?
                    )
                ;
