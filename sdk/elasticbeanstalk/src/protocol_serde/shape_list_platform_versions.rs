...
> {
    ...
    Err(match error_code {
        "ElasticBeanstalkServiceException" => {
            crate::operation::list_platform_versions::ListPlatformVersionsError::ElasticBeanstalkServiceException({
                #[allow(unused_mut)]
                let mut tmp = {
                    #[allow(unused_mut)]
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_platform_versions::de_list_platform_versions(_response_body, output)
            .map_err(crate::operation::list_platform_versions::ListPlatformVersionsError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("ListPlatformVersionsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected ListPlatformVersionsResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("ListPlatformVersionsResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected ListPlatformVersionsResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("PlatformSummaryList") /* PlatformSummaryList com.amazonaws.elasticbeanstalk.synthetic#ListPlatformVersionsOutput$PlatformSummaryList */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_platform_summary_list::de_platform_summary_list(&mut tag, depth + 1)
                        ?
                    )
                ;
