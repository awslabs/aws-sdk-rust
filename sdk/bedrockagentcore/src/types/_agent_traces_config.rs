// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>The configuration specifying where to read agent traces from for recommendation analysis.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq)]
pub enum AgentTracesConfig {
    /// <p>Agent traces read from CloudWatch Logs.</p>
    CloudwatchLogs(crate::types::CloudWatchLogsTraceConfig),
    /// <p>Agent traces provided as inline session spans in OpenTelemetry format.</p>
    SessionSpans(::std::vec::Vec<::aws_smithy_types::Document>),
    /// The `Unknown` variant represents cases where new union variant was received. Consider upgrading the SDK to the latest available version.
    /// An unknown enum variant
    ///
    /// _Note: If you encounter this error, consider upgrading your SDK to the latest version._
    /// The `Unknown` variant represents cases where the server sent a value that wasn't recognized
    /// by the client. This can happen when the server adds new functionality, but the client has not been updated.
    /// To investigate this, consider turning on debug logging to print the raw HTTP response.
    #[non_exhaustive]
    Unknown,
}
impl AgentTracesConfig {
    /// Tries to convert the enum instance into [`CloudwatchLogs`](crate::types::AgentTracesConfig::CloudwatchLogs), extracting the inner [`CloudWatchLogsTraceConfig`](crate::types::CloudWatchLogsTraceConfig).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_cloudwatch_logs(&self) -> ::std::result::Result<&crate::types::CloudWatchLogsTraceConfig, &Self> {
        if let AgentTracesConfig::CloudwatchLogs(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`CloudwatchLogs`](crate::types::AgentTracesConfig::CloudwatchLogs).
    pub fn is_cloudwatch_logs(&self) -> bool {
        self.as_cloudwatch_logs().is_ok()
    }
    /// Tries to convert the enum instance into [`SessionSpans`](crate::types::AgentTracesConfig::SessionSpans), extracting the inner [`Vec`](::std::vec::Vec).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_session_spans(&self) -> ::std::result::Result<&::std::vec::Vec<::aws_smithy_types::Document>, &Self> {
        if let AgentTracesConfig::SessionSpans(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`SessionSpans`](crate::types::AgentTracesConfig::SessionSpans).
    pub fn is_session_spans(&self) -> bool {
        self.as_session_spans().is_ok()
    }
    /// Returns true if the enum instance is the `Unknown` variant.
    pub fn is_unknown(&self) -> bool {
        matches!(self, Self::Unknown)
    }
}
impl ::std::fmt::Debug for AgentTracesConfig {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        match self {
            AgentTracesConfig::CloudwatchLogs(val) => f.debug_tuple("CloudwatchLogs").field(&val).finish(),
            AgentTracesConfig::SessionSpans(_) => f.debug_tuple("*** Sensitive Data Redacted ***").finish(),
            AgentTracesConfig::Unknown => f.debug_tuple("Unknown").finish(),
        }
    }
}
