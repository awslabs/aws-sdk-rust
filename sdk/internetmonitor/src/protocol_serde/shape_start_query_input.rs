...
pub fn ser_start_query_input_input(
    ...
    if let Some(var_2) = &input.filter_parameters {
        let mut array_3 = object.key("FilterParameters").start_array();
        for item_4 in var_2 {
             {
                #[allow(unused_mut)]
                let mut object_5 = array_3.value().start_object();
                crate::protocol_serde::shape_filter_parameter::ser_filter_parameter(&mut object_5, item_4)?;
                object_5.finish();
            }
        }
        array_3.finish();
    ...
    if let Some(var_7) = &input.start_time {
        object.key("StartTime").date_time(var_7, ::aws_smithy_types::date_time::Format::DateTime)?;
    }
    Ok(())
}
