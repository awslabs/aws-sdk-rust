            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_ground_stations_output_next_token(resp);
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
                                && new_token == input.next_token.as_ref()
                                && self.stop_on_duplicate_token
...
impl ListContactsPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_contacts_output_contact_list(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
...
impl ListDataflowEndpointGroupsPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_dataflow_endpoint_groups_output_dataflow_endpoint_group_list(
                page,
            )
            .unwrap_or_default()
            .into_iter()
...
impl ListGroundStationsPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_ground_stations_output_ground_station_list(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
