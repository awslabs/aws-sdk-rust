// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct ListAntennasInput {
    /// <p>ID of a ground station.</p>
    pub ground_station_id: ::std::option::Option<::std::string::String>,
    /// <p>Maximum number of antennas returned.</p>
    pub max_results: ::std::option::Option<i32>,
    /// <p>Next token returned in the request of a previous <code>ListAntennas</code> call. Used to get the next page of results.</p>
    pub next_token: ::std::option::Option<::std::string::String>,
}
impl ListAntennasInput {
    /// <p>ID of a ground station.</p>
    pub fn ground_station_id(&self) -> ::std::option::Option<&str> {
        self.ground_station_id.as_deref()
    }
    /// <p>Maximum number of antennas returned.</p>
    pub fn max_results(&self) -> ::std::option::Option<i32> {
        self.max_results
    }
    /// <p>Next token returned in the request of a previous <code>ListAntennas</code> call. Used to get the next page of results.</p>
    pub fn next_token(&self) -> ::std::option::Option<&str> {
        self.next_token.as_deref()
    }
}
impl ListAntennasInput {
    /// Creates a new builder-style object to manufacture [`ListAntennasInput`](crate::operation::list_antennas::ListAntennasInput).
    pub fn builder() -> crate::operation::list_antennas::builders::ListAntennasInputBuilder {
        crate::operation::list_antennas::builders::ListAntennasInputBuilder::default()
    }
}

/// A builder for [`ListAntennasInput`](crate::operation::list_antennas::ListAntennasInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct ListAntennasInputBuilder {
    pub(crate) ground_station_id: ::std::option::Option<::std::string::String>,
    pub(crate) max_results: ::std::option::Option<i32>,
    pub(crate) next_token: ::std::option::Option<::std::string::String>,
}
impl ListAntennasInputBuilder {
    /// <p>ID of a ground station.</p>
    /// This field is required.
    pub fn ground_station_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.ground_station_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>ID of a ground station.</p>
    pub fn set_ground_station_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.ground_station_id = input;
        self
    }
    /// <p>ID of a ground station.</p>
    pub fn get_ground_station_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.ground_station_id
    }
    /// <p>Maximum number of antennas returned.</p>
    pub fn max_results(mut self, input: i32) -> Self {
        self.max_results = ::std::option::Option::Some(input);
        self
    }
    /// <p>Maximum number of antennas returned.</p>
    pub fn set_max_results(mut self, input: ::std::option::Option<i32>) -> Self {
        self.max_results = input;
        self
    }
    /// <p>Maximum number of antennas returned.</p>
    pub fn get_max_results(&self) -> &::std::option::Option<i32> {
        &self.max_results
    }
    /// <p>Next token returned in the request of a previous <code>ListAntennas</code> call. Used to get the next page of results.</p>
    pub fn next_token(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.next_token = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>Next token returned in the request of a previous <code>ListAntennas</code> call. Used to get the next page of results.</p>
    pub fn set_next_token(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.next_token = input;
        self
    }
    /// <p>Next token returned in the request of a previous <code>ListAntennas</code> call. Used to get the next page of results.</p>
    pub fn get_next_token(&self) -> &::std::option::Option<::std::string::String> {
        &self.next_token
    }
    /// Consumes the builder and constructs a [`ListAntennasInput`](crate::operation::list_antennas::ListAntennasInput).
    pub fn build(
        self,
    ) -> ::std::result::Result<crate::operation::list_antennas::ListAntennasInput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(crate::operation::list_antennas::ListAntennasInput {
            ground_station_id: self.ground_station_id,
            max_results: self.max_results,
            next_token: self.next_token,
        })
    }
}
