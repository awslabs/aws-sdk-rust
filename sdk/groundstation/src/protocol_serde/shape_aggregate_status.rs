// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_aggregate_status(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::types::AggregateStatus) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.status {
        object.key("status").string(var_1.as_str());
    }
    if let Some(var_2) = &input.signature_map {
        #[allow(unused_mut)]
        let mut object_3 = object.key("signatureMap").start_object();
        for (key_4, value_5) in var_2 {
             {
                object_3.key(key_4.as_str()).boolean(*value_5);
            }
        }
        object_3.finish();
    }
    Ok(())
}

