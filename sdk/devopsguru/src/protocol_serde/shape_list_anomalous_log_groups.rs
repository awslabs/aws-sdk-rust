...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_anomalous_log_groups::ListAnomalousLogGroupsError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        crate::serde_util::list_anomalous_log_groups_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_anomalous_log_groups::ListAnomalousLogGroupsError::unhandled)?
    })
}
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "AnomalousLogGroups" => {
                    builder = builder.set_anomalous_log_groups(crate::protocol_serde::shape_anomalous_log_groups::de_anomalous_log_groups(tokens)?);
                }
                "InsightId" => {
