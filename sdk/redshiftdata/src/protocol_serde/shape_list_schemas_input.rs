// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_list_schemas_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::ListSchemasInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.cluster_identifier {
        object.key("ClusterIdentifier").string(var_1.as_str());
    }
    if let Some(var_2) = &input.secret_arn {
        object.key("SecretArn").string(var_2.as_str());
    }
    if let Some(var_3) = &input.db_user {
        object.key("DbUser").string(var_3.as_str());
    }
    if let Some(var_4) = &input.database {
        object.key("Database").string(var_4.as_str());
    }
    if let Some(var_5) = &input.connected_database {
        object.key("ConnectedDatabase").string(var_5.as_str());
    }
    if let Some(var_6) = &input.schema_pattern {
        object.key("SchemaPattern").string(var_6.as_str());
    }
    if let Some(var_7) = &input.next_token {
        object.key("NextToken").string(var_7.as_str());
    }
    if input.max_results != 0 {
        object.key("MaxResults").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((input.max_results).into()));
    }
    if let Some(var_8) = &input.workgroup_name {
        object.key("WorkgroupName").string(var_8.as_str());
    }
    Ok(())
}

