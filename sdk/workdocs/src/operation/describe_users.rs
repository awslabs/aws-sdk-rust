...
impl DescribeUsersInput {
    ...
    pub async fn make_operation(&self, _config: &crate::config::Config) -> std::result::Result<aws_smithy_http::operation::Operation<crate::operation::describe_users::DescribeUsers, aws_http::retry::AwsResponseRetryClassifier>, aws_smithy_http::operation::error::BuildError> {
        ...
        let mut request = {
            ...
            fn uri_query(_input: &crate::operation::describe_users::DescribeUsersInput, mut output: &mut String) -> Result<(), aws_smithy_http::operation::error::BuildError> {
                let mut query = aws_smithy_http::query::Writer::new(&mut output);
                if let Some(inner_1) = &_input.organization_id {
                     {
                        query.push_kv("organizationId", &aws_smithy_http::query::fmt_string(&inner_1));
                    }
                }
                if let Some(inner_2) = &_input.user_ids {
                     {
                        query.push_kv("userIds", &aws_smithy_http::query::fmt_string(&inner_2));
                    }
                }
                if let Some(inner_3) = &_input.query {
                     {
                        query.push_kv("query", &aws_smithy_http::query::fmt_string(&inner_3));
                    }
                }
                if let Some(inner_4) = &_input.include {
                     {
                ...
                if let Some(inner_6) = &_input.sort {
                     {
                        query.push_kv("sort", &aws_smithy_http::query::fmt_string(&inner_6));
                    }
                }
                if let Some(inner_7) = &_input.marker {
                     {
                ...
                if let Some(inner_9) = &_input.fields {
                     {
