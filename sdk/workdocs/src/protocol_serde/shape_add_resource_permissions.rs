...
> {
    ...
    let error_code =
        match generic.code() {
            Some(code) => code,
            None => return Err(
                crate::operation::add_resource_permissions::AddResourcePermissionsError::unhandled(
                    generic,
                ),
            ),
        };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_add_resource_permissions_http_response_with_props(
    _response_status: u16,
    _response_headers: &::http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_add_resource_permissions::de_add_resource_permissions(
                _response_body,
                output,
            )
            .map_err(
                ...
                match key.to_unescaped()?.as_ref() {
                    "ShareResults" => {
                        builder = builder.set_share_results(
                            crate::protocol_serde::shape_share_results_list::de_share_results_list(
                                tokens,
                            )?,
                        );
                    }
