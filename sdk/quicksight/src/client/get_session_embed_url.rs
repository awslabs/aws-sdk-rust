...
impl super::Client {
    ...
    ///   - [`user_arn(impl ::std::convert::Into<String>)`](crate::operation::get_session_embed_url::builders::GetSessionEmbedUrlFluentBuilder::user_arn) / [`set_user_arn(Option<String>)`](crate::operation::get_session_embed_url::builders::GetSessionEmbedUrlFluentBuilder::set_user_arn): <p>The Amazon QuickSight user's Amazon Resource Name (ARN), for use with <code>QUICKSIGHT</code> identity type. You can use this for any type of Amazon QuickSight users in your account (readers, authors, or admins). They need to be authenticated as one of the following:</p>  <ol>   <li> <p>Active Directory (AD) users or group members</p> </li>   <li> <p>Invited nonfederated users</p> </li>   <li> <p>IAM users and IAM role-based sessions authenticated through Federated Single Sign-On using SAML, OpenID Connect, or IAM federation</p> </li>  </ol>  <p>Omit this parameter for users in the third group, IAM users and IAM role-based sessions.</p>
