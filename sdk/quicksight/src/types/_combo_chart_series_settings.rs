// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Options that determine the presentation of a series in the visual.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct ComboChartSeriesSettings {
    /// <p>Line styles options for the line series in the visual.</p>
    pub line_style_settings: ::std::option::Option<crate::types::LineChartLineStyleSettings>,
    /// <p>Marker styles options for the line series in the visual.</p>
    pub marker_style_settings: ::std::option::Option<crate::types::LineChartMarkerStyleSettings>,
    /// <p>Decal settings for the series in the visual.</p>
    pub decal_settings: ::std::option::Option<crate::types::DecalSettings>,
    /// <p>Border settings for the bar series in the visual.</p>
    pub border_settings: ::std::option::Option<crate::types::BorderSettings>,
}
impl ComboChartSeriesSettings {
    /// <p>Line styles options for the line series in the visual.</p>
    pub fn line_style_settings(&self) -> ::std::option::Option<&crate::types::LineChartLineStyleSettings> {
        self.line_style_settings.as_ref()
    }
    /// <p>Marker styles options for the line series in the visual.</p>
    pub fn marker_style_settings(&self) -> ::std::option::Option<&crate::types::LineChartMarkerStyleSettings> {
        self.marker_style_settings.as_ref()
    }
    /// <p>Decal settings for the series in the visual.</p>
    pub fn decal_settings(&self) -> ::std::option::Option<&crate::types::DecalSettings> {
        self.decal_settings.as_ref()
    }
    /// <p>Border settings for the bar series in the visual.</p>
    pub fn border_settings(&self) -> ::std::option::Option<&crate::types::BorderSettings> {
        self.border_settings.as_ref()
    }
}
impl ComboChartSeriesSettings {
    /// Creates a new builder-style object to manufacture [`ComboChartSeriesSettings`](crate::types::ComboChartSeriesSettings).
    pub fn builder() -> crate::types::builders::ComboChartSeriesSettingsBuilder {
        crate::types::builders::ComboChartSeriesSettingsBuilder::default()
    }
}

/// A builder for [`ComboChartSeriesSettings`](crate::types::ComboChartSeriesSettings).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct ComboChartSeriesSettingsBuilder {
    pub(crate) line_style_settings: ::std::option::Option<crate::types::LineChartLineStyleSettings>,
    pub(crate) marker_style_settings: ::std::option::Option<crate::types::LineChartMarkerStyleSettings>,
    pub(crate) decal_settings: ::std::option::Option<crate::types::DecalSettings>,
    pub(crate) border_settings: ::std::option::Option<crate::types::BorderSettings>,
}
impl ComboChartSeriesSettingsBuilder {
    /// <p>Line styles options for the line series in the visual.</p>
    pub fn line_style_settings(mut self, input: crate::types::LineChartLineStyleSettings) -> Self {
        self.line_style_settings = ::std::option::Option::Some(input);
        self
    }
    /// <p>Line styles options for the line series in the visual.</p>
    pub fn set_line_style_settings(mut self, input: ::std::option::Option<crate::types::LineChartLineStyleSettings>) -> Self {
        self.line_style_settings = input;
        self
    }
    /// <p>Line styles options for the line series in the visual.</p>
    pub fn get_line_style_settings(&self) -> &::std::option::Option<crate::types::LineChartLineStyleSettings> {
        &self.line_style_settings
    }
    /// <p>Marker styles options for the line series in the visual.</p>
    pub fn marker_style_settings(mut self, input: crate::types::LineChartMarkerStyleSettings) -> Self {
        self.marker_style_settings = ::std::option::Option::Some(input);
        self
    }
    /// <p>Marker styles options for the line series in the visual.</p>
    pub fn set_marker_style_settings(mut self, input: ::std::option::Option<crate::types::LineChartMarkerStyleSettings>) -> Self {
        self.marker_style_settings = input;
        self
    }
    /// <p>Marker styles options for the line series in the visual.</p>
    pub fn get_marker_style_settings(&self) -> &::std::option::Option<crate::types::LineChartMarkerStyleSettings> {
        &self.marker_style_settings
    }
    /// <p>Decal settings for the series in the visual.</p>
    pub fn decal_settings(mut self, input: crate::types::DecalSettings) -> Self {
        self.decal_settings = ::std::option::Option::Some(input);
        self
    }
    /// <p>Decal settings for the series in the visual.</p>
    pub fn set_decal_settings(mut self, input: ::std::option::Option<crate::types::DecalSettings>) -> Self {
        self.decal_settings = input;
        self
    }
    /// <p>Decal settings for the series in the visual.</p>
    pub fn get_decal_settings(&self) -> &::std::option::Option<crate::types::DecalSettings> {
        &self.decal_settings
    }
    /// <p>Border settings for the bar series in the visual.</p>
    pub fn border_settings(mut self, input: crate::types::BorderSettings) -> Self {
        self.border_settings = ::std::option::Option::Some(input);
        self
    }
    /// <p>Border settings for the bar series in the visual.</p>
    pub fn set_border_settings(mut self, input: ::std::option::Option<crate::types::BorderSettings>) -> Self {
        self.border_settings = input;
        self
    }
    /// <p>Border settings for the bar series in the visual.</p>
    pub fn get_border_settings(&self) -> &::std::option::Option<crate::types::BorderSettings> {
        &self.border_settings
    }
    /// Consumes the builder and constructs a [`ComboChartSeriesSettings`](crate::types::ComboChartSeriesSettings).
    pub fn build(self) -> crate::types::ComboChartSeriesSettings {
        crate::types::ComboChartSeriesSettings {
            line_style_settings: self.line_style_settings,
            marker_style_settings: self.marker_style_settings,
            decal_settings: self.decal_settings,
            border_settings: self.border_settings,
        }
    }
}
