// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct DeleteRoleMembershipInput  {
    /// <p>The name of the group.</p>
    pub member_name: ::std::option::Option<::std::string::String>,
    /// <p>The role that you want to remove permissions from.</p>
    pub role: ::std::option::Option<crate::types::Role>,
    /// <p>The ID for the Amazon Web Services account that you want to create a group in. The Amazon Web Services account ID that you provide must be the same Amazon Web Services account that contains your Amazon QuickSight account.</p>
    pub aws_account_id: ::std::option::Option<::std::string::String>,
    /// <p>The namespace that contains the role.</p>
    pub namespace: ::std::option::Option<::std::string::String>,
}
impl  DeleteRoleMembershipInput  {
    /// <p>The name of the group.</p>
    pub fn member_name(&self) -> ::std::option::Option<& str> {
        self.member_name.as_deref()
    }
    /// <p>The role that you want to remove permissions from.</p>
    pub fn role(&self) -> ::std::option::Option<& crate::types::Role> {
        self.role.as_ref()
    }
    /// <p>The ID for the Amazon Web Services account that you want to create a group in. The Amazon Web Services account ID that you provide must be the same Amazon Web Services account that contains your Amazon QuickSight account.</p>
    pub fn aws_account_id(&self) -> ::std::option::Option<& str> {
        self.aws_account_id.as_deref()
    }
    /// <p>The namespace that contains the role.</p>
    pub fn namespace(&self) -> ::std::option::Option<& str> {
        self.namespace.as_deref()
    }
}
impl DeleteRoleMembershipInput {
    /// Creates a new builder-style object to manufacture [`DeleteRoleMembershipInput`](crate::operation::delete_role_membership::DeleteRoleMembershipInput).
    pub fn builder() -> crate::operation::delete_role_membership::builders::DeleteRoleMembershipInputBuilder {
        crate::operation::delete_role_membership::builders::DeleteRoleMembershipInputBuilder::default()
    }
}

/// A builder for [`DeleteRoleMembershipInput`](crate::operation::delete_role_membership::DeleteRoleMembershipInput).
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
pub struct DeleteRoleMembershipInputBuilder {
    pub(crate) member_name: ::std::option::Option<::std::string::String>,
    pub(crate) role: ::std::option::Option<crate::types::Role>,
    pub(crate) aws_account_id: ::std::option::Option<::std::string::String>,
    pub(crate) namespace: ::std::option::Option<::std::string::String>,
}
impl DeleteRoleMembershipInputBuilder {
    /// <p>The name of the group.</p>
    /// This field is required.
    pub fn member_name(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.member_name = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The name of the group.</p>
    pub fn set_member_name(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.member_name = input; self
    }
    /// <p>The name of the group.</p>
    pub fn get_member_name(&self) -> &::std::option::Option<::std::string::String> {
        &self.member_name
    }
    /// <p>The role that you want to remove permissions from.</p>
    /// This field is required.
    pub fn role(mut self, input: crate::types::Role) -> Self {
        self.role = ::std::option::Option::Some(input);
        self
    }
    /// <p>The role that you want to remove permissions from.</p>
    pub fn set_role(mut self, input: ::std::option::Option<crate::types::Role>) -> Self {
        self.role = input; self
    }
    /// <p>The role that you want to remove permissions from.</p>
    pub fn get_role(&self) -> &::std::option::Option<crate::types::Role> {
        &self.role
    }
    /// <p>The ID for the Amazon Web Services account that you want to create a group in. The Amazon Web Services account ID that you provide must be the same Amazon Web Services account that contains your Amazon QuickSight account.</p>
    /// This field is required.
    pub fn aws_account_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.aws_account_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ID for the Amazon Web Services account that you want to create a group in. The Amazon Web Services account ID that you provide must be the same Amazon Web Services account that contains your Amazon QuickSight account.</p>
    pub fn set_aws_account_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.aws_account_id = input; self
    }
    /// <p>The ID for the Amazon Web Services account that you want to create a group in. The Amazon Web Services account ID that you provide must be the same Amazon Web Services account that contains your Amazon QuickSight account.</p>
    pub fn get_aws_account_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.aws_account_id
    }
    /// <p>The namespace that contains the role.</p>
    /// This field is required.
    pub fn namespace(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.namespace = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The namespace that contains the role.</p>
    pub fn set_namespace(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.namespace = input; self
    }
    /// <p>The namespace that contains the role.</p>
    pub fn get_namespace(&self) -> &::std::option::Option<::std::string::String> {
        &self.namespace
    }
    /// Consumes the builder and constructs a [`DeleteRoleMembershipInput`](crate::operation::delete_role_membership::DeleteRoleMembershipInput).
    pub fn build(self) -> ::std::result::Result<crate::operation::delete_role_membership::DeleteRoleMembershipInput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(
            crate::operation::delete_role_membership::DeleteRoleMembershipInput {
                member_name: self.member_name
                ,
                role: self.role
                ,
                aws_account_id: self.aws_account_id
                ,
                namespace: self.namespace
                ,
            }
        )
    }
}

