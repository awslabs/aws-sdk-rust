// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct CreateActionConnectorOutput {
    /// <p>The Amazon Resource Name (ARN) of the created action connector.</p>
    pub arn: ::std::option::Option<::std::string::String>,
    /// <p>The creation status of the action connector.</p>
    pub creation_status: ::std::option::Option<crate::types::ResourceStatus>,
    /// <p>The unique identifier of the created action connector.</p>
    pub action_connector_id: ::std::option::Option<::std::string::String>,
    /// <p>The Amazon Web Services request ID for this operation.</p>
    pub request_id: ::std::option::Option<::std::string::String>,
    /// <p>The HTTP status code of the request.</p>
    pub status: i32,
    _request_id: Option<String>,
}
impl CreateActionConnectorOutput {
    /// <p>The Amazon Resource Name (ARN) of the created action connector.</p>
    pub fn arn(&self) -> ::std::option::Option<&str> {
        self.arn.as_deref()
    }
    /// <p>The creation status of the action connector.</p>
    pub fn creation_status(&self) -> ::std::option::Option<&crate::types::ResourceStatus> {
        self.creation_status.as_ref()
    }
    /// <p>The unique identifier of the created action connector.</p>
    pub fn action_connector_id(&self) -> ::std::option::Option<&str> {
        self.action_connector_id.as_deref()
    }
    /// <p>The Amazon Web Services request ID for this operation.</p>
    pub fn request_id(&self) -> ::std::option::Option<&str> {
        self.request_id.as_deref()
    }
    /// <p>The HTTP status code of the request.</p>
    pub fn status(&self) -> i32 {
        self.status
    }
}
impl ::aws_types::request_id::RequestId for CreateActionConnectorOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl CreateActionConnectorOutput {
    /// Creates a new builder-style object to manufacture [`CreateActionConnectorOutput`](crate::operation::create_action_connector::CreateActionConnectorOutput).
    pub fn builder() -> crate::operation::create_action_connector::builders::CreateActionConnectorOutputBuilder {
        crate::operation::create_action_connector::builders::CreateActionConnectorOutputBuilder::default()
    }
}

/// A builder for [`CreateActionConnectorOutput`](crate::operation::create_action_connector::CreateActionConnectorOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct CreateActionConnectorOutputBuilder {
    pub(crate) arn: ::std::option::Option<::std::string::String>,
    pub(crate) creation_status: ::std::option::Option<crate::types::ResourceStatus>,
    pub(crate) action_connector_id: ::std::option::Option<::std::string::String>,
    pub(crate) request_id: ::std::option::Option<::std::string::String>,
    pub(crate) status: ::std::option::Option<i32>,
    _request_id: Option<String>,
}
impl CreateActionConnectorOutputBuilder {
    /// <p>The Amazon Resource Name (ARN) of the created action connector.</p>
    pub fn arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the created action connector.</p>
    pub fn set_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.arn = input;
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the created action connector.</p>
    pub fn get_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.arn
    }
    /// <p>The creation status of the action connector.</p>
    pub fn creation_status(mut self, input: crate::types::ResourceStatus) -> Self {
        self.creation_status = ::std::option::Option::Some(input);
        self
    }
    /// <p>The creation status of the action connector.</p>
    pub fn set_creation_status(mut self, input: ::std::option::Option<crate::types::ResourceStatus>) -> Self {
        self.creation_status = input;
        self
    }
    /// <p>The creation status of the action connector.</p>
    pub fn get_creation_status(&self) -> &::std::option::Option<crate::types::ResourceStatus> {
        &self.creation_status
    }
    /// <p>The unique identifier of the created action connector.</p>
    pub fn action_connector_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.action_connector_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The unique identifier of the created action connector.</p>
    pub fn set_action_connector_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.action_connector_id = input;
        self
    }
    /// <p>The unique identifier of the created action connector.</p>
    pub fn get_action_connector_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.action_connector_id
    }
    /// <p>The Amazon Web Services request ID for this operation.</p>
    pub fn request_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.request_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Amazon Web Services request ID for this operation.</p>
    pub fn set_request_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.request_id = input;
        self
    }
    /// <p>The Amazon Web Services request ID for this operation.</p>
    pub fn get_request_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.request_id
    }
    /// <p>The HTTP status code of the request.</p>
    pub fn status(mut self, input: i32) -> Self {
        self.status = ::std::option::Option::Some(input);
        self
    }
    /// <p>The HTTP status code of the request.</p>
    pub fn set_status(mut self, input: ::std::option::Option<i32>) -> Self {
        self.status = input;
        self
    }
    /// <p>The HTTP status code of the request.</p>
    pub fn get_status(&self) -> &::std::option::Option<i32> {
        &self.status
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`CreateActionConnectorOutput`](crate::operation::create_action_connector::CreateActionConnectorOutput).
    pub fn build(self) -> crate::operation::create_action_connector::CreateActionConnectorOutput {
        crate::operation::create_action_connector::CreateActionConnectorOutput {
            arn: self.arn,
            creation_status: self.creation_status,
            action_connector_id: self.action_connector_id,
            request_id: self.request_id,
            status: self.status.unwrap_or_default(),
            _request_id: self._request_id,
        }
    }
}
