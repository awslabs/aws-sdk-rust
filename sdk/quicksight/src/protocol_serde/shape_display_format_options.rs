...
pub fn ser_display_format_options(
    ...
    if input.fraction_digits != 0 {
        object.key("FractionDigits").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((input.fraction_digits).into()));
    }
...
pub(crate) fn de_display_format_options<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::DisplayFormatOptions>, ::aws_smithy_json::deserialize::error::DeserializeError>
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "DecimalSeparator" => {
                                builder = builder.set_decimal_separator(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::TopicNumericSeparatorSymbol::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
