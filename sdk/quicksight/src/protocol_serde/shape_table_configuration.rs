// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_table_configuration(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::types::TableConfiguration,
) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.field_wells {
        #[allow(unused_mut)]
        let mut object_2 = object.key("FieldWells").start_object();
        crate::protocol_serde::shape_table_field_wells::ser_table_field_wells(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.sort_configuration {
        #[allow(unused_mut)]
        let mut object_4 = object.key("SortConfiguration").start_object();
        crate::protocol_serde::shape_table_sort_configuration::ser_table_sort_configuration(&mut object_4, var_3)?;
        object_4.finish();
    }
    if let Some(var_5) = &input.table_options {
        #[allow(unused_mut)]
        let mut object_6 = object.key("TableOptions").start_object();
        crate::protocol_serde::shape_table_options::ser_table_options(&mut object_6, var_5)?;
        object_6.finish();
    }
    if let Some(var_7) = &input.total_options {
        #[allow(unused_mut)]
        let mut object_8 = object.key("TotalOptions").start_object();
        crate::protocol_serde::shape_total_options::ser_total_options(&mut object_8, var_7)?;
        object_8.finish();
    }
    if let Some(var_9) = &input.field_options {
        #[allow(unused_mut)]
        let mut object_10 = object.key("FieldOptions").start_object();
        crate::protocol_serde::shape_table_field_options::ser_table_field_options(&mut object_10, var_9)?;
        object_10.finish();
    }
    if let Some(var_11) = &input.paginated_report_options {
        #[allow(unused_mut)]
        let mut object_12 = object.key("PaginatedReportOptions").start_object();
        crate::protocol_serde::shape_table_paginated_report_options::ser_table_paginated_report_options(&mut object_12, var_11)?;
        object_12.finish();
    }
    if let Some(var_13) = &input.table_inline_visualizations {
        let mut array_14 = object.key("TableInlineVisualizations").start_array();
        for item_15 in var_13 {
            {
                #[allow(unused_mut)]
                let mut object_16 = array_14.value().start_object();
                crate::protocol_serde::shape_table_inline_visualization::ser_table_inline_visualization(&mut object_16, item_15)?;
                object_16.finish();
            }
        }
        array_14.finish();
    }
    if let Some(var_17) = &input.dashboard_customization_visual_options {
        #[allow(unused_mut)]
        let mut object_18 = object.key("DashboardCustomizationVisualOptions").start_object();
        crate::protocol_serde::shape_dashboard_customization_visual_options::ser_dashboard_customization_visual_options(&mut object_18, var_17)?;
        object_18.finish();
    }
    if let Some(var_19) = &input.interactions {
        #[allow(unused_mut)]
        let mut object_20 = object.key("Interactions").start_object();
        crate::protocol_serde::shape_visual_interaction_options::ser_visual_interaction_options(&mut object_20, var_19)?;
        object_20.finish();
    }
    Ok(())
}

pub(crate) fn de_table_configuration<'a, I>(
    tokens: &mut ::std::iter::Peekable<I>,
    _value: &'a [u8],
) -> ::std::result::Result<Option<crate::types::TableConfiguration>, ::aws_smithy_json::deserialize::error::DeserializeError>
where
    I: Iterator<Item = Result<::aws_smithy_json::deserialize::Token<'a>, ::aws_smithy_json::deserialize::error::DeserializeError>>,
{
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
        Some(::aws_smithy_json::deserialize::Token::StartObject { .. }) => {
            #[allow(unused_mut)]
            let mut builder = crate::types::builders::TableConfigurationBuilder::default();
            loop {
                match tokens.next().transpose()? {
                    Some(::aws_smithy_json::deserialize::Token::EndObject { .. }) => break,
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "FieldWells" => {
                            builder = builder.set_field_wells(crate::protocol_serde::shape_table_field_wells::de_table_field_wells(tokens, _value)?);
                        }
                        "SortConfiguration" => {
                            builder = builder.set_sort_configuration(
                                crate::protocol_serde::shape_table_sort_configuration::de_table_sort_configuration(tokens, _value)?,
                            );
                        }
                        "TableOptions" => {
                            builder = builder.set_table_options(crate::protocol_serde::shape_table_options::de_table_options(tokens, _value)?);
                        }
                        "TotalOptions" => {
                            builder = builder.set_total_options(crate::protocol_serde::shape_total_options::de_total_options(tokens, _value)?);
                        }
                        "FieldOptions" => {
                            builder =
                                builder.set_field_options(crate::protocol_serde::shape_table_field_options::de_table_field_options(tokens, _value)?);
                        }
                        "PaginatedReportOptions" => {
                            builder = builder.set_paginated_report_options(
                                crate::protocol_serde::shape_table_paginated_report_options::de_table_paginated_report_options(tokens, _value)?,
                            );
                        }
                        "TableInlineVisualizations" => {
                            builder = builder.set_table_inline_visualizations(
                                crate::protocol_serde::shape_table_inline_visualization_list::de_table_inline_visualization_list(tokens, _value)?,
                            );
                        }
                        "DashboardCustomizationVisualOptions" => {
                            builder = builder.set_dashboard_customization_visual_options(
                                crate::protocol_serde::shape_dashboard_customization_visual_options::de_dashboard_customization_visual_options(
                                    tokens, _value,
                                )?,
                            );
                        }
                        "Interactions" => {
                            builder = builder.set_interactions(
                                crate::protocol_serde::shape_visual_interaction_options::de_visual_interaction_options(tokens, _value)?,
                            );
                        }
                        _ => ::aws_smithy_json::deserialize::token::skip_value(tokens)?,
                    },
                    other => {
                        return Err(::aws_smithy_json::deserialize::error::DeserializeError::custom(format!(
                            "expected object key or end object, found: {other:?}"
                        )))
                    }
                }
            }
            Ok(Some(builder.build()))
        }
        _ => Err(::aws_smithy_json::deserialize::error::DeserializeError::custom(
            "expected start object or null",
        )),
    }
}
