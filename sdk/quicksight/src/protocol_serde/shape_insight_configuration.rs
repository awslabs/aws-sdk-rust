...
pub fn ser_insight_configuration(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::types::InsightConfiguration,
            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "Computations" => {
                            builder = builder.set_computations(crate::protocol_serde::shape_computation_list::de_computation_list(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "CustomNarrative" => {
                            builder = builder.set_custom_narrative(
                                crate::protocol_serde::shape_custom_narrative_options::de_custom_narrative_options(tokens, _value, depth + 1)?,
                            );
                        }
                        "Interactions" => {
