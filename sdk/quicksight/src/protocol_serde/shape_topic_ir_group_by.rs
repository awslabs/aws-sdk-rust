            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "FieldName" => {
                            builder = builder.set_field_name(crate::protocol_serde::shape_identifier::de_identifier(tokens, _value, depth + 1)?);
                        }
                        "TimeGranularity" => {
                        ...
                        "NamedEntity" => {
                            builder = builder.set_named_entity(crate::protocol_serde::shape_named_entity_ref::de_named_entity_ref(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
