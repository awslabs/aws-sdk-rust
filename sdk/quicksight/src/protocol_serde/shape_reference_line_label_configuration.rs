...
pub fn ser_reference_line_label_configuration(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::model::ReferenceLineLabelConfiguration) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
...
pub(crate) fn de_reference_line_label_configuration<'a, I>(tokens: &mut std::iter::Peekable<I>) -> Result<Option<crate::model::ReferenceLineLabelConfiguration>, aws_smithy_json::deserialize::error::DeserializeError>
                    where I: Iterator<Item = Result<aws_smithy_json::deserialize::Token<'a>, aws_smithy_json::deserialize::error::DeserializeError>> {
    match tokens.next().transpose()? {
        Some(aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        Some(aws_smithy_json::deserialize::Token::StartObject { .. }) => {
            #[allow(unused_mut)]
            let mut builder = crate::model::reference_line_label_configuration::Builder::default();
            loop {
                match tokens.next().transpose()? {
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "HorizontalPosition" => {
                                builder = builder.set_horizontal_position(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::model::ReferenceLineLabelHorizontalPosition::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
                            "VerticalPosition" => {
                                builder = builder.set_vertical_position(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::model::ReferenceLineLabelVerticalPosition::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
