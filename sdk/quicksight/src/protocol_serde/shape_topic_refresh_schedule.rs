...
pub fn ser_topic_refresh_schedule(
    ...
    if let Some(var_1) = &input.starting_at {
        object.key("StartingAt").date_time(var_1, ::aws_smithy_types::date_time::Format::EpochSeconds)?;
    }
...
pub(crate) fn de_topic_refresh_schedule<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::TopicRefreshSchedule>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                    ...
                    Some(::aws_smithy_json::deserialize::Token::EndObject { .. }) => break,
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "BasedOnSpiceSchedule" => {
                                builder = builder.set_based_on_spice_schedule(
                                    ::aws_smithy_json::deserialize::token::expect_bool_or_null(tokens.next())?
                                );
                            }
                            "StartingAt" => {
                                builder = builder.set_starting_at(
                            ...
                            "TopicScheduleType" => {
                                builder = builder.set_topic_schedule_type(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::TopicScheduleType::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
            ...
            Ok(Some(crate::serde_util::topic_refresh_schedule_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
