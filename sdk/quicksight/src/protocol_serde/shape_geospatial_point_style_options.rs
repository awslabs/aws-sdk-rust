// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_geospatial_point_style_options(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::types::GeospatialPointStyleOptions,
) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.selected_point_style {
        object.key("SelectedPointStyle").string(var_1.as_str());
    }
    if let Some(var_2) = &input.cluster_marker_configuration {
        #[allow(unused_mut)]
        let mut object_3 = object.key("ClusterMarkerConfiguration").start_object();
        crate::protocol_serde::shape_cluster_marker_configuration::ser_cluster_marker_configuration(&mut object_3, var_2)?;
        object_3.finish();
    }
    if let Some(var_4) = &input.heatmap_configuration {
        #[allow(unused_mut)]
        let mut object_5 = object.key("HeatmapConfiguration").start_object();
        crate::protocol_serde::shape_geospatial_heatmap_configuration::ser_geospatial_heatmap_configuration(&mut object_5, var_4)?;
        object_5.finish();
    }
    Ok(())
}

pub(crate) fn de_geospatial_point_style_options<'a, I>(
    tokens: &mut ::std::iter::Peekable<I>,
    _value: &'a [u8],
    depth: u32,
) -> ::std::result::Result<Option<crate::types::GeospatialPointStyleOptions>, ::aws_smithy_json::deserialize::error::DeserializeError>
where
    I: Iterator<Item = Result<::aws_smithy_json::deserialize::Token<'a>, ::aws_smithy_json::deserialize::error::DeserializeError>>,
{
    if depth >= 128u32 {
        return Err(::aws_smithy_json::deserialize::error::DeserializeError::custom(
            "maximum nesting depth exceeded",
        ));
    }
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
        Some(::aws_smithy_json::deserialize::Token::StartObject { .. }) => {
            #[allow(unused_mut)]
            let mut builder = crate::types::builders::GeospatialPointStyleOptionsBuilder::default();
            loop {
                match tokens.next().transpose()? {
                    Some(::aws_smithy_json::deserialize::Token::EndObject { .. }) => break,
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "SelectedPointStyle" => {
                            builder = builder.set_selected_point_style(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::GeospatialSelectedPointStyle::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "ClusterMarkerConfiguration" => {
                            builder = builder.set_cluster_marker_configuration(
                                crate::protocol_serde::shape_cluster_marker_configuration::de_cluster_marker_configuration(
                                    tokens,
                                    _value,
                                    depth + 1,
                                )?,
                            );
                        }
                        "HeatmapConfiguration" => {
                            builder = builder.set_heatmap_configuration(
                                crate::protocol_serde::shape_geospatial_heatmap_configuration::de_geospatial_heatmap_configuration(
                                    tokens,
                                    _value,
                                    depth + 1,
                                )?,
                            );
                        }
                        _ => ::aws_smithy_json::deserialize::token::skip_value(tokens)?,
                    },
                    other => {
                        return Err(::aws_smithy_json::deserialize::error::DeserializeError::custom(format!(
                            "expected object key or end object, found: {other:?}"
                        )))
                    }
                }
            }
            Ok(Some(builder.build()))
        }
        _ => Err(::aws_smithy_json::deserialize::error::DeserializeError::custom(
            "expected start object or null",
        )),
    }
}
