                ...
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                        match key.to_unescaped()?.as_ref() {
                            "SortBy" => {
                                builder = builder.set_sort_by(crate::protocol_serde::shape_column_identifier::de_column_identifier(
                                    tokens,
                                    _value,
                                    depth + 1,
                                )?);
                            ...
                            "AggregationFunction" => {
                                builder = builder.set_aggregation_function(
                                    crate::protocol_serde::shape_aggregation_function::de_aggregation_function(tokens, _value, depth + 1)?,
                                );
                            }
