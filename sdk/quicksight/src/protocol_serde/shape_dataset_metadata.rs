...
pub(crate) fn de_dataset_metadata<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::DatasetMetadata>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "Filters" => {
                                builder = builder.set_filters(
                                    crate::protocol_serde::shape_topic_filters::de_topic_filters(tokens)?
                                );
                            }
                            "Columns" => {
                                builder = builder.set_columns(
                                    crate::protocol_serde::shape_topic_columns::de_topic_columns(tokens)?
                                );
                            }
                            "CalculatedFields" => {
            ...
            Ok(Some(crate::serde_util::dataset_metadata_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
