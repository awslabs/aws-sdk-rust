...
> {
    ...
    Ok({
        ...
        output = output.set_request_id(
            crate::protocol_serde::shape_start_stream_transcription_output::de_request_id_header(
                response.headers(),
            )
            .map_err(|_| {
                crate::error::StartStreamTranscriptionError::unhandled(
                    "Failed to parse RequestId from header `x-amzn-request-id",
                )
            })?,
        );
        output = output.set_session_id(
            crate::protocol_serde::shape_start_stream_transcription_output::de_session_id_header(
                response.headers(),
            )
            .map_err(|_| {
                crate::error::StartStreamTranscriptionError::unhandled(
