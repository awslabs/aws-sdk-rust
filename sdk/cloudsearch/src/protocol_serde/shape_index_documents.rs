...
pub fn de_index_documents(
    ...
    if !(start_el.matches("IndexDocumentsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected IndexDocumentsResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("IndexDocumentsResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected IndexDocumentsResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("FieldNames") /* FieldNames com.amazonaws.cloudsearch.synthetic#IndexDocumentsOutput$FieldNames */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_field_name_list::de_field_name_list(&mut tag, depth + 1)
                        ?
                    )
                ;
