...
pub fn de_expression_status(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
    depth: u32,
) -> ::std::result::Result<crate::types::ExpressionStatus, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("Options") /* Options com.amazonaws.cloudsearch#ExpressionStatus$Options */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_expression::de_expression(&mut tag, depth + 1)
                        ?
                    )
                ;
