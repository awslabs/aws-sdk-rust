...
pub fn de_create_domain(
    ...
    if !(start_el.matches("CreateDomainResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected CreateDomainResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("CreateDomainResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected CreateDomainResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("DomainStatus") /* DomainStatus com.amazonaws.cloudsearch.synthetic#CreateDomainOutput$DomainStatus */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_domain_status::de_domain_status(&mut tag, depth + 1)
                        ?
                    )
                ;
