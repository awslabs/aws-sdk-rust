// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn de_attached_permissions_boundary(decoder: &mut aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::model::AttachedPermissionsBoundary, aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    let mut builder = crate::model::AttachedPermissionsBoundary::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("PermissionsBoundaryType") /* PermissionsBoundaryType com.amazonaws.iam#AttachedPermissionsBoundary$PermissionsBoundaryType */ =>  {
                let var_1 =
                    Some(
                        Result::<crate::model::PermissionsBoundaryAttachmentType, aws_smithy_xml::decode::XmlDecodeError>::Ok(
                            crate::model::PermissionsBoundaryAttachmentType::from(
                                aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            )
                        )
                        ?
                    )
                ;
                builder = builder.set_permissions_boundary_type(var_1);
            }
            ,
            s if s.matches("PermissionsBoundaryArn") /* PermissionsBoundaryArn com.amazonaws.iam#AttachedPermissionsBoundary$PermissionsBoundaryArn */ =>  {
                let var_2 =
                    Some(
                        Result::<std::string::String, aws_smithy_xml::decode::XmlDecodeError>::Ok(
                            aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            .into()
                        )
                        ?
                    )
                ;
                builder = builder.set_permissions_boundary_arn(var_2);
            }
            ,
            _ => {}
        }
    }
    Ok(builder.build()
    )
}

