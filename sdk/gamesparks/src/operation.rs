...
impl aws_smithy_http::response::ParseStrictResponse for DisconnectPlayer {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_disconnect_player::de_disconnect_player_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_disconnect_player::de_disconnect_player_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for GetExtensionVersion {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_get_extension_version::de_get_extension_version_http_error(
                response,
            )
        } else {
...
impl aws_smithy_http::response::ParseStrictResponse for GetStageDeployment {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_get_stage_deployment::de_get_stage_deployment_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_get_stage_deployment::de_get_stage_deployment_http_response(
                response,
            )
        }
    }
