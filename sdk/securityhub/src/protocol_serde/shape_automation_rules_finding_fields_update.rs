...
pub fn ser_automation_rules_finding_fields_update(
    ...
    if let Some(var_6) = &input.confidence {
        object.key("Confidence").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_6).into()));
    }
    if let Some(var_7) = &input.criticality {
        object.key("Criticality").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_7).into()));
    }
...
pub(crate) fn de_automation_rules_finding_fields_update<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::AutomationRulesFindingFieldsUpdate>, ::aws_smithy_json::deserialize::error::DeserializeError>
                        ...
                        match key.to_unescaped()?.as_ref() {
                            "Note" => {
                                builder = builder.set_note(
                                    crate::protocol_serde::shape_note_update::de_note_update(tokens)?
                                );
                            }
                            "Severity" => {
