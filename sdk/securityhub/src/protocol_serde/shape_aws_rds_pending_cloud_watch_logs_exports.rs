            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "LogTypesToEnable" => {
                            builder =
                                builder.set_log_types_to_enable(crate::protocol_serde::shape_string_list::de_string_list(tokens, _value, depth + 1)?);
                        }
                        "LogTypesToDisable" => {
                            builder = builder.set_log_types_to_disable(crate::protocol_serde::shape_string_list::de_string_list(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
