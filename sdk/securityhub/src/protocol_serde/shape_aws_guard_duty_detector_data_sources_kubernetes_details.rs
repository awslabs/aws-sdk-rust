                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "AuditLogs" => {
                            builder = builder.set_audit_logs(
                                    crate::protocol_serde::shape_aws_guard_duty_detector_data_sources_kubernetes_audit_logs_details::de_aws_guard_duty_detector_data_sources_kubernetes_audit_logs_details(tokens, _value, depth + 1)?
                                );
                        }
