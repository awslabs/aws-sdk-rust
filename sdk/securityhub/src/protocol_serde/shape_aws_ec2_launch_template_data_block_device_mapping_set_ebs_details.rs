...
pub fn ser_aws_ec2_launch_template_data_block_device_mapping_set_ebs_details(
    ...
    if let Some(var_6) = &input.throughput {
        object.key("Throughput").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_6).into()));
    }
    if let Some(var_7) = &input.volume_size {
        object.key("VolumeSize").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_7).into()));
    }
    if let Some(var_8) = &input.volume_type {
        object.key("VolumeType").string(var_8.as_str());
    }
    Ok(())
}
