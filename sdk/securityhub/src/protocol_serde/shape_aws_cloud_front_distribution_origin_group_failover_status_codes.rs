                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "Items" => {
                            builder = builder.set_items(
                                    crate::protocol_serde::shape_aws_cloud_front_distribution_origin_group_failover_status_codes_item_list::de_aws_cloud_front_distribution_origin_group_failover_status_codes_item_list(tokens, _value, depth + 1)?
                                );
                        }
                        "Quantity" => {
