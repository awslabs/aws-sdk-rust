                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "exactResponseFields" => {
                            builder = builder.set_exact_response_fields(
                                crate::protocol_serde::shape_exact_response_fields::de_exact_response_fields(tokens, _value, depth + 1)?,
                            );
                        }
                        "includeFields" => {
                            builder = builder.set_include_fields(crate::protocol_serde::shape_os_include_fields::de_os_include_fields(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
