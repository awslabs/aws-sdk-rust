            ...
            let mut tmp = {
                ...
                output = output.set_retry_after_seconds(
                    crate::protocol_serde::shape_throttling_exception::de_retry_after_seconds_header(_response_headers).map_err(|_| {
                        crate::operation::list_exports::ListExportsError::unhandled("Failed to parse retryAfterSeconds from header `Retry-After")
                    })?,
                );
                let output = output.meta(generic);
                output.build()
...
) -> Result<crate::operation::list_exports::builders::ListExportsOutputBuilder, ::aws_smithy_json::deserialize::error::DeserializeError> {
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "exportSummaries" => {
                    builder = builder.set_export_summaries(crate::protocol_serde::shape_export_summary_list::de_export_summary_list(tokens)?);
                }
                "localeId" => {
