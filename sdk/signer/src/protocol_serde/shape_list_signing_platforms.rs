...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_signing_platforms::de_list_signing_platforms(_response_body, output)
            .map_err(crate::operation::list_signing_platforms::ListSigningPlatformsError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "platforms" => {
                    builder = builder.set_platforms(crate::protocol_serde::shape_signing_platforms::de_signing_platforms(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
