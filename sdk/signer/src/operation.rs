...
impl aws_smithy_http::response::ParseStrictResponse for DescribeSigningJob {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_describe_signing_job::de_describe_signing_job_http_error(
                response,
            )
        } else {
...
impl aws_smithy_http::response::ParseStrictResponse for ListSigningJobs {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_list_signing_jobs::de_list_signing_jobs_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_list_signing_jobs::de_list_signing_jobs_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for StartSigningJob {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_start_signing_job::de_start_signing_job_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_start_signing_job::de_start_signing_job_http_response(
                response,
            )
        }
    }
