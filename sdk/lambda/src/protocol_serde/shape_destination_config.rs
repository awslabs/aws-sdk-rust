            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "OnSuccess" => {
                            builder = builder.set_on_success(crate::protocol_serde::shape_on_success::de_on_success(tokens, _value)?);
                        }
                        "OnFailure" => {
                            builder = builder.set_on_failure(crate::protocol_serde::shape_on_failure::de_on_failure(tokens, _value)?);
                        }
