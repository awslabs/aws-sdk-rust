...
impl aws_smithy_http::response::ParseStrictResponse for ListFunctions {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !success && status != 200 {
            crate::protocol_serde::shape_list_functions::de_list_functions_http_error(
                status, headers, body,
            )
        } else {
            crate::protocol_serde::shape_list_functions::de_list_functions_http_response_with_props(
                status, headers, body,
            )
        }
    }
