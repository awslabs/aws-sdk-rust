...
) -> std::result::Result<crate::operation::detect_stack_set_drift::builders::DetectStackSetDriftOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError>
{
    ...
    if !(start_el.matches("DetectStackSetDriftResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DetectStackSetDriftResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DetectStackSetDriftResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DetectStackSetDriftResult got {start_el:?}"
            )));
        }
