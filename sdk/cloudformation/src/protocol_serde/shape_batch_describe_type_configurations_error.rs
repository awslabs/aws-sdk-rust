...
pub fn de_batch_describe_type_configurations_error(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::BatchDescribeTypeConfigurationsError, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("TypeConfigurationIdentifier") /* TypeConfigurationIdentifier com.amazonaws.cloudformation#BatchDescribeTypeConfigurationsError$TypeConfigurationIdentifier */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_type_configuration_identifier::de_type_configuration_identifier(&mut tag, depth + 1)
                        ?
                    )
                ;
