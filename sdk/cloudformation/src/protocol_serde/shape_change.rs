...
pub fn de_change(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::Change, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("ResourceChange") /* ResourceChange com.amazonaws.cloudformation#Change$ResourceChange */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_resource_change::de_resource_change(&mut tag, depth + 1)
                        ?
                    )
                ;
