...
pub fn de_list_stack_resources_http_error(
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_list_stack_resources_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
...
) -> std::result::Result<crate::operation::list_stack_resources::builders::ListStackResourcesOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError>
{
    ...
    if !(start_el.matches("ListStackResourcesResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected ListStackResourcesResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("ListStackResourcesResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected ListStackResourcesResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("StackResourceSummaries") /* StackResourceSummaries com.amazonaws.cloudformation.synthetic#ListStackResourcesOutput$StackResourceSummaries */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_stack_resource_summaries::de_stack_resource_summaries(&mut tag, depth + 1)
                        ?
                    )
                ;
