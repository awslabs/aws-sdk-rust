...
impl aws_smithy_http::response::ParseStrictResponse for DeleteFHIRDatastore {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_delete_fhir_datastore::de_delete_fhir_datastore_http_error(
                response,
            )
        } else {
...
impl aws_smithy_http::response::ParseStrictResponse for StartFHIRImportJob {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_start_fhir_import_job::de_start_fhir_import_job_http_error(
                response,
            )
        } else {
