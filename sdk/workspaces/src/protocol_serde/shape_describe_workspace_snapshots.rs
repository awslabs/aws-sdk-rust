    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "RebuildSnapshots" => {
                    builder = builder.set_rebuild_snapshots(crate::protocol_serde::shape_snapshot_list::de_snapshot_list(tokens, _value, depth + 1)?);
                }
                "RestoreSnapshots" => {
                    builder = builder.set_restore_snapshots(crate::protocol_serde::shape_snapshot_list::de_snapshot_list(tokens, _value, depth + 1)?);
                }
