...
pub fn ser_target_tracking_scaling_policy_configuration(
    ...
    if let Some(var_5) = &input.scale_out_cooldown {
        object.key("ScaleOutCooldown").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_5).into()));
    }
    if let Some(var_6) = &input.scale_in_cooldown {
        object.key("ScaleInCooldown").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_6).into()));
    }
...
pub(crate) fn de_target_tracking_scaling_policy_configuration<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::TargetTrackingScalingPolicyConfiguration>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
            ...
            Ok(Some(crate::serde_util::target_tracking_scaling_policy_configuration_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
