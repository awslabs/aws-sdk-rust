// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_create_storage_lens_group_input_input_input(input: &crate::operation::create_storage_lens_group::CreateStorageLensGroupInput, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
    if let Some(var_1) = &input.storage_lens_group {
        let inner_writer = scope.start_el("StorageLensGroup");
        crate::protocol_serde::shape_storage_lens_group::ser_storage_lens_group(var_1, inner_writer)?
    }
    if let Some(var_2) = &input.tags {
        let mut inner_writer = scope.start_el("Tags").finish();
        for list_item_3 in var_2 {
             {
                let inner_writer = inner_writer.start_el("Tag");
                crate::protocol_serde::shape_tag::ser_tag(list_item_3, inner_writer)?
            }
        }
    }
    scope.finish();
    Ok(())
}

