// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn de_noncurrent_version_transition_list(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
) -> ::std::result::Result<::std::vec::Vec<crate::types::NoncurrentVersionTransition>, ::aws_smithy_xml::decode::XmlDecodeError> {
    let mut out = std::vec::Vec::new();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("NoncurrentVersionTransition") /* member com.amazonaws.s3control#NoncurrentVersionTransitionList$member */ =>  {
                out.push(
                    crate::protocol_serde::shape_noncurrent_version_transition::de_noncurrent_version_transition(&mut tag)
                    ?
                );
            }
            ,
            _ => {}
        }
    }
    Ok(out)
}
