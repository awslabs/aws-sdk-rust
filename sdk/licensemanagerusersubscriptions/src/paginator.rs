            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_user_associations_output_next_token(resp);
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
                                && new_token == input.next_token.as_ref()
                                && self.stop_on_duplicate_token
...
impl ListInstancesPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_instances_output_instance_summaries(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
...
impl ListUserAssociationsPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_user_associations_output_instance_user_summaries(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
