// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_start_product_subscription_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::operation::start_product_subscription::StartProductSubscriptionInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.domain {
        object.key("Domain").string(var_1.as_str());
    }
    if let Some(var_2) = &input.identity_provider {
        #[allow(unused_mut)]
        let mut object_3 = object.key("IdentityProvider").start_object();
        crate::protocol_serde::shape_identity_provider::ser_identity_provider(&mut object_3, var_2)?;
        object_3.finish();
    }
    if let Some(var_4) = &input.product {
        object.key("Product").string(var_4.as_str());
    }
    if let Some(var_5) = &input.username {
        object.key("Username").string(var_5.as_str());
    }
    Ok(())
}

