// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_update_identity_provider_settings_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::UpdateIdentityProviderSettingsInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.identity_provider {
        #[allow(unused_mut)]
        let mut object_2 = object.key("IdentityProvider").start_object();
        crate::protocol_serde::shape_identity_provider::ser_identity_provider(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.product {
        object.key("Product").string(var_3.as_str());
    }
    if let Some(var_4) = &input.update_settings {
        #[allow(unused_mut)]
        let mut object_5 = object.key("UpdateSettings").start_object();
        crate::protocol_serde::shape_update_settings::ser_update_settings(&mut object_5, var_4)?;
        object_5.finish();
    }
    Ok(())
}

