    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "Attributes" => {
                    builder = builder.set_attributes(crate::protocol_serde::shape_workflow_attributes::de_workflow_attributes(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
                ...
                "Metrics" => {
                    builder = builder.set_metrics(crate::protocol_serde::shape_workflow_metrics::de_workflow_metrics(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
