...
/// <p>Configuration settings that define a stream group's stream capacity for a location. When configuring a location for the first time, you must specify a numeric value for at least one of the two capacity types. To update the capacity for an existing stream group, call <code>UpdateStreamGroup</code>. To add a new location and specify its capacity, call <code>AddStreamGroupLocations</code>.</p>
#[non_exhaustive]
