// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_describe_custom_key_stores_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::DescribeCustomKeyStoresInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.custom_key_store_id {
        object.key("CustomKeyStoreId").string(var_1.as_str());
    }
    if let Some(var_2) = &input.custom_key_store_name {
        object.key("CustomKeyStoreName").string(var_2.as_str());
    }
    if let Some(var_3) = &input.limit {
        object.key("Limit").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_3).into()));
    }
    if let Some(var_4) = &input.marker {
        object.key("Marker").string(var_4.as_str());
    }
    Ok(())
}

