...
impl CodeArtifact {
    ...
    /// Tries to convert the enum instance into [`Uri`](crate::types::CodeArtifact::Uri), extracting the inner [`String`](::std::string::String).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_uri(&self) -> ::std::result::Result<&::std::string::String, &Self> {
        if let CodeArtifact::Uri(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`Uri`](crate::types::CodeArtifact::Uri).
