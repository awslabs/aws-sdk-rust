...
/// When writing a match expression against `PublishStatus`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `publishstatus` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `PublishStatus::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `PublishStatus::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `PublishStatus::NewFeature` is defined.
/// Specifically, when `publishstatus` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `PublishStatus::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for PublishStatus {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => PublishStatus::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
impl ::std::str::FromStr for PublishStatus {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(PublishStatus::from(s))
    }
}
impl PublishStatus {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            PublishStatus::Unknown(value) => value.as_str(),
        }
    }
...
impl ::std::fmt::Display for PublishStatus {
    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        match self {
            ...
            PublishStatus::Unknown(value) => write!(f, "{value}"),
        }
    }
}
