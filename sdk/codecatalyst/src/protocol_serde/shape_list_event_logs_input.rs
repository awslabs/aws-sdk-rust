// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_list_event_logs_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::ListEventLogsInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.end_time {
        object.key("endTime").date_time(var_1, aws_smithy_types::date_time::Format::DateTime)?;
    }
    if let Some(var_2) = &input.event_name {
        object.key("eventName").string(var_2.as_str());
    }
    if let Some(var_3) = &input.max_results {
        object.key("maxResults").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_3).into()));
    }
    if let Some(var_4) = &input.next_token {
        object.key("nextToken").string(var_4.as_str());
    }
    if let Some(var_5) = &input.start_time {
        object.key("startTime").date_time(var_5, aws_smithy_types::date_time::Format::DateTime)?;
    }
    Ok(())
}

