...
/// When writing a match expression against `NotebookStatus`, it is important to ensure
...
///
/// ```text
/// # let notebookstatus = unimplemented!();
...
/// }
/// ```
/// The above code demonstrates that when `notebookstatus` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `NotebookStatus::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `NotebookStatus::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `NotebookStatus::NewFeature` is defined.
/// Specifically, when `notebookstatus` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `NotebookStatus::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for NotebookStatus {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for NotebookStatus {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(NotebookStatus::from(s))
    }
}
impl NotebookStatus {
