...
pub fn ser_lake_formation_configuration(
    ...
    if let Some(var_1) = &input.location_registration_role {
        object.key("locationRegistrationRole").string(var_1.as_str());
    }
    if let Some(var_2) = &input.location_registration_exclude_s3_locations {
        let mut array_3 = object.key("locationRegistrationExcludeS3Locations").start_array();
        for item_4 in var_2 {
            {
                ...
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "locationRegistrationRole" => {
                            builder = builder.set_location_registration_role(
                        ...
                        "locationRegistrationExcludeS3Locations" => {
                            builder = builder.set_location_registration_exclude_s3_locations(
                                crate::protocol_serde::shape_s3_location_list::de_s3_location_list(tokens, _value, depth + 1)?,
                            );
                        }
