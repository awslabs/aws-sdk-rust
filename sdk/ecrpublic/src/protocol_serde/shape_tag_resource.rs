            ...
            let mut tmp =
                 {
                    ...
                    let _ = response;
                    output = crate::protocol_serde::shape_unsupported_command_exception::de_unsupported_command_exception_json_err(response.body().as_ref(), output).map_err(crate::operation::tag_resource::TagResourceError::unhandled)?;
                    let output = output.meta(generic);
                    output.build()
                }
            ;
