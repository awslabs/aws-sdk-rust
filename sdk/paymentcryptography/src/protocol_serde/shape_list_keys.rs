...
pub fn de_list_keys_http_response(
    ...
    Ok({
        ...
        crate::serde_util::list_keys_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_keys::ListKeysError::unhandled)?
    })
}
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "Keys" => {
                    builder = builder.set_keys(crate::protocol_serde::shape_key_summary_list::de_key_summary_list(tokens, _value)?);
                }
                "NextToken" => {
