    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "RequiredParameters" => {
                    builder =
                        builder.set_required_parameters(crate::protocol_serde::shape_parameter_list::de_parameter_list(tokens, _value, depth + 1)?);
                }
                "MutuallyExclusiveParameters" => {
                    builder = builder.set_mutually_exclusive_parameters(crate::protocol_serde::shape_parameter_list::de_parameter_list(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
