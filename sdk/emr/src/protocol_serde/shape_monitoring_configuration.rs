...
pub fn ser_monitoring_configuration(
    ...
    if let Some(var_1) = &input.cloud_watch_log_configuration {
        #[allow(unused_mut)]
        let mut object_2 = object.key("CloudWatchLogConfiguration").start_object();
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "CloudWatchLogConfiguration" => {
                            builder = builder.set_cloud_watch_log_configuration(
                                crate::protocol_serde::shape_cloud_watch_log_configuration::de_cloud_watch_log_configuration(
                                    tokens,
                                    _value,
                                    depth + 1,
                                )?,
                        ...
                        "S3LoggingConfiguration" => {
                            builder = builder.set_s3_logging_configuration(
                                crate::protocol_serde::shape_s3_logging_configuration::de_s3_logging_configuration(tokens, _value, depth + 1)?,
                            );
                        }
