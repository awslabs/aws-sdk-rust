            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "S3InputDefinition" => {
                            builder =
                                builder.set_s3_input_definition(crate::protocol_serde::shape_s3_location::de_s3_location(tokens, _value, depth + 1)?);
                        }
                        "DataCatalogInputDefinition" => {
                            builder = builder.set_data_catalog_input_definition(
                                crate::protocol_serde::shape_data_catalog_input_definition::de_data_catalog_input_definition(
                                    tokens,
                                    _value,
                                    depth + 1,
                                )?,
                        ...
                        "DatabaseInputDefinition" => {
                            builder = builder.set_database_input_definition(
                                crate::protocol_serde::shape_database_input_definition::de_database_input_definition(tokens, _value, depth + 1)?,
                            );
                        }
                        "Metadata" => {
