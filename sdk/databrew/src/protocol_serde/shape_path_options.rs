            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "FilesLimit" => {
                            builder = builder.set_files_limit(crate::protocol_serde::shape_files_limit::de_files_limit(tokens, _value, depth + 1)?);
                        }
                        "Parameters" => {
                            builder = builder.set_parameters(crate::protocol_serde::shape_path_parameters_map::de_path_parameters_map(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
