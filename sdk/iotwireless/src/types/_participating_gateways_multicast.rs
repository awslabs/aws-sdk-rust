...
pub struct ParticipatingGatewaysMulticast {
    /// <p>The list of gateways that you want to use for sending the multicast downlink. Each downlink will be sent to all the gateways in the list with transmission interval between them. If list is empty the gateway list will be dynamically selected similar to the case of no ParticipatingGateways</p>
    pub gateway_list: ::std::option::Option<::std::vec::Vec<::std::string::String>>,
    /// <p>The duration of time for which AWS IoT Core for LoRaWAN will wait before transmitting the multicast payload to the next gateway in the list.</p>
