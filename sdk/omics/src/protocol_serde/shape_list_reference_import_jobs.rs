...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_reference_import_jobs::ListReferenceImportJobsError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_reference_import_jobs::de_list_reference_import_jobs(_response_body, output)
            .map_err(crate::operation::list_reference_import_jobs::ListReferenceImportJobsError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "importJobs" => {
                    builder = builder.set_import_jobs(crate::protocol_serde::shape_import_reference_job_list::de_import_reference_job_list(
                        tokens,
                    )?);
                }
                "nextToken" => {
