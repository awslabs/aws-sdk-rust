// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn serialize_structure_put_session_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::PutSessionInput,
) {
    if let Some(var_1) = &input.messages {
        let mut array_2 = object.key("messages").start_array();
        for item_3 in var_1 {
            {
                let mut object_4 = array_2.value().start_object();
                crate::json_ser::serialize_structure_message(&mut object_4, item_3);
                object_4.finish();
            }
        }
        array_2.finish();
    }
    if let Some(var_5) = &input.request_attributes {
        let mut object_6 = object.key("requestAttributes").start_object();
        for (key_7, value_8) in var_5 {
            {
                object_6.key(key_7).string(value_8);
            }
        }
        object_6.finish();
    }
    if let Some(var_9) = &input.session_state {
        let mut object_10 = object.key("sessionState").start_object();
        crate::json_ser::serialize_structure_session_state(&mut object_10, var_9);
        object_10.finish();
    }
}

pub fn serialize_structure_recognize_text_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::RecognizeTextInput,
) {
    if let Some(var_11) = &input.request_attributes {
        let mut object_12 = object.key("requestAttributes").start_object();
        for (key_13, value_14) in var_11 {
            {
                object_12.key(key_13).string(value_14);
            }
        }
        object_12.finish();
    }
    if let Some(var_15) = &input.session_state {
        let mut object_16 = object.key("sessionState").start_object();
        crate::json_ser::serialize_structure_session_state(&mut object_16, var_15);
        object_16.finish();
    }
    if let Some(var_17) = &input.text {
        object.key("text").string(var_17);
    }
}

pub fn serialize_structure_message(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::Message,
) {
    if let Some(var_18) = &input.content {
        object.key("content").string(var_18);
    }
    if let Some(var_19) = &input.content_type {
        object.key("contentType").string(var_19.as_str());
    }
    if let Some(var_20) = &input.image_response_card {
        let mut object_21 = object.key("imageResponseCard").start_object();
        crate::json_ser::serialize_structure_image_response_card(&mut object_21, var_20);
        object_21.finish();
    }
}

pub fn serialize_structure_session_state(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::SessionState,
) {
    if let Some(var_22) = &input.dialog_action {
        let mut object_23 = object.key("dialogAction").start_object();
        crate::json_ser::serialize_structure_dialog_action(&mut object_23, var_22);
        object_23.finish();
    }
    if let Some(var_24) = &input.intent {
        let mut object_25 = object.key("intent").start_object();
        crate::json_ser::serialize_structure_intent(&mut object_25, var_24);
        object_25.finish();
    }
    if let Some(var_26) = &input.active_contexts {
        let mut array_27 = object.key("activeContexts").start_array();
        for item_28 in var_26 {
            {
                let mut object_29 = array_27.value().start_object();
                crate::json_ser::serialize_structure_active_context(&mut object_29, item_28);
                object_29.finish();
            }
        }
        array_27.finish();
    }
    if let Some(var_30) = &input.session_attributes {
        let mut object_31 = object.key("sessionAttributes").start_object();
        for (key_32, value_33) in var_30 {
            {
                object_31.key(key_32).string(value_33);
            }
        }
        object_31.finish();
    }
    if let Some(var_34) = &input.originating_request_id {
        object.key("originatingRequestId").string(var_34);
    }
}

pub fn serialize_structure_image_response_card(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ImageResponseCard,
) {
    if let Some(var_35) = &input.title {
        object.key("title").string(var_35);
    }
    if let Some(var_36) = &input.subtitle {
        object.key("subtitle").string(var_36);
    }
    if let Some(var_37) = &input.image_url {
        object.key("imageUrl").string(var_37);
    }
    if let Some(var_38) = &input.buttons {
        let mut array_39 = object.key("buttons").start_array();
        for item_40 in var_38 {
            {
                let mut object_41 = array_39.value().start_object();
                crate::json_ser::serialize_structure_button(&mut object_41, item_40);
                object_41.finish();
            }
        }
        array_39.finish();
    }
}

pub fn serialize_structure_dialog_action(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::DialogAction,
) {
    if let Some(var_42) = &input.r#type {
        object.key("type").string(var_42.as_str());
    }
    if let Some(var_43) = &input.slot_to_elicit {
        object.key("slotToElicit").string(var_43);
    }
}

pub fn serialize_structure_intent(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::Intent,
) {
    if let Some(var_44) = &input.name {
        object.key("name").string(var_44);
    }
    if let Some(var_45) = &input.slots {
        let mut object_46 = object.key("slots").start_object();
        for (key_47, value_48) in var_45 {
            {
                let mut object_49 = object_46.key(key_47).start_object();
                crate::json_ser::serialize_structure_slot(&mut object_49, value_48);
                object_49.finish();
            }
        }
        object_46.finish();
    }
    if let Some(var_50) = &input.state {
        object.key("state").string(var_50.as_str());
    }
    if let Some(var_51) = &input.confirmation_state {
        object.key("confirmationState").string(var_51.as_str());
    }
}

pub fn serialize_structure_active_context(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ActiveContext,
) {
    if let Some(var_52) = &input.name {
        object.key("name").string(var_52);
    }
    if let Some(var_53) = &input.time_to_live {
        let mut object_54 = object.key("timeToLive").start_object();
        crate::json_ser::serialize_structure_active_context_time_to_live(&mut object_54, var_53);
        object_54.finish();
    }
    if let Some(var_55) = &input.context_attributes {
        let mut object_56 = object.key("contextAttributes").start_object();
        for (key_57, value_58) in var_55 {
            {
                object_56.key(key_57).string(value_58);
            }
        }
        object_56.finish();
    }
}

pub fn serialize_structure_button(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::Button,
) {
    if let Some(var_59) = &input.text {
        object.key("text").string(var_59);
    }
    if let Some(var_60) = &input.value {
        object.key("value").string(var_60);
    }
}

pub fn serialize_structure_slot(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::Slot,
) {
    if let Some(var_61) = &input.value {
        let mut object_62 = object.key("value").start_object();
        crate::json_ser::serialize_structure_value(&mut object_62, var_61);
        object_62.finish();
    }
    if let Some(var_63) = &input.shape {
        object.key("shape").string(var_63.as_str());
    }
    if let Some(var_64) = &input.values {
        let mut array_65 = object.key("values").start_array();
        for item_66 in var_64 {
            {
                let mut object_67 = array_65.value().start_object();
                crate::json_ser::serialize_structure_slot(&mut object_67, item_66);
                object_67.finish();
            }
        }
        array_65.finish();
    }
}

pub fn serialize_structure_active_context_time_to_live(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ActiveContextTimeToLive,
) {
    if let Some(var_68) = &input.time_to_live_in_seconds {
        object.key("timeToLiveInSeconds").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_68).into()),
        );
    }
    if let Some(var_69) = &input.turns_to_live {
        object.key("turnsToLive").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_69).into()),
        );
    }
}

pub fn serialize_structure_value(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::Value,
) {
    if let Some(var_70) = &input.original_value {
        object.key("originalValue").string(var_70);
    }
    if let Some(var_71) = &input.interpreted_value {
        object.key("interpretedValue").string(var_71);
    }
    if let Some(var_72) = &input.resolved_values {
        let mut array_73 = object.key("resolvedValues").start_array();
        for item_74 in var_72 {
            {
                array_73.value().string(item_74);
            }
        }
        array_73.finish();
    }
}
