            ...
            let mut tmp = {
                ...
                output = output.set_retry_after_seconds(
                    crate::protocol_serde::shape_internal_server_exception::de_retry_after_seconds_header(_response_headers).map_err(|_| {
                        crate::operation::create_project::CreateProjectError::unhandled("Failed to parse RetryAfterSeconds from header `Retry-After")
                    })?,
                );
                let output = output.meta(generic);
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "ProjectMetadata" => {
                    builder = builder.set_project_metadata(crate::protocol_serde::shape_project_metadata::de_project_metadata(tokens)?);
                }
