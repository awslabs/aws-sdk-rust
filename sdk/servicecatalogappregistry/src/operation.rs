...
impl aws_smithy_http::response::ParseStrictResponse for ListAttributeGroups {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_list_attribute_groups::de_list_attribute_groups_http_error(
                response,
            )
        } else {
...
impl aws_smithy_http::response::ParseStrictResponse for PutConfiguration {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_put_configuration::de_put_configuration_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_put_configuration::de_put_configuration_http_response(
                response,
            )
        }
    }
