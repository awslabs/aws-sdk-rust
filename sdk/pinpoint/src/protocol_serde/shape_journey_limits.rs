...
pub fn ser_journey_limits(
    ...
    if let Some(var_1) = &input.daily_cap {
        object.key("DailyCap").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_1).into()));
    }
    if let Some(var_2) = &input.endpoint_reentry_cap {
        object.key("EndpointReentryCap").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_2).into()));
    }
    if let Some(var_3) = &input.messages_per_second {
        object.key("MessagesPerSecond").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_3).into()));
    }
    ...
    if let Some(var_7) = &input.total_cap {
        object.key("TotalCap").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_7).into()));
    }
    Ok(())
}
                ...
                match tokens.next().transpose()? {
                    Some(::aws_smithy_json::deserialize::Token::EndObject { .. }) => break,
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "TimeframeCap" => {
                                builder = builder.set_timeframe_cap(
                                    crate::protocol_serde::shape_journey_timeframe_cap::de_journey_timeframe_cap(tokens)?
                                );
                            }
                            "TotalCap" => {
