// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>A filter used to narrow down results based on specific criteria.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct Filter {
    /// <p>The name of the filter field to apply.</p>
    pub name: crate::types::AutomationRuleFilterName,
    /// <p>The list of values to filter by for the specified filter field.</p>
    pub values: ::std::vec::Vec<::std::string::String>,
}
impl Filter {
    /// <p>The name of the filter field to apply.</p>
    pub fn name(&self) -> &crate::types::AutomationRuleFilterName {
        &self.name
    }
    /// <p>The list of values to filter by for the specified filter field.</p>
    pub fn values(&self) -> &[::std::string::String] {
        use std::ops::Deref;
        self.values.deref()
    }
}
impl Filter {
    /// Creates a new builder-style object to manufacture [`Filter`](crate::types::Filter).
    pub fn builder() -> crate::types::builders::FilterBuilder {
        crate::types::builders::FilterBuilder::default()
    }
}

/// A builder for [`Filter`](crate::types::Filter).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct FilterBuilder {
    pub(crate) name: ::std::option::Option<crate::types::AutomationRuleFilterName>,
    pub(crate) values: ::std::option::Option<::std::vec::Vec<::std::string::String>>,
}
impl FilterBuilder {
    /// <p>The name of the filter field to apply.</p>
    /// This field is required.
    pub fn name(mut self, input: crate::types::AutomationRuleFilterName) -> Self {
        self.name = ::std::option::Option::Some(input);
        self
    }
    /// <p>The name of the filter field to apply.</p>
    pub fn set_name(mut self, input: ::std::option::Option<crate::types::AutomationRuleFilterName>) -> Self {
        self.name = input;
        self
    }
    /// <p>The name of the filter field to apply.</p>
    pub fn get_name(&self) -> &::std::option::Option<crate::types::AutomationRuleFilterName> {
        &self.name
    }
    /// Appends an item to `values`.
    ///
    /// To override the contents of this collection use [`set_values`](Self::set_values).
    ///
    /// <p>The list of values to filter by for the specified filter field.</p>
    pub fn values(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        let mut v = self.values.unwrap_or_default();
        v.push(input.into());
        self.values = ::std::option::Option::Some(v);
        self
    }
    /// <p>The list of values to filter by for the specified filter field.</p>
    pub fn set_values(mut self, input: ::std::option::Option<::std::vec::Vec<::std::string::String>>) -> Self {
        self.values = input;
        self
    }
    /// <p>The list of values to filter by for the specified filter field.</p>
    pub fn get_values(&self) -> &::std::option::Option<::std::vec::Vec<::std::string::String>> {
        &self.values
    }
    /// Consumes the builder and constructs a [`Filter`](crate::types::Filter).
    /// This method will fail if any of the following fields are not set:
    /// - [`name`](crate::types::builders::FilterBuilder::name)
    /// - [`values`](crate::types::builders::FilterBuilder::values)
    pub fn build(self) -> ::std::result::Result<crate::types::Filter, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(crate::types::Filter {
            name: self.name.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "name",
                    "name was not specified but it is required when building Filter",
                )
            })?,
            values: self.values.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "values",
                    "values was not specified but it is required when building Filter",
                )
            })?,
        })
    }
}
