...
> {
    ...
    if !(start_el.matches("GetCustomVerificationEmailTemplateResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected GetCustomVerificationEmailTemplateResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("GetCustomVerificationEmailTemplateResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected GetCustomVerificationEmailTemplateResult got {start_el:?}"
            )));
        }
