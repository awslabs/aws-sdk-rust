    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("entry") => {
                crate::protocol_serde::shape_mail_from_domain_attributes::de_mail_from_domain_attributes_entry(&mut tag, &mut out, depth)?;
            }
            _ => {}
        }
    }
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("value") /* value com.amazonaws.ses#MailFromDomainAttributes$value */ =>  {
                v = Some(
                    crate::protocol_serde::shape_identity_mail_from_domain_attributes::de_identity_mail_from_domain_attributes(&mut tag, depth + 1)
                    ?
                )
            }
            ,
