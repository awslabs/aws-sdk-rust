    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("entry") => {
                crate::protocol_serde::shape_dkim_attributes::de_dkim_attributes_entry(&mut tag, &mut out, depth)?;
            }
            _ => {}
        }
    }
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("value") /* value com.amazonaws.ses#DkimAttributes$value */ =>  {
                v = Some(
                    crate::protocol_serde::shape_identity_dkim_attributes::de_identity_dkim_attributes(&mut tag, depth + 1)
                    ?
                )
            }
            ,
