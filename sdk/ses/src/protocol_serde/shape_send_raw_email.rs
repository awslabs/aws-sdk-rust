...
pub fn de_send_raw_email_http_response(
    ...
    Ok({
        ...
        crate::serde_util::send_raw_email_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::send_raw_email::SendRawEmailError::unhandled)?
    })
}
...
pub fn de_send_raw_email(
    ...
    if !(start_el.matches("SendRawEmailResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected SendRawEmailResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("SendRawEmailResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected SendRawEmailResult got {start_el:?}"
            )));
        }
