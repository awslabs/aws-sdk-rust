...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::describe_receipt_rule_set::DescribeReceiptRuleSetError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_receipt_rule_set::de_describe_receipt_rule_set(_response_body, output)
            .map_err(crate::operation::describe_receipt_rule_set::DescribeReceiptRuleSetError::unhandled)?;
...
> {
    ...
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DescribeReceiptRuleSetResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DescribeReceiptRuleSetResult got {start_el:?}"
            )));
        }
