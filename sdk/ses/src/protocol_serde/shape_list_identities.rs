...
pub fn de_list_identities_http_response(
    ...
    Ok({
        ...
        crate::serde_util::list_identities_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_identities::ListIdentitiesError::unhandled)?
    })
}
...
pub fn de_list_identities(
...
) -> std::result::Result<crate::operation::list_identities::builders::ListIdentitiesOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    if !(start_el.matches("ListIdentitiesResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected ListIdentitiesResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("ListIdentitiesResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected ListIdentitiesResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("Identities") /* Identities com.amazonaws.ses.synthetic#ListIdentitiesOutput$Identities */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_identity_list::de_identity_list(&mut tag, depth + 1)
                        ?
                    )
                ;
