...
#[allow(clippy::unnecessary_wraps)]
pub fn de_verify_domain_identity_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    ...
    if !(start_el.matches("VerifyDomainIdentityResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected VerifyDomainIdentityResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("VerifyDomainIdentityResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected VerifyDomainIdentityResult got {start_el:?}"
            )));
        }
