...
impl ::aws_smithy_http::response::ParseStrictResponse for SendRawEmail {
    ...
    fn parse(&self, response: &::http::Response<::bytes::Bytes>) -> Self::Output {
        ...
        if !success && status != 200 {
            crate::protocol_serde::shape_send_raw_email::de_send_raw_email_http_error(
                status, headers, body,
            )
        } else {
            crate::protocol_serde::shape_send_raw_email::de_send_raw_email_http_response_with_props(
                status, headers, body,
            )
        }
    }
