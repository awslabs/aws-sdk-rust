...
pub fn de_start_loader_job_http_response(
    ...
    Ok({
        ...
        crate::serde_util::start_loader_job_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::start_loader_job::StartLoaderJobError::unhandled)?
    })
}
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "payload" => {
                    builder = builder.set_payload(crate::protocol_serde::shape_string_valued_map::de_string_valued_map(tokens, _value)?);
                }
                "status" => {
