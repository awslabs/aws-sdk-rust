...
pub fn ser_start_ml_data_processing_job_input_input(
    ...
    if let Some(var_9) = &input.processing_instance_volume_size_in_gb {
        object.key("processingInstanceVolumeSizeInGB").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_9).into()));
    }
    if let Some(var_10) = &input.processing_time_out_in_seconds {
        object.key("processingTimeOutInSeconds").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_10).into()));
    }
