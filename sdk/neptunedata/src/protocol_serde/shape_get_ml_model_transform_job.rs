        ...
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                match key.to_unescaped()?.as_ref() {
                    "baseProcessingJob" => {
                        builder = builder.set_base_processing_job(crate::protocol_serde::shape_ml_resource_definition::de_ml_resource_definition(
                            tokens,
                            _value,
                            depth + 1,
                        )?);
                    ...
                    "models" => {
                        builder = builder.set_models(crate::protocol_serde::shape_models::de_models(tokens, _value, depth + 1)?);
                    }
                    "remoteModelTransformJob" => {
                        builder = builder.set_remote_model_transform_job(
                            crate::protocol_serde::shape_ml_resource_definition::de_ml_resource_definition(tokens, _value, depth + 1)?,
                        );
                    }
                    "status" => {
