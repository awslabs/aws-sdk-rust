...
impl  CsvMappingParameters  {
    ...
    pub fn record_row_delimiter(&self) -> & str {
        use std::ops::Deref; self.record_row_delimiter.deref()
    }
    /// <p>Column delimiter. For example, in a CSV format, a comma (",") is the typical column delimiter.</p>
    pub fn record_column_delimiter(&self) -> & str {
        use std::ops::Deref; self.record_column_delimiter.deref()
    }
}
impl CsvMappingParameters {
        ...
        ::std::result::Result::Ok(
            crate::types::CsvMappingParameters {
                record_row_delimiter: self.record_row_delimiter
                    .ok_or_else(||
                        ::aws_smithy_types::error::operation::BuildError::missing_field("record_row_delimiter", "record_row_delimiter was not specified but it is required when building CsvMappingParameters")
                    )?
                ,
                record_column_delimiter: self.record_column_delimiter
                    .ok_or_else(||
                        ::aws_smithy_types::error::operation::BuildError::missing_field("record_column_delimiter", "record_column_delimiter was not specified but it is required when building CsvMappingParameters")
                    )?
                ,
            }
        )
