...
pub(crate) fn de_metrics_data_source<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::MetricsDataSource>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        ...
                        match key.to_unescaped()?.as_ref() {
                            "Dimensions" => {
                                builder = builder.set_dimensions(
                                    crate::protocol_serde::shape_export_dimensions::de_export_dimensions(tokens)?
                                );
                            }
                            "Namespace" => {
                            ...
                            "Metrics" => {
                                builder = builder.set_metrics(
                                    crate::protocol_serde::shape_export_metrics::de_export_metrics(tokens)?
                                );
                            }
                            "StartDate" => {
            ...
            Ok(Some(crate::serde_util::metrics_data_source_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
