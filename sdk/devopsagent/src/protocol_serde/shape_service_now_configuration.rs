...
pub fn ser_service_now_configuration(
    ...
    if let Some(var_2) = &input.auth_scopes {
        let mut array_3 = object.key("authScopes").start_array();
        for item_4 in var_2 {
            {
            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "authScopes" => {
                            builder = builder.set_auth_scopes(
                                crate::protocol_serde::shape_service_now_authentication_scope_list::de_service_now_authentication_scope_list(
                                    tokens,
                                    _value,
                                    depth + 1,
                                )?,
