...
impl aws_smithy_http::response::ParseStrictResponse for CompleteSnapshot {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !response.status().is_success() && response.status().as_u16() != 202 {
            crate::protocol_serde::shape_complete_snapshot::de_complete_snapshot_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_complete_snapshot::de_complete_snapshot_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseHttpResponse for GetSnapshotBlock {
    ...
    fn parse_unloaded(
        ...
        Some(
            crate::protocol_serde::shape_get_snapshot_block::de_get_snapshot_block_http_response(
                response,
            ),
        )
    }
...
impl aws_smithy_http::response::ParseStrictResponse for ListChangedBlocks {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_list_changed_blocks::de_list_changed_blocks_http_error(
                response,
            )
        } else {
            crate::protocol_serde::shape_list_changed_blocks::de_list_changed_blocks_http_response(
                response,
            )
        }
    }
