...
impl super::Client {
    ...
    ///   - [`filter_pattern(impl ::std::convert::Into<String>)`](crate::operation::test_metric_filter::builders::TestMetricFilterFluentBuilder::filter_pattern) / [`set_filter_pattern(Option<String>)`](crate::operation::test_metric_filter::builders::TestMetricFilterFluentBuilder::set_filter_pattern): <p>A symbolic description of how CloudWatch Logs should interpret the data in each log event. For example, a log event can contain timestamps, IP addresses, strings, and so on. You use the filter pattern to specify what to look for in the log event message.</p>
