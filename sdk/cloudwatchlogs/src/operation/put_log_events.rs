...
impl aws_smithy_http::response::ParseStrictResponse for PutLogEvents {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !success && status != 200 {
            crate::protocol_serde::shape_put_log_events::de_put_log_events_http_error(
                status, headers, body,
            )
        } else {
            crate::protocol_serde::shape_put_log_events::de_put_log_events_http_response_with_props(
                status, headers, body,
            )
        }
    }
