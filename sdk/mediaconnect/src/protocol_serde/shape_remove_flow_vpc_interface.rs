...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::remove_flow_vpc_interface::RemoveFlowVpcInterfaceError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_remove_flow_vpc_interface::de_remove_flow_vpc_interface(_response_body, output)
            .map_err(crate::operation::remove_flow_vpc_interface::RemoveFlowVpcInterfaceError::unhandled)?;
        ...
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "nonDeletedNetworkInterfaceIds" => {
                    builder = builder.set_non_deleted_network_interface_ids(crate::protocol_serde::shape_list_of_string::de_list_of_string(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
