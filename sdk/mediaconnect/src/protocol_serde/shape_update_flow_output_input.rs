...
pub fn ser_update_flow_output_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::operation::update_flow_output::UpdateFlowOutputInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    ...
    if let Some(var_12) = &input.protocol {
        object.key("protocol").string(var_12.as_str());
    }
    if let Some(var_13) = &input.remote_id {
        object.key("remoteId").string(var_13.as_str());
    }
    ...
    if let Some(var_14) = &input.sender_ip_address {
        object.key("senderIpAddress").string(var_14.as_str());
    }
