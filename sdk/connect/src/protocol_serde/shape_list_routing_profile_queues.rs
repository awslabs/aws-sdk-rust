...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::list_routing_profile_queues::ListRoutingProfileQueuesError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_routing_profile_queues::ListRoutingProfileQueuesError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_routing_profile_queues::de_list_routing_profile_queues(_response_body, output)
            .map_err(crate::operation::list_routing_profile_queues::ListRoutingProfileQueuesError::unhandled)?;
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "RoutingProfileQueueConfigSummaryList" => {
                    builder = builder.set_routing_profile_queue_config_summary_list(
                        crate::protocol_serde::shape_routing_profile_queue_config_summary_list::de_routing_profile_queue_config_summary_list(
                            tokens, _value,
                        )?,
                    );
                }
