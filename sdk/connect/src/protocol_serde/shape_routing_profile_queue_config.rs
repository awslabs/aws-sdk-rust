// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_routing_profile_queue_config(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::model::RoutingProfileQueueConfig) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.queue_reference {
        #[allow(unused_mut)]
        let mut object_2 = object.key("QueueReference").start_object();
        crate::protocol_serde::shape_routing_profile_queue_reference::ser_routing_profile_queue_reference(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.priority {
        object.key("Priority").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_3).into()));
    }
    if let Some(var_4) = &input.delay {
        object.key("Delay").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_4).into()));
    }
    Ok(())
}

