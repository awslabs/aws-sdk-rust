// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
impl super::Client {
    /// Constructs a fluent builder for the [`UpdateContactFlowModuleContent`](crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder) operation.
    ///
    /// - The fluent builder is configurable:
    ///   - [`instance_id(impl Into<String>)`](crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder::instance_id) / [`set_instance_id(Option<String>)`](crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder::set_instance_id): <p>The identifier of the Amazon Connect instance. You can <a href="https://docs.aws.amazon.com/connect/latest/adminguide/find-instance-arn.html">find the instance ID</a> in the Amazon Resource Name (ARN) of the instance.</p>
    ///   - [`contact_flow_module_id(impl Into<String>)`](crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder::contact_flow_module_id) / [`set_contact_flow_module_id(Option<String>)`](crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder::set_contact_flow_module_id): <p>The identifier of the flow module.</p>
    ///   - [`content(impl Into<String>)`](crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder::content) / [`set_content(Option<String>)`](crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder::set_content): <p>The JSON string that represents the content of the flow. For an example, see <a href="https://docs.aws.amazon.com/connect/latest/APIReference/flow-language-example.html">Example flow in Amazon Connect Flow language</a>. </p>
    /// - On success, responds with [`UpdateContactFlowModuleContentOutput`](crate::operation::update_contact_flow_module_content::UpdateContactFlowModuleContentOutput)
    /// - On failure, responds with [`SdkError<UpdateContactFlowModuleContentError>`](crate::operation::update_contact_flow_module_content::UpdateContactFlowModuleContentError)
    pub fn update_contact_flow_module_content(
        &self,
    ) -> crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder {
        crate::operation::update_contact_flow_module_content::builders::UpdateContactFlowModuleContentFluentBuilder::new(self.handle.clone())
    }
}
