            ...
            let mut tmp =
                 {
                    ...
                    let _ = response;
                    output = crate::protocol_serde::shape_concurrent_access_exception::de_concurrent_access_exception_json_err(response.body().as_ref(), output).map_err(crate::error::TagResourceError::unhandled)?;
                    let output = output.meta(generic);
                    output.build()
                }
            ;
