...
pub fn ser_multi_measure_attribute_mapping(
    ...
    {
        object.key("MeasureValue").string(input.measure_value.as_str());
    }
    {
    ...
    {
        object
            .key("MultiMeasureAttributeName")
            .string(input.multi_measure_attribute_name.as_str());
    }
    Ok(())
}
            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "MeasureValue" => {
                            builder = builder.set_measure_value(
                        ...
                        "MultiMeasureAttributeName" => {
                            builder = builder.set_multi_measure_attribute_name(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| u.into_owned()))
                                    .transpose()?,
                            );
