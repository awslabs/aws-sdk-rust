...
pub fn ser_pipe_source_managed_streaming_kafka_parameters(
    ...
    if let Some(var_2) = &input.batch_size {
        object.key("BatchSize").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_2).into()));
    }
    if let Some(var_3) = &input.maximum_batching_window_in_seconds {
        object.key("MaximumBatchingWindowInSeconds").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_3).into()));
    }
...
pub(crate) fn de_pipe_source_managed_streaming_kafka_parameters<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::PipeSourceManagedStreamingKafkaParameters>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
            ...
            Ok(Some(crate::serde_util::pipe_source_managed_streaming_kafka_parameters_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
