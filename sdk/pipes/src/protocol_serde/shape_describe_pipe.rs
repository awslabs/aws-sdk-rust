...
pub fn de_describe_pipe_http_error(
    ...
    _response_body: &[u8],
) -> std::result::Result<crate::operation::describe_pipe::DescribePipeOutput, crate::operation::describe_pipe::DescribePipeError> {
    #[allow(unused_mut)]
            ...
            let mut tmp = {
                ...
                output = output.set_retry_after_seconds(
                    crate::protocol_serde::shape_internal_exception::de_retry_after_seconds_header(_response_headers).map_err(|_| {
                        crate::operation::describe_pipe::DescribePipeError::unhandled("Failed to parse retryAfterSeconds from header `Retry-After")
                    })?,
                );
...
pub fn de_describe_pipe_http_response_with_props(
    ...
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_pipe::de_describe_pipe(_response_body, output)
            .map_err(crate::operation::describe_pipe::DescribePipeError::unhandled)?;
                ...
                "DesiredState" => {
                    builder = builder.set_desired_state(
                        ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                            .map(|s| {
                                s.to_unescaped()
                                    .map(|u| crate::types::RequestedPipeStateDescribeResponse::from(u.as_ref()))
                            })
                            .transpose()?,
                    );
                }
