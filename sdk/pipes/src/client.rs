...
/// For example, the [`CreatePipe`](crate::operation::create_pipe) operation has
/// a [`Client::create_pipe`], function which returns a builder for that operation.
/// The fluent builder ultimately has a `send()` function that returns an async future that
/// returns a result, as illustrated below:
///
/// ```rust,ignore
/// let result = client.create_pipe()
///     .name("example")
///     .send()
///     .await;
/// ```
...
///
/// ```ignore
/// # async fn wrapper() -> ::std::result::Result<(), aws_sdk_pipes::Error> {
