...
pub(crate) fn de_launch_configuration_template<'a, I>(tokens: &mut std::iter::Peekable<I>) -> Result<Option<crate::model::LaunchConfigurationTemplate>, aws_smithy_json::deserialize::error::DeserializeError>
                    where I: Iterator<Item = Result<aws_smithy_json::deserialize::Token<'a>, aws_smithy_json::deserialize::error::DeserializeError>> {
    match tokens.next().transpose()? {
        Some(aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        Some(aws_smithy_json::deserialize::Token::StartObject { .. }) => {
            #[allow(unused_mut)]
            let mut builder = crate::model::launch_configuration_template::Builder::default();
            loop {
                match tokens.next().transpose()? {
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "launchDisposition" => {
                                builder = builder.set_launch_disposition(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::model::LaunchDisposition::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
                            "targetInstanceTypeRightSizingMethod" => {
                                builder = builder.set_target_instance_type_right_sizing_method(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::model::TargetInstanceTypeRightSizingMethod::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
                            ...
                            "bootMode" => {
                                builder = builder.set_boot_mode(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::model::BootMode::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
