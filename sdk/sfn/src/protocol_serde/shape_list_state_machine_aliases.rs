...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_state_machine_aliases::ListStateMachineAliasesError::unhandled(
                generic,
            ))
        }
    };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_list_state_machine_aliases_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
                ...
                match key.to_unescaped()?.as_ref() {
                    "stateMachineAliases" => {
                        builder = builder.set_state_machine_aliases(
                            crate::protocol_serde::shape_state_machine_alias_list::de_state_machine_alias_list(tokens, _value, depth + 1)?,
                        );
                    }
                    "nextToken" => {
