// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct ListPlanExecutionsInput {
    /// <p>The ARN for the plan.</p>
    pub plan_arn: ::std::option::Option<::std::string::String>,
    /// <p>The number of objects that you want to return with this call.</p>
    pub max_results: ::std::option::Option<i32>,
    /// <p>Specifies that you want to receive the next page of results. Valid only if you received a <code>nextToken</code> response in the previous request. If you did, it indicates that more output is available. Set this parameter to the value provided by the previous call's <code>nextToken</code> response to request the next page of results.</p>
    pub next_token: ::std::option::Option<::std::string::String>,
    /// <p>The state of the plan execution. For example, the plan execution might be In Progress.</p>
    pub state: ::std::option::Option<crate::types::ExecutionState>,
}
impl ListPlanExecutionsInput {
    /// <p>The ARN for the plan.</p>
    pub fn plan_arn(&self) -> ::std::option::Option<&str> {
        self.plan_arn.as_deref()
    }
    /// <p>The number of objects that you want to return with this call.</p>
    pub fn max_results(&self) -> ::std::option::Option<i32> {
        self.max_results
    }
    /// <p>Specifies that you want to receive the next page of results. Valid only if you received a <code>nextToken</code> response in the previous request. If you did, it indicates that more output is available. Set this parameter to the value provided by the previous call's <code>nextToken</code> response to request the next page of results.</p>
    pub fn next_token(&self) -> ::std::option::Option<&str> {
        self.next_token.as_deref()
    }
    /// <p>The state of the plan execution. For example, the plan execution might be In Progress.</p>
    pub fn state(&self) -> ::std::option::Option<&crate::types::ExecutionState> {
        self.state.as_ref()
    }
}
impl ListPlanExecutionsInput {
    /// Creates a new builder-style object to manufacture [`ListPlanExecutionsInput`](crate::operation::list_plan_executions::ListPlanExecutionsInput).
    pub fn builder() -> crate::operation::list_plan_executions::builders::ListPlanExecutionsInputBuilder {
        crate::operation::list_plan_executions::builders::ListPlanExecutionsInputBuilder::default()
    }
}

/// A builder for [`ListPlanExecutionsInput`](crate::operation::list_plan_executions::ListPlanExecutionsInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct ListPlanExecutionsInputBuilder {
    pub(crate) plan_arn: ::std::option::Option<::std::string::String>,
    pub(crate) max_results: ::std::option::Option<i32>,
    pub(crate) next_token: ::std::option::Option<::std::string::String>,
    pub(crate) state: ::std::option::Option<crate::types::ExecutionState>,
}
impl ListPlanExecutionsInputBuilder {
    /// <p>The ARN for the plan.</p>
    /// This field is required.
    pub fn plan_arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.plan_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ARN for the plan.</p>
    pub fn set_plan_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.plan_arn = input;
        self
    }
    /// <p>The ARN for the plan.</p>
    pub fn get_plan_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.plan_arn
    }
    /// <p>The number of objects that you want to return with this call.</p>
    pub fn max_results(mut self, input: i32) -> Self {
        self.max_results = ::std::option::Option::Some(input);
        self
    }
    /// <p>The number of objects that you want to return with this call.</p>
    pub fn set_max_results(mut self, input: ::std::option::Option<i32>) -> Self {
        self.max_results = input;
        self
    }
    /// <p>The number of objects that you want to return with this call.</p>
    pub fn get_max_results(&self) -> &::std::option::Option<i32> {
        &self.max_results
    }
    /// <p>Specifies that you want to receive the next page of results. Valid only if you received a <code>nextToken</code> response in the previous request. If you did, it indicates that more output is available. Set this parameter to the value provided by the previous call's <code>nextToken</code> response to request the next page of results.</p>
    pub fn next_token(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.next_token = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>Specifies that you want to receive the next page of results. Valid only if you received a <code>nextToken</code> response in the previous request. If you did, it indicates that more output is available. Set this parameter to the value provided by the previous call's <code>nextToken</code> response to request the next page of results.</p>
    pub fn set_next_token(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.next_token = input;
        self
    }
    /// <p>Specifies that you want to receive the next page of results. Valid only if you received a <code>nextToken</code> response in the previous request. If you did, it indicates that more output is available. Set this parameter to the value provided by the previous call's <code>nextToken</code> response to request the next page of results.</p>
    pub fn get_next_token(&self) -> &::std::option::Option<::std::string::String> {
        &self.next_token
    }
    /// <p>The state of the plan execution. For example, the plan execution might be In Progress.</p>
    pub fn state(mut self, input: crate::types::ExecutionState) -> Self {
        self.state = ::std::option::Option::Some(input);
        self
    }
    /// <p>The state of the plan execution. For example, the plan execution might be In Progress.</p>
    pub fn set_state(mut self, input: ::std::option::Option<crate::types::ExecutionState>) -> Self {
        self.state = input;
        self
    }
    /// <p>The state of the plan execution. For example, the plan execution might be In Progress.</p>
    pub fn get_state(&self) -> &::std::option::Option<crate::types::ExecutionState> {
        &self.state
    }
    /// Consumes the builder and constructs a [`ListPlanExecutionsInput`](crate::operation::list_plan_executions::ListPlanExecutionsInput).
    pub fn build(
        self,
    ) -> ::std::result::Result<crate::operation::list_plan_executions::ListPlanExecutionsInput, ::aws_smithy_types::error::operation::BuildError>
    {
        ::std::result::Result::Ok(crate::operation::list_plan_executions::ListPlanExecutionsInput {
            plan_arn: self.plan_arn,
            max_results: self.max_results,
            next_token: self.next_token,
            state: self.state,
        })
    }
}
