// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub(crate) fn de_plan(
    decoder: &mut ::aws_smithy_cbor::Decoder,
    depth: u32,
) -> ::std::result::Result<crate::types::Plan, ::aws_smithy_cbor::decode::DeserializeError> {
    if depth >= 128u32 {
        return Err(::aws_smithy_cbor::decode::DeserializeError::custom(
            "maximum nesting depth exceeded",
            decoder.position(),
        ));
    }
    #[allow(clippy::match_single_binding, unused_variables)]
    fn pair(
        mut builder: crate::types::builders::PlanBuilder,
        decoder: &mut ::aws_smithy_cbor::Decoder,
        depth: u32,
    ) -> ::std::result::Result<crate::types::builders::PlanBuilder, ::aws_smithy_cbor::decode::DeserializeError> {
        builder = match decoder.str()?.as_ref() {
            "arn" => builder.set_arn(Some(decoder.string()?)),
            "description" => {
                ::aws_smithy_cbor::decode::set_optional(builder, decoder, |builder, decoder| Ok(builder.set_description(Some(decoder.string()?))))?
            }
            "workflows" => builder.set_workflows(Some(crate::protocol_serde::shape_workflow_list::de_workflow_list(decoder, depth + 1)?)),
            "executionRole" => builder.set_execution_role(Some(decoder.string()?)),
            "recoveryTimeObjectiveMinutes" => ::aws_smithy_cbor::decode::set_optional(builder, decoder, |builder, decoder| {
                Ok(builder.set_recovery_time_objective_minutes(Some(decoder.integer()?)))
            })?,
            "associatedAlarms" => ::aws_smithy_cbor::decode::set_optional(builder, decoder, |builder, decoder| {
                Ok(
                    builder.set_associated_alarms(Some(crate::protocol_serde::shape_associated_alarm_map::de_associated_alarm_map(
                        decoder,
                        depth + 1,
                    )?)),
                )
            })?,
            "triggers" => ::aws_smithy_cbor::decode::set_optional(builder, decoder, |builder, decoder| {
                Ok(builder.set_triggers(Some(crate::protocol_serde::shape_trigger_list::de_trigger_list(decoder, depth + 1)?)))
            })?,
            "reportConfiguration" => ::aws_smithy_cbor::decode::set_optional(builder, decoder, |builder, decoder| {
                Ok(
                    builder.set_report_configuration(Some(crate::protocol_serde::shape_report_configuration::de_report_configuration(
                        decoder,
                        depth + 1,
                    )?)),
                )
            })?,
            "name" => builder.set_name(Some(decoder.string()?)),
            "regions" => builder.set_regions(Some(crate::protocol_serde::shape_region_list::de_region_list(decoder, depth + 1)?)),
            "recoveryApproach" => builder.set_recovery_approach(Some(decoder.string().map(|s| crate::types::RecoveryApproach::from(s.as_ref()))?)),
            "primaryRegion" => ::aws_smithy_cbor::decode::set_optional(builder, decoder, |builder, decoder| {
                Ok(builder.set_primary_region(Some(decoder.string()?)))
            })?,
            "owner" => builder.set_owner(Some(decoder.string()?)),
            "version" => {
                ::aws_smithy_cbor::decode::set_optional(builder, decoder, |builder, decoder| Ok(builder.set_version(Some(decoder.string()?))))?
            }
            "updatedAt" => {
                ::aws_smithy_cbor::decode::set_optional(
                    builder,
                    decoder,
                    |builder, decoder| Ok(builder.set_updated_at(Some(decoder.timestamp()?))),
                )?
            }
            _ => {
                decoder.skip()?;
                builder
            }
        };
        Ok(builder)
    }

    let mut builder = crate::types::builders::PlanBuilder::default();

    match decoder.map()? {
        None => loop {
            match decoder.datatype()? {
                ::aws_smithy_cbor::data::Type::Break => {
                    decoder.skip()?;
                    break;
                }
                _ => {
                    builder = pair(builder, decoder, depth)?;
                }
            };
        },
        Some(n) => {
            for _ in 0..n {
                builder = pair(builder, decoder, depth)?;
            }
        }
    };
    #[allow(clippy::needless_question_mark)]
    {
        return Ok(crate::serde_util::plan_correct_errors(builder)
            .build()
            .map_err(|err| ::aws_smithy_cbor::decode::DeserializeError::custom(err.to_string(), decoder.position()))?);
    }
}
