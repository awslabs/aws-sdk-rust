...
pub fn ser_create_cluster_input(
    ...
    if let Some(var_18) = &input.initial_cluster_size {
        object.key("InitialClusterSize").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_18).into()));
    }
    if let Some(var_19) = &input.force_create_jobs {
        object.key("ForceCreateJobs").boolean(*var_19);
    }
    if let Some(var_20) = &input.long_term_pricing_ids {
        let mut array_21 = object.key("LongTermPricingIds").start_array();
