...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::delete_experiment_template::DeleteExperimentTemplateError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::delete_experiment_template::DeleteExperimentTemplateError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_delete_experiment_template::de_delete_experiment_template(_response_body, output)
            .map_err(crate::operation::delete_experiment_template::DeleteExperimentTemplateError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "experimentTemplate" => {
                    builder = builder.set_experiment_template(crate::protocol_serde::shape_experiment_template::de_experiment_template(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
