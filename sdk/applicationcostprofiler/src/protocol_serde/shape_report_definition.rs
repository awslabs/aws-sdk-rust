                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "reportFrequency" => {
                            builder = builder.set_report_frequency(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::ReportFrequency::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "format" => {
                        ...
                        "destinationS3Location" => {
                            builder = builder.set_destination_s3_location(crate::protocol_serde::shape_s3_location::de_s3_location(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
