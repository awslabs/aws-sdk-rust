            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "Action" => {
                            builder = builder.set_action(crate::protocol_serde::shape_recipe_action::de_recipe_action(tokens, _value, depth + 1)?);
                        }
                        "ConditionExpressions" => {
                            builder = builder.set_condition_expressions(
                                crate::protocol_serde::shape_condition_expression_list::de_condition_expression_list(tokens, _value, depth + 1)?,
                            );
                        }
