...
pub(crate) fn de_s3_delta_direct_target<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::S3DeltaDirectTarget>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "Compression" => {
                                builder = builder.set_compression(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::DeltaTargetCompressionType::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
            ...
            Ok(Some(crate::serde_util::s3_delta_direct_target_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
