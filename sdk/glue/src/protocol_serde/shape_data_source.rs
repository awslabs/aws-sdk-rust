            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "GlueTable" => {
                            builder = builder.set_glue_table(crate::protocol_serde::shape_glue_table::de_glue_table(tokens, _value, depth + 1)?);
                        }
                        "DataQualityGlueTable" => {
                            builder = builder.set_data_quality_glue_table(
                                crate::protocol_serde::shape_data_quality_glue_table::de_data_quality_glue_table(tokens, _value, depth + 1)?,
                            );
                        }
