// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_get_schema_version_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::input::GetSchemaVersionInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.schema_id {
        #[allow(unused_mut)]
        let mut object_2 = object.key("SchemaId").start_object();
        crate::protocol_serde::shape_schema_id::ser_schema_id(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.schema_version_id {
        object.key("SchemaVersionId").string(var_3.as_str());
    }
    if let Some(var_4) = &input.schema_version_number {
        #[allow(unused_mut)]
        let mut object_5 = object.key("SchemaVersionNumber").start_object();
        crate::protocol_serde::shape_schema_version_number::ser_schema_version_number(&mut object_5, var_4)?;
        object_5.finish();
    }
    Ok(())
}

