// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_get_maintenance_window_execution_task_input_input(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::operation::get_maintenance_window_execution_task::GetMaintenanceWindowExecutionTaskInput,
) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.window_execution_id {
        object.key("WindowExecutionId").string(var_1.as_str());
    }
    if let Some(var_2) = &input.task_id {
        object.key("TaskId").string(var_2.as_str());
    }
    Ok(())
}
