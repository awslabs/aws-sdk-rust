...
pub(crate) fn de_maintenance_window_execution_task_identity<'a, I>(tokens: &mut std::iter::Peekable<I>) -> Result<Option<crate::model::MaintenanceWindowExecutionTaskIdentity>, aws_smithy_json::deserialize::error::DeserializeError>
                    where I: Iterator<Item = Result<aws_smithy_json::deserialize::Token<'a>, aws_smithy_json::deserialize::error::DeserializeError>> {
    match tokens.next().transpose()? {
        Some(aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        Some(aws_smithy_json::deserialize::Token::StartObject { .. }) => {
            #[allow(unused_mut)]
            let mut builder = crate::model::maintenance_window_execution_task_identity::Builder::default();
            loop {
                match tokens.next().transpose()? {
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "Status" => {
                                builder = builder.set_status(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::model::MaintenanceWindowExecutionStatus::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
                            ...
                            "TaskType" => {
                                builder = builder.set_task_type(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::model::MaintenanceWindowTaskType::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
