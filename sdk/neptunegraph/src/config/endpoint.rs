...
mod test {
    ...
    fn test_71() {
        ...
        let endpoint = resolver.resolve_endpoint(&params);
        let error = endpoint
            .expect_err("expected error: Invalid Configuration: FIPS and custom endpoint are not supported [Both Fips and dualstack enabled]");
    ...
    fn test_72() {
        ...
        let endpoint = resolver.resolve_endpoint(&params);
        let error = endpoint
            .expect_err("expected error: Invalid Configuration: FIPS and custom endpoint are not supported [Fips enabled and dualstack disabled]");
        assert_eq!(format!("{}", error), "Invalid Configuration: FIPS and custom endpoint are not supported")
    }
    ...
    fn test_73() {
        ...
        let error = endpoint.expect_err(
            "expected error: Invalid Configuration: Dualstack and custom endpoint are not supported [Fips disabled and dualstack enabled]",
        );
        assert_eq!(
            format!("{}", error),
    ...
    pub fn build(self) -> ::std::result::Result<crate::config::endpoint::Params, crate::config::endpoint::InvalidParams> {
        Ok(
            ...
            crate::config::endpoint::Params {
                ...
                api_type: self.api_type.ok_or_else(|| crate::config::endpoint::InvalidParams::missing("api_type"))?,
            },
        )
    }
