...
pub fn ser_create_graph_using_import_task_input_input(
    ...
    if let Some(var_8) = &input.max_provisioned_memory {
        object.key("maxProvisionedMemory").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_8).into()));
    }
    if let Some(var_9) = &input.min_provisioned_memory {
        object.key("minProvisionedMemory").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_9).into()));
    }
    ...
    if let Some(var_11) = &input.replica_count {
        object.key("replicaCount").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_11).into()));
    }
