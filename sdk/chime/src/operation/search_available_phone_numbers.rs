...
impl SearchAvailablePhoneNumbersInput {
    ...
    pub async fn make_operation(&self, _config: &crate::config::Config) -> std::result::Result<aws_smithy_http::operation::Operation<crate::operation::search_available_phone_numbers::SearchAvailablePhoneNumbers, aws_http::retry::AwsResponseRetryClassifier>, aws_smithy_http::operation::error::BuildError> {
        ...
        let mut request = {
            fn uri_base(_input: &crate::operation::search_available_phone_numbers::SearchAvailablePhoneNumbersInput, output: &mut String) -> std::result::Result<(), aws_smithy_http::operation::error::BuildError> {
                use std::fmt::Write as _;
                write!(output, "/search").expect("formatting should succeed");
                Ok(())
            }
            fn uri_query(_input: &crate::operation::search_available_phone_numbers::SearchAvailablePhoneNumbersInput, mut output: &mut String) -> Result<(), aws_smithy_http::operation::error::BuildError> {
                let mut query = aws_smithy_http::query::Writer::new(output);
                query.push_kv("type", "phone-numbers");
                if let Some(inner_1) = &_input.area_code {
                     {
                        query.push_kv("area-code", &aws_smithy_http::query::fmt_string(&inner_1));
                    }
                }
                if let Some(inner_2) = &_input.city {
                     {
                        query.push_kv("city", &aws_smithy_http::query::fmt_string(&inner_2));
                    }
                }
                if let Some(inner_3) = &_input.country {
                     {
                ...
                if let Some(inner_5) = &_input.toll_free_prefix {
                     {
                        query.push_kv("toll-free-prefix", &aws_smithy_http::query::fmt_string(&inner_5));
                    }
                }
                if let Some(inner_6) = &_input.phone_number_type {
                     {
        ...
        let op = aws_smithy_http::operation::Operation::new(request, crate::operation::search_available_phone_numbers::SearchAvailablePhoneNumbers::new())
                            .with_metadata(aws_smithy_http::operation::Metadata::new("SearchAvailablePhoneNumbers", "chime"));
