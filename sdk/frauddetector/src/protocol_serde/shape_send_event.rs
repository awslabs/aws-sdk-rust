    ...
    Err(match error_code {
        ...
        "ConflictException" => crate::operation::send_event::SendEventError::ConflictException({
            ...
            let mut tmp = {
                ...
                let _ = response;
                output = crate::protocol_serde::shape_conflict_exception::de_conflict_exception_json_err(response.body().as_ref(), output).map_err(crate::operation::send_event::SendEventError::unhandled)?;
                let output = output.meta(generic);
                output.build()
            };
            if tmp.message.is_none() {
        ...
        "ResourceNotFoundException" => {
            crate::operation::send_event::SendEventError::ResourceNotFoundException({
                ...
                let mut tmp = {
                    ...
                    let _ = response;
                    output = crate::protocol_serde::shape_resource_not_found_exception::de_resource_not_found_exception_json_err(response.body().as_ref(), output).map_err(crate::operation::send_event::SendEventError::unhandled)?;
                    let output = output.meta(generic);
                    output.build()
                };
                if tmp.message.is_none() {
