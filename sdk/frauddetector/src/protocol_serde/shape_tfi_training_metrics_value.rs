                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "metricDataPoints" => {
                            builder = builder.set_metric_data_points(
                                crate::protocol_serde::shape_tfi_metric_data_points_list::de_tfi_metric_data_points_list(tokens, _value, depth + 1)?,
                            );
                        }
                        "modelPerformance" => {
                            builder = builder.set_model_performance(crate::protocol_serde::shape_tfi_model_performance::de_tfi_model_performance(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
