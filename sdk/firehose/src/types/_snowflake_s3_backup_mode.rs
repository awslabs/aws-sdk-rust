...
/// When writing a match expression against `SnowflakeS3BackupMode`, it is important to ensure
...
///     SnowflakeS3BackupMode::AllData => { /* ... */ },
///     SnowflakeS3BackupMode::FailedDataOnly => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `snowflakes3backupmode` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `SnowflakeS3BackupMode::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `SnowflakeS3BackupMode::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `SnowflakeS3BackupMode::NewFeature` is defined.
