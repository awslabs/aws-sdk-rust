...
pub(crate) fn de_elasticsearch_destination_description<'a, I>(
    tokens: &mut ::std::iter::Peekable<I>,
    _value: &'a [u8],
) -> ::std::result::Result<Option<crate::types::ElasticsearchDestinationDescription>, ::aws_smithy_json::deserialize::error::DeserializeError>
where
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "IndexRotationPeriod" => {
                            builder = builder.set_index_rotation_period(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::ElasticsearchIndexRotationPeriod::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "BufferingHints" => {
                            builder = builder.set_buffering_hints(
                                crate::protocol_serde::shape_elasticsearch_buffering_hints::de_elasticsearch_buffering_hints(tokens, _value)?,
                            );
                        }
                        "RetryOptions" => {
                            builder = builder.set_retry_options(
                                crate::protocol_serde::shape_elasticsearch_retry_options::de_elasticsearch_retry_options(tokens, _value)?,
                            );
                        }
                        "S3BackupMode" => {
                            builder = builder.set_s3_backup_mode(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::ElasticsearchS3BackupMode::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
