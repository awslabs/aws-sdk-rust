...
pub fn ser_iceberg_destination_update(
    ...
    if let Some(var_15) = &input.catalog_configuration {
        #[allow(unused_mut)]
        let mut object_16 = object.key("CatalogConfiguration").start_object();
        crate::protocol_serde::shape_catalog_configuration::ser_catalog_configuration(&mut object_16, var_15)?;
        object_16.finish();
    }
    if let Some(var_17) = &input.s3_configuration {
        #[allow(unused_mut)]
        let mut object_18 = object.key("S3Configuration").start_object();
        crate::protocol_serde::shape_s3_destination_configuration::ser_s3_destination_configuration(&mut object_18, var_17)?;
        object_18.finish();
    }
    Ok(())
}
