// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn serialize_structure_batch_delete_builds_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::BatchDeleteBuildsInput,
) {
    if let Some(var_1) = &input.ids {
        let mut array_2 = object.key("ids").start_array();
        for item_3 in var_1 {
            {
                array_2.value().string(item_3);
            }
        }
        array_2.finish();
    }
}

pub fn serialize_structure_batch_get_build_batches_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::BatchGetBuildBatchesInput,
) {
    if let Some(var_4) = &input.ids {
        let mut array_5 = object.key("ids").start_array();
        for item_6 in var_4 {
            {
                array_5.value().string(item_6);
            }
        }
        array_5.finish();
    }
}

pub fn serialize_structure_batch_get_builds_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::BatchGetBuildsInput,
) {
    if let Some(var_7) = &input.ids {
        let mut array_8 = object.key("ids").start_array();
        for item_9 in var_7 {
            {
                array_8.value().string(item_9);
            }
        }
        array_8.finish();
    }
}

pub fn serialize_structure_batch_get_projects_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::BatchGetProjectsInput,
) {
    if let Some(var_10) = &input.names {
        let mut array_11 = object.key("names").start_array();
        for item_12 in var_10 {
            {
                array_11.value().string(item_12);
            }
        }
        array_11.finish();
    }
}

pub fn serialize_structure_batch_get_report_groups_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::BatchGetReportGroupsInput,
) {
    if let Some(var_13) = &input.report_group_arns {
        let mut array_14 = object.key("reportGroupArns").start_array();
        for item_15 in var_13 {
            {
                array_14.value().string(item_15);
            }
        }
        array_14.finish();
    }
}

pub fn serialize_structure_batch_get_reports_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::BatchGetReportsInput,
) {
    if let Some(var_16) = &input.report_arns {
        let mut array_17 = object.key("reportArns").start_array();
        for item_18 in var_16 {
            {
                array_17.value().string(item_18);
            }
        }
        array_17.finish();
    }
}

pub fn serialize_structure_create_project_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::CreateProjectInput,
) {
    if let Some(var_19) = &input.name {
        object.key("name").string(var_19);
    }
    if let Some(var_20) = &input.description {
        object.key("description").string(var_20);
    }
    if let Some(var_21) = &input.source {
        let mut object_22 = object.key("source").start_object();
        crate::json_ser::serialize_structure_project_source(&mut object_22, var_21);
        object_22.finish();
    }
    if let Some(var_23) = &input.secondary_sources {
        let mut array_24 = object.key("secondarySources").start_array();
        for item_25 in var_23 {
            {
                let mut object_26 = array_24.value().start_object();
                crate::json_ser::serialize_structure_project_source(&mut object_26, item_25);
                object_26.finish();
            }
        }
        array_24.finish();
    }
    if let Some(var_27) = &input.source_version {
        object.key("sourceVersion").string(var_27);
    }
    if let Some(var_28) = &input.secondary_source_versions {
        let mut array_29 = object.key("secondarySourceVersions").start_array();
        for item_30 in var_28 {
            {
                let mut object_31 = array_29.value().start_object();
                crate::json_ser::serialize_structure_project_source_version(
                    &mut object_31,
                    item_30,
                );
                object_31.finish();
            }
        }
        array_29.finish();
    }
    if let Some(var_32) = &input.artifacts {
        let mut object_33 = object.key("artifacts").start_object();
        crate::json_ser::serialize_structure_project_artifacts(&mut object_33, var_32);
        object_33.finish();
    }
    if let Some(var_34) = &input.secondary_artifacts {
        let mut array_35 = object.key("secondaryArtifacts").start_array();
        for item_36 in var_34 {
            {
                let mut object_37 = array_35.value().start_object();
                crate::json_ser::serialize_structure_project_artifacts(&mut object_37, item_36);
                object_37.finish();
            }
        }
        array_35.finish();
    }
    if let Some(var_38) = &input.cache {
        let mut object_39 = object.key("cache").start_object();
        crate::json_ser::serialize_structure_project_cache(&mut object_39, var_38);
        object_39.finish();
    }
    if let Some(var_40) = &input.environment {
        let mut object_41 = object.key("environment").start_object();
        crate::json_ser::serialize_structure_project_environment(&mut object_41, var_40);
        object_41.finish();
    }
    if let Some(var_42) = &input.service_role {
        object.key("serviceRole").string(var_42);
    }
    if let Some(var_43) = &input.timeout_in_minutes {
        object.key("timeoutInMinutes").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_43).into()),
        );
    }
    if let Some(var_44) = &input.queued_timeout_in_minutes {
        object.key("queuedTimeoutInMinutes").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_44).into()),
        );
    }
    if let Some(var_45) = &input.encryption_key {
        object.key("encryptionKey").string(var_45);
    }
    if let Some(var_46) = &input.tags {
        let mut array_47 = object.key("tags").start_array();
        for item_48 in var_46 {
            {
                let mut object_49 = array_47.value().start_object();
                crate::json_ser::serialize_structure_tag(&mut object_49, item_48);
                object_49.finish();
            }
        }
        array_47.finish();
    }
    if let Some(var_50) = &input.vpc_config {
        let mut object_51 = object.key("vpcConfig").start_object();
        crate::json_ser::serialize_structure_vpc_config(&mut object_51, var_50);
        object_51.finish();
    }
    if let Some(var_52) = &input.badge_enabled {
        object.key("badgeEnabled").boolean(*var_52);
    }
    if let Some(var_53) = &input.logs_config {
        let mut object_54 = object.key("logsConfig").start_object();
        crate::json_ser::serialize_structure_logs_config(&mut object_54, var_53);
        object_54.finish();
    }
    if let Some(var_55) = &input.file_system_locations {
        let mut array_56 = object.key("fileSystemLocations").start_array();
        for item_57 in var_55 {
            {
                let mut object_58 = array_56.value().start_object();
                crate::json_ser::serialize_structure_project_file_system_location(
                    &mut object_58,
                    item_57,
                );
                object_58.finish();
            }
        }
        array_56.finish();
    }
    if let Some(var_59) = &input.build_batch_config {
        let mut object_60 = object.key("buildBatchConfig").start_object();
        crate::json_ser::serialize_structure_project_build_batch_config(&mut object_60, var_59);
        object_60.finish();
    }
    if let Some(var_61) = &input.concurrent_build_limit {
        object.key("concurrentBuildLimit").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_61).into()),
        );
    }
}

pub fn serialize_structure_create_report_group_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::CreateReportGroupInput,
) {
    if let Some(var_62) = &input.name {
        object.key("name").string(var_62);
    }
    if let Some(var_63) = &input.r#type {
        object.key("type").string(var_63.as_str());
    }
    if let Some(var_64) = &input.export_config {
        let mut object_65 = object.key("exportConfig").start_object();
        crate::json_ser::serialize_structure_report_export_config(&mut object_65, var_64);
        object_65.finish();
    }
    if let Some(var_66) = &input.tags {
        let mut array_67 = object.key("tags").start_array();
        for item_68 in var_66 {
            {
                let mut object_69 = array_67.value().start_object();
                crate::json_ser::serialize_structure_tag(&mut object_69, item_68);
                object_69.finish();
            }
        }
        array_67.finish();
    }
}

pub fn serialize_structure_create_webhook_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::CreateWebhookInput,
) {
    if let Some(var_70) = &input.project_name {
        object.key("projectName").string(var_70);
    }
    if let Some(var_71) = &input.branch_filter {
        object.key("branchFilter").string(var_71);
    }
    if let Some(var_72) = &input.filter_groups {
        let mut array_73 = object.key("filterGroups").start_array();
        for item_74 in var_72 {
            {
                let mut array_75 = array_73.value().start_array();
                for item_76 in item_74 {
                    {
                        let mut object_77 = array_75.value().start_object();
                        crate::json_ser::serialize_structure_webhook_filter(
                            &mut object_77,
                            item_76,
                        );
                        object_77.finish();
                    }
                }
                array_75.finish();
            }
        }
        array_73.finish();
    }
    if let Some(var_78) = &input.build_type {
        object.key("buildType").string(var_78.as_str());
    }
}

pub fn serialize_structure_delete_build_batch_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DeleteBuildBatchInput,
) {
    if let Some(var_79) = &input.id {
        object.key("id").string(var_79);
    }
}

pub fn serialize_structure_delete_project_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DeleteProjectInput,
) {
    if let Some(var_80) = &input.name {
        object.key("name").string(var_80);
    }
}

pub fn serialize_structure_delete_report_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DeleteReportInput,
) {
    if let Some(var_81) = &input.arn {
        object.key("arn").string(var_81);
    }
}

pub fn serialize_structure_delete_report_group_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DeleteReportGroupInput,
) {
    if let Some(var_82) = &input.arn {
        object.key("arn").string(var_82);
    }
    if input.delete_reports {
        object.key("deleteReports").boolean(input.delete_reports);
    }
}

pub fn serialize_structure_delete_resource_policy_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DeleteResourcePolicyInput,
) {
    if let Some(var_83) = &input.resource_arn {
        object.key("resourceArn").string(var_83);
    }
}

pub fn serialize_structure_delete_source_credentials_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DeleteSourceCredentialsInput,
) {
    if let Some(var_84) = &input.arn {
        object.key("arn").string(var_84);
    }
}

pub fn serialize_structure_delete_webhook_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DeleteWebhookInput,
) {
    if let Some(var_85) = &input.project_name {
        object.key("projectName").string(var_85);
    }
}

pub fn serialize_structure_describe_code_coverages_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DescribeCodeCoveragesInput,
) {
    if let Some(var_86) = &input.report_arn {
        object.key("reportArn").string(var_86);
    }
    if let Some(var_87) = &input.next_token {
        object.key("nextToken").string(var_87);
    }
    if let Some(var_88) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_88).into()),
        );
    }
    if let Some(var_89) = &input.sort_order {
        object.key("sortOrder").string(var_89.as_str());
    }
    if let Some(var_90) = &input.sort_by {
        object.key("sortBy").string(var_90.as_str());
    }
    if let Some(var_91) = &input.min_line_coverage_percentage {
        object.key("minLineCoveragePercentage").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::Float((*var_91).into()),
        );
    }
    if let Some(var_92) = &input.max_line_coverage_percentage {
        object.key("maxLineCoveragePercentage").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::Float((*var_92).into()),
        );
    }
}

pub fn serialize_structure_describe_test_cases_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::DescribeTestCasesInput,
) {
    if let Some(var_93) = &input.report_arn {
        object.key("reportArn").string(var_93);
    }
    if let Some(var_94) = &input.next_token {
        object.key("nextToken").string(var_94);
    }
    if let Some(var_95) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_95).into()),
        );
    }
    if let Some(var_96) = &input.filter {
        let mut object_97 = object.key("filter").start_object();
        crate::json_ser::serialize_structure_test_case_filter(&mut object_97, var_96);
        object_97.finish();
    }
}

pub fn serialize_structure_get_report_group_trend_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::GetReportGroupTrendInput,
) {
    if let Some(var_98) = &input.report_group_arn {
        object.key("reportGroupArn").string(var_98);
    }
    if let Some(var_99) = &input.num_of_reports {
        object.key("numOfReports").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_99).into()),
        );
    }
    if let Some(var_100) = &input.trend_field {
        object.key("trendField").string(var_100.as_str());
    }
}

pub fn serialize_structure_get_resource_policy_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::GetResourcePolicyInput,
) {
    if let Some(var_101) = &input.resource_arn {
        object.key("resourceArn").string(var_101);
    }
}

pub fn serialize_structure_import_source_credentials_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ImportSourceCredentialsInput,
) {
    if let Some(var_102) = &input.username {
        object.key("username").string(var_102);
    }
    if let Some(var_103) = &input.token {
        object.key("token").string(var_103);
    }
    if let Some(var_104) = &input.server_type {
        object.key("serverType").string(var_104.as_str());
    }
    if let Some(var_105) = &input.auth_type {
        object.key("authType").string(var_105.as_str());
    }
    if let Some(var_106) = &input.should_overwrite {
        object.key("shouldOverwrite").boolean(*var_106);
    }
}

pub fn serialize_structure_invalidate_project_cache_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::InvalidateProjectCacheInput,
) {
    if let Some(var_107) = &input.project_name {
        object.key("projectName").string(var_107);
    }
}

pub fn serialize_structure_list_build_batches_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListBuildBatchesInput,
) {
    if let Some(var_108) = &input.filter {
        let mut object_109 = object.key("filter").start_object();
        crate::json_ser::serialize_structure_build_batch_filter(&mut object_109, var_108);
        object_109.finish();
    }
    if let Some(var_110) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_110).into()),
        );
    }
    if let Some(var_111) = &input.sort_order {
        object.key("sortOrder").string(var_111.as_str());
    }
    if let Some(var_112) = &input.next_token {
        object.key("nextToken").string(var_112);
    }
}

pub fn serialize_structure_list_build_batches_for_project_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListBuildBatchesForProjectInput,
) {
    if let Some(var_113) = &input.project_name {
        object.key("projectName").string(var_113);
    }
    if let Some(var_114) = &input.filter {
        let mut object_115 = object.key("filter").start_object();
        crate::json_ser::serialize_structure_build_batch_filter(&mut object_115, var_114);
        object_115.finish();
    }
    if let Some(var_116) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_116).into()),
        );
    }
    if let Some(var_117) = &input.sort_order {
        object.key("sortOrder").string(var_117.as_str());
    }
    if let Some(var_118) = &input.next_token {
        object.key("nextToken").string(var_118);
    }
}

pub fn serialize_structure_list_builds_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListBuildsInput,
) {
    if let Some(var_119) = &input.sort_order {
        object.key("sortOrder").string(var_119.as_str());
    }
    if let Some(var_120) = &input.next_token {
        object.key("nextToken").string(var_120);
    }
}

pub fn serialize_structure_list_builds_for_project_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListBuildsForProjectInput,
) {
    if let Some(var_121) = &input.project_name {
        object.key("projectName").string(var_121);
    }
    if let Some(var_122) = &input.sort_order {
        object.key("sortOrder").string(var_122.as_str());
    }
    if let Some(var_123) = &input.next_token {
        object.key("nextToken").string(var_123);
    }
}

pub fn serialize_structure_list_projects_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListProjectsInput,
) {
    if let Some(var_124) = &input.sort_by {
        object.key("sortBy").string(var_124.as_str());
    }
    if let Some(var_125) = &input.sort_order {
        object.key("sortOrder").string(var_125.as_str());
    }
    if let Some(var_126) = &input.next_token {
        object.key("nextToken").string(var_126);
    }
}

pub fn serialize_structure_list_report_groups_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListReportGroupsInput,
) {
    if let Some(var_127) = &input.sort_order {
        object.key("sortOrder").string(var_127.as_str());
    }
    if let Some(var_128) = &input.sort_by {
        object.key("sortBy").string(var_128.as_str());
    }
    if let Some(var_129) = &input.next_token {
        object.key("nextToken").string(var_129);
    }
    if let Some(var_130) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_130).into()),
        );
    }
}

pub fn serialize_structure_list_reports_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListReportsInput,
) {
    if let Some(var_131) = &input.sort_order {
        object.key("sortOrder").string(var_131.as_str());
    }
    if let Some(var_132) = &input.next_token {
        object.key("nextToken").string(var_132);
    }
    if let Some(var_133) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_133).into()),
        );
    }
    if let Some(var_134) = &input.filter {
        let mut object_135 = object.key("filter").start_object();
        crate::json_ser::serialize_structure_report_filter(&mut object_135, var_134);
        object_135.finish();
    }
}

pub fn serialize_structure_list_reports_for_report_group_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListReportsForReportGroupInput,
) {
    if let Some(var_136) = &input.report_group_arn {
        object.key("reportGroupArn").string(var_136);
    }
    if let Some(var_137) = &input.next_token {
        object.key("nextToken").string(var_137);
    }
    if let Some(var_138) = &input.sort_order {
        object.key("sortOrder").string(var_138.as_str());
    }
    if let Some(var_139) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_139).into()),
        );
    }
    if let Some(var_140) = &input.filter {
        let mut object_141 = object.key("filter").start_object();
        crate::json_ser::serialize_structure_report_filter(&mut object_141, var_140);
        object_141.finish();
    }
}

pub fn serialize_structure_list_shared_projects_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListSharedProjectsInput,
) {
    if let Some(var_142) = &input.sort_by {
        object.key("sortBy").string(var_142.as_str());
    }
    if let Some(var_143) = &input.sort_order {
        object.key("sortOrder").string(var_143.as_str());
    }
    if let Some(var_144) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_144).into()),
        );
    }
    if let Some(var_145) = &input.next_token {
        object.key("nextToken").string(var_145);
    }
}

pub fn serialize_structure_list_shared_report_groups_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::ListSharedReportGroupsInput,
) {
    if let Some(var_146) = &input.sort_order {
        object.key("sortOrder").string(var_146.as_str());
    }
    if let Some(var_147) = &input.sort_by {
        object.key("sortBy").string(var_147.as_str());
    }
    if let Some(var_148) = &input.next_token {
        object.key("nextToken").string(var_148);
    }
    if let Some(var_149) = &input.max_results {
        object.key("maxResults").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_149).into()),
        );
    }
}

pub fn serialize_structure_put_resource_policy_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::PutResourcePolicyInput,
) {
    if let Some(var_150) = &input.policy {
        object.key("policy").string(var_150);
    }
    if let Some(var_151) = &input.resource_arn {
        object.key("resourceArn").string(var_151);
    }
}

pub fn serialize_structure_retry_build_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::RetryBuildInput,
) {
    if let Some(var_152) = &input.id {
        object.key("id").string(var_152);
    }
    if let Some(var_153) = &input.idempotency_token {
        object.key("idempotencyToken").string(var_153);
    }
}

pub fn serialize_structure_retry_build_batch_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::RetryBuildBatchInput,
) {
    if let Some(var_154) = &input.id {
        object.key("id").string(var_154);
    }
    if let Some(var_155) = &input.idempotency_token {
        object.key("idempotencyToken").string(var_155);
    }
    if let Some(var_156) = &input.retry_type {
        object.key("retryType").string(var_156.as_str());
    }
}

pub fn serialize_structure_start_build_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::StartBuildInput,
) {
    if let Some(var_157) = &input.project_name {
        object.key("projectName").string(var_157);
    }
    if let Some(var_158) = &input.secondary_sources_override {
        let mut array_159 = object.key("secondarySourcesOverride").start_array();
        for item_160 in var_158 {
            {
                let mut object_161 = array_159.value().start_object();
                crate::json_ser::serialize_structure_project_source(&mut object_161, item_160);
                object_161.finish();
            }
        }
        array_159.finish();
    }
    if let Some(var_162) = &input.secondary_sources_version_override {
        let mut array_163 = object.key("secondarySourcesVersionOverride").start_array();
        for item_164 in var_162 {
            {
                let mut object_165 = array_163.value().start_object();
                crate::json_ser::serialize_structure_project_source_version(
                    &mut object_165,
                    item_164,
                );
                object_165.finish();
            }
        }
        array_163.finish();
    }
    if let Some(var_166) = &input.source_version {
        object.key("sourceVersion").string(var_166);
    }
    if let Some(var_167) = &input.artifacts_override {
        let mut object_168 = object.key("artifactsOverride").start_object();
        crate::json_ser::serialize_structure_project_artifacts(&mut object_168, var_167);
        object_168.finish();
    }
    if let Some(var_169) = &input.secondary_artifacts_override {
        let mut array_170 = object.key("secondaryArtifactsOverride").start_array();
        for item_171 in var_169 {
            {
                let mut object_172 = array_170.value().start_object();
                crate::json_ser::serialize_structure_project_artifacts(&mut object_172, item_171);
                object_172.finish();
            }
        }
        array_170.finish();
    }
    if let Some(var_173) = &input.environment_variables_override {
        let mut array_174 = object.key("environmentVariablesOverride").start_array();
        for item_175 in var_173 {
            {
                let mut object_176 = array_174.value().start_object();
                crate::json_ser::serialize_structure_environment_variable(
                    &mut object_176,
                    item_175,
                );
                object_176.finish();
            }
        }
        array_174.finish();
    }
    if let Some(var_177) = &input.source_type_override {
        object.key("sourceTypeOverride").string(var_177.as_str());
    }
    if let Some(var_178) = &input.source_location_override {
        object.key("sourceLocationOverride").string(var_178);
    }
    if let Some(var_179) = &input.source_auth_override {
        let mut object_180 = object.key("sourceAuthOverride").start_object();
        crate::json_ser::serialize_structure_source_auth(&mut object_180, var_179);
        object_180.finish();
    }
    if let Some(var_181) = &input.git_clone_depth_override {
        object.key("gitCloneDepthOverride").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_181).into()),
        );
    }
    if let Some(var_182) = &input.git_submodules_config_override {
        let mut object_183 = object.key("gitSubmodulesConfigOverride").start_object();
        crate::json_ser::serialize_structure_git_submodules_config(&mut object_183, var_182);
        object_183.finish();
    }
    if let Some(var_184) = &input.buildspec_override {
        object.key("buildspecOverride").string(var_184);
    }
    if let Some(var_185) = &input.insecure_ssl_override {
        object.key("insecureSslOverride").boolean(*var_185);
    }
    if let Some(var_186) = &input.report_build_status_override {
        object.key("reportBuildStatusOverride").boolean(*var_186);
    }
    if let Some(var_187) = &input.build_status_config_override {
        let mut object_188 = object.key("buildStatusConfigOverride").start_object();
        crate::json_ser::serialize_structure_build_status_config(&mut object_188, var_187);
        object_188.finish();
    }
    if let Some(var_189) = &input.environment_type_override {
        object
            .key("environmentTypeOverride")
            .string(var_189.as_str());
    }
    if let Some(var_190) = &input.image_override {
        object.key("imageOverride").string(var_190);
    }
    if let Some(var_191) = &input.compute_type_override {
        object.key("computeTypeOverride").string(var_191.as_str());
    }
    if let Some(var_192) = &input.certificate_override {
        object.key("certificateOverride").string(var_192);
    }
    if let Some(var_193) = &input.cache_override {
        let mut object_194 = object.key("cacheOverride").start_object();
        crate::json_ser::serialize_structure_project_cache(&mut object_194, var_193);
        object_194.finish();
    }
    if let Some(var_195) = &input.service_role_override {
        object.key("serviceRoleOverride").string(var_195);
    }
    if let Some(var_196) = &input.privileged_mode_override {
        object.key("privilegedModeOverride").boolean(*var_196);
    }
    if let Some(var_197) = &input.timeout_in_minutes_override {
        object.key("timeoutInMinutesOverride").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_197).into()),
        );
    }
    if let Some(var_198) = &input.queued_timeout_in_minutes_override {
        object.key("queuedTimeoutInMinutesOverride").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_198).into()),
        );
    }
    if let Some(var_199) = &input.encryption_key_override {
        object.key("encryptionKeyOverride").string(var_199);
    }
    if let Some(var_200) = &input.idempotency_token {
        object.key("idempotencyToken").string(var_200);
    }
    if let Some(var_201) = &input.logs_config_override {
        let mut object_202 = object.key("logsConfigOverride").start_object();
        crate::json_ser::serialize_structure_logs_config(&mut object_202, var_201);
        object_202.finish();
    }
    if let Some(var_203) = &input.registry_credential_override {
        let mut object_204 = object.key("registryCredentialOverride").start_object();
        crate::json_ser::serialize_structure_registry_credential(&mut object_204, var_203);
        object_204.finish();
    }
    if let Some(var_205) = &input.image_pull_credentials_type_override {
        object
            .key("imagePullCredentialsTypeOverride")
            .string(var_205.as_str());
    }
    if let Some(var_206) = &input.debug_session_enabled {
        object.key("debugSessionEnabled").boolean(*var_206);
    }
}

pub fn serialize_structure_start_build_batch_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::StartBuildBatchInput,
) {
    if let Some(var_207) = &input.project_name {
        object.key("projectName").string(var_207);
    }
    if let Some(var_208) = &input.secondary_sources_override {
        let mut array_209 = object.key("secondarySourcesOverride").start_array();
        for item_210 in var_208 {
            {
                let mut object_211 = array_209.value().start_object();
                crate::json_ser::serialize_structure_project_source(&mut object_211, item_210);
                object_211.finish();
            }
        }
        array_209.finish();
    }
    if let Some(var_212) = &input.secondary_sources_version_override {
        let mut array_213 = object.key("secondarySourcesVersionOverride").start_array();
        for item_214 in var_212 {
            {
                let mut object_215 = array_213.value().start_object();
                crate::json_ser::serialize_structure_project_source_version(
                    &mut object_215,
                    item_214,
                );
                object_215.finish();
            }
        }
        array_213.finish();
    }
    if let Some(var_216) = &input.source_version {
        object.key("sourceVersion").string(var_216);
    }
    if let Some(var_217) = &input.artifacts_override {
        let mut object_218 = object.key("artifactsOverride").start_object();
        crate::json_ser::serialize_structure_project_artifacts(&mut object_218, var_217);
        object_218.finish();
    }
    if let Some(var_219) = &input.secondary_artifacts_override {
        let mut array_220 = object.key("secondaryArtifactsOverride").start_array();
        for item_221 in var_219 {
            {
                let mut object_222 = array_220.value().start_object();
                crate::json_ser::serialize_structure_project_artifacts(&mut object_222, item_221);
                object_222.finish();
            }
        }
        array_220.finish();
    }
    if let Some(var_223) = &input.environment_variables_override {
        let mut array_224 = object.key("environmentVariablesOverride").start_array();
        for item_225 in var_223 {
            {
                let mut object_226 = array_224.value().start_object();
                crate::json_ser::serialize_structure_environment_variable(
                    &mut object_226,
                    item_225,
                );
                object_226.finish();
            }
        }
        array_224.finish();
    }
    if let Some(var_227) = &input.source_type_override {
        object.key("sourceTypeOverride").string(var_227.as_str());
    }
    if let Some(var_228) = &input.source_location_override {
        object.key("sourceLocationOverride").string(var_228);
    }
    if let Some(var_229) = &input.source_auth_override {
        let mut object_230 = object.key("sourceAuthOverride").start_object();
        crate::json_ser::serialize_structure_source_auth(&mut object_230, var_229);
        object_230.finish();
    }
    if let Some(var_231) = &input.git_clone_depth_override {
        object.key("gitCloneDepthOverride").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_231).into()),
        );
    }
    if let Some(var_232) = &input.git_submodules_config_override {
        let mut object_233 = object.key("gitSubmodulesConfigOverride").start_object();
        crate::json_ser::serialize_structure_git_submodules_config(&mut object_233, var_232);
        object_233.finish();
    }
    if let Some(var_234) = &input.buildspec_override {
        object.key("buildspecOverride").string(var_234);
    }
    if let Some(var_235) = &input.insecure_ssl_override {
        object.key("insecureSslOverride").boolean(*var_235);
    }
    if let Some(var_236) = &input.report_build_batch_status_override {
        object
            .key("reportBuildBatchStatusOverride")
            .boolean(*var_236);
    }
    if let Some(var_237) = &input.environment_type_override {
        object
            .key("environmentTypeOverride")
            .string(var_237.as_str());
    }
    if let Some(var_238) = &input.image_override {
        object.key("imageOverride").string(var_238);
    }
    if let Some(var_239) = &input.compute_type_override {
        object.key("computeTypeOverride").string(var_239.as_str());
    }
    if let Some(var_240) = &input.certificate_override {
        object.key("certificateOverride").string(var_240);
    }
    if let Some(var_241) = &input.cache_override {
        let mut object_242 = object.key("cacheOverride").start_object();
        crate::json_ser::serialize_structure_project_cache(&mut object_242, var_241);
        object_242.finish();
    }
    if let Some(var_243) = &input.service_role_override {
        object.key("serviceRoleOverride").string(var_243);
    }
    if let Some(var_244) = &input.privileged_mode_override {
        object.key("privilegedModeOverride").boolean(*var_244);
    }
    if let Some(var_245) = &input.build_timeout_in_minutes_override {
        object.key("buildTimeoutInMinutesOverride").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_245).into()),
        );
    }
    if let Some(var_246) = &input.queued_timeout_in_minutes_override {
        object.key("queuedTimeoutInMinutesOverride").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_246).into()),
        );
    }
    if let Some(var_247) = &input.encryption_key_override {
        object.key("encryptionKeyOverride").string(var_247);
    }
    if let Some(var_248) = &input.idempotency_token {
        object.key("idempotencyToken").string(var_248);
    }
    if let Some(var_249) = &input.logs_config_override {
        let mut object_250 = object.key("logsConfigOverride").start_object();
        crate::json_ser::serialize_structure_logs_config(&mut object_250, var_249);
        object_250.finish();
    }
    if let Some(var_251) = &input.registry_credential_override {
        let mut object_252 = object.key("registryCredentialOverride").start_object();
        crate::json_ser::serialize_structure_registry_credential(&mut object_252, var_251);
        object_252.finish();
    }
    if let Some(var_253) = &input.image_pull_credentials_type_override {
        object
            .key("imagePullCredentialsTypeOverride")
            .string(var_253.as_str());
    }
    if let Some(var_254) = &input.build_batch_config_override {
        let mut object_255 = object.key("buildBatchConfigOverride").start_object();
        crate::json_ser::serialize_structure_project_build_batch_config(&mut object_255, var_254);
        object_255.finish();
    }
    if let Some(var_256) = &input.debug_session_enabled {
        object.key("debugSessionEnabled").boolean(*var_256);
    }
}

pub fn serialize_structure_stop_build_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::StopBuildInput,
) {
    if let Some(var_257) = &input.id {
        object.key("id").string(var_257);
    }
}

pub fn serialize_structure_stop_build_batch_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::StopBuildBatchInput,
) {
    if let Some(var_258) = &input.id {
        object.key("id").string(var_258);
    }
}

pub fn serialize_structure_update_project_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::UpdateProjectInput,
) {
    if let Some(var_259) = &input.name {
        object.key("name").string(var_259);
    }
    if let Some(var_260) = &input.description {
        object.key("description").string(var_260);
    }
    if let Some(var_261) = &input.source {
        let mut object_262 = object.key("source").start_object();
        crate::json_ser::serialize_structure_project_source(&mut object_262, var_261);
        object_262.finish();
    }
    if let Some(var_263) = &input.secondary_sources {
        let mut array_264 = object.key("secondarySources").start_array();
        for item_265 in var_263 {
            {
                let mut object_266 = array_264.value().start_object();
                crate::json_ser::serialize_structure_project_source(&mut object_266, item_265);
                object_266.finish();
            }
        }
        array_264.finish();
    }
    if let Some(var_267) = &input.source_version {
        object.key("sourceVersion").string(var_267);
    }
    if let Some(var_268) = &input.secondary_source_versions {
        let mut array_269 = object.key("secondarySourceVersions").start_array();
        for item_270 in var_268 {
            {
                let mut object_271 = array_269.value().start_object();
                crate::json_ser::serialize_structure_project_source_version(
                    &mut object_271,
                    item_270,
                );
                object_271.finish();
            }
        }
        array_269.finish();
    }
    if let Some(var_272) = &input.artifacts {
        let mut object_273 = object.key("artifacts").start_object();
        crate::json_ser::serialize_structure_project_artifacts(&mut object_273, var_272);
        object_273.finish();
    }
    if let Some(var_274) = &input.secondary_artifacts {
        let mut array_275 = object.key("secondaryArtifacts").start_array();
        for item_276 in var_274 {
            {
                let mut object_277 = array_275.value().start_object();
                crate::json_ser::serialize_structure_project_artifacts(&mut object_277, item_276);
                object_277.finish();
            }
        }
        array_275.finish();
    }
    if let Some(var_278) = &input.cache {
        let mut object_279 = object.key("cache").start_object();
        crate::json_ser::serialize_structure_project_cache(&mut object_279, var_278);
        object_279.finish();
    }
    if let Some(var_280) = &input.environment {
        let mut object_281 = object.key("environment").start_object();
        crate::json_ser::serialize_structure_project_environment(&mut object_281, var_280);
        object_281.finish();
    }
    if let Some(var_282) = &input.service_role {
        object.key("serviceRole").string(var_282);
    }
    if let Some(var_283) = &input.timeout_in_minutes {
        object.key("timeoutInMinutes").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_283).into()),
        );
    }
    if let Some(var_284) = &input.queued_timeout_in_minutes {
        object.key("queuedTimeoutInMinutes").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_284).into()),
        );
    }
    if let Some(var_285) = &input.encryption_key {
        object.key("encryptionKey").string(var_285);
    }
    if let Some(var_286) = &input.tags {
        let mut array_287 = object.key("tags").start_array();
        for item_288 in var_286 {
            {
                let mut object_289 = array_287.value().start_object();
                crate::json_ser::serialize_structure_tag(&mut object_289, item_288);
                object_289.finish();
            }
        }
        array_287.finish();
    }
    if let Some(var_290) = &input.vpc_config {
        let mut object_291 = object.key("vpcConfig").start_object();
        crate::json_ser::serialize_structure_vpc_config(&mut object_291, var_290);
        object_291.finish();
    }
    if let Some(var_292) = &input.badge_enabled {
        object.key("badgeEnabled").boolean(*var_292);
    }
    if let Some(var_293) = &input.logs_config {
        let mut object_294 = object.key("logsConfig").start_object();
        crate::json_ser::serialize_structure_logs_config(&mut object_294, var_293);
        object_294.finish();
    }
    if let Some(var_295) = &input.file_system_locations {
        let mut array_296 = object.key("fileSystemLocations").start_array();
        for item_297 in var_295 {
            {
                let mut object_298 = array_296.value().start_object();
                crate::json_ser::serialize_structure_project_file_system_location(
                    &mut object_298,
                    item_297,
                );
                object_298.finish();
            }
        }
        array_296.finish();
    }
    if let Some(var_299) = &input.build_batch_config {
        let mut object_300 = object.key("buildBatchConfig").start_object();
        crate::json_ser::serialize_structure_project_build_batch_config(&mut object_300, var_299);
        object_300.finish();
    }
    if let Some(var_301) = &input.concurrent_build_limit {
        object.key("concurrentBuildLimit").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_301).into()),
        );
    }
}

pub fn serialize_structure_update_report_group_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::UpdateReportGroupInput,
) {
    if let Some(var_302) = &input.arn {
        object.key("arn").string(var_302);
    }
    if let Some(var_303) = &input.export_config {
        let mut object_304 = object.key("exportConfig").start_object();
        crate::json_ser::serialize_structure_report_export_config(&mut object_304, var_303);
        object_304.finish();
    }
    if let Some(var_305) = &input.tags {
        let mut array_306 = object.key("tags").start_array();
        for item_307 in var_305 {
            {
                let mut object_308 = array_306.value().start_object();
                crate::json_ser::serialize_structure_tag(&mut object_308, item_307);
                object_308.finish();
            }
        }
        array_306.finish();
    }
}

pub fn serialize_structure_update_webhook_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::UpdateWebhookInput,
) {
    if let Some(var_309) = &input.project_name {
        object.key("projectName").string(var_309);
    }
    if let Some(var_310) = &input.branch_filter {
        object.key("branchFilter").string(var_310);
    }
    if input.rotate_secret {
        object.key("rotateSecret").boolean(input.rotate_secret);
    }
    if let Some(var_311) = &input.filter_groups {
        let mut array_312 = object.key("filterGroups").start_array();
        for item_313 in var_311 {
            {
                let mut array_314 = array_312.value().start_array();
                for item_315 in item_313 {
                    {
                        let mut object_316 = array_314.value().start_object();
                        crate::json_ser::serialize_structure_webhook_filter(
                            &mut object_316,
                            item_315,
                        );
                        object_316.finish();
                    }
                }
                array_314.finish();
            }
        }
        array_312.finish();
    }
    if let Some(var_317) = &input.build_type {
        object.key("buildType").string(var_317.as_str());
    }
}

pub fn serialize_structure_project_source(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ProjectSource,
) {
    if let Some(var_318) = &input.r#type {
        object.key("type").string(var_318.as_str());
    }
    if let Some(var_319) = &input.location {
        object.key("location").string(var_319);
    }
    if let Some(var_320) = &input.git_clone_depth {
        object.key("gitCloneDepth").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_320).into()),
        );
    }
    if let Some(var_321) = &input.git_submodules_config {
        let mut object_322 = object.key("gitSubmodulesConfig").start_object();
        crate::json_ser::serialize_structure_git_submodules_config(&mut object_322, var_321);
        object_322.finish();
    }
    if let Some(var_323) = &input.buildspec {
        object.key("buildspec").string(var_323);
    }
    if let Some(var_324) = &input.auth {
        let mut object_325 = object.key("auth").start_object();
        crate::json_ser::serialize_structure_source_auth(&mut object_325, var_324);
        object_325.finish();
    }
    if let Some(var_326) = &input.report_build_status {
        object.key("reportBuildStatus").boolean(*var_326);
    }
    if let Some(var_327) = &input.build_status_config {
        let mut object_328 = object.key("buildStatusConfig").start_object();
        crate::json_ser::serialize_structure_build_status_config(&mut object_328, var_327);
        object_328.finish();
    }
    if let Some(var_329) = &input.insecure_ssl {
        object.key("insecureSsl").boolean(*var_329);
    }
    if let Some(var_330) = &input.source_identifier {
        object.key("sourceIdentifier").string(var_330);
    }
}

pub fn serialize_structure_project_source_version(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ProjectSourceVersion,
) {
    if let Some(var_331) = &input.source_identifier {
        object.key("sourceIdentifier").string(var_331);
    }
    if let Some(var_332) = &input.source_version {
        object.key("sourceVersion").string(var_332);
    }
}

pub fn serialize_structure_project_artifacts(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ProjectArtifacts,
) {
    if let Some(var_333) = &input.r#type {
        object.key("type").string(var_333.as_str());
    }
    if let Some(var_334) = &input.location {
        object.key("location").string(var_334);
    }
    if let Some(var_335) = &input.path {
        object.key("path").string(var_335);
    }
    if let Some(var_336) = &input.namespace_type {
        object.key("namespaceType").string(var_336.as_str());
    }
    if let Some(var_337) = &input.name {
        object.key("name").string(var_337);
    }
    if let Some(var_338) = &input.packaging {
        object.key("packaging").string(var_338.as_str());
    }
    if let Some(var_339) = &input.override_artifact_name {
        object.key("overrideArtifactName").boolean(*var_339);
    }
    if let Some(var_340) = &input.encryption_disabled {
        object.key("encryptionDisabled").boolean(*var_340);
    }
    if let Some(var_341) = &input.artifact_identifier {
        object.key("artifactIdentifier").string(var_341);
    }
}

pub fn serialize_structure_project_cache(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ProjectCache,
) {
    if let Some(var_342) = &input.r#type {
        object.key("type").string(var_342.as_str());
    }
    if let Some(var_343) = &input.location {
        object.key("location").string(var_343);
    }
    if let Some(var_344) = &input.modes {
        let mut array_345 = object.key("modes").start_array();
        for item_346 in var_344 {
            {
                array_345.value().string(item_346.as_str());
            }
        }
        array_345.finish();
    }
}

pub fn serialize_structure_project_environment(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ProjectEnvironment,
) {
    if let Some(var_347) = &input.r#type {
        object.key("type").string(var_347.as_str());
    }
    if let Some(var_348) = &input.image {
        object.key("image").string(var_348);
    }
    if let Some(var_349) = &input.compute_type {
        object.key("computeType").string(var_349.as_str());
    }
    if let Some(var_350) = &input.environment_variables {
        let mut array_351 = object.key("environmentVariables").start_array();
        for item_352 in var_350 {
            {
                let mut object_353 = array_351.value().start_object();
                crate::json_ser::serialize_structure_environment_variable(
                    &mut object_353,
                    item_352,
                );
                object_353.finish();
            }
        }
        array_351.finish();
    }
    if let Some(var_354) = &input.privileged_mode {
        object.key("privilegedMode").boolean(*var_354);
    }
    if let Some(var_355) = &input.certificate {
        object.key("certificate").string(var_355);
    }
    if let Some(var_356) = &input.registry_credential {
        let mut object_357 = object.key("registryCredential").start_object();
        crate::json_ser::serialize_structure_registry_credential(&mut object_357, var_356);
        object_357.finish();
    }
    if let Some(var_358) = &input.image_pull_credentials_type {
        object
            .key("imagePullCredentialsType")
            .string(var_358.as_str());
    }
}

pub fn serialize_structure_tag(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::Tag,
) {
    if let Some(var_359) = &input.key {
        object.key("key").string(var_359);
    }
    if let Some(var_360) = &input.value {
        object.key("value").string(var_360);
    }
}

pub fn serialize_structure_vpc_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::VpcConfig,
) {
    if let Some(var_361) = &input.vpc_id {
        object.key("vpcId").string(var_361);
    }
    if let Some(var_362) = &input.subnets {
        let mut array_363 = object.key("subnets").start_array();
        for item_364 in var_362 {
            {
                array_363.value().string(item_364);
            }
        }
        array_363.finish();
    }
    if let Some(var_365) = &input.security_group_ids {
        let mut array_366 = object.key("securityGroupIds").start_array();
        for item_367 in var_365 {
            {
                array_366.value().string(item_367);
            }
        }
        array_366.finish();
    }
}

pub fn serialize_structure_logs_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::LogsConfig,
) {
    if let Some(var_368) = &input.cloud_watch_logs {
        let mut object_369 = object.key("cloudWatchLogs").start_object();
        crate::json_ser::serialize_structure_cloud_watch_logs_config(&mut object_369, var_368);
        object_369.finish();
    }
    if let Some(var_370) = &input.s3_logs {
        let mut object_371 = object.key("s3Logs").start_object();
        crate::json_ser::serialize_structure_s3_logs_config(&mut object_371, var_370);
        object_371.finish();
    }
}

pub fn serialize_structure_project_file_system_location(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ProjectFileSystemLocation,
) {
    if let Some(var_372) = &input.r#type {
        object.key("type").string(var_372.as_str());
    }
    if let Some(var_373) = &input.location {
        object.key("location").string(var_373);
    }
    if let Some(var_374) = &input.mount_point {
        object.key("mountPoint").string(var_374);
    }
    if let Some(var_375) = &input.identifier {
        object.key("identifier").string(var_375);
    }
    if let Some(var_376) = &input.mount_options {
        object.key("mountOptions").string(var_376);
    }
}

pub fn serialize_structure_project_build_batch_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ProjectBuildBatchConfig,
) {
    if let Some(var_377) = &input.service_role {
        object.key("serviceRole").string(var_377);
    }
    if let Some(var_378) = &input.combine_artifacts {
        object.key("combineArtifacts").boolean(*var_378);
    }
    if let Some(var_379) = &input.restrictions {
        let mut object_380 = object.key("restrictions").start_object();
        crate::json_ser::serialize_structure_batch_restrictions(&mut object_380, var_379);
        object_380.finish();
    }
    if let Some(var_381) = &input.timeout_in_mins {
        object.key("timeoutInMins").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_381).into()),
        );
    }
}

pub fn serialize_structure_report_export_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ReportExportConfig,
) {
    if let Some(var_382) = &input.export_config_type {
        object.key("exportConfigType").string(var_382.as_str());
    }
    if let Some(var_383) = &input.s3_destination {
        let mut object_384 = object.key("s3Destination").start_object();
        crate::json_ser::serialize_structure_s3_report_export_config(&mut object_384, var_383);
        object_384.finish();
    }
}

pub fn serialize_structure_webhook_filter(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::WebhookFilter,
) {
    if let Some(var_385) = &input.r#type {
        object.key("type").string(var_385.as_str());
    }
    if let Some(var_386) = &input.pattern {
        object.key("pattern").string(var_386);
    }
    if let Some(var_387) = &input.exclude_matched_pattern {
        object.key("excludeMatchedPattern").boolean(*var_387);
    }
}

pub fn serialize_structure_test_case_filter(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::TestCaseFilter,
) {
    if let Some(var_388) = &input.status {
        object.key("status").string(var_388);
    }
    if let Some(var_389) = &input.keyword {
        object.key("keyword").string(var_389);
    }
}

pub fn serialize_structure_build_batch_filter(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::BuildBatchFilter,
) {
    if let Some(var_390) = &input.status {
        object.key("status").string(var_390.as_str());
    }
}

pub fn serialize_structure_report_filter(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ReportFilter,
) {
    if let Some(var_391) = &input.status {
        object.key("status").string(var_391.as_str());
    }
}

pub fn serialize_structure_environment_variable(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::EnvironmentVariable,
) {
    if let Some(var_392) = &input.name {
        object.key("name").string(var_392);
    }
    if let Some(var_393) = &input.value {
        object.key("value").string(var_393);
    }
    if let Some(var_394) = &input.r#type {
        object.key("type").string(var_394.as_str());
    }
}

pub fn serialize_structure_source_auth(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::SourceAuth,
) {
    if let Some(var_395) = &input.r#type {
        object.key("type").string(var_395.as_str());
    }
    if let Some(var_396) = &input.resource {
        object.key("resource").string(var_396);
    }
}

pub fn serialize_structure_git_submodules_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::GitSubmodulesConfig,
) {
    if let Some(var_397) = &input.fetch_submodules {
        object.key("fetchSubmodules").boolean(*var_397);
    }
}

pub fn serialize_structure_build_status_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::BuildStatusConfig,
) {
    if let Some(var_398) = &input.context {
        object.key("context").string(var_398);
    }
    if let Some(var_399) = &input.target_url {
        object.key("targetUrl").string(var_399);
    }
}

pub fn serialize_structure_registry_credential(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::RegistryCredential,
) {
    if let Some(var_400) = &input.credential {
        object.key("credential").string(var_400);
    }
    if let Some(var_401) = &input.credential_provider {
        object.key("credentialProvider").string(var_401.as_str());
    }
}

pub fn serialize_structure_cloud_watch_logs_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::CloudWatchLogsConfig,
) {
    if let Some(var_402) = &input.status {
        object.key("status").string(var_402.as_str());
    }
    if let Some(var_403) = &input.group_name {
        object.key("groupName").string(var_403);
    }
    if let Some(var_404) = &input.stream_name {
        object.key("streamName").string(var_404);
    }
}

pub fn serialize_structure_s3_logs_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::S3LogsConfig,
) {
    if let Some(var_405) = &input.status {
        object.key("status").string(var_405.as_str());
    }
    if let Some(var_406) = &input.location {
        object.key("location").string(var_406);
    }
    if let Some(var_407) = &input.encryption_disabled {
        object.key("encryptionDisabled").boolean(*var_407);
    }
}

pub fn serialize_structure_batch_restrictions(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::BatchRestrictions,
) {
    if let Some(var_408) = &input.maximum_builds_allowed {
        object.key("maximumBuildsAllowed").number(
            #[allow(clippy::useless_conversion)]
            smithy_types::Number::NegInt((*var_408).into()),
        );
    }
    if let Some(var_409) = &input.compute_types_allowed {
        let mut array_410 = object.key("computeTypesAllowed").start_array();
        for item_411 in var_409 {
            {
                array_410.value().string(item_411);
            }
        }
        array_410.finish();
    }
}

pub fn serialize_structure_s3_report_export_config(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::S3ReportExportConfig,
) {
    if let Some(var_412) = &input.bucket {
        object.key("bucket").string(var_412);
    }
    if let Some(var_413) = &input.bucket_owner {
        object.key("bucketOwner").string(var_413);
    }
    if let Some(var_414) = &input.path {
        object.key("path").string(var_414);
    }
    if let Some(var_415) = &input.packaging {
        object.key("packaging").string(var_415.as_str());
    }
    if let Some(var_416) = &input.encryption_key {
        object.key("encryptionKey").string(var_416);
    }
    if let Some(var_417) = &input.encryption_disabled {
        object.key("encryptionDisabled").boolean(*var_417);
    }
}
