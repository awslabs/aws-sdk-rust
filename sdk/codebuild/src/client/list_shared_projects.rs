...
impl super::Client {
    ...
    ///   - [`next_token(impl Into<String>)`](crate::operation::list_shared_projects::builders::ListSharedProjectsFluentBuilder::next_token) / [`set_next_token(Option<String>)`](crate::operation::list_shared_projects::builders::ListSharedProjectsFluentBuilder::set_next_token): <p> During a previous call, the maximum number of items that can be returned is the value specified in <code>maxResults</code>. If there more items in the list, then a unique string called a <i>nextToken</i> is returned. To get the next batch of items in the list, call this operation again, adding the next token to the call. To get all of the items in the list, keep calling this operation with each subsequent next token that is returned, until no more next tokens are returned. </p>
