...
pub fn ser_start_build_batch_input(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::operation::start_build_batch::StartBuildBatchInput) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    ...
    if let Some(var_39) = &input.build_timeout_in_minutes_override {
        object.key("buildTimeoutInMinutesOverride").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_39).into()));
    }
    if let Some(var_40) = &input.queued_timeout_in_minutes_override {
        object.key("queuedTimeoutInMinutesOverride").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_40).into()));
    }
