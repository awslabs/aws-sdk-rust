...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_medical_vocabularies::ListMedicalVocabulariesError::unhandled(
                generic,
            ))
        }
    };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_list_medical_vocabularies_http_response_with_props(
    _response_status: u16,
    _response_headers: &::http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_medical_vocabularies::de_list_medical_vocabularies(_response_body, output)
            .map_err(crate::operation::list_medical_vocabularies::ListMedicalVocabulariesError::unhandled)?;
