...
impl GetMigrationsInput {
    ...
    pub async fn make_operation(&self, _config: &crate::config::Config) -> std::result::Result<aws_smithy_http::operation::Operation<crate::operation::get_migrations::GetMigrations, aws_http::retry::AwsResponseRetryClassifier>, aws_smithy_http::operation::error::BuildError> {
        ...
        let mut request = {
            fn uri_base(_input: &crate::operation::get_migrations::GetMigrationsInput, output: &mut String) -> std::result::Result<(), aws_smithy_http::operation::error::BuildError> {
                use std::fmt::Write as _;
                write!(output, "/migrations").expect("formatting should succeed");
                Ok(())
            }
            fn uri_query(_input: &crate::operation::get_migrations::GetMigrationsInput, mut output: &mut String) -> Result<(), aws_smithy_http::operation::error::BuildError> {
                let mut query = aws_smithy_http::query::Writer::new(output);
                if let Some(inner_1) = &_input.sort_by_attribute {
                     {
                        query.push_kv("sortByAttribute", &aws_smithy_http::query::fmt_string(&inner_1));
                    }
                }
                if let Some(inner_2) = &_input.sort_by_order {
                     {
                        query.push_kv("sortByOrder", &aws_smithy_http::query::fmt_string(&inner_2));
                    }
                }
                if let Some(inner_3) = &_input.v1_bot_name_contains {
                     {
                        query.push_kv("v1BotNameContains", &aws_smithy_http::query::fmt_string(&inner_3));
                    }
                }
                if let Some(inner_4) = &_input.migration_status_equals {
                     {
