...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_asset_relationships::ListAssetRelationshipsError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        crate::serde_util::list_asset_relationships_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_asset_relationships::ListAssetRelationshipsError::unhandled)?
    })
}
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "assetRelationshipSummaries" => {
                    builder = builder.set_asset_relationship_summaries(
                        crate::protocol_serde::shape_asset_relationship_summaries::de_asset_relationship_summaries(tokens, _value, depth + 1)?,
                    );
                }
                "nextToken" => {
