// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct UpdateComputationModelOutput {
    /// <p>The status of the computation model. It contains a state (UPDATING after successfully calling this operation) and an error message if any.</p>
    pub computation_model_status: ::std::option::Option<crate::types::ComputationModelStatus>,
    _request_id: Option<String>,
}
impl UpdateComputationModelOutput {
    /// <p>The status of the computation model. It contains a state (UPDATING after successfully calling this operation) and an error message if any.</p>
    pub fn computation_model_status(&self) -> ::std::option::Option<&crate::types::ComputationModelStatus> {
        self.computation_model_status.as_ref()
    }
}
impl ::aws_types::request_id::RequestId for UpdateComputationModelOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl UpdateComputationModelOutput {
    /// Creates a new builder-style object to manufacture [`UpdateComputationModelOutput`](crate::operation::update_computation_model::UpdateComputationModelOutput).
    pub fn builder() -> crate::operation::update_computation_model::builders::UpdateComputationModelOutputBuilder {
        crate::operation::update_computation_model::builders::UpdateComputationModelOutputBuilder::default()
    }
}

/// A builder for [`UpdateComputationModelOutput`](crate::operation::update_computation_model::UpdateComputationModelOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct UpdateComputationModelOutputBuilder {
    pub(crate) computation_model_status: ::std::option::Option<crate::types::ComputationModelStatus>,
    _request_id: Option<String>,
}
impl UpdateComputationModelOutputBuilder {
    /// <p>The status of the computation model. It contains a state (UPDATING after successfully calling this operation) and an error message if any.</p>
    /// This field is required.
    pub fn computation_model_status(mut self, input: crate::types::ComputationModelStatus) -> Self {
        self.computation_model_status = ::std::option::Option::Some(input);
        self
    }
    /// <p>The status of the computation model. It contains a state (UPDATING after successfully calling this operation) and an error message if any.</p>
    pub fn set_computation_model_status(mut self, input: ::std::option::Option<crate::types::ComputationModelStatus>) -> Self {
        self.computation_model_status = input;
        self
    }
    /// <p>The status of the computation model. It contains a state (UPDATING after successfully calling this operation) and an error message if any.</p>
    pub fn get_computation_model_status(&self) -> &::std::option::Option<crate::types::ComputationModelStatus> {
        &self.computation_model_status
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`UpdateComputationModelOutput`](crate::operation::update_computation_model::UpdateComputationModelOutput).
    pub fn build(self) -> crate::operation::update_computation_model::UpdateComputationModelOutput {
        crate::operation::update_computation_model::UpdateComputationModelOutput {
            computation_model_status: self.computation_model_status,
            _request_id: self._request_id,
        }
    }
}
