// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
impl super::Client {
    /// Constructs a fluent builder for the [`RevokeVpcEndpointAccess`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder) operation.
    ///
    /// - The fluent builder is configurable:
    ///   - [`domain_name(impl Into<String>)`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::domain_name) / [`set_domain_name(Option<String>)`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::set_domain_name):<br>required: **true**<br><p>The name of the OpenSearch Service domain.</p><br>
    ///   - [`account(impl Into<String>)`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::account) / [`set_account(Option<String>)`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::set_account):<br>required: **false**<br><p>The account ID to revoke access from.</p><br>
    ///   - [`service(AwsServicePrincipal)`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::service) / [`set_service(Option<AwsServicePrincipal>)`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::set_service):<br>required: **false**<br><p>The service SP to revoke access from.</p><br>
    ///   - [`service_options(ServiceOptions)`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::service_options) / [`set_service_options(Option<ServiceOptions>)`](crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::set_service_options):<br>required: **false**<br><p>The options for the service, including the supported Regions for the endpoint access.</p><br>
    /// - On success, responds with [`RevokeVpcEndpointAccessOutput`](crate::operation::revoke_vpc_endpoint_access::RevokeVpcEndpointAccessOutput)
    /// - On failure, responds with [`SdkError<RevokeVpcEndpointAccessError>`](crate::operation::revoke_vpc_endpoint_access::RevokeVpcEndpointAccessError)
    pub fn revoke_vpc_endpoint_access(&self) -> crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder {
        crate::operation::revoke_vpc_endpoint_access::builders::RevokeVpcEndpointAccessFluentBuilder::new(self.handle.clone())
    }
}
