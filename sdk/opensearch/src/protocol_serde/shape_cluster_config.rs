            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "WarmType" => {
                            builder = builder.set_warm_type(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| {
                                        s.to_unescaped()
                                            .map(|u| crate::types::OpenSearchWarmPartitionInstanceType::from(u.as_ref()))
                                    })
                                    .transpose()?,
                            );
                        }
                        ...
                        "NodeOptions" => {
                            builder = builder.set_node_options(crate::protocol_serde::shape_node_options_list::de_node_options_list(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
