...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_dry_run_progress::DescribeDryRunProgressError::unhandled)?;
    generic_builder = ::aws_http::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::describe_dry_run_progress::DescribeDryRunProgressError::unhandled(
                generic,
            ))
        }
    };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_describe_dry_run_progress_http_response_with_props(
    _response_status: u16,
    _response_headers: &::http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_dry_run_progress::de_describe_dry_run_progress(_response_body, output)
            .map_err(crate::operation::describe_dry_run_progress::DescribeDryRunProgressError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "DryRunConfig" => {
                    builder = builder.set_dry_run_config(crate::protocol_serde::shape_domain_status::de_domain_status(tokens)?);
                }
