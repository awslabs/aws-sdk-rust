...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::delete_inbound_connection::DeleteInboundConnectionError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::delete_inbound_connection::DeleteInboundConnectionError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_delete_inbound_connection::de_delete_inbound_connection(_response_body, output)
            .map_err(crate::operation::delete_inbound_connection::DeleteInboundConnectionError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "Connection" => {
                    builder = builder.set_connection(crate::protocol_serde::shape_inbound_connection::de_inbound_connection(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
