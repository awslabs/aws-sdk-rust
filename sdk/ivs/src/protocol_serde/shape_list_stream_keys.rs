            ...
            let mut tmp = {
                ...
                output = output.set_access_control_allow_origin(
                    crate::protocol_serde::shape_access_denied_exception::de_access_control_allow_origin_header(_response_headers).map_err(|_| {
                        crate::operation::list_stream_keys::ListStreamKeysError::unhandled(
                ...
                output = output.set_cache_control(
                    crate::protocol_serde::shape_access_denied_exception::de_cache_control_header(_response_headers).map_err(|_| {
                        crate::operation::list_stream_keys::ListStreamKeysError::unhandled("Failed to parse cacheControl from header `Cache-Control")
                    })?,
                );
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "streamKeys" => {
                    builder = builder.set_stream_keys(crate::protocol_serde::shape_stream_key_list::de_stream_key_list(tokens, _value)?);
                }
