...
pub(crate) fn de_field_level_encryption_profile_list_payload(
    body: &[u8],
) -> std::result::Result<
...
pub fn de_field_level_encryption_profile_list(
    ...
    if !(start_el.matches("FieldLevelEncryptionProfileList")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected FieldLevelEncryptionProfileList got {start_el:?}"
        )));
    }
    #[allow(unused_variables)]
    let depth = 0u32;
    crate::protocol_serde::shape_field_level_encryption_profile_list::de_field_level_encryption_profile_list(&mut decoder, depth + 1)
}
