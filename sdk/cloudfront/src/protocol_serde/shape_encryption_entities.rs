...
pub fn de_encryption_entities(
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("Items") /* Items com.amazonaws.cloudfront#EncryptionEntities$Items */ =>  {
                let var_4 =
                    Some(
                        crate::protocol_serde::shape_encryption_entity_list::de_encryption_entity_list(&mut tag, depth + 1)
                        ?
                    )
                ;
    ...
    Ok(crate::serde_util::encryption_entities_correct_errors(builder)
        .build()
