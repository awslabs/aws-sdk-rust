...
pub fn de_cache_policy(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::CachePolicy, ::aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    let mut builder = crate::types::CachePolicy::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("CachePolicyConfig") /* CachePolicyConfig com.amazonaws.cloudfront#CachePolicy$CachePolicyConfig */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_cache_policy_config::de_cache_policy_config(&mut tag)
                        ?
                    )
                ;
                builder = builder.set_cache_policy_config(var_3);
            }
            ,
            _ => {}
        }
    }
    Ok(crate::serde_util::cache_policy_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
