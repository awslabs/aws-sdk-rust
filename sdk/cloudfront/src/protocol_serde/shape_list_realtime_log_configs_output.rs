...
pub(crate) fn de_realtime_log_configs_payload(
    body: &[u8],
) -> std::result::Result<
...
> {
    (!body.is_empty())
        .then(|| {
            crate::protocol_serde::shape_list_realtime_log_configs_output::de_realtime_log_configs(body)
                .map_err(crate::operation::list_realtime_log_configs::ListRealtimeLogConfigsError::unhandled)
        })
        .transpose()
}

pub fn de_realtime_log_configs(inp: &[u8]) -> std::result::Result<crate::types::RealtimeLogConfigs, ::aws_smithy_xml::decode::XmlDecodeError> {
