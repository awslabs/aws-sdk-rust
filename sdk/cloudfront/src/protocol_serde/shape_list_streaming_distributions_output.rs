...
pub(crate) fn de_streaming_distribution_list_payload(
    body: &[u8],
) -> std::result::Result<
...
pub fn de_streaming_distribution_list(
    ...
    if !(start_el.matches("StreamingDistributionList")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected StreamingDistributionList got {start_el:?}"
        )));
    }
    #[allow(unused_variables)]
    let depth = 0u32;
    crate::protocol_serde::shape_streaming_distribution_list::de_streaming_distribution_list(&mut decoder, depth + 1)
}
