...
pub fn de_active_trusted_signers(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::ActiveTrustedSigners, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("Items") /* Items com.amazonaws.cloudfront#ActiveTrustedSigners$Items */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_signer_list::de_signer_list(&mut tag, depth + 1)
                        ?
                    )
                ;
