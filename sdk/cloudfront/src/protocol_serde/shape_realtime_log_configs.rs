...
pub fn de_realtime_log_configs(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::RealtimeLogConfigs, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("Items") /* Items com.amazonaws.cloudfront#RealtimeLogConfigs$Items */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_realtime_log_config_list::de_realtime_log_config_list(&mut tag, depth + 1)
                        ?
                    )
                ;
    ...
    Ok(crate::serde_util::realtime_log_configs_correct_errors(builder)
        .build()
