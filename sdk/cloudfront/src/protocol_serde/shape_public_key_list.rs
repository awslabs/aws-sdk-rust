...
pub fn de_public_key_list(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::PublicKeyList, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("Items") /* Items com.amazonaws.cloudfront#PublicKeyList$Items */ =>  {
                let var_4 =
                    Some(
                        crate::protocol_serde::shape_public_key_summary_list::de_public_key_summary_list(&mut tag, depth + 1)
                        ?
                    )
                ;
    ...
    Ok(crate::serde_util::public_key_list_correct_errors(builder)
        .build()
