...
pub fn ser_forwarded_values(input: &crate::types::ForwardedValues, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
     {
        let mut inner_writer = scope.start_el("QueryString").finish();
        inner_writer.data(
            ::aws_smithy_types::primitive::Encoder::from(input.query_string).encode()
        );
    }
...
pub fn de_forwarded_values(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::ForwardedValues, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    Ok(crate::serde_util::forwarded_values_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
