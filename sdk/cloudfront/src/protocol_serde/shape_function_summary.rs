...
pub fn de_function_summary(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::FunctionSummary, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("FunctionConfig") /* FunctionConfig com.amazonaws.cloudfront#FunctionSummary$FunctionConfig */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_function_config::de_function_config(&mut tag, depth + 1)
                        ?
                    )
                ;
            ...
            s if s.matches("FunctionMetadata") /* FunctionMetadata com.amazonaws.cloudfront#FunctionSummary$FunctionMetadata */ =>  {
                let var_4 =
                    Some(
                        crate::protocol_serde::shape_function_metadata::de_function_metadata(&mut tag, depth + 1)
                        ?
                    )
                ;
    ...
    Ok(crate::serde_util::function_summary_correct_errors(builder)
        .build()
