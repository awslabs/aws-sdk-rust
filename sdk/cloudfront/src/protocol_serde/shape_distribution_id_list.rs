...
pub fn de_distribution_id_list(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::DistributionIdList, ::aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    let mut builder = crate::types::DistributionIdList::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("Items") /* Items com.amazonaws.cloudfront#DistributionIdList$Items */ =>  {
                let var_6 =
                    Some(
                        crate::protocol_serde::shape_distribution_id_list_summary::de_distribution_id_list_summary(&mut tag)
                        ?
                    )
                ;
    ...
    Ok(crate::serde_util::distribution_id_list_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
