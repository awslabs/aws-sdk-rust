...
pub(crate) fn de_distribution_id_list_payload(
    body: &[u8],
) -> std::result::Result<
...
> {
    (!body.is_empty())
        .then(|| {
            crate::protocol_serde::shape_list_distributions_by_vpc_origin_id_output::de_distribution_id_list(body)
...
pub fn de_distribution_id_list(inp: &[u8]) -> std::result::Result<crate::types::DistributionIdList, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    let depth = 0u32;
    crate::protocol_serde::shape_distribution_id_list::de_distribution_id_list(&mut decoder, depth + 1)
}
