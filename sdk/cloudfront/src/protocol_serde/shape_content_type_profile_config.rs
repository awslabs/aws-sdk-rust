...
pub fn ser_content_type_profile_config(input: &crate::types::ContentTypeProfileConfig, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
     {
        let mut inner_writer = scope.start_el("ForwardWhenContentTypeIsUnknown").finish();
        inner_writer.data(
            ::aws_smithy_types::primitive::Encoder::from(input.forward_when_content_type_is_unknown).encode()
        );
    }
...
pub fn de_content_type_profile_config(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::ContentTypeProfileConfig, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    Ok(crate::serde_util::content_type_profile_config_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
