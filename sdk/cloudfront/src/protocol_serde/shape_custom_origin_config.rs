...
pub fn ser_custom_origin_config(input: &crate::types::CustomOriginConfig, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
     {
        let mut inner_writer = scope.start_el("HTTPPort").finish();
        inner_writer.data(
            ::aws_smithy_types::primitive::Encoder::from(input.http_port).encode()
        );
    }
     {
        let mut inner_writer = scope.start_el("HTTPSPort").finish();
        inner_writer.data(
            ::aws_smithy_types::primitive::Encoder::from(input.https_port).encode()
        );
    }
     {
        let mut inner_writer = scope.start_el("OriginProtocolPolicy").finish();
        inner_writer.data(
            input.origin_protocol_policy.as_str()
        );
    }
...
pub fn de_custom_origin_config(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::CustomOriginConfig, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    Ok(crate::serde_util::custom_origin_config_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
