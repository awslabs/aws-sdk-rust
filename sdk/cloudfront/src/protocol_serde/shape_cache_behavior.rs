...
pub fn ser_cache_behavior(input: &crate::types::CacheBehavior, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
     {
        let mut inner_writer = scope.start_el("PathPattern").finish();
        inner_writer.data(
            input.path_pattern.as_str()
        );
    }
     {
...
pub fn de_cache_behavior(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::CacheBehavior, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    Ok(crate::serde_util::cache_behavior_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
