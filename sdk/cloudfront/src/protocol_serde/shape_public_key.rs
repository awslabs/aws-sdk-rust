...
pub fn de_public_key(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::PublicKey, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("PublicKeyConfig") /* PublicKeyConfig com.amazonaws.cloudfront#PublicKey$PublicKeyConfig */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_public_key_config::de_public_key_config(&mut tag, depth + 1)
                        ?
                    )
                ;
    ...
    Ok(crate::serde_util::public_key_correct_errors(builder)
        .build()
