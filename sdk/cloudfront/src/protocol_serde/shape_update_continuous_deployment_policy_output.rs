...
> {
    (!body.is_empty())
        .then(|| {
            crate::protocol_serde::shape_update_continuous_deployment_policy_output::de_continuous_deployment_policy(body)
...
pub fn de_continuous_deployment_policy(
    ...
    if !(start_el.matches("ContinuousDeploymentPolicy")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected ContinuousDeploymentPolicy got {start_el:?}"
        )));
    }
    #[allow(unused_variables)]
    let depth = 0u32;
    crate::protocol_serde::shape_continuous_deployment_policy::de_continuous_deployment_policy(&mut decoder, depth + 1)
}
