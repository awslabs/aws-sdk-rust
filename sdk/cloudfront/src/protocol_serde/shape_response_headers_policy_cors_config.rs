...
pub fn ser_response_headers_policy_cors_config(input: &crate::types::ResponseHeadersPolicyCorsConfig, writer: ::aws_smithy_xml::encode::ElWriter) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    ...
    if let Some(var_3) = &input.access_control_allow_methods {
    ...
    }
     {
        ...
        inner_writer.data(
            ::aws_smithy_types::primitive::Encoder::from(input.access_control_allow_credentials).encode()
        );
    }
    if let Some(var_4) = &input.access_control_expose_headers {
        let inner_writer = scope.start_el("AccessControlExposeHeaders");
        crate::protocol_serde::shape_response_headers_policy_access_control_expose_headers::ser_response_headers_policy_access_control_expose_headers(var_4, inner_writer)?
    }
    if let Some(var_5) = &input.access_control_max_age_sec {
    ...
    }
     {
        ...
        inner_writer.data(
            ::aws_smithy_types::primitive::Encoder::from(input.origin_override).encode()
        );
    }
    scope.finish();
    Ok(())
...
pub fn de_response_headers_policy_cors_config(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::ResponseHeadersPolicyCorsConfig, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    Ok(crate::serde_util::response_headers_policy_cors_config_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
