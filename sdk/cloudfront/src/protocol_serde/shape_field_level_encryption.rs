...
pub fn de_field_level_encryption(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::FieldLevelEncryption, ::aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("FieldLevelEncryptionConfig") /* FieldLevelEncryptionConfig com.amazonaws.cloudfront#FieldLevelEncryption$FieldLevelEncryptionConfig */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_field_level_encryption_config::de_field_level_encryption_config(&mut tag)
                        ?
                    )
                ;
                builder = builder.set_field_level_encryption_config(var_3);
            }
            ,
            _ => {}
        }
    }
    Ok(crate::serde_util::field_level_encryption_correct_errors(builder).build().map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("missing field"))?)
}
