...
pub fn de_field_level_encryption(
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("FieldLevelEncryptionConfig") /* FieldLevelEncryptionConfig com.amazonaws.cloudfront#FieldLevelEncryption$FieldLevelEncryptionConfig */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_field_level_encryption_config::de_field_level_encryption_config(&mut tag, depth + 1)
                        ?
                    )
                ;
    ...
    Ok(crate::serde_util::field_level_encryption_correct_errors(builder)
        .build()
