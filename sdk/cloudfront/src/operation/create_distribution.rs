...
impl ::aws_smithy_runtime_api::client::ser_de::DeserializeResponse for CreateDistributionResponseDeserializer {
    fn deserialize_nonstreaming_with_config(
        ...
        let parse_result = if !success && status != 201 || force_error {
            crate::protocol_serde::shape_create_distribution::de_create_distribution_http_error(status, headers, body, _cfg)
        } else {
            crate::protocol_serde::shape_create_distribution::de_create_distribution_http_response(status, headers, body, _cfg)
        };
...
impl CreateDistributionError {
    ...
    /// Returns `true` if the error kind is `CreateDistributionError::EntityLimitExceeded`.
    pub fn is_entity_limit_exceeded(&self) -> bool {
        matches!(self, Self::EntityLimitExceeded(_))
    }
    /// Returns `true` if the error kind is `CreateDistributionError::EntityNotFound`.
