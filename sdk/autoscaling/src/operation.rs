...
impl aws_smithy_http::response::ParseStrictResponse for AttachInstances {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !response.status().is_success() && response.status().as_u16() != 200 {
            ...
            crate::protocol_serde::shape_attach_instances::de_attach_instances_http_response(
                response,
            )
        }
    }
...
impl aws_smithy_http::response::ParseStrictResponse for DetachLoadBalancers {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::protocol_serde::shape_detach_load_balancers::de_detach_load_balancers_http_error(
                response,
            )
        } else {
...
impl aws_smithy_http::response::ParseStrictResponse for ResumeProcesses {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !response.status().is_success() && response.status().as_u16() != 200 {
            ...
            crate::protocol_serde::shape_resume_processes::de_resume_processes_http_response(
                response,
            )
        }
    }
