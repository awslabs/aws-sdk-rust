            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "type" => {
                            builder = builder.set_type(crate::protocol_serde::shape_edi_type::de_edi_type(tokens, _value, depth + 1)?);
                        }
                        "inputLocation" => {
                            builder =
                                builder.set_input_location(crate::protocol_serde::shape_s3_location::de_s3_location(tokens, _value, depth + 1)?);
                        }
                        "outputLocation" => {
                            builder =
                                builder.set_output_location(crate::protocol_serde::shape_s3_location::de_s3_location(tokens, _value, depth + 1)?);
                        }
                        "transformerId" => {
