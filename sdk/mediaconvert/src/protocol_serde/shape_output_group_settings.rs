            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "cmafGroupSettings" => {
                            builder = builder.set_cmaf_group_settings(crate::protocol_serde::shape_cmaf_group_settings::de_cmaf_group_settings(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "dashIsoGroupSettings" => {
                            builder = builder.set_dash_iso_group_settings(
                                crate::protocol_serde::shape_dash_iso_group_settings::de_dash_iso_group_settings(tokens, _value, depth + 1)?,
                            );
                        }
                        "fileGroupSettings" => {
                            builder = builder.set_file_group_settings(crate::protocol_serde::shape_file_group_settings::de_file_group_settings(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "type" => {
                            builder = builder.set_type(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::OutputGroupType::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
