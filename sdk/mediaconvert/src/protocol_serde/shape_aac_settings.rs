...
pub fn ser_aac_settings(
    ...
    if let Some(var_7) = &input.sample_rate {
        object.key("sampleRate").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_7).into()));
    }
                                ...
                                builder = builder.set_audio_description_broadcaster_mix(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::AacAudioDescriptionBroadcasterMix::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
