...
pub fn ser_ac3_settings(
    ...
    if let Some(var_4) = &input.dialnorm {
        object.key("dialnorm").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_4).into()));
    }
    ...
    if let Some(var_10) = &input.sample_rate {
        object.key("sampleRate").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_10).into()));
    }
    Ok(())
}
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "dynamicRangeCompressionLine" => {
                                builder = builder.set_dynamic_range_compression_line(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::Ac3DynamicRangeCompressionLine::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
                            "dynamicRangeCompressionProfile" => {
                                builder = builder.set_dynamic_range_compression_profile(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::Ac3DynamicRangeCompressionProfile::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
