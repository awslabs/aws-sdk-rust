...
pub fn ser_color_corrector(
    ...
    if let Some(var_10) = &input.max_luminance {
        object.key("maxLuminance").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_10).into()));
    }
    ...
    if let Some(var_12) = &input.saturation {
        object.key("saturation").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_12).into()));
    }
    if let Some(var_13) = &input.sdr_reference_white_level {
        object.key("sdrReferenceWhiteLevel").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_13).into()));
    }
    Ok(())
}
