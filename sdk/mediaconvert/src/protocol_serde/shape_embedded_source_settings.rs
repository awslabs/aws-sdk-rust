...
pub fn ser_embedded_source_settings(
    ...
    if let Some(var_2) = &input.source608_channel_number {
        object.key("source608ChannelNumber").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_2).into()));
    }
    if let Some(var_3) = &input.source608_track_number {
        object.key("source608TrackNumber").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_3).into()));
    }
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "terminateCaptions" => {
                                builder = builder.set_terminate_captions(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::EmbeddedTerminateCaptions::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
