            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_backup_plans_output_next_token(resp);
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
                                && new_token == input.next_token.as_ref()
                                && self.stop_on_duplicate_token
            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_backup_plan_versions_output_next_token(
                                    resp,
                                );
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_backup_selections_output_next_token(resp);
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
                                && new_token == input.next_token.as_ref()
                                && self.stop_on_duplicate_token
            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_copy_jobs_output_next_token(resp);
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
                                && new_token == input.next_token.as_ref()
                                && self.stop_on_duplicate_token
            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_report_jobs_output_next_token(resp);
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
                                && new_token == input.next_token.as_ref()
                                && self.stop_on_duplicate_token
            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_report_plans_output_next_token(resp);
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
                                && new_token == input.next_token.as_ref()
                                && self.stop_on_duplicate_token
...
impl ListBackupPlansPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_backup_plans_output_backup_plans_list(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
...
impl ListBackupPlanVersionsPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_backup_plan_versions_output_backup_plan_versions_list(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
...
impl ListBackupSelectionsPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_backup_selections_output_backup_selections_list(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
...
impl ListCopyJobsPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_copy_jobs_output_copy_jobs(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
...
impl ListRecoveryPointsByBackupVaultPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_recovery_points_by_backup_vault_output_recovery_points(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
