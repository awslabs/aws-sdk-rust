// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn serialize_structure_create_managed_endpoint_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::CreateManagedEndpointInput,
) {
    if let Some(var_1) = &input.certificate_arn {
        object.key("certificateArn").string(var_1);
    }
    if let Some(var_2) = &input.client_token {
        object.key("clientToken").string(var_2);
    }
    if let Some(var_3) = &input.configuration_overrides {
        let mut object_4 = object.key("configurationOverrides").start_object();
        crate::json_ser::serialize_structure_configuration_overrides(&mut object_4, var_3);
        object_4.finish();
    }
    if let Some(var_5) = &input.execution_role_arn {
        object.key("executionRoleArn").string(var_5);
    }
    if let Some(var_6) = &input.name {
        object.key("name").string(var_6);
    }
    if let Some(var_7) = &input.release_label {
        object.key("releaseLabel").string(var_7);
    }
    if let Some(var_8) = &input.tags {
        let mut object_9 = object.key("tags").start_object();
        for (key_10, value_11) in var_8 {
            {
                object_9.key(key_10).string(value_11);
            }
        }
        object_9.finish();
    }
    if let Some(var_12) = &input.r#type {
        object.key("type").string(var_12);
    }
}

pub fn serialize_structure_create_virtual_cluster_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::CreateVirtualClusterInput,
) {
    if let Some(var_13) = &input.client_token {
        object.key("clientToken").string(var_13);
    }
    if let Some(var_14) = &input.container_provider {
        let mut object_15 = object.key("containerProvider").start_object();
        crate::json_ser::serialize_structure_container_provider(&mut object_15, var_14);
        object_15.finish();
    }
    if let Some(var_16) = &input.name {
        object.key("name").string(var_16);
    }
    if let Some(var_17) = &input.tags {
        let mut object_18 = object.key("tags").start_object();
        for (key_19, value_20) in var_17 {
            {
                object_18.key(key_19).string(value_20);
            }
        }
        object_18.finish();
    }
}

pub fn serialize_structure_start_job_run_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::StartJobRunInput,
) {
    if let Some(var_21) = &input.client_token {
        object.key("clientToken").string(var_21);
    }
    if let Some(var_22) = &input.configuration_overrides {
        let mut object_23 = object.key("configurationOverrides").start_object();
        crate::json_ser::serialize_structure_configuration_overrides(&mut object_23, var_22);
        object_23.finish();
    }
    if let Some(var_24) = &input.execution_role_arn {
        object.key("executionRoleArn").string(var_24);
    }
    if let Some(var_25) = &input.job_driver {
        let mut object_26 = object.key("jobDriver").start_object();
        crate::json_ser::serialize_structure_job_driver(&mut object_26, var_25);
        object_26.finish();
    }
    if let Some(var_27) = &input.name {
        object.key("name").string(var_27);
    }
    if let Some(var_28) = &input.release_label {
        object.key("releaseLabel").string(var_28);
    }
    if let Some(var_29) = &input.tags {
        let mut object_30 = object.key("tags").start_object();
        for (key_31, value_32) in var_29 {
            {
                object_30.key(key_31).string(value_32);
            }
        }
        object_30.finish();
    }
}

pub fn serialize_structure_tag_resource_input(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::input::TagResourceInput,
) {
    if let Some(var_33) = &input.tags {
        let mut object_34 = object.key("tags").start_object();
        for (key_35, value_36) in var_33 {
            {
                object_34.key(key_35).string(value_36);
            }
        }
        object_34.finish();
    }
}

pub fn serialize_structure_configuration_overrides(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ConfigurationOverrides,
) {
    if let Some(var_37) = &input.application_configuration {
        let mut array_38 = object.key("applicationConfiguration").start_array();
        for item_39 in var_37 {
            {
                let mut object_40 = array_38.value().start_object();
                crate::json_ser::serialize_structure_configuration(&mut object_40, item_39);
                object_40.finish();
            }
        }
        array_38.finish();
    }
    if let Some(var_41) = &input.monitoring_configuration {
        let mut object_42 = object.key("monitoringConfiguration").start_object();
        crate::json_ser::serialize_structure_monitoring_configuration(&mut object_42, var_41);
        object_42.finish();
    }
}

pub fn serialize_structure_container_provider(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ContainerProvider,
) {
    if let Some(var_43) = &input.r#type {
        object.key("type").string(var_43.as_str());
    }
    if let Some(var_44) = &input.id {
        object.key("id").string(var_44);
    }
    if let Some(var_45) = &input.info {
        let mut object_46 = object.key("info").start_object();
        crate::json_ser::serialize_union_container_info(&mut object_46, var_45);
        object_46.finish();
    }
}

pub fn serialize_structure_job_driver(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::JobDriver,
) {
    if let Some(var_47) = &input.spark_submit_job_driver {
        let mut object_48 = object.key("sparkSubmitJobDriver").start_object();
        crate::json_ser::serialize_structure_spark_submit_job_driver(&mut object_48, var_47);
        object_48.finish();
    }
}

pub fn serialize_structure_configuration(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::Configuration,
) {
    if let Some(var_49) = &input.classification {
        object.key("classification").string(var_49);
    }
    if let Some(var_50) = &input.properties {
        let mut object_51 = object.key("properties").start_object();
        for (key_52, value_53) in var_50 {
            {
                object_51.key(key_52).string(value_53);
            }
        }
        object_51.finish();
    }
    if let Some(var_54) = &input.configurations {
        let mut array_55 = object.key("configurations").start_array();
        for item_56 in var_54 {
            {
                let mut object_57 = array_55.value().start_object();
                crate::json_ser::serialize_structure_configuration(&mut object_57, item_56);
                object_57.finish();
            }
        }
        array_55.finish();
    }
}

pub fn serialize_structure_monitoring_configuration(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::MonitoringConfiguration,
) {
    if let Some(var_58) = &input.persistent_app_ui {
        object.key("persistentAppUI").string(var_58.as_str());
    }
    if let Some(var_59) = &input.cloud_watch_monitoring_configuration {
        let mut object_60 = object
            .key("cloudWatchMonitoringConfiguration")
            .start_object();
        crate::json_ser::serialize_structure_cloud_watch_monitoring_configuration(
            &mut object_60,
            var_59,
        );
        object_60.finish();
    }
    if let Some(var_61) = &input.s3_monitoring_configuration {
        let mut object_62 = object.key("s3MonitoringConfiguration").start_object();
        crate::json_ser::serialize_structure_s3_monitoring_configuration(&mut object_62, var_61);
        object_62.finish();
    }
}

pub fn serialize_union_container_info(
    object_46: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::ContainerInfo,
) {
    match input {
        crate::model::ContainerInfo::EksInfo(inner) => {
            let mut object_63 = object_46.key("eksInfo").start_object();
            crate::json_ser::serialize_structure_eks_info(&mut object_63, inner);
            object_63.finish();
        }
    }
}

pub fn serialize_structure_spark_submit_job_driver(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::SparkSubmitJobDriver,
) {
    if let Some(var_64) = &input.entry_point {
        object.key("entryPoint").string(var_64);
    }
    if let Some(var_65) = &input.entry_point_arguments {
        let mut array_66 = object.key("entryPointArguments").start_array();
        for item_67 in var_65 {
            {
                array_66.value().string(item_67);
            }
        }
        array_66.finish();
    }
    if let Some(var_68) = &input.spark_submit_parameters {
        object.key("sparkSubmitParameters").string(var_68);
    }
}

pub fn serialize_structure_cloud_watch_monitoring_configuration(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::CloudWatchMonitoringConfiguration,
) {
    if let Some(var_69) = &input.log_group_name {
        object.key("logGroupName").string(var_69);
    }
    if let Some(var_70) = &input.log_stream_name_prefix {
        object.key("logStreamNamePrefix").string(var_70);
    }
}

pub fn serialize_structure_s3_monitoring_configuration(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::S3MonitoringConfiguration,
) {
    if let Some(var_71) = &input.log_uri {
        object.key("logUri").string(var_71);
    }
}

pub fn serialize_structure_eks_info(
    object: &mut smithy_json::serialize::JsonObjectWriter,
    input: &crate::model::EksInfo,
) {
    if let Some(var_72) = &input.namespace {
        object.key("namespace").string(var_72);
    }
}
