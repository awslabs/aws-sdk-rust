...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::describe_virtual_cluster::DescribeVirtualClusterError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_virtual_cluster::de_describe_virtual_cluster(_response_body, output)
            .map_err(crate::operation::describe_virtual_cluster::DescribeVirtualClusterError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "virtualCluster" => {
                    builder = builder.set_virtual_cluster(crate::protocol_serde::shape_virtual_cluster::de_virtual_cluster(tokens)?);
                }
