...
impl super::Client {
    ...
    ///   - [`ip_address_type(IpAddressType)`](crate::operation::set_ip_address_type::builders::SetIpAddressTypeFluentBuilder::ip_address_type) / [`set_ip_address_type(Option<IpAddressType>)`](crate::operation::set_ip_address_type::builders::SetIpAddressTypeFluentBuilder::set_ip_address_type):<br>required: **true**<br><p>The IP address type. Internal load balancers must use <code>ipv4</code>.</p> <p>\[Application Load Balancers\] The possible values are ipv4 (IPv4 addresses), dualstack (IPv4 and IPv6 addresses), and dualstack-without-public-ipv4 (public IPv6 addresses and private IPv4 and IPv6 addresses).</p> <p>Application Load Balancer authentication supports IPv4 addresses only when connecting to an Identity Provider (IdP) or Amazon Cognito endpoint. Without a public IPv4 address the load balancer can't complete the authentication process, resulting in HTTP 500 errors.</p> <p>\[Network Load Balancers and Gateway Load Balancers\] The possible values are ipv4 (IPv4 addresses) and dualstack (IPv4 and IPv6 addresses).</p><br>
