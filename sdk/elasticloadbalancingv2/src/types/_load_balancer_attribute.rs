...
pub struct LoadBalancerAttribute {
    ...
    /// <li>
    /// <p><code>health_check_logs.s3.enabled</code> - Indicates whether health check logs are enabled. The value is <code>true</code> or <code>false</code>. The default is <code>false</code>.</p></li>
    /// <li>
    /// <p><code>health_check_logs.s3.bucket</code> - The name of the S3 bucket for the health check logs. This attribute is required if health check logs are enabled. The bucket must exist in the same region as the load balancer and have a bucket policy that grants Elastic Load Balancing permissions to write to the bucket.</p></li>
    /// <li>
    /// <p><code>health_check_logs.s3.prefix</code> - The prefix for the location in the S3 bucket for the health check logs.</p></li>
    /// <li>
    ...
    /// </ul></li>
    /// <li>
    /// <p><code>routing.http2.enabled</code> - Indicates whether clients can connect to the load balancer using HTTP/2. If <code>true</code>, clients can connect using HTTP/2 or HTTP/1.1. However, all client requests are subject to the stricter HTTP/2 header validation rules. For example, message header names must contain only alphanumeric characters and hyphens. If <code>false</code>, clients must connect using HTTP/1.1. The default is <code>true</code>.</p></li>
    /// <li>
    ...
    /// <li>
    /// <p><code>secondary_ips.auto_assigned.per_subnet</code> - The number of secondary IP addresses to configure for your load balancer nodes. Use to address port allocation errors if you can't add targets. The valid range is 0 to 7. The default is 0. After you set this value, you can't decrease it.</p></li>
    /// </ul>
