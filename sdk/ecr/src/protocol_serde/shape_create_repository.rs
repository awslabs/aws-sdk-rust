            ...
            let mut tmp =
                 {
                    ...
                    let _ = response;
                    output = crate::protocol_serde::shape_kms_exception::de_kms_exception_json_err(response.body().as_ref(), output).map_err(crate::error::CreateRepositoryError::unhandled)?;
                    let output = output.meta(generic);
                    output.build()
                }
            ;
