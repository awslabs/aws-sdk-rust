                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "Credentials" => {
                            builder = builder.set_credentials(
                                crate::protocol_serde::shape_application_credential_list::de_application_credential_list(tokens, _value, depth + 1)?,
                            );
                        }
                        "DatabaseId" => {
                        ...
                        "Status" => {
                            builder = builder.set_status(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::DatabaseStatus::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "PrimaryHost" => {
                        ...
                        "ConnectedComponentArns" => {
                            builder = builder.set_connected_component_arns(crate::protocol_serde::shape_component_arn_list::de_component_arn_list(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
