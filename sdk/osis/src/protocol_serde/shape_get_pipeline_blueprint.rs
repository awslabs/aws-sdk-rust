...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_get_pipeline_blueprint::de_get_pipeline_blueprint(_response_body, output)
            .map_err(crate::operation::get_pipeline_blueprint::GetPipelineBlueprintError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "Blueprint" => {
                    builder = builder.set_blueprint(crate::protocol_serde::shape_pipeline_blueprint::de_pipeline_blueprint(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
