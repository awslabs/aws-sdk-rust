...
pub fn ser_efs_volume_configuration(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::types::EfsVolumeConfiguration) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    ...
    if let Some(var_4) = &input.transit_encryption_port {
        object.key("transitEncryptionPort").number(#[allow(clippy::useless_conversion)]aws_smithy_types::Number::NegInt((*var_4).into()));
    }
...
pub(crate) fn de_efs_volume_configuration<'a, I>(tokens: &mut std::iter::Peekable<I>) -> Result<Option<crate::types::EfsVolumeConfiguration>, aws_smithy_json::deserialize::error::DeserializeError>
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "transitEncryption" => {
                                builder = builder.set_transit_encryption(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::EfsTransitEncryption::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
