...
> {
    ...
    Err(match error_code {
        "AccessDeniedException" => crate::operation::describe_capacity_providers::DescribeCapacityProvidersError::AccessDeniedException({
            #[allow(unused_mut)]
            let mut tmp = {
                #[allow(unused_mut)]
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "capacityProviders" => {
                    builder = builder.set_capacity_providers(crate::protocol_serde::shape_capacity_providers::de_capacity_providers(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
