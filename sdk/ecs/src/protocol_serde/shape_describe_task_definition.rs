...
> {
    ...
    let error_code =
        match generic.code() {
            Some(code) => code,
            None => return Err(
                crate::operation::describe_task_definition::DescribeTaskDefinitionError::unhandled(
                    generic,
                ),
            ),
        };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_describe_task_definition_http_response_with_props(
    _response_status: u16,
    _response_headers: &http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_describe_task_definition::de_describe_task_definition(
                _response_body,
                output,
            )
            .map_err(
