...
> {
    ...
    Err(match error_code {
        "AccessDeniedException" => crate::operation::describe_task_definition::DescribeTaskDefinitionError::AccessDeniedException({
            #[allow(unused_mut)]
            let mut tmp = {
                #[allow(unused_mut)]
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "taskDefinition" => {
                    builder = builder.set_task_definition(crate::protocol_serde::shape_task_definition::de_task_definition(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
