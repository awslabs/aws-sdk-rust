...
> {
    ...
    let error_code =
        match generic.code() {
            Some(code) => code,
            None => return Err(
                crate::operation::list_container_instances::ListContainerInstancesError::unhandled(
                    generic,
                ),
            ),
        };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_list_container_instances_http_response_with_props(
    _response_status: u16,
    _response_headers: &http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_list_container_instances::de_list_container_instances(
                _response_body,
                output,
            )
            .map_err(
