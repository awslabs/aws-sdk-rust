            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "driverOpts" => {
                            builder = builder.set_driver_opts(crate::protocol_serde::shape_string_map::de_string_map(tokens, _value, depth + 1)?);
                        }
                        "labels" => {
                            builder = builder.set_labels(crate::protocol_serde::shape_string_map::de_string_map(tokens, _value, depth + 1)?);
                        }
