...
pub(crate) fn de_auto_scaling_group_provider<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::AutoScalingGroupProvider>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "managedDraining" => {
                                builder = builder.set_managed_draining(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::ManagedDraining::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
            ...
            Ok(Some(crate::serde_util::auto_scaling_group_provider_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
