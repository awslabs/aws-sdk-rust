...
pub fn ser_service_managed_ebs_volume_configuration(
    ...
    if let Some(var_4) = &input.size_in_gib {
        object.key("sizeInGiB").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_4).into()));
    }
    ...
    if let Some(var_7) = &input.throughput {
        object.key("throughput").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_7).into()));
    }
...
pub(crate) fn de_service_managed_ebs_volume_configuration<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::ServiceManagedEbsVolumeConfiguration>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                ...
                match tokens.next().transpose()? {
                    Some(::aws_smithy_json::deserialize::Token::EndObject { .. }) => break,
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "tagSpecifications" => {
                                builder = builder.set_tag_specifications(
                                    crate::protocol_serde::shape_ebs_tag_specifications::de_ebs_tag_specifications(tokens)?
                                );
                            }
                            "roleArn" => {
                            ...
                            "filesystemType" => {
                                builder = builder.set_filesystem_type(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::TaskFilesystemType::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
            ...
            Ok(Some(crate::serde_util::service_managed_ebs_volume_configuration_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
