...
/// <p>Port mappings expose your container's network ports to the outside world. this allows clients to access your application. It's also used for inter-container communication within the same task.</p>
/// <p>For task definitions (both the Fargate and EC2 launch type) that use the <code>awsvpc</code> network mode, only specify the <code>containerPort</code>. The <code>hostPort</code> is always ignored, and the container port is automatically mapped to a random high-numbered port on the host.</p>
...
pub struct PortMapping {
    ...
    /// <p>For tasks that use the Fargate launch type or EC2 tasks that use the <code>awsvpc</code> network mode, you use <code>containerPort</code> to specify the exposed ports.</p>
    /// <p>For Windows containers on Fargate, you can't use port 3150 for the <code>containerPort</code>. This is because it's reserved.</p>
    /// <p>Suppose that you're using containers in a task with the EC2 launch type and you specify a container port and not a host port. Then, your container automatically receives a host port in the ephemeral port range. For more information, see <code>hostPort</code>. Port mappings that are automatically assigned in this way don't count toward the 100 reserved ports quota of a container instance.</p>
