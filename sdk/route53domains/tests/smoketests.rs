...
//! ```
//!
use aws_sdk_route53domains::{config, Client};
/// Smoke tests for the `get_domain_detail` operation
#[::tokio::test]
async fn test_get_domain_detail_failure() {
    ...
    let res = client
        .get_domain_detail()
        .set_domain_name(::std::option::Option::Some("fake-domain-name".to_owned()))
        .send()
        .await;
...
async fn test_list_domains_success() {
    ...
    let conf = config::Config::from(&config)
        ...
        .build();
    let client = Client::from_conf(conf);
    let res = client.list_domains().send().await;
