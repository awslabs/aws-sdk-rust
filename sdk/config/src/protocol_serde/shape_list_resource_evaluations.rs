...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_resource_evaluations::ListResourceEvaluationsError::unhandled(
                generic,
            ))
        }
    };
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "ResourceEvaluations" => {
                    builder = builder.set_resource_evaluations(crate::protocol_serde::shape_resource_evaluations::de_resource_evaluations(tokens)?);
                }
                "NextToken" => {
