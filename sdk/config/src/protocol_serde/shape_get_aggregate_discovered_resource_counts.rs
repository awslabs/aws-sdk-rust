...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::get_aggregate_discovered_resource_counts::GetAggregateDiscoveredResourceCountsError::unhandled(generic))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_get_aggregate_discovered_resource_counts::de_get_aggregate_discovered_resource_counts(
            _response_body,
            output,
        )
        .map_err(crate::operation::get_aggregate_discovered_resource_counts::GetAggregateDiscoveredResourceCountsError::unhandled)?;
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "GroupedResourceCounts" => {
                    builder = builder.set_grouped_resource_counts(
                        crate::protocol_serde::shape_grouped_resource_count_list::de_grouped_resource_count_list(tokens, _value, depth + 1)?,
                    );
                }
                "NextToken" => {
