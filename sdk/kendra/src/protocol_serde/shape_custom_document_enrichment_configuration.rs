                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "InlineConfigurations" => {
                            builder = builder.set_inline_configurations(
                                    crate::protocol_serde::shape_inline_custom_document_enrichment_configuration_list::de_inline_custom_document_enrichment_configuration_list(tokens, _value, depth + 1)?
                                );
                        }
                        "PreExtractionHookConfiguration" => {
                            builder = builder.set_pre_extraction_hook_configuration(
                                crate::protocol_serde::shape_hook_configuration::de_hook_configuration(tokens, _value, depth + 1)?,
                            );
                        }
