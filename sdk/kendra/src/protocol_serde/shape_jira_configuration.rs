...
pub(crate) fn de_jira_configuration<'a, I>(
    ...
    _value: &'a [u8],
    depth: u32,
) -> ::std::result::Result<Option<crate::types::JiraConfiguration>, ::aws_smithy_json::deserialize::error::DeserializeError>
where
                ...
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "IssueType" => {
                                builder = builder.set_issue_type(crate::protocol_serde::shape_issue_type::de_issue_type(tokens, _value, depth + 1)?);
                            }
                            "Status" => {
                                builder = builder.set_status(crate::protocol_serde::shape_jira_status::de_jira_status(tokens, _value, depth + 1)?);
                            }
                            "IssueSubEntityFilter" => {
                                builder = builder.set_issue_sub_entity_filter(
                                    crate::protocol_serde::shape_issue_sub_entity_filter::de_issue_sub_entity_filter(tokens, _value, depth + 1)?,
                                );
                            }
