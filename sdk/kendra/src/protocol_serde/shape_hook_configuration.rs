// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_hook_configuration(object: &mut aws_smithy_json::serialize::JsonObjectWriter, input: &crate::model::HookConfiguration) -> Result<(), aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.invocation_condition {
        #[allow(unused_mut)]
        let mut object_2 = object.key("InvocationCondition").start_object();
        crate::protocol_serde::shape_document_attribute_condition::ser_document_attribute_condition(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.lambda_arn {
        object.key("LambdaArn").string(var_3.as_str());
    }
    if let Some(var_4) = &input.s3_bucket {
        object.key("S3Bucket").string(var_4.as_str());
    }
    Ok(())
}

pub(crate) fn de_hook_configuration<'a, I>(tokens: &mut std::iter::Peekable<I>) -> Result<Option<crate::model::HookConfiguration>, aws_smithy_json::deserialize::error::DeserializeError>
                    where I: Iterator<Item = Result<aws_smithy_json::deserialize::Token<'a>, aws_smithy_json::deserialize::error::DeserializeError>> {
    match tokens.next().transpose()? {
        Some(aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
                        Some(aws_smithy_json::deserialize::Token::StartObject { .. }) => {
            #[allow(unused_mut)]
            let mut builder = crate::model::hook_configuration::Builder::default();
            loop {
                match tokens.next().transpose()? {
                    Some(aws_smithy_json::deserialize::Token::EndObject { .. }) => break,
                                            Some(aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                        match key.to_unescaped()?.as_ref() {
                            "InvocationCondition" => {
                                builder = builder.set_invocation_condition(
                                    crate::protocol_serde::shape_document_attribute_condition::de_document_attribute_condition(tokens)?
                                );
                            }
                            "LambdaArn" => {
                                builder = builder.set_lambda_arn(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            u.into_owned()
                                        )
                                    ).transpose()?
                                );
                            }
                            "S3Bucket" => {
                                builder = builder.set_s3_bucket(
                                    aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            u.into_owned()
                                        )
                                    ).transpose()?
                                );
                            }
                            _ => aws_smithy_json::deserialize::token::skip_value(tokens)?
                        }
                    }
                    other => return Err(aws_smithy_json::deserialize::error::DeserializeError::custom(format!("expected object key or end object, found: {:?}", other)))
                }
            }
            Ok(Some(builder.build()))
        }
        _ => {
            Err(aws_smithy_json::deserialize::error::DeserializeError::custom("expected start object or null"))
        }
    }
}

