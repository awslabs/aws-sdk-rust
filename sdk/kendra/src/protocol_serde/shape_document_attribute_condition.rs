...
pub(crate) fn de_document_attribute_condition<'a, I>(
    ...
    _value: &'a [u8],
    depth: u32,
) -> ::std::result::Result<Option<crate::types::DocumentAttributeCondition>, ::aws_smithy_json::deserialize::error::DeserializeError>
where
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "Operator" => {
                            builder = builder.set_operator(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::ConditionOperator::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        "ConditionOnValue" => {
                            builder = builder.set_condition_on_value(
                                crate::protocol_serde::shape_document_attribute_value::de_document_attribute_value(tokens, _value, depth + 1)?,
                            );
                        }
