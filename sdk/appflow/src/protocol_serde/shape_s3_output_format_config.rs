            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "prefixConfig" => {
                            builder =
                                builder.set_prefix_config(crate::protocol_serde::shape_prefix_config::de_prefix_config(tokens, _value, depth + 1)?);
                        }
                        "aggregationConfig" => {
                            builder = builder.set_aggregation_config(crate::protocol_serde::shape_aggregation_config::de_aggregation_config(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
