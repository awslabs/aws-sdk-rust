                ...
                pub fn send(self) -> impl tokio_stream::Stream<Item = std::result::Result<crate::output::ListApplicationsOutput, aws_smithy_http::result::SdkError<crate::error::ListApplicationsError>>> + Unpin
                 {
                    ...
                    aws_smithy_async::future::fn_stream::FnStream::new(move |tx| Box::pin(async move {
                        ...
                        loop {
                            ...
                            let done = match resp {
                                Ok(ref resp) => {
                                    let new_token = crate::lens::reflens_list_applications_output_next_token(resp);
...
/// Paginator for [`ListJobRuns`](crate::operation::ListJobRuns)
            ...
            impl ListJobRunsPaginator  {
                /// Create a new paginator-wrapper
                pub(crate) fn new(handle: std::sync::Arc<crate::client::Handle>, builder: crate::input::list_job_runs_input::Builder) -> Self {
                    Self {
                        handle,
                        builder,
                        stop_on_duplicate_token: true,
                ...
                ///
                /// _Note:_ No requests will be dispatched until the stream is used (eg. with [`.next().await`](tokio_stream::StreamExt::next)).
                pub fn send(self) -> impl tokio_stream::Stream<Item = std::result::Result<crate::output::ListJobRunsOutput, aws_smithy_http::result::SdkError<crate::error::ListJobRunsError>>> + Unpin
                 {
                    // Move individual fields out of self for the borrow checker
                    let builder = self.builder;
                    let handle = self.handle;
                    aws_smithy_async::future::fn_stream::FnStream::new(move |tx| Box::pin(async move {
                        ...
                        loop {
                            ...
                            let done = match resp {
                                Ok(ref resp) => {
                                    let new_token = crate::lens::reflens_list_job_runs_output_next_token(resp);
                                    let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                                    if !is_empty && new_token == input.next_token.as_ref() && self.stop_on_duplicate_token {
                                        true
                                    } else {
...
/// Flattened paginator for `ListApplicationsPaginator`
                ...
                impl  ListApplicationsPaginatorItems  {
                    ...
                    pub fn send(self) -> impl tokio_stream::Stream<Item = std::result::Result<crate::model::ApplicationSummary, aws_smithy_http::result::SdkError<crate::error::ListApplicationsError>>> + Unpin
                     {
                        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| crate::lens::lens_list_applications_output_applications(page).unwrap_or_default().into_iter())
                    }
                }
...
/// Flattened paginator for `ListJobRunsPaginator`
                ...
                impl  ListJobRunsPaginatorItems  {
                    ...
                    ///
                    /// To read the entirety of the paginator, use [`.collect::<Result<Vec<_>, _>()`](tokio_stream::StreamExt::collect).
                    pub fn send(self) -> impl tokio_stream::Stream<Item = std::result::Result<crate::model::JobRunSummary, aws_smithy_http::result::SdkError<crate::error::ListJobRunsError>>> + Unpin
                     {
                        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| crate::lens::lens_list_job_runs_output_job_runs(page).unwrap_or_default().into_iter())
                    }
                }
