...
#[allow(clippy::unnecessary_wraps)]
pub fn de_get_evidence_folders_by_assessment_control_http_response_with_props(
    _response_status: u16,
    _response_headers: &::http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "evidenceFolders" => {
                    builder = builder.set_evidence_folders(crate::protocol_serde::shape_assessment_evidence_folders::de_assessment_evidence_folders(
                        tokens,
                    )?);
                }
                "nextToken" => {
