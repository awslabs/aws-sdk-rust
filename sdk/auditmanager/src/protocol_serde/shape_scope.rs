            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "awsAccounts" => {
                            builder =
                                builder.set_aws_accounts(crate::protocol_serde::shape_aws_accounts::de_aws_accounts(tokens, _value, depth + 1)?);
                        }
                        "awsServices" => {
                            builder =
                                builder.set_aws_services(crate::protocol_serde::shape_aws_services::de_aws_services(tokens, _value, depth + 1)?);
                        }
