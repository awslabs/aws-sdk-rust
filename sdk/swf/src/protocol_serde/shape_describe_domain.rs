    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "domainInfo" => {
                    builder = builder.set_domain_info(crate::protocol_serde::shape_domain_info::de_domain_info(tokens, _value, depth + 1)?);
                }
                "configuration" => {
                    builder = builder.set_configuration(crate::protocol_serde::shape_domain_configuration::de_domain_configuration(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
