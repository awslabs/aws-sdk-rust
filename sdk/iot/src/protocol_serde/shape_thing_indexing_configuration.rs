...
pub(crate) fn de_thing_indexing_configuration<'a, I>(
    ...
    _value: &'a [u8],
    depth: u32,
) -> ::std::result::Result<Option<crate::types::ThingIndexingConfiguration>, ::aws_smithy_json::deserialize::error::DeserializeError>
where
            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "managedFields" => {
                            builder = builder.set_managed_fields(crate::protocol_serde::shape_fields::de_fields(tokens, _value, depth + 1)?);
                        }
                        "customFields" => {
                            builder = builder.set_custom_fields(crate::protocol_serde::shape_fields::de_fields(tokens, _value, depth + 1)?);
                        }
                        "filter" => {
                            builder = builder.set_filter(crate::protocol_serde::shape_indexing_filter::de_indexing_filter(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
