            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "headers" => {
                            builder = builder.set_headers(crate::protocol_serde::shape_header_list::de_header_list(tokens, _value, depth + 1)?);
                        }
                        "auth" => {
                            builder = builder.set_auth(crate::protocol_serde::shape_http_authorization::de_http_authorization(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "batchConfig" => {
                            builder =
                                builder.set_batch_config(crate::protocol_serde::shape_batch_config::de_batch_config(tokens, _value, depth + 1)?);
                        }
