                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "securityProfileIdentifier" => {
                            builder = builder.set_security_profile_identifier(
                                crate::protocol_serde::shape_security_profile_identifier::de_security_profile_identifier(tokens, _value, depth + 1)?,
                            );
                        }
                        "target" => {
                            builder = builder.set_target(crate::protocol_serde::shape_security_profile_target::de_security_profile_target(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
