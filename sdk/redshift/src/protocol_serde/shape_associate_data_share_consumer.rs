...
> {
    ...
    if !(start_el.matches("AssociateDataShareConsumerResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected AssociateDataShareConsumerResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("AssociateDataShareConsumerResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected AssociateDataShareConsumerResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            ,
            s if s.matches("DataShareType") /* DataShareType com.amazonaws.redshift.synthetic#AssociateDataShareConsumerOutput$DataShareType */ =>  {
                let var_6 =
                    Some(
