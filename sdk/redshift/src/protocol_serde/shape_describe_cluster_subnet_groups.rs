...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_cluster_subnet_groups::DescribeClusterSubnetGroupsError::unhandled)?;
    ...
    Err(match error_code {
        "ClusterSubnetGroupNotFoundFault" => {
            crate::operation::describe_cluster_subnet_groups::DescribeClusterSubnetGroupsError::ClusterSubnetGroupNotFoundFault({
                #[allow(unused_mut)]
                let mut tmp = {
                    #[allow(unused_mut)]
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_cluster_subnet_groups::de_describe_cluster_subnet_groups(_response_body, output)
            .map_err(crate::operation::describe_cluster_subnet_groups::DescribeClusterSubnetGroupsError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("DescribeClusterSubnetGroupsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeClusterSubnetGroupsResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DescribeClusterSubnetGroupsResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DescribeClusterSubnetGroupsResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("ClusterSubnetGroups") /* ClusterSubnetGroups com.amazonaws.redshift.synthetic#DescribeClusterSubnetGroupsOutput$ClusterSubnetGroups */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_cluster_subnet_groups::de_cluster_subnet_groups(&mut tag, depth + 1)
                        ?
                    )
                ;
