...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_logging_status::de_describe_logging_status(_response_body, output)
            .map_err(crate::operation::describe_logging_status::DescribeLoggingStatusError::unhandled)?;
...
> {
    ...
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DescribeLoggingStatusResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DescribeLoggingStatusResult got {start_el:?}"
            )));
        }
