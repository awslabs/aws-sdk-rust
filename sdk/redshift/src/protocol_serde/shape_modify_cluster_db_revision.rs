...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::modify_cluster_db_revision::ModifyClusterDbRevisionError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::modify_cluster_db_revision::ModifyClusterDbRevisionError::unhandled(
                generic,
            ))
        }
    };
    ...
    Err(match error_code {
        ...
        "ClusterOnLatestRevision" => crate::operation::modify_cluster_db_revision::ModifyClusterDbRevisionError::ClusterOnLatestRevisionFault({
            ...
            let mut tmp = {
                ...
                output = crate::protocol_serde::shape_cluster_on_latest_revision_fault::de_cluster_on_latest_revision_fault_xml_err(
                    _response_body,
                    output,
                )
                .map_err(crate::operation::modify_cluster_db_revision::ModifyClusterDbRevisionError::unhandled)?;
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_modify_cluster_db_revision::de_modify_cluster_db_revision(_response_body, output)
            .map_err(crate::operation::modify_cluster_db_revision::ModifyClusterDbRevisionError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("ModifyClusterDbRevisionResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected ModifyClusterDbRevisionResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("ModifyClusterDbRevisionResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected ModifyClusterDbRevisionResult got {start_el:?}"
            )));
        }
