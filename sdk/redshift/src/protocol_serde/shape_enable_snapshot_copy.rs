...
) -> std::result::Result<crate::operation::enable_snapshot_copy::builders::EnableSnapshotCopyOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError>
{
    ...
    if !(start_el.matches("EnableSnapshotCopyResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected EnableSnapshotCopyResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("EnableSnapshotCopyResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected EnableSnapshotCopyResult got {start_el:?}"
            )));
        }
