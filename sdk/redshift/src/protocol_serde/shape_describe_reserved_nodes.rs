...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_reserved_nodes::DescribeReservedNodesError::unhandled)?;
    ...
    Err(match error_code {
        "DependentServiceUnavailableFault" => {
            crate::operation::describe_reserved_nodes::DescribeReservedNodesError::DependentServiceUnavailableFault({
                #[allow(unused_mut)]
                let mut tmp = {
                    #[allow(unused_mut)]
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_reserved_nodes::de_describe_reserved_nodes(_response_body, output)
            .map_err(crate::operation::describe_reserved_nodes::DescribeReservedNodesError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("DescribeReservedNodesResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeReservedNodesResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DescribeReservedNodesResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DescribeReservedNodesResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("ReservedNodes") /* ReservedNodes com.amazonaws.redshift.synthetic#DescribeReservedNodesOutput$ReservedNodes */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_reserved_node_list::de_reserved_node_list(&mut tag, depth + 1)
                        ?
                    )
                ;
