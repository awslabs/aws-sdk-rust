...
pub fn de_association(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::Association, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("CertificateAssociations") /* CertificateAssociations com.amazonaws.redshift#Association$CertificateAssociations */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_certificate_association_list::de_certificate_association_list(&mut tag, depth + 1)
                        ?
                    )
                ;
