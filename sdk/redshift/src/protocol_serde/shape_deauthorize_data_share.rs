...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_deauthorize_data_share::de_deauthorize_data_share(_response_body, output)
            .map_err(crate::operation::deauthorize_data_share::DeauthorizeDataShareError::unhandled)?;
...
> {
    ...
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DeauthorizeDataShareResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DeauthorizeDataShareResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            ,
            s if s.matches("DataShareType") /* DataShareType com.amazonaws.redshift.synthetic#DeauthorizeDataShareOutput$DataShareType */ =>  {
                let var_6 =
                    Some(
