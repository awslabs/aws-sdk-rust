...
#[allow(clippy::unnecessary_wraps)]
pub fn de_create_custom_domain_association_http_response_with_props(
    _response_status: u16,
    _response_headers: &::http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    ...
    if !(start_el.matches("CreateCustomDomainAssociationResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected CreateCustomDomainAssociationResponse got {:?}",
            start_el
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("CreateCustomDomainAssociationResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected CreateCustomDomainAssociationResult got {:?}",
                start_el
            )));
        }
