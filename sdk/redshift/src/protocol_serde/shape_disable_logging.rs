...
pub fn de_disable_logging(
    ...
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("DisableLoggingResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected DisableLoggingResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            ,
            s if s.matches("S3Tables") /* S3Tables com.amazonaws.redshift.synthetic#DisableLoggingOutput$S3Tables */ =>  {
                let var_9 =
                    Some(
