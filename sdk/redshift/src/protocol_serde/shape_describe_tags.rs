...
pub fn de_describe_tags_http_error(
    ...
    Err(match error_code {
        "InvalidTagFault" => {
            crate::error::DescribeTagsError::InvalidTagFault({
                #[allow(unused_mut)]
                let mut tmp = {
                    #[allow(unused_mut)]
                    let mut output = crate::error::invalid_tag_fault::Builder::default();
                    let _ = response;
                    output = crate::protocol_serde::shape_invalid_tag_fault::de_invalid_tag_fault_xml_err(response.body().as_ref(), output).map_err(crate::error::DescribeTagsError::unhandled)?;
        ...
        "ResourceNotFoundFault" => crate::error::DescribeTagsError::ResourceNotFoundFault({
            ...
            let mut tmp = {
                ...
                let _ = response;
                output = crate::protocol_serde::shape_resource_not_found_fault::de_resource_not_found_fault_xml_err(response.body().as_ref(), output).map_err(crate::error::DescribeTagsError::unhandled)?;
                let output = output.meta(generic);
                output.build()
            };
            if tmp.message.is_none() {
