...
pub fn de_s3_table_last_ingestion_time_map(
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("entry") => {
                crate::protocol_serde::shape_s3_table_last_ingestion_time_map::de_s3_table_last_ingestion_time_map_entry(&mut tag, &mut out, depth)?;
            }
            _ => {}
        }
    }
...
pub fn de_s3_table_last_ingestion_time_map_entry(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("key") /* key com.amazonaws.redshift#S3TableLastIngestionTimeMap$key */ =>  {
                k = Some(
            ...
            ,
            s if s.matches("value") /* value com.amazonaws.redshift#S3TableLastIngestionTimeMap$value */ =>  {
                v = Some(
                    Result::<::std::string::String, ::aws_smithy_xml::decode::XmlDecodeError>::Ok(
                        ::aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                        .into()
