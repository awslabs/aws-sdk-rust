...
/// When writing a match expression against `UsageLimitFeatureType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `usagelimitfeaturetype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `UsageLimitFeatureType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `UsageLimitFeatureType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `UsageLimitFeatureType::NewFeature` is defined.
...
impl ::std::convert::From<&str> for UsageLimitFeatureType {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => UsageLimitFeatureType::Unknown(crate::primitives::UnknownVariantValue(other.to_owned())),
        }
    }
}
impl ::std::str::FromStr for UsageLimitFeatureType {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(UsageLimitFeatureType::from(s))
    }
}
