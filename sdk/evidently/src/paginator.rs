            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_launches_output_next_token(resp);
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
                                && new_token == input.next_token.as_ref()
                                && self.stop_on_duplicate_token
            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let done = match resp {
                        Ok(ref resp) => {
                            let new_token =
                                crate::lens::reflens_list_segment_references_output_next_token(
                                    resp,
                                );
                            let is_empty = new_token.map(|token| token.is_empty()).unwrap_or(true);
                            if !is_empty
...
impl ListExperimentsPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_experiments_output_experiments(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
...
impl ListLaunchesPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_launches_output_launches(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
...
impl ListSegmentReferencesPaginatorItems {
    ...
    > + Unpin {
        aws_smithy_async::future::fn_stream::TryFlatMap::new(self.0.send()).flat_map(|page| {
            crate::lens::lens_list_segment_references_output_referenced_by(page)
                .unwrap_or_default()
                .into_iter()
        })
    }
