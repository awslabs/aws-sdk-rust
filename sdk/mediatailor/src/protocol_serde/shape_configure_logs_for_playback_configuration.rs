...
pub fn de_configure_logs_for_playback_configuration_http_error(
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_configure_logs_for_playback_configuration_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_configure_logs_for_playback_configuration::de_configure_logs_for_playback_configuration(
            _response_body,
            output,
        )
        .map_err(crate::operation::configure_logs_for_playback_configuration::ConfigureLogsForPlaybackConfigurationError::unhandled)?;
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "AdsInteractionLog" => {
                    builder = builder.set_ads_interaction_log(crate::protocol_serde::shape_ads_interaction_log::de_ads_interaction_log(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
                ...
                "EnabledLoggingStrategies" => {
                    builder = builder.set_enabled_logging_strategies(
                        crate::protocol_serde::shape_list_of_logging_strategies::de_list_of_logging_strategies(tokens, _value, depth + 1)?,
                    );
                }
                "ManifestServiceInteractionLog" => {
                    builder = builder.set_manifest_service_interaction_log(
                        crate::protocol_serde::shape_manifest_service_interaction_log::de_manifest_service_interaction_log(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
