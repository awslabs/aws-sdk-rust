...
pub struct AdminCreateUserConfigType {
    /// <p>The setting for allowing self-service sign-up. When <code>true</code>, only administrators can create new user profiles. When <code>false</code>, users can register themselves and create a new user profile with the <a href="https://docs.aws.amazon.com/cognito-user-identity-pools/latest/APIReference/API_SignUp.html">SignUp</a> operation.</p>
    pub allow_admin_create_user_only: bool,
    /// <p>This parameter is no longer in use. Configure the duration of temporary passwords with the <code>TemporaryPasswordValidityDays</code> parameter of <a href="https://docs.aws.amazon.com/cognito-user-identity-pools/latest/APIReference/API_PasswordPolicyType.html">PasswordPolicyType</a>. For older user pools that have a <code>UnusedAccountValidityDays</code> configuration, that value is effective until you set a value for <code>TemporaryPasswordValidityDays</code>.</p>
