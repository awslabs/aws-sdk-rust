...
impl GetIceServerConfigInput {
    ...
    > {
        Ok({
            ...
            let body = crate::operation_ser::serialize_operation_get_ice_server_config(&self)
                .map_err(|err| {
...
impl SendAlexaOfferToMasterInput {
    ...
    > {
        Ok({
            let properties = smithy_http::property_bag::SharedPropertyBag::new();
            let request = self.request_builder_base()?;
            let body = crate::operation_ser::serialize_operation_send_alexa_offer_to_master(&self)
                .map_err(|err| {
                    smithy_http::operation::BuildError::SerializationError(err.into())
                })?;
            let request = Self::assemble(request, body);
