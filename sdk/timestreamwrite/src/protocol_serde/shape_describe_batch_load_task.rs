...
> {
    ...
    let error_code =
        match generic.code() {
            Some(code) => code,
            None => return Err(
                crate::operation::describe_batch_load_task::DescribeBatchLoadTaskError::unhandled(
                    generic,
                ),
            ),
        };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_describe_batch_load_task_http_response_with_props(
    _response_status: u16,
    _response_headers: &::http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_describe_batch_load_task::de_describe_batch_load_task(
                _response_body,
                output,
            )
            .map_err(
