            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "DataModel" => {
                            builder = builder.set_data_model(crate::protocol_serde::shape_data_model::de_data_model(tokens, _value, depth + 1)?);
                        }
                        "DataModelS3Configuration" => {
                            builder = builder.set_data_model_s3_configuration(
                                crate::protocol_serde::shape_data_model_s3_configuration::de_data_model_s3_configuration(tokens, _value, depth + 1)?,
                            );
                        }
