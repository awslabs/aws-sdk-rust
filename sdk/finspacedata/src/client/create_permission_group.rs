// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
impl super::Client {
    /// Constructs a fluent builder for the [`CreatePermissionGroup`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder) operation.
    ///
    /// - The fluent builder is configurable:
    ///   - [`name(impl ::std::convert::Into<String>)`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::name) / [`set_name(Option<String>)`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::set_name): <p>The name of the permission group.</p>
    ///   - [`description(impl ::std::convert::Into<String>)`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::description) / [`set_description(Option<String>)`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::set_description): <p>A brief description for the permission group.</p>
    ///   - [`application_permissions(Vec<ApplicationPermission>)`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::application_permissions) / [`set_application_permissions(Option<Vec<ApplicationPermission>>)`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::set_application_permissions): <p>The option to indicate FinSpace application permissions that are granted to a specific group.</p> <important>   <p>When assigning application permissions, be aware that the permission <code>ManageUsersAndGroups</code> allows users to grant themselves or others access to any functionality in their FinSpace environment's application. It should only be granted to trusted users.</p>  </important>  <ul>   <li> <p> <code>CreateDataset</code> – Group members can create new datasets.</p> </li>   <li> <p> <code>ManageClusters</code> – Group members can manage Apache Spark clusters from FinSpace notebooks.</p> </li>   <li> <p> <code>ManageUsersAndGroups</code> – Group members can manage users and permission groups. This is a privileged permission that allows users to grant themselves or others access to any functionality in the application. It should only be granted to trusted users.</p> </li>   <li> <p> <code>ManageAttributeSets</code> – Group members can manage attribute sets.</p> </li>   <li> <p> <code>ViewAuditData</code> – Group members can view audit data.</p> </li>   <li> <p> <code>AccessNotebooks</code> – Group members will have access to FinSpace notebooks.</p> </li>   <li> <p> <code>GetTemporaryCredentials</code> – Group members can get temporary API credentials.</p> </li>  </ul>
    ///   - [`client_token(impl ::std::convert::Into<String>)`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::client_token) / [`set_client_token(Option<String>)`](crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::set_client_token): <p>A token that ensures idempotency. This token expires in 10 minutes.</p>
    /// - On success, responds with [`CreatePermissionGroupOutput`](crate::operation::create_permission_group::CreatePermissionGroupOutput) with field(s):
    ///   - [`permission_group_id(Option<String>)`](crate::operation::create_permission_group::CreatePermissionGroupOutput::permission_group_id): <p>The unique identifier for the permission group.</p>
    /// - On failure, responds with [`SdkError<CreatePermissionGroupError>`](crate::operation::create_permission_group::CreatePermissionGroupError)
    pub fn create_permission_group(
        &self,
    ) -> crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder
    {
        crate::operation::create_permission_group::builders::CreatePermissionGroupFluentBuilder::new(
            self.handle.clone(),
        )
    }
}
