...
pub fn ser_schema_definition(
    ...
    if let Some(var_5) = &input.primary_key_columns {
        let mut array_6 = object.key("primaryKeyColumns").start_array();
        for item_7 in var_5 {
            {
            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "primaryKeyColumns" => {
                            builder = builder.set_primary_key_columns(crate::protocol_serde::shape_column_name_list::de_column_name_list(tokens)?);
                        }
