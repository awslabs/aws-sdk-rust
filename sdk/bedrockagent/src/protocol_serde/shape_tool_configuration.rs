            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "tools" => {
                            builder = builder.set_tools(crate::protocol_serde::shape_tools::de_tools(tokens, _value, depth + 1)?);
                        }
                        "toolChoice" => {
                            builder = builder.set_tool_choice(crate::protocol_serde::shape_tool_choice::de_tool_choice(tokens, _value, depth + 1)?);
                        }
