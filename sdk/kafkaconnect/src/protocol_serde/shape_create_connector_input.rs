...
pub fn ser_create_connector_input_input(
    ...
    if let Some(var_18) = &input.network_type {
        object.key("networkType").string(var_18.as_str());
    }
    if let Some(var_19) = &input.plugins {
        let mut array_20 = object.key("plugins").start_array();
        for item_21 in var_19 {
            {
                #[allow(unused_mut)]
                let mut object_22 = array_20.value().start_object();
                crate::protocol_serde::shape_plugin::ser_plugin(&mut object_22, item_21)?;
                object_22.finish();
            }
        }
        array_20.finish();
    }
    if let Some(var_23) = &input.service_execution_role_arn {
        object.key("serviceExecutionRoleArn").string(var_23.as_str());
    }
    ...
    if let Some(var_28) = &input.worker_configuration {
        #[allow(unused_mut)]
        let mut object_29 = object.key("workerConfiguration").start_object();
        crate::protocol_serde::shape_worker_configuration::ser_worker_configuration(&mut object_29, var_28)?;
        object_29.finish();
    }
    Ok(())
}
