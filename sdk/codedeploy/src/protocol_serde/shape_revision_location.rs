            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "gitHubLocation" => {
                            builder = builder.set_git_hub_location(crate::protocol_serde::shape_git_hub_location::de_git_hub_location(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "string" => {
                            builder = builder.set_string(crate::protocol_serde::shape_raw_string::de_raw_string(tokens, _value, depth + 1)?);
                        }
                        "appSpecContent" => {
                            builder = builder.set_app_spec_content(crate::protocol_serde::shape_app_spec_content::de_app_spec_content(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
