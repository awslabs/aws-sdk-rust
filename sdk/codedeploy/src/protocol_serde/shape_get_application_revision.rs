...
> {
    ...
    .map_err(crate::operation::get_application_revision::GetApplicationRevisionError::unhandled)?;
    ...
    let error_code =
        match generic.code() {
            Some(code) => code,
            None => return Err(
                crate::operation::get_application_revision::GetApplicationRevisionError::unhandled(
                    generic,
                ),
            ),
        };
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_get_application_revision_http_response_with_props(
    _response_status: u16,
    _response_headers: &::http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
