...
/// Configuration for a aws_sdk_lexruntime service client.
///
...
impl Builder {
    ...
    /// impl aws_sdk_lexruntime::config::auth::ResolveAuthScheme for CustomAuthSchemeResolver {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a aws_sdk_lexruntime::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    /// ```no_run
    /// use aws_sdk_lexruntime::config::endpoint::{ResolveEndpoint, EndpointFuture, Params, Endpoint};
    /// #[derive(Debug)]
    ...
    /// let config = aws_sdk_lexruntime::Config::builder().endpoint_resolver(resolver).build();
    /// let client = aws_sdk_lexruntime::Client::from_conf(config);
    /// ```
    ...
    ///
    /// ```no_run
    /// use aws_sdk_lexruntime::config::{AsyncSleep, Config, SharedAsyncSleep, Sleep};
    ///
    /// #[derive(Debug)]
    ...
    ///
    /// ```no_run
    /// use aws_sdk_lexruntime::config::{AsyncSleep, Builder, Config, SharedAsyncSleep, Sleep};
    ///
    /// #[derive(Debug)]
    /// pub struct ForeverSleep;
    ///
    ...
    /// # Default Behavior
    ///
    /// When no retry partition is explicitly set, the SDK automatically creates a default retry partition named `lexruntimeservice`
    /// (or `lexruntimeservice-<region>` if a region is configured).
    /// All Lex Runtime Service clients without an explicit retry partition will share this default partition.
    ///
    /// # Notes
    ///
    ...
    /// ```no_run
    /// use aws_sdk_lexruntime::config::Config;
    /// use aws_sdk_lexruntime::config::retry::{RetryPartition, TokenBucket};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_lexruntime::config::Config;
    /// use aws_sdk_lexruntime::config::retry::{ClientRateLimiter, RetryConfig, RetryPartition};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_lexruntime::config::IdentityCache;
    ///
    /// let config = aws_sdk_lexruntime::Config::builder()
    ...
    /// #    fn meta(&self) -> &aws_sdk_lexruntime::error::ErrorMetadata { todo!() }
    /// # }
    ///
    ...
    /// ```no_run
    /// use aws_sdk_lexruntime::config::BehaviorVersion;
    ///
    /// let config = aws_sdk_lexruntime::Config::builder()
    ...
    pub fn build(mut self) -> Config {
        ...
        Config {
            config: crate::config::Layer::from(layer.clone())
                .with_name("aws_sdk_lexruntime::config::Config")
                .freeze(),
            cloneable: layer,
...
impl ServiceRuntimePlugin {
    pub fn new(_service_config: crate::config::Config) -> Self {
        let config = {
            let mut cfg = ::aws_smithy_types::config_bag::Layer::new("AWSDeepSenseRunTimeService");
    ...
    pub(crate) fn new(
        ...
        Self {
            config: ::aws_smithy_types::config_bag::Layer::from(layer)
                .with_name("aws_sdk_lexruntime::config::ConfigOverrideRuntimePlugin")
                .freeze(),
            components,
        }
    }
        ...
        if input.get_origin("endpoint_url").is_client_config() {
            ...
            builder.set_endpoint_url(
                input
                    .service_config()
                    .and_then(|conf| {
                        conf.load_config(service_config_key("Lex Runtime Service", "AWS_ENDPOINT_URL", "endpoint_url"))
...
pub(crate) fn base_client_runtime_plugins(mut config: crate::Config) -> ::aws_smithy_runtime_api::client::runtime_plugin::RuntimePlugins {
    ...
    let default_retry_partition = "lexruntimeservice";
