    ...
    R: aws_smithy_client::retry::NewRequestPolicy,
{
    ...
    ///
    /// - Takes [`DeleteSessionInput`](crate::input::DeleteSessionInput) with field(s):
    ///   - [`bot_name(Option<String>)`](crate::input::DeleteSessionInput::bot_name): <p>The name of the bot that contains the session data.</p>
    ///   - [`bot_alias(Option<String>)`](crate::input::DeleteSessionInput::bot_alias): <p>The alias in use for the bot that contains the session data.</p>
    ///   - [`user_id(Option<String>)`](crate::input::DeleteSessionInput::user_id): <p>The identifier of the user associated with the session data.</p>
    ...
    ///   - [`bot_name(Option<String>)`](crate::input::GetSessionInput::bot_name): <p>The name of the bot that contains the session data.</p>
    ///   - [`bot_alias(Option<String>)`](crate::input::GetSessionInput::bot_alias): <p>The alias in use for the bot that contains the session data.</p>
    ///   - [`user_id(Option<String>)`](crate::input::GetSessionInput::user_id): <p>The ID of the client application user. Amazon Lex uses this to identify a user's conversation with your bot. </p>
    ///   - [`checkpoint_label_filter(Option<String>)`](crate::input::GetSessionInput::checkpoint_label_filter): <p>A string used to filter the intents returned in the <code>recentIntentSummaryView</code> structure. </p>  <p>When you specify a filter, only intents with their <code>checkpointLabel</code> field set to that string are returned.</p>
    ...
    ///
    /// - Takes [`PostTextInput`](crate::input::PostTextInput) with field(s):
    ///   - [`bot_name(Option<String>)`](crate::input::PostTextInput::bot_name): <p>The name of the Amazon Lex bot.</p>
    ///   - [`bot_alias(Option<String>)`](crate::input::PostTextInput::bot_alias): <p>The alias of the Amazon Lex bot.</p>
    ///   - [`user_id(Option<String>)`](crate::input::PostTextInput::user_id): <p>The ID of the client application user. Amazon Lex uses this to identify a user's conversation with your bot. At runtime, each request must contain the <code>userID</code> field.</p>  <p>To decide the user ID to use for your application, consider the following factors.</p>  <ul>   <li> <p>The <code>userID</code> field must not contain any personally identifiable information of the user, for example, name, personal identification numbers, or other end user personal information.</p> </li>   <li> <p>If you want a user to start a conversation on one device and continue on another device, use a user-specific identifier.</p> </li>   <li> <p>If you want the same user to be able to have two independent conversations on two different devices, choose a device-specific identifier.</p> </li>   <li> <p>A user can't have two independent conversations with two different versions of the same bot. For example, a user can't have a conversation with the PROD and BETA versions of the same bot. If you anticipate that a user will need to have conversation with two different versions, for example, while testing, include the bot alias in the user ID to separate the two conversations.</p> </li>  </ul>
    ///   - [`session_attributes(Option<HashMap<String, String>>)`](crate::input::PostTextInput::session_attributes): <p>Application-specific information passed between Amazon Lex and a client application.</p>  <p>For more information, see <a href="https://docs.aws.amazon.com/lex/latest/dg/context-mgmt.html#context-mgmt-session-attribs">Setting Session Attributes</a>.</p>
    ///   - [`request_attributes(Option<HashMap<String, String>>)`](crate::input::PostTextInput::request_attributes): <p>Request-specific information passed between Amazon Lex and a client application.</p>  <p>The namespace <code>x-amz-lex:</code> is reserved for special attributes. Don't create any request attributes with the prefix <code>x-amz-lex:</code>.</p>  <p>For more information, see <a href="https://docs.aws.amazon.com/lex/latest/dg/context-mgmt.html#context-mgmt-request-attribs">Setting Request Attributes</a>.</p>
    ///   - [`input_text(Option<String>)`](crate::input::PostTextInput::input_text): <p>The text that the user entered (Amazon Lex interprets this text).</p>
    ///   - [`active_contexts(Option<Vec<ActiveContext>>)`](crate::input::PostTextInput::active_contexts): <p>A list of contexts active for the request. A context can be activated when a previous intent is fulfilled, or by including the context in the request,</p>  <p>If you don't specify a list of contexts, Amazon Lex will use the current list of contexts for the session. If you specify an empty list, all contexts for the session are cleared.</p>
    ...
    ///
    /// - Takes [`PutSessionInput`](crate::input::PutSessionInput) with field(s):
    ///   - [`bot_name(Option<String>)`](crate::input::PutSessionInput::bot_name): <p>The name of the bot that contains the session data.</p>
    ///   - [`bot_alias(Option<String>)`](crate::input::PutSessionInput::bot_alias): <p>The alias in use for the bot that contains the session data.</p>
    ///   - [`user_id(Option<String>)`](crate::input::PutSessionInput::user_id): <p>The ID of the client application user. Amazon Lex uses this to identify a user's conversation with your bot. </p>
    ///   - [`session_attributes(Option<HashMap<String, String>>)`](crate::input::PutSessionInput::session_attributes): <p>Map of key/value pairs representing the session-specific context information. It contains application information passed between Amazon Lex and a client application.</p>
    ///   - [`dialog_action(Option<DialogAction>)`](crate::input::PutSessionInput::dialog_action): <p>Sets the next action that the bot should take to fulfill the conversation.</p>
    ///   - [`recent_intent_summary_view(Option<Vec<IntentSummary>>)`](crate::input::PutSessionInput::recent_intent_summary_view): <p>A summary of the recent intents for the bot. You can use the intent summary view to set a checkpoint label on an intent and modify attributes of intents. You can also use it to remove or add intent summary objects to the list.</p>  <p>An intent that you modify or add to the list must make sense for the bot. For example, the intent name must be valid for the bot. You must provide valid values for:</p>  <ul>   <li> <p> <code>intentName</code> </p> </li>   <li> <p>slot names</p> </li>   <li> <p> <code>slotToElict</code> </p> </li>  </ul>  <p>If you send the <code>recentIntentSummaryView</code> parameter in a <code>PutSession</code> request, the contents of the new summary view replaces the old summary view. For example, if a <code>GetSession</code> request returns three intents in the summary view and you call <code>PutSession</code> with one intent in the summary view, the next call to <code>GetSession</code> will only return one intent.</p>
    ///   - [`accept(Option<String>)`](crate::input::PutSessionInput::accept): <p>The message that Amazon Lex returns in the response can be either text or speech based depending on the value of this field.</p>  <ul>   <li> <p>If the value is <code>text/plain; charset=utf-8</code>, Amazon Lex returns text in the response.</p> </li>   <li> <p>If the value begins with <code>audio/</code>, Amazon Lex returns speech in the response. Amazon Lex uses Amazon Polly to generate the speech in the configuration that you specify. For example, if you specify <code>audio/mpeg</code> as the value, Amazon Lex returns speech in the MPEG format.</p> </li>   <li> <p>If the value is <code>audio/pcm</code>, the speech is returned as <code>audio/pcm</code> in 16-bit, little endian format.</p> </li>   <li> <p>The following are the accepted values:</p>    <ul>     <li> <p> <code>audio/mpeg</code> </p> </li>     <li> <p> <code>audio/ogg</code> </p> </li>     <li> <p> <code>audio/pcm</code> </p> </li>     <li> <p> <code>audio/*</code> (defaults to mpeg)</p> </li>     <li> <p> <code>text/plain; charset=utf-8</code> </p> </li>    </ul> </li>  </ul>
    ///   - [`active_contexts(Option<Vec<ActiveContext>>)`](crate::input::PutSessionInput::active_contexts): <p>A list of contexts active for the request. A context can be activated when a previous intent is fulfilled, or by including the context in the request,</p>  <p>If you don't specify a list of contexts, Amazon Lex will use the current list of contexts for the session. If you specify an empty list, all contexts for the session are cleared.</p>
