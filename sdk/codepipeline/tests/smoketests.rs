...
/// Smoke tests for the `get_pipeline` operation
#[::tokio::test]
async fn test_get_pipeline_failure() {
    ...
    let res = client
        .get_pipeline()
        .set_name(::std::option::Option::Some("fake-pipeline".to_owned()))
        .send()
        .await;
    res.expect_err("request should fail");
}
/// Smoke tests for the `list_pipelines` operation
#[::tokio::test]
async fn test_list_pipelines_success() {
    ...
    let conf = config::Config::from(&config)
        ...
        .build();
    let client = Client::from_conf(conf);
    let res = client.list_pipelines().send().await;
