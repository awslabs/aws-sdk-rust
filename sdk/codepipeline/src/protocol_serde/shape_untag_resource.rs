            ...
            let mut tmp =
                 {
                    ...
                    let _ = response;
                    output = crate::protocol_serde::shape_invalid_tags_exception::de_invalid_tags_exception_json_err(response.body().as_ref(), output).map_err(crate::operation::untag_resource::UntagResourceError::unhandled)?;
                    let output = output.meta(generic);
                    output.build()
                }
            ;
