...
impl super::Client {
    ...
    ///   - [`updates(RuleGroupUpdate)`](crate::operation::update_rule_group::builders::UpdateRuleGroupFluentBuilder::updates) / [`set_updates(Option<Vec<RuleGroupUpdate>>)`](crate::operation::update_rule_group::builders::UpdateRuleGroupFluentBuilder::set_updates): <p>An array of <code>RuleGroupUpdate</code> objects that you want to insert into or delete from a <code>RuleGroup</code>.</p>  <p>You can only insert <code>REGULAR</code> rules into a rule group.</p>  <p> <code>ActivatedRule|OverrideAction</code> applies only when updating or adding a <code>RuleGroup</code> to a <code>WebACL</code>. In this case you do not use <code>ActivatedRule|Action</code>. For all other update requests, <code>ActivatedRule|Action</code> is used instead of <code>ActivatedRule|OverrideAction</code>.</p>
