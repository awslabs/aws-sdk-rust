...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::list_application_instances::ListApplicationInstancesError::unhandled)?;
    generic_builder = ::aws_http::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_application_instances::ListApplicationInstancesError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_application_instances::de_list_application_instances(_response_body, output)
            .map_err(crate::operation::list_application_instances::ListApplicationInstancesError::unhandled)?;
        ...
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "ApplicationInstances" => {
                    builder =
                        builder.set_application_instances(crate::protocol_serde::shape_application_instances::de_application_instances(tokens)?);
                }
                "NextToken" => {
