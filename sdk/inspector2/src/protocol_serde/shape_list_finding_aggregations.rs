...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_finding_aggregations::ListFindingAggregationsError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        crate::serde_util::list_finding_aggregations_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::list_finding_aggregations::ListFindingAggregationsError::unhandled)?
    })
}
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "responses" => {
                    builder = builder.set_responses(crate::protocol_serde::shape_aggregation_response_list::de_aggregation_response_list(
                        tokens,
                    )?);
                }
