...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::batch_get_free_trial_info::BatchGetFreeTrialInfoError::unhandled(
                generic,
            ))
        }
    };
...
> {
    Ok({
        ...
        crate::serde_util::batch_get_free_trial_info_output_output_correct_errors(output)
            .build()
            .map_err(crate::operation::batch_get_free_trial_info::BatchGetFreeTrialInfoError::unhandled)?
    })
}
                ...
                match key.to_unescaped()?.as_ref() {
                    "accounts" => {
                        builder = builder.set_accounts(
                            crate::protocol_serde::shape_free_trial_account_info_list::de_free_trial_account_info_list(tokens, _value, depth + 1)?,
                        );
                    }
                    "failedAccounts" => {
                        builder = builder.set_failed_accounts(
                            crate::protocol_serde::shape_free_trial_info_error_list::de_free_trial_info_error_list(tokens, _value, depth + 1)?,
                        );
                    }
