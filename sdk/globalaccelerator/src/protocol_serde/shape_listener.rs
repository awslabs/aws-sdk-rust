            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "PortRanges" => {
                            builder = builder.set_port_ranges(crate::protocol_serde::shape_port_ranges::de_port_ranges(tokens, _value, depth + 1)?);
                        }
                        "Protocol" => {
                        ...
                        "ClientAffinity" => {
                            builder = builder.set_client_affinity(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::ClientAffinity::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
