            ...
            let mut tmp = {
                ...
                output = output.set_retry_after_seconds(
                    crate::protocol_serde::shape_internal_server_error_exception::de_retry_after_seconds_header(_response_headers).map_err(|_| {
                        crate::operation::list_queues::ListQueuesError::unhandled("Failed to parse retryAfterSeconds from header `Retry-After")
                    })?,
                );
                let output = output.meta(generic);
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "queues" => {
                    builder = builder.set_queues(crate::protocol_serde::shape_queue_summaries::de_queue_summaries(tokens, _value)?);
                }
