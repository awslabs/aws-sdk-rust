                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "invocationLogSource" => {
                            builder = builder.set_invocation_log_source(
                                crate::protocol_serde::shape_invocation_log_source::de_invocation_log_source(tokens, _value, depth + 1)?,
                            );
                        }
                        "requestMetadataFilters" => {
                            builder = builder.set_request_metadata_filters(
                                crate::protocol_serde::shape_request_metadata_filters::de_request_metadata_filters(tokens, _value, depth + 1)?,
                            );
                        }
