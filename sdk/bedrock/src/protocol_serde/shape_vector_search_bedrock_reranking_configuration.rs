                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "modelConfiguration" => {
                            builder = builder.set_model_configuration(
                                    crate::protocol_serde::shape_vector_search_bedrock_reranking_model_configuration::de_vector_search_bedrock_reranking_model_configuration(tokens, _value, depth + 1)?
                                );
                        }
