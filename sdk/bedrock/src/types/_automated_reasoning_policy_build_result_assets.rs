// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Contains the various assets generated during a policy build workflow, including logs, quality reports, test cases, and the final policy definition.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub enum AutomatedReasoningPolicyBuildResultAssets {
    /// <p>A manifest listing all available artifacts produced by the build workflow. This provides a catalog of all assets that can be retrieved, including their types, names, and identifiers.</p>
    AssetManifest(crate::types::AutomatedReasoningPolicyBuildResultAssetManifest),
    /// <p>The complete build log containing detailed information about each step in the policy generation process.</p>
    BuildLog(crate::types::AutomatedReasoningPolicyBuildLog),
    /// <p>A source document that was used as input during the build workflow. This allows you to retrieve the original documents that were processed to generate the policy.</p>
    Document(crate::types::AutomatedReasoningPolicySourceDocument),
    /// <p>A comprehensive fidelity report that measures how accurately the generated policy represents the source documents. The report includes coverage and accuracy scores, along with detailed grounding information for rules and variables.</p>
    FidelityReport(crate::types::AutomatedReasoningPolicyFidelityReport),
    /// <p>A comprehensive test suite generated by the build workflow, providing validation capabilities for automated reasoning policies.</p>
    GeneratedTestCases(crate::types::AutomatedReasoningPolicyGeneratedTestCases),
    /// <p>The complete policy definition generated by the build workflow, containing all rules, variables, and custom types extracted from the source documents.</p>
    PolicyDefinition(crate::types::AutomatedReasoningPolicyDefinition),
    /// <p>An entity encompassing all the policy scenarios generated by the build workflow, which can be used to validate an Automated Reasoning policy.</p>
    PolicyScenarios(crate::types::AutomatedReasoningPolicyScenarios),
    /// <p>A comprehensive report analyzing the quality of the generated policy, including metrics about rule coverage, potential conflicts, and unused elements.</p>
    QualityReport(crate::types::AutomatedReasoningPolicyDefinitionQualityReport),
    /// The `Unknown` variant represents cases where new union variant was received. Consider upgrading the SDK to the latest available version.
    /// An unknown enum variant
    ///
    /// _Note: If you encounter this error, consider upgrading your SDK to the latest version._
    /// The `Unknown` variant represents cases where the server sent a value that wasn't recognized
    /// by the client. This can happen when the server adds new functionality, but the client has not been updated.
    /// To investigate this, consider turning on debug logging to print the raw HTTP response.
    #[non_exhaustive]
    Unknown,
}
impl AutomatedReasoningPolicyBuildResultAssets {
    /// Tries to convert the enum instance into [`AssetManifest`](crate::types::AutomatedReasoningPolicyBuildResultAssets::AssetManifest), extracting the inner [`AutomatedReasoningPolicyBuildResultAssetManifest`](crate::types::AutomatedReasoningPolicyBuildResultAssetManifest).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_asset_manifest(&self) -> ::std::result::Result<&crate::types::AutomatedReasoningPolicyBuildResultAssetManifest, &Self> {
        if let AutomatedReasoningPolicyBuildResultAssets::AssetManifest(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`AssetManifest`](crate::types::AutomatedReasoningPolicyBuildResultAssets::AssetManifest).
    pub fn is_asset_manifest(&self) -> bool {
        self.as_asset_manifest().is_ok()
    }
    /// Tries to convert the enum instance into [`BuildLog`](crate::types::AutomatedReasoningPolicyBuildResultAssets::BuildLog), extracting the inner [`AutomatedReasoningPolicyBuildLog`](crate::types::AutomatedReasoningPolicyBuildLog).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_build_log(&self) -> ::std::result::Result<&crate::types::AutomatedReasoningPolicyBuildLog, &Self> {
        if let AutomatedReasoningPolicyBuildResultAssets::BuildLog(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`BuildLog`](crate::types::AutomatedReasoningPolicyBuildResultAssets::BuildLog).
    pub fn is_build_log(&self) -> bool {
        self.as_build_log().is_ok()
    }
    /// Tries to convert the enum instance into [`Document`](crate::types::AutomatedReasoningPolicyBuildResultAssets::Document), extracting the inner [`AutomatedReasoningPolicySourceDocument`](crate::types::AutomatedReasoningPolicySourceDocument).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_document(&self) -> ::std::result::Result<&crate::types::AutomatedReasoningPolicySourceDocument, &Self> {
        if let AutomatedReasoningPolicyBuildResultAssets::Document(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`Document`](crate::types::AutomatedReasoningPolicyBuildResultAssets::Document).
    pub fn is_document(&self) -> bool {
        self.as_document().is_ok()
    }
    /// Tries to convert the enum instance into [`FidelityReport`](crate::types::AutomatedReasoningPolicyBuildResultAssets::FidelityReport), extracting the inner [`AutomatedReasoningPolicyFidelityReport`](crate::types::AutomatedReasoningPolicyFidelityReport).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_fidelity_report(&self) -> ::std::result::Result<&crate::types::AutomatedReasoningPolicyFidelityReport, &Self> {
        if let AutomatedReasoningPolicyBuildResultAssets::FidelityReport(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`FidelityReport`](crate::types::AutomatedReasoningPolicyBuildResultAssets::FidelityReport).
    pub fn is_fidelity_report(&self) -> bool {
        self.as_fidelity_report().is_ok()
    }
    /// Tries to convert the enum instance into [`GeneratedTestCases`](crate::types::AutomatedReasoningPolicyBuildResultAssets::GeneratedTestCases), extracting the inner [`AutomatedReasoningPolicyGeneratedTestCases`](crate::types::AutomatedReasoningPolicyGeneratedTestCases).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_generated_test_cases(&self) -> ::std::result::Result<&crate::types::AutomatedReasoningPolicyGeneratedTestCases, &Self> {
        if let AutomatedReasoningPolicyBuildResultAssets::GeneratedTestCases(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`GeneratedTestCases`](crate::types::AutomatedReasoningPolicyBuildResultAssets::GeneratedTestCases).
    pub fn is_generated_test_cases(&self) -> bool {
        self.as_generated_test_cases().is_ok()
    }
    /// Tries to convert the enum instance into [`PolicyDefinition`](crate::types::AutomatedReasoningPolicyBuildResultAssets::PolicyDefinition), extracting the inner [`AutomatedReasoningPolicyDefinition`](crate::types::AutomatedReasoningPolicyDefinition).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_policy_definition(&self) -> ::std::result::Result<&crate::types::AutomatedReasoningPolicyDefinition, &Self> {
        if let AutomatedReasoningPolicyBuildResultAssets::PolicyDefinition(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`PolicyDefinition`](crate::types::AutomatedReasoningPolicyBuildResultAssets::PolicyDefinition).
    pub fn is_policy_definition(&self) -> bool {
        self.as_policy_definition().is_ok()
    }
    /// Tries to convert the enum instance into [`PolicyScenarios`](crate::types::AutomatedReasoningPolicyBuildResultAssets::PolicyScenarios), extracting the inner [`AutomatedReasoningPolicyScenarios`](crate::types::AutomatedReasoningPolicyScenarios).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_policy_scenarios(&self) -> ::std::result::Result<&crate::types::AutomatedReasoningPolicyScenarios, &Self> {
        if let AutomatedReasoningPolicyBuildResultAssets::PolicyScenarios(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`PolicyScenarios`](crate::types::AutomatedReasoningPolicyBuildResultAssets::PolicyScenarios).
    pub fn is_policy_scenarios(&self) -> bool {
        self.as_policy_scenarios().is_ok()
    }
    /// Tries to convert the enum instance into [`QualityReport`](crate::types::AutomatedReasoningPolicyBuildResultAssets::QualityReport), extracting the inner [`AutomatedReasoningPolicyDefinitionQualityReport`](crate::types::AutomatedReasoningPolicyDefinitionQualityReport).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_quality_report(&self) -> ::std::result::Result<&crate::types::AutomatedReasoningPolicyDefinitionQualityReport, &Self> {
        if let AutomatedReasoningPolicyBuildResultAssets::QualityReport(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`QualityReport`](crate::types::AutomatedReasoningPolicyBuildResultAssets::QualityReport).
    pub fn is_quality_report(&self) -> bool {
        self.as_quality_report().is_ok()
    }
    /// Returns true if the enum instance is the `Unknown` variant.
    pub fn is_unknown(&self) -> bool {
        matches!(self, Self::Unknown)
    }
}
