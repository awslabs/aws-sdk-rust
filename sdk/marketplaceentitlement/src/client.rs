    ...
    R: aws_smithy_client::retry::NewRequestPolicy,
{
    ...
    ///
    /// - Takes [`GetEntitlementsInput`](crate::input::GetEntitlementsInput) with field(s):
    ///   - [`product_code(Option<String>)`](crate::input::GetEntitlementsInput::product_code): <p>Product code is used to uniquely identify a product in AWS Marketplace. The product code will be provided by AWS Marketplace when the product listing is created.</p>
    ///   - [`filter(Option<HashMap<GetEntitlementFilterName, Vec<String>>>)`](crate::input::GetEntitlementsInput::filter): <p>Filter is used to return entitlements for a specific customer or for a specific dimension. Filters are described as keys mapped to a lists of values. Filtered requests are <i>unioned</i> for each value in the value list, and then <i>intersected</i> for each filter key.</p>
    ///   - [`next_token(Option<String>)`](crate::input::GetEntitlementsInput::next_token): <p>For paginated calls to GetEntitlements, pass the NextToken from the previous GetEntitlementsResult.</p>
    ///   - [`max_results(Option<i32>)`](crate::input::GetEntitlementsInput::max_results): <p>The maximum number of items to retrieve from the GetEntitlements operation. For pagination, use the NextToken field in subsequent calls to GetEntitlements.</p>
