            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "Start" => {
                            builder = builder.set_start(crate::protocol_serde::shape_hand_off_time::de_hand_off_time(tokens, _value, depth + 1)?);
                        }
                        "End" => {
                            builder = builder.set_end(crate::protocol_serde::shape_hand_off_time::de_hand_off_time(tokens, _value, depth + 1)?);
                        }
