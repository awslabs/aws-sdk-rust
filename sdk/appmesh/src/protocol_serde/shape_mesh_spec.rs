            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "egressFilter" => {
                            builder = builder.set_egress_filter(crate::protocol_serde::shape_egress_filter::de_egress_filter(tokens, _value)?);
                        }
                        "serviceDiscovery" => {
                            builder = builder.set_service_discovery(crate::protocol_serde::shape_mesh_service_discovery::de_mesh_service_discovery(
                                tokens, _value,
                            )?);
                        }
