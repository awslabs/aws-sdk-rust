            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "serviceDiscovery" => {
                            builder = builder.set_service_discovery(crate::protocol_serde::shape_service_discovery::de_service_discovery(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
                        ...
                        "listeners" => {
                            builder = builder.set_listeners(crate::protocol_serde::shape_listeners::de_listeners(tokens, _value, depth + 1)?);
                        }
                        "backends" => {
                            builder = builder.set_backends(crate::protocol_serde::shape_backends::de_backends(tokens, _value, depth + 1)?);
                        }
                        "backendDefaults" => {
                            builder = builder.set_backend_defaults(crate::protocol_serde::shape_backend_defaults::de_backend_defaults(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
