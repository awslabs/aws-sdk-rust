            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        ...
                        "interval" => {
                            builder = builder.set_interval(crate::protocol_serde::shape_duration::de_duration(tokens, _value, depth + 1)?);
                        }
                        "baseEjectionDuration" => {
                            builder =
                                builder.set_base_ejection_duration(crate::protocol_serde::shape_duration::de_duration(tokens, _value, depth + 1)?);
                        }
                        "maxEjectionPercent" => {
