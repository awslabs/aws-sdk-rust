    ...
    Err(match error_code {
        "AccessDeniedFault" => crate::error::DeleteConnectionError::AccessDeniedFault({
            ...
            let mut tmp = {
                ...
                let _ = response;
                output = crate::protocol_serde::shape_access_denied_fault::de_access_denied_fault_json_err(response.body().as_ref(), output).map_err(crate::error::DeleteConnectionError::unhandled)?;
        ...
        "InvalidResourceStateFault" => {
            crate::error::DeleteConnectionError::InvalidResourceStateFault({
                ...
                let mut tmp = {
                    ...
                    let _ = response;
                    output = crate::protocol_serde::shape_invalid_resource_state_fault::de_invalid_resource_state_fault_json_err(response.body().as_ref(), output).map_err(crate::error::DeleteConnectionError::unhandled)?;
                    let output = output.meta(generic);
                    output.build()
                };
                if tmp.message.is_none() {
        ...
        "ResourceNotFoundFault" => crate::error::DeleteConnectionError::ResourceNotFoundFault({
            ...
            let mut tmp = {
                ...
                let _ = response;
                output = crate::protocol_serde::shape_resource_not_found_fault::de_resource_not_found_fault_json_err(response.body().as_ref(), output).map_err(crate::error::DeleteConnectionError::unhandled)?;
                let output = output.meta(generic);
                output.build()
            };
            if tmp.message.is_none() {
