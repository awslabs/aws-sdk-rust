// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_create_instance_profile_input_input(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::operation::create_instance_profile::CreateInstanceProfileInput,
) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.availability_zone {
        object.key("AvailabilityZone").string(var_1.as_str());
    }
    if let Some(var_2) = &input.kms_key_arn {
        object.key("KmsKeyArn").string(var_2.as_str());
    }
    if let Some(var_3) = &input.publicly_accessible {
        object.key("PubliclyAccessible").boolean(*var_3);
    }
    if let Some(var_4) = &input.tags {
        let mut array_5 = object.key("Tags").start_array();
        for item_6 in var_4 {
            {
                #[allow(unused_mut)]
                let mut object_7 = array_5.value().start_object();
                crate::protocol_serde::shape_tag::ser_tag(&mut object_7, item_6)?;
                object_7.finish();
            }
        }
        array_5.finish();
    }
    if let Some(var_8) = &input.network_type {
        object.key("NetworkType").string(var_8.as_str());
    }
    if let Some(var_9) = &input.instance_profile_name {
        object.key("InstanceProfileName").string(var_9.as_str());
    }
    if let Some(var_10) = &input.description {
        object.key("Description").string(var_10.as_str());
    }
    if let Some(var_11) = &input.subnet_group_identifier {
        object.key("SubnetGroupIdentifier").string(var_11.as_str());
    }
    if let Some(var_12) = &input.vpc_security_groups {
        let mut array_13 = object.key("VpcSecurityGroups").start_array();
        for item_14 in var_12 {
            {
                array_13.value().string(item_14.as_str());
            }
        }
        array_13.finish();
    }
    Ok(())
}
