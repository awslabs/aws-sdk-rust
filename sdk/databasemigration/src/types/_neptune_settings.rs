...
impl  NeptuneSettings  {
    ...
    pub fn s3_bucket_folder(&self) -> & str {
        use std::ops::Deref; self.s3_bucket_folder.deref()
    }
    /// <p>The number of milliseconds for DMS to wait to retry a bulk-load of migrated graph data to the Neptune target database before raising an error. The default is 250.</p>
        ...
        ::std::result::Result::Ok(
            crate::types::NeptuneSettings {
                ...
                s3_bucket_name: self.s3_bucket_name
                    .ok_or_else(||
                        ::aws_smithy_types::error::operation::BuildError::missing_field("s3_bucket_name", "s3_bucket_name was not specified but it is required when building NeptuneSettings")
                    )?
                ,
                s3_bucket_folder: self.s3_bucket_folder
                    .ok_or_else(||
                        ::aws_smithy_types::error::operation::BuildError::missing_field("s3_bucket_folder", "s3_bucket_folder was not specified but it is required when building NeptuneSettings")
                    )?
                ,
