...
pub fn ser_route_waypoint(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::types::RouteWaypoint,
    ...
    if let Some(var_4) = &input.pass_through {
        object.key("PassThrough").boolean(*var_4);
    }
    {
        let mut array_5 = object.key("Position").start_array();
        for item_6 in &input.position {
            {
    ...
    if let Some(var_7) = &input.side_of_street {
        #[allow(unused_mut)]
        let mut object_8 = object.key("SideOfStreet").start_object();
        crate::protocol_serde::shape_route_side_of_street_options::ser_route_side_of_street_options(&mut object_8, var_7)?;
        object_8.finish();
    }
