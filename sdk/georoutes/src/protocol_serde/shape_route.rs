            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "Legs" => {
                            builder = builder.set_legs(crate::protocol_serde::shape_route_leg_list::de_route_leg_list(tokens)?);
                        }
                        "MajorRoadLabels" => {
                            builder = builder.set_major_road_labels(
                                crate::protocol_serde::shape_route_major_road_label_list::de_route_major_road_label_list(tokens)?,
                            );
                        }
                        "Summary" => {
                            builder = builder.set_summary(crate::protocol_serde::shape_route_summary::de_route_summary(tokens)?);
                        }
