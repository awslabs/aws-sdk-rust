...
pub fn ser_route_avoidance_area_geometry(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::types::RouteAvoidanceAreaGeometry,
) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.corridor {
        #[allow(unused_mut)]
        let mut object_2 = object.key("Corridor").start_object();
        crate::protocol_serde::shape_corridor::ser_corridor(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.bounding_box {
        let mut array_4 = object.key("BoundingBox").start_array();
        for item_5 in var_3 {
            {
