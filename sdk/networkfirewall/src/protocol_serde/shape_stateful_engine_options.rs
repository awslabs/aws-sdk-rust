            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "RuleOrder" => {
                            builder = builder.set_rule_order(
                                ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                    .map(|s| s.to_unescaped().map(|u| crate::types::RuleOrder::from(u.as_ref())))
                                    .transpose()?,
                            );
                        }
                        ...
                        "FlowTimeouts" => {
                            builder =
                                builder.set_flow_timeouts(crate::protocol_serde::shape_flow_timeouts::de_flow_timeouts(tokens, _value, depth + 1)?);
                        }
