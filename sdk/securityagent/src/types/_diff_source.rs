...
impl DiffSource {
    ...
    /// Tries to convert the enum instance into [`S3Uri`](crate::types::DiffSource::S3Uri), extracting the inner [`String`](::std::string::String).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_s3_uri(&self) -> ::std::result::Result<&::std::string::String, &Self> {
        if let DiffSource::S3Uri(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`S3Uri`](crate::types::DiffSource::S3Uri).
