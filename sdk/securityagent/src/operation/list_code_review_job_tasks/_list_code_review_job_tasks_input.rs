// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Input for listing tasks associated with a code review job.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct ListCodeReviewJobTasksInput {
    /// <p>The unique identifier of the agent space.</p>
    pub agent_space_id: ::std::option::Option<::std::string::String>,
    /// <p>The maximum number of results to return in a single call.</p>
    pub max_results: ::std::option::Option<i32>,
    /// <p>The unique identifier of the code review job to list tasks for.</p>
    pub code_review_job_id: ::std::option::Option<::std::string::String>,
    /// <p>Filter tasks by step name.</p>
    pub step_name: ::std::option::Option<crate::types::StepName>,
    /// <p>Filter tasks by category name.</p>
    pub category_name: ::std::option::Option<::std::string::String>,
    /// <p>A token to use for paginating results that are returned in the response. Set the value of this parameter to null for the first request. For subsequent calls, use the nextToken value returned from the previous request.</p>
    pub next_token: ::std::option::Option<::std::string::String>,
}
impl ListCodeReviewJobTasksInput {
    /// <p>The unique identifier of the agent space.</p>
    pub fn agent_space_id(&self) -> ::std::option::Option<&str> {
        self.agent_space_id.as_deref()
    }
    /// <p>The maximum number of results to return in a single call.</p>
    pub fn max_results(&self) -> ::std::option::Option<i32> {
        self.max_results
    }
    /// <p>The unique identifier of the code review job to list tasks for.</p>
    pub fn code_review_job_id(&self) -> ::std::option::Option<&str> {
        self.code_review_job_id.as_deref()
    }
    /// <p>Filter tasks by step name.</p>
    pub fn step_name(&self) -> ::std::option::Option<&crate::types::StepName> {
        self.step_name.as_ref()
    }
    /// <p>Filter tasks by category name.</p>
    pub fn category_name(&self) -> ::std::option::Option<&str> {
        self.category_name.as_deref()
    }
    /// <p>A token to use for paginating results that are returned in the response. Set the value of this parameter to null for the first request. For subsequent calls, use the nextToken value returned from the previous request.</p>
    pub fn next_token(&self) -> ::std::option::Option<&str> {
        self.next_token.as_deref()
    }
}
impl ListCodeReviewJobTasksInput {
    /// Creates a new builder-style object to manufacture [`ListCodeReviewJobTasksInput`](crate::operation::list_code_review_job_tasks::ListCodeReviewJobTasksInput).
    pub fn builder() -> crate::operation::list_code_review_job_tasks::builders::ListCodeReviewJobTasksInputBuilder {
        crate::operation::list_code_review_job_tasks::builders::ListCodeReviewJobTasksInputBuilder::default()
    }
}

/// A builder for [`ListCodeReviewJobTasksInput`](crate::operation::list_code_review_job_tasks::ListCodeReviewJobTasksInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct ListCodeReviewJobTasksInputBuilder {
    pub(crate) agent_space_id: ::std::option::Option<::std::string::String>,
    pub(crate) max_results: ::std::option::Option<i32>,
    pub(crate) code_review_job_id: ::std::option::Option<::std::string::String>,
    pub(crate) step_name: ::std::option::Option<crate::types::StepName>,
    pub(crate) category_name: ::std::option::Option<::std::string::String>,
    pub(crate) next_token: ::std::option::Option<::std::string::String>,
}
impl ListCodeReviewJobTasksInputBuilder {
    /// <p>The unique identifier of the agent space.</p>
    /// This field is required.
    pub fn agent_space_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.agent_space_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The unique identifier of the agent space.</p>
    pub fn set_agent_space_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.agent_space_id = input;
        self
    }
    /// <p>The unique identifier of the agent space.</p>
    pub fn get_agent_space_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.agent_space_id
    }
    /// <p>The maximum number of results to return in a single call.</p>
    pub fn max_results(mut self, input: i32) -> Self {
        self.max_results = ::std::option::Option::Some(input);
        self
    }
    /// <p>The maximum number of results to return in a single call.</p>
    pub fn set_max_results(mut self, input: ::std::option::Option<i32>) -> Self {
        self.max_results = input;
        self
    }
    /// <p>The maximum number of results to return in a single call.</p>
    pub fn get_max_results(&self) -> &::std::option::Option<i32> {
        &self.max_results
    }
    /// <p>The unique identifier of the code review job to list tasks for.</p>
    pub fn code_review_job_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.code_review_job_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The unique identifier of the code review job to list tasks for.</p>
    pub fn set_code_review_job_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.code_review_job_id = input;
        self
    }
    /// <p>The unique identifier of the code review job to list tasks for.</p>
    pub fn get_code_review_job_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.code_review_job_id
    }
    /// <p>Filter tasks by step name.</p>
    pub fn step_name(mut self, input: crate::types::StepName) -> Self {
        self.step_name = ::std::option::Option::Some(input);
        self
    }
    /// <p>Filter tasks by step name.</p>
    pub fn set_step_name(mut self, input: ::std::option::Option<crate::types::StepName>) -> Self {
        self.step_name = input;
        self
    }
    /// <p>Filter tasks by step name.</p>
    pub fn get_step_name(&self) -> &::std::option::Option<crate::types::StepName> {
        &self.step_name
    }
    /// <p>Filter tasks by category name.</p>
    pub fn category_name(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.category_name = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>Filter tasks by category name.</p>
    pub fn set_category_name(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.category_name = input;
        self
    }
    /// <p>Filter tasks by category name.</p>
    pub fn get_category_name(&self) -> &::std::option::Option<::std::string::String> {
        &self.category_name
    }
    /// <p>A token to use for paginating results that are returned in the response. Set the value of this parameter to null for the first request. For subsequent calls, use the nextToken value returned from the previous request.</p>
    pub fn next_token(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.next_token = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>A token to use for paginating results that are returned in the response. Set the value of this parameter to null for the first request. For subsequent calls, use the nextToken value returned from the previous request.</p>
    pub fn set_next_token(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.next_token = input;
        self
    }
    /// <p>A token to use for paginating results that are returned in the response. Set the value of this parameter to null for the first request. For subsequent calls, use the nextToken value returned from the previous request.</p>
    pub fn get_next_token(&self) -> &::std::option::Option<::std::string::String> {
        &self.next_token
    }
    /// Consumes the builder and constructs a [`ListCodeReviewJobTasksInput`](crate::operation::list_code_review_job_tasks::ListCodeReviewJobTasksInput).
    pub fn build(
        self,
    ) -> ::std::result::Result<
        crate::operation::list_code_review_job_tasks::ListCodeReviewJobTasksInput,
        ::aws_smithy_types::error::operation::BuildError,
    > {
        ::std::result::Result::Ok(crate::operation::list_code_review_job_tasks::ListCodeReviewJobTasksInput {
            agent_space_id: self.agent_space_id,
            max_results: self.max_results,
            code_review_job_id: self.code_review_job_id,
            step_name: self.step_name,
            category_name: self.category_name,
            next_token: self.next_token,
        })
    }
}
