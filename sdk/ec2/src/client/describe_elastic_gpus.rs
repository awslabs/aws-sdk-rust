// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
impl super::Client {
    /// Constructs a fluent builder for the [`DescribeElasticGpus`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder) operation.
    ///
    /// - The fluent builder is configurable:
    ///   - [`elastic_gpu_ids(Vec<String>)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::elastic_gpu_ids) / [`set_elastic_gpu_ids(Option<Vec<String>>)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::set_elastic_gpu_ids): <p>The Elastic Graphics accelerator IDs.</p>
    ///   - [`dry_run(bool)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::dry_run) / [`set_dry_run(Option<bool>)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::set_dry_run): <p>Checks whether you have the required permissions for the action, without actually making the request, and provides an error response. If you have the required permissions, the error response is <code>DryRunOperation</code>. Otherwise, it is <code>UnauthorizedOperation</code>.</p>
    ///   - [`filters(Vec<Filter>)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::filters) / [`set_filters(Option<Vec<Filter>>)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::set_filters): <p>The filters.</p>  <ul>   <li> <p> <code>availability-zone</code> - The Availability Zone in which the Elastic Graphics accelerator resides.</p> </li>   <li> <p> <code>elastic-gpu-health</code> - The status of the Elastic Graphics accelerator (<code>OK</code> | <code>IMPAIRED</code>).</p> </li>   <li> <p> <code>elastic-gpu-state</code> - The state of the Elastic Graphics accelerator (<code>ATTACHED</code>).</p> </li>   <li> <p> <code>elastic-gpu-type</code> - The type of Elastic Graphics accelerator; for example, <code>eg1.medium</code>.</p> </li>   <li> <p> <code>instance-id</code> - The ID of the instance to which the Elastic Graphics accelerator is associated.</p> </li>  </ul>
    ///   - [`max_results(i32)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::max_results) / [`set_max_results(Option<i32>)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::set_max_results): <p>The maximum number of results to return in a single call. To retrieve the remaining results, make another call with the returned <code>NextToken</code> value. This value can be between 5 and 1000.</p>
    ///   - [`next_token(impl ::std::convert::Into<String>)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::next_token) / [`set_next_token(Option<String>)`](crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::set_next_token): <p>The token to request the next page of results.</p>
    /// - On success, responds with [`DescribeElasticGpusOutput`](crate::operation::describe_elastic_gpus::DescribeElasticGpusOutput) with field(s):
    ///   - [`elastic_gpu_set(Option<Vec<ElasticGpus>>)`](crate::operation::describe_elastic_gpus::DescribeElasticGpusOutput::elastic_gpu_set): <p>Information about the Elastic Graphics accelerators.</p>
    ///   - [`max_results(Option<i32>)`](crate::operation::describe_elastic_gpus::DescribeElasticGpusOutput::max_results): <p>The total number of items to return. If the total number of items available is more than the value specified in max-items then a Next-Token will be provided in the output that you can use to resume pagination.</p>
    ///   - [`next_token(Option<String>)`](crate::operation::describe_elastic_gpus::DescribeElasticGpusOutput::next_token): <p>The token to use to retrieve the next page of results. This value is <code>null</code> when there are no more results to return.</p>
    /// - On failure, responds with [`SdkError<DescribeElasticGpusError>`](crate::operation::describe_elastic_gpus::DescribeElasticGpusError)
    pub fn describe_elastic_gpus(
        &self,
    ) -> crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder {
        crate::operation::describe_elastic_gpus::builders::DescribeElasticGpusFluentBuilder::new(
            self.handle.clone(),
        )
    }
}
