...
impl aws_smithy_http::response::ParseStrictResponse for DescribeVerifiedAccessTrustProviders {
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        ...
        if !response.status().is_success() && response.status().as_u16() != 200 {
            ...
            crate::operation_deser::parse_describe_verified_access_trust_providers_response(
                response,
            )
        }
    }
    ...
    for DisableAwsNetworkPerformanceMetricSubscription
{
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::operation_deser::parse_disable_aws_network_performance_metric_subscription_error(
                response,
            )
        } else {
    ...
    for EnableAwsNetworkPerformanceMetricSubscription
{
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::operation_deser::parse_enable_aws_network_performance_metric_subscription_error(
                response,
            )
        } else {
    ...
    for EnableReachabilityAnalyzerOrganizationSharing
{
    ...
    fn parse(&self, response: &http::Response<bytes::Bytes>) -> Self::Output {
        tracing::debug!(request_id = ?aws_http::request_id::RequestId::request_id(response));
        if !response.status().is_success() && response.status().as_u16() != 200 {
            crate::operation_deser::parse_enable_reachability_analyzer_organization_sharing_error(
                response,
            )
        } else {
            crate::operation_deser::parse_enable_reachability_analyzer_organization_sharing_response(
                response,
            )
        }
    }
