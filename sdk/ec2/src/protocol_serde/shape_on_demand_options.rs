...
pub fn de_on_demand_options(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::OnDemandOptions, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("capacityReservationOptions") /* CapacityReservationOptions com.amazonaws.ec2#OnDemandOptions$CapacityReservationOptions */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_capacity_reservation_options::de_capacity_reservation_options(&mut tag, depth + 1)
                        ?
                    )
                ;
