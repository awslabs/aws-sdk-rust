    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("routeSet") /* Routes com.amazonaws.ec2#VerifiedAccessInstanceOpenVpnClientConfiguration$Routes */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_verified_access_instance_open_vpn_client_configuration_route_list::de_verified_access_instance_open_vpn_client_configuration_route_list(&mut tag, depth + 1)
                        ?
                    )
                ;
