...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::create_reserved_instances_listing::CreateReservedInstancesListingError::unhandled)?;
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_create_reserved_instances_listing::de_create_reserved_instances_listing(_response_body, output)
            .map_err(crate::operation::create_reserved_instances_listing::CreateReservedInstancesListingError::unhandled)?;
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("reservedInstancesListingsSet") /* ReservedInstancesListings com.amazonaws.ec2.synthetic#CreateReservedInstancesListingOutput$ReservedInstancesListings */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_reserved_instances_listing_list::de_reserved_instances_listing_list(&mut tag, depth + 1)
                        ?
                    )
                ;
