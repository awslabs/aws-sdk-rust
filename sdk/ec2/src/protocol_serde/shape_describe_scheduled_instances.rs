...
pub fn de_describe_scheduled_instances(
    ...
    crate::operation::describe_scheduled_instances::builders::DescribeScheduledInstancesOutputBuilder,
...
> {
    ...
    if !(start_el.matches("DescribeScheduledInstancesResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeScheduledInstancesResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("scheduledInstanceSet") /* ScheduledInstanceSet com.amazonaws.ec2.synthetic#DescribeScheduledInstancesOutput$ScheduledInstanceSet */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_scheduled_instance_set::de_scheduled_instance_set(&mut tag, depth + 1)
                        ?
                    )
                ;
