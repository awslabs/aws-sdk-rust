...
> {
    ...
    if !(start_el.matches("DescribeVerifiedAccessGroupsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeVerifiedAccessGroupsResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("verifiedAccessGroupSet") /* VerifiedAccessGroups com.amazonaws.ec2.synthetic#DescribeVerifiedAccessGroupsOutput$VerifiedAccessGroups */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_verified_access_group_list::de_verified_access_group_list(&mut tag, depth + 1)
                        ?
                    )
                ;
