...
pub fn de_inference_device_info(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::InferenceDeviceInfo, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("memoryInfo") /* MemoryInfo com.amazonaws.ec2#InferenceDeviceInfo$MemoryInfo */ =>  {
                let var_4 =
                    Some(
                        crate::protocol_serde::shape_inference_device_memory_info::de_inference_device_memory_info(&mut tag, depth + 1)
                        ?
                    )
                ;
