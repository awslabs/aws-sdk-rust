...
pub fn de_available_capacity(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::AvailableCapacity, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("availableInstanceCapacity") /* AvailableInstanceCapacity com.amazonaws.ec2#AvailableCapacity$AvailableInstanceCapacity */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_available_instance_capacity_list::de_available_instance_capacity_list(&mut tag, depth + 1)
                        ?
                    )
                ;
