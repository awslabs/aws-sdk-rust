...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_customer_gateways::DescribeCustomerGatewaysError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    Err(crate::operation::describe_customer_gateways::DescribeCustomerGatewaysError::generic(
        generic,
    ))
}

#[allow(clippy::unnecessary_wraps)]
pub fn de_describe_customer_gateways_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_customer_gateways::de_describe_customer_gateways(_response_body, output)
            .map_err(crate::operation::describe_customer_gateways::DescribeCustomerGatewaysError::unhandled)?;
...
#[allow(unused_mut)]
pub fn de_describe_customer_gateways(
    inp: &[u8],
...
> {
    ...
    if !(start_el.matches("DescribeCustomerGatewaysResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeCustomerGatewaysResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("customerGatewaySet") /* CustomerGateways com.amazonaws.ec2.synthetic#DescribeCustomerGatewaysOutput$CustomerGateways */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_customer_gateway_list::de_customer_gateway_list(&mut tag, depth + 1)
                        ?
                    )
                ;
