// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn de_egress_only_internet_gateway(decoder: &mut aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::EgressOnlyInternetGateway, aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    let mut builder = crate::types::EgressOnlyInternetGateway::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("attachmentSet") /* Attachments com.amazonaws.ec2#EgressOnlyInternetGateway$Attachments */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_internet_gateway_attachment_list::de_internet_gateway_attachment_list(&mut tag)
                        ?
                    )
                ;
                builder = builder.set_attachments(var_1);
            }
            ,
            s if s.matches("egressOnlyInternetGatewayId") /* EgressOnlyInternetGatewayId com.amazonaws.ec2#EgressOnlyInternetGateway$EgressOnlyInternetGatewayId */ =>  {
                let var_2 =
                    Some(
                        Result::<std::string::String, aws_smithy_xml::decode::XmlDecodeError>::Ok(
                            aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            .into()
                        )
                        ?
                    )
                ;
                builder = builder.set_egress_only_internet_gateway_id(var_2);
            }
            ,
            s if s.matches("tagSet") /* Tags com.amazonaws.ec2#EgressOnlyInternetGateway$Tags */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_tag_list::de_tag_list(&mut tag)
                        ?
                    )
                ;
                builder = builder.set_tags(var_3);
            }
            ,
            _ => {}
        }
    }
    Ok(builder.build()
    )
}

