// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(unused_mut)]
pub fn ser_access_scope_path_request(mut writer: ::aws_smithy_query::QueryValueWriter, input: &crate::types::AccessScopePathRequest) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope_1 = writer.prefix("Source");
    if let Some(var_2) = &input.source {
        crate::protocol_serde::shape_path_statement_request::ser_path_statement_request(scope_1, var_2)?;
    }
    #[allow(unused_mut)]
    let mut scope_3 = writer.prefix("Destination");
    if let Some(var_4) = &input.destination {
        crate::protocol_serde::shape_path_statement_request::ser_path_statement_request(scope_3, var_4)?;
    }
    #[allow(unused_mut)]
    let mut scope_5 = writer.prefix("ThroughResource");
    if let Some(var_6) = &input.through_resources {
        let mut list_8 = scope_5.start_list(true, Some("item"));
        for item_7 in var_6 {
            #[allow(unused_mut)]
            let mut entry_9 = list_8.entry();
            crate::protocol_serde::shape_through_resources_statement_request::ser_through_resources_statement_request(entry_9, item_7)?;
        }
        list_8.finish();
    }
    Ok(())
}

