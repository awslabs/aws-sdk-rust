...
pub fn de_describe_trunk_interface_associations_http_error(
...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_trunk_interface_associations::DescribeTrunkInterfaceAssociationsError::unhandled)?;
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_describe_trunk_interface_associations_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_trunk_interface_associations::de_describe_trunk_interface_associations(_response_body, output)
            .map_err(crate::operation::describe_trunk_interface_associations::DescribeTrunkInterfaceAssociationsError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("DescribeTrunkInterfaceAssociationsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeTrunkInterfaceAssociationsResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("interfaceAssociationSet") /* InterfaceAssociations com.amazonaws.ec2.synthetic#DescribeTrunkInterfaceAssociationsOutput$InterfaceAssociations */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_trunk_interface_association_list::de_trunk_interface_association_list(&mut tag, depth + 1)
                        ?
                    )
                ;
