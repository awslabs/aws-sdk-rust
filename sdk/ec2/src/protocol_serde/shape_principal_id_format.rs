// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn de_principal_id_format(decoder: &mut aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::model::PrincipalIdFormat, aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    let mut builder = crate::model::PrincipalIdFormat::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("arn") /* Arn com.amazonaws.ec2#PrincipalIdFormat$Arn */ =>  {
                let var_1 =
                    Some(
                        Result::<std::string::String, aws_smithy_xml::decode::XmlDecodeError>::Ok(
                            aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            .into()
                        )
                        ?
                    )
                ;
                builder = builder.set_arn(var_1);
            }
            ,
            s if s.matches("statusSet") /* Statuses com.amazonaws.ec2#PrincipalIdFormat$Statuses */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_id_format_list::de_id_format_list(&mut tag)
                        ?
                    )
                ;
                builder = builder.set_statuses(var_2);
            }
            ,
            _ => {}
        }
    }
    Ok(builder.build()
    )
}

