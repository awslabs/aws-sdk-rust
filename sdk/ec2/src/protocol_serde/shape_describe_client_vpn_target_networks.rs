...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_client_vpn_target_networks::DescribeClientVpnTargetNetworksError::unhandled)?;
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_client_vpn_target_networks::de_describe_client_vpn_target_networks(_response_body, output)
            .map_err(crate::operation::describe_client_vpn_target_networks::DescribeClientVpnTargetNetworksError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("DescribeClientVpnTargetNetworksResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeClientVpnTargetNetworksResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("clientVpnTargetNetworks") /* ClientVpnTargetNetworks com.amazonaws.ec2.synthetic#DescribeClientVpnTargetNetworksOutput$ClientVpnTargetNetworks */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_target_network_set::de_target_network_set(&mut tag, depth + 1)
                        ?
                    )
                ;
