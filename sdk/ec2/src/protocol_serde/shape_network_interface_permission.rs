...
pub fn de_network_interface_permission(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::NetworkInterfacePermission, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("permissionState") /* PermissionState com.amazonaws.ec2#NetworkInterfacePermission$PermissionState */ =>  {
                let var_6 =
                    Some(
                        crate::protocol_serde::shape_network_interface_permission_state::de_network_interface_permission_state(&mut tag, depth + 1)
                        ?
                    )
                ;
