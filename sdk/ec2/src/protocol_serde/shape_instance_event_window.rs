...
pub fn de_instance_event_window(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::InstanceEventWindow, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("timeRangeSet") /* TimeRanges com.amazonaws.ec2#InstanceEventWindow$TimeRanges */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_instance_event_window_time_range_list::de_instance_event_window_time_range_list(&mut tag, depth + 1)
                        ?
                    )
                ;
            ...
            s if s.matches("associationTarget") /* AssociationTarget com.amazonaws.ec2#InstanceEventWindow$AssociationTarget */ =>  {
                let var_5 =
                    Some(
                        crate::protocol_serde::shape_instance_event_window_association_target::de_instance_event_window_association_target(&mut tag, depth + 1)
                        ?
                    )
                ;
