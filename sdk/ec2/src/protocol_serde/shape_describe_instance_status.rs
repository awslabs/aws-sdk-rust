...
> {
    ...
    if !(start_el.matches("DescribeInstanceStatusResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeInstanceStatusResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("instanceStatusSet") /* InstanceStatuses com.amazonaws.ec2.synthetic#DescribeInstanceStatusOutput$InstanceStatuses */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_instance_status_list::de_instance_status_list(&mut tag, depth + 1)
                        ?
                    )
                ;
