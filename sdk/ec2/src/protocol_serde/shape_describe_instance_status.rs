...
#[allow(clippy::unnecessary_wraps)]
pub fn de_describe_instance_status_http_response_with_props(
    _response_status: u16,
    _response_headers: &http::header::HeaderMap,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_describe_instance_status::de_describe_instance_status(
                _response_body,
                output,
            )
            .map_err(
                crate::operation::describe_instance_status::DescribeInstanceStatusError::unhandled,
            )?;
