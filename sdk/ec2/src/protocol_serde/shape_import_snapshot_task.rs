// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(clippy::needless_question_mark)]
pub fn de_import_snapshot_task(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
    depth: u32,
) -> ::std::result::Result<crate::types::ImportSnapshotTask, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom("maximum nesting depth exceeded"));
    }
    #[allow(unused_mut)]
    let mut builder = crate::types::ImportSnapshotTask::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("description") /* Description com.amazonaws.ec2#ImportSnapshotTask$Description */ =>  {
                let var_1 =
                    Some(
                        Result::<::std::string::String, ::aws_smithy_xml::decode::XmlDecodeError>::Ok(
                            ::aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            .into()
                        )
                        ?
                    )
                ;
                builder = builder.set_description(var_1);
            }
            ,
            s if s.matches("importTaskId") /* ImportTaskId com.amazonaws.ec2#ImportSnapshotTask$ImportTaskId */ =>  {
                let var_2 =
                    Some(
                        Result::<::std::string::String, ::aws_smithy_xml::decode::XmlDecodeError>::Ok(
                            ::aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            .into()
                        )
                        ?
                    )
                ;
                builder = builder.set_import_task_id(var_2);
            }
            ,
            s if s.matches("snapshotTaskDetail") /* SnapshotTaskDetail com.amazonaws.ec2#ImportSnapshotTask$SnapshotTaskDetail */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_snapshot_task_detail::de_snapshot_task_detail(&mut tag, depth + 1)
                        ?
                    )
                ;
                builder = builder.set_snapshot_task_detail(var_3);
            }
            ,
            s if s.matches("tagSet") /* Tags com.amazonaws.ec2#ImportSnapshotTask$Tags */ =>  {
                let var_4 =
                    Some(
                        crate::protocol_serde::shape_tag_list::de_tag_list(&mut tag, depth + 1)
                        ?
                    )
                ;
                builder = builder.set_tags(var_4);
            }
            ,
            _ => {}
        }
    }
    Ok(builder.build())
}
