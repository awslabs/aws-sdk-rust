...
pub fn de_create_local_gateway_route_table_virtual_interface_group_association_http_error(response: &http::Response<bytes::Bytes>) -> std::result::Result<crate::output::CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationOutput, crate::error::CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationError> {
    #[allow(unused_mut)]
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(response).map_err(crate::error::CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationError::unhandled)?;
