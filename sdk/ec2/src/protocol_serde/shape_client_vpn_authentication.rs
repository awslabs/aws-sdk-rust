    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("activeDirectory") /* ActiveDirectory com.amazonaws.ec2#ClientVpnAuthentication$ActiveDirectory */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_directory_service_authentication::de_directory_service_authentication(&mut tag, depth + 1)
                        ?
                    )
                ;
            ...
            s if s.matches("mutualAuthentication") /* MutualAuthentication com.amazonaws.ec2#ClientVpnAuthentication$MutualAuthentication */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_certificate_authentication::de_certificate_authentication(&mut tag, depth + 1)
                        ?
                    )
                ;
            ...
            s if s.matches("federatedAuthentication") /* FederatedAuthentication com.amazonaws.ec2#ClientVpnAuthentication$FederatedAuthentication */ =>  {
                let var_4 =
                    Some(
                        crate::protocol_serde::shape_federated_authentication::de_federated_authentication(&mut tag, depth + 1)
                        ?
                    )
                ;
