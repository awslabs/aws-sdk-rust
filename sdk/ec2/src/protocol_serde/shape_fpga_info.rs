...
pub fn de_fpga_info(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::FpgaInfo, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("fpgas") /* Fpgas com.amazonaws.ec2#FpgaInfo$Fpgas */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_fpga_device_info_list::de_fpga_device_info_list(&mut tag, depth + 1)
                        ?
                    )
                ;
