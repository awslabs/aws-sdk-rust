...
pub fn de_fpga_device_info(decoder: &mut aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::FpgaDeviceInfo, aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    let mut builder = crate::types::FpgaDeviceInfo::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("memoryInfo") /* MemoryInfo com.amazonaws.ec2#FpgaDeviceInfo$MemoryInfo */ =>  {
                let var_4 =
                    Some(
                        crate::protocol_serde::shape_fpga_device_memory_info::de_fpga_device_memory_info(&mut tag)
                        ?
                    )
                ;
