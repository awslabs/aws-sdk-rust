...
pub fn de_disable_fast_snapshot_restore_state_error_item(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::DisableFastSnapshotRestoreStateErrorItem, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("error") /* Error com.amazonaws.ec2#DisableFastSnapshotRestoreStateErrorItem$Error */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_disable_fast_snapshot_restore_state_error::de_disable_fast_snapshot_restore_state_error(&mut tag, depth + 1)
                        ?
                    )
                ;
