...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_network_interfaces::DescribeNetworkInterfacesError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    Err(crate::operation::describe_network_interfaces::DescribeNetworkInterfacesError::generic(
        generic,
    ))
}
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_network_interfaces::de_describe_network_interfaces(_response_body, output)
            .map_err(crate::operation::describe_network_interfaces::DescribeNetworkInterfacesError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("DescribeNetworkInterfacesResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeNetworkInterfacesResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("networkInterfaceSet") /* NetworkInterfaces com.amazonaws.ec2.synthetic#DescribeNetworkInterfacesOutput$NetworkInterfaces */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_network_interface_list::de_network_interface_list(&mut tag, depth + 1)
                        ?
                    )
                ;
