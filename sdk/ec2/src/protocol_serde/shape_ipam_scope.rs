...
pub fn de_ipam_scope(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::IpamScope, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("externalAuthorityConfiguration") /* ExternalAuthorityConfiguration com.amazonaws.ec2#IpamScope$ExternalAuthorityConfiguration */ =>  {
                let var_12 =
                    Some(
                        crate::protocol_serde::shape_ipam_scope_external_authority_configuration::de_ipam_scope_external_authority_configuration(&mut tag, depth + 1)
                        ?
                    )
                ;
