...
#[allow(unused_mut)]
pub fn de_describe_vpc_endpoints(
    inp: &[u8],
...
> {
    ...
    if !(start_el.matches("DescribeVpcEndpointsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeVpcEndpointsResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("vpcEndpointSet") /* VpcEndpoints com.amazonaws.ec2.synthetic#DescribeVpcEndpointsOutput$VpcEndpoints */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_vpc_endpoint_set::de_vpc_endpoint_set(&mut tag, depth + 1)
                        ?
                    )
                ;
