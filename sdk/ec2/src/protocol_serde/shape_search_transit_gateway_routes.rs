...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::search_transit_gateway_routes::SearchTransitGatewayRoutesError::unhandled)?;
    generic_builder = ::aws_types::request_id::apply_request_id(generic_builder, _response_headers);
    let generic = generic_builder.build();
    Err(crate::operation::search_transit_gateway_routes::SearchTransitGatewayRoutesError::generic(
        generic,
    ))
}
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_search_transit_gateway_routes::de_search_transit_gateway_routes(_response_body, output)
            .map_err(crate::operation::search_transit_gateway_routes::SearchTransitGatewayRoutesError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("SearchTransitGatewayRoutesResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected SearchTransitGatewayRoutesResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("routeSet") /* Routes com.amazonaws.ec2.synthetic#SearchTransitGatewayRoutesOutput$Routes */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_transit_gateway_route_list::de_transit_gateway_route_list(&mut tag, depth + 1)
                        ?
                    )
                ;
