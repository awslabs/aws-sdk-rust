...
pub fn de_describe_flow_logs(
...
) -> std::result::Result<crate::operation::describe_flow_logs::builders::DescribeFlowLogsOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError> {
    ...
    if !(start_el.matches("DescribeFlowLogsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeFlowLogsResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("flowLogSet") /* FlowLogs com.amazonaws.ec2.synthetic#DescribeFlowLogsOutput$FlowLogs */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_flow_log_set::de_flow_log_set(&mut tag, depth + 1)
                        ?
                    )
                ;
