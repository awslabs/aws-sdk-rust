...
> {
    ...
    if !(start_el.matches("DescribeClientVpnConnectionsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeClientVpnConnectionsResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("connections") /* Connections com.amazonaws.ec2.synthetic#DescribeClientVpnConnectionsOutput$Connections */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_client_vpn_connection_set::de_client_vpn_connection_set(&mut tag, depth + 1)
                        ?
                    )
                ;
