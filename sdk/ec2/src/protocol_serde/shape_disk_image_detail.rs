// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(unused_mut)]
pub fn ser_disk_image_detail(mut writer: ::aws_smithy_query::QueryValueWriter, input: &crate::types::DiskImageDetail) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope_1 = writer.prefix("Bytes");
    if let Some(var_2) = &input.bytes {
        scope_1.number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_2).into()));
    }
    #[allow(unused_mut)]
    let mut scope_3 = writer.prefix("Format");
    if let Some(var_4) = &input.format {
        scope_3.string(var_4.as_str());
    }
    #[allow(unused_mut)]
    let mut scope_5 = writer.prefix("ImportManifestUrl");
    if let Some(var_6) = &input.import_manifest_url {
        scope_5.string(var_6);
    }
    Ok(())
}

