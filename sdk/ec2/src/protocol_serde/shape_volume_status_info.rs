...
pub fn de_volume_status_info(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::VolumeStatusInfo, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("details") /* Details com.amazonaws.ec2#VolumeStatusInfo$Details */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_volume_status_details_list::de_volume_status_details_list(&mut tag, depth + 1)
                        ?
                    )
                ;
