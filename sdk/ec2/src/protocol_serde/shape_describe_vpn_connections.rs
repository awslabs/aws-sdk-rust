...
> {
    ...
    if !(start_el.matches("DescribeVpnConnectionsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeVpnConnectionsResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("vpnConnectionSet") /* VpnConnections com.amazonaws.ec2.synthetic#DescribeVpnConnectionsOutput$VpnConnections */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_vpn_connection_list::de_vpn_connection_list(&mut tag, depth + 1)
                        ?
                    )
                ;
