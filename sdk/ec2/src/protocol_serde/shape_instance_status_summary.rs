// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn de_instance_status_summary(decoder: &mut aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::model::InstanceStatusSummary, aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    let mut builder = crate::model::InstanceStatusSummary::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("details") /* Details com.amazonaws.ec2#InstanceStatusSummary$Details */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_instance_status_details_list::de_instance_status_details_list(&mut tag)
                        ?
                    )
                ;
                builder = builder.set_details(var_1);
            }
            ,
            s if s.matches("status") /* Status com.amazonaws.ec2#InstanceStatusSummary$Status */ =>  {
                let var_2 =
                    Some(
                        Result::<crate::model::SummaryStatus, aws_smithy_xml::decode::XmlDecodeError>::Ok(
                            crate::model::SummaryStatus::from(
                                aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            )
                        )
                        ?
                    )
                ;
                builder = builder.set_status(var_2);
            }
            ,
            _ => {}
        }
    }
    Ok(builder.build()
    )
}

