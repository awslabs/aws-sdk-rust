...
pub fn de_access_scope_path(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::AccessScopePath, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("source") /* Source com.amazonaws.ec2#AccessScopePath$Source */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_path_statement::de_path_statement(&mut tag, depth + 1)
                        ?
                    )
                ;
            ...
            s if s.matches("throughResourceSet") /* ThroughResources com.amazonaws.ec2#AccessScopePath$ThroughResources */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_through_resources_statement_list::de_through_resources_statement_list(&mut tag, depth + 1)
                        ?
                    )
                ;
