// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(clippy::needless_question_mark)]
pub fn de_history_record_entry(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
    depth: u32,
) -> ::std::result::Result<crate::types::HistoryRecordEntry, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom("maximum nesting depth exceeded"));
    }
    #[allow(unused_mut)]
    let mut builder = crate::types::HistoryRecordEntry::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("eventInformation") /* EventInformation com.amazonaws.ec2#HistoryRecordEntry$EventInformation */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_event_information::de_event_information(&mut tag, depth + 1)
                        ?
                    )
                ;
                builder = builder.set_event_information(var_1);
            }
            ,
            s if s.matches("eventType") /* EventType com.amazonaws.ec2#HistoryRecordEntry$EventType */ =>  {
                let var_2 =
                    Some(
                        Result::<crate::types::FleetEventType, ::aws_smithy_xml::decode::XmlDecodeError>::Ok(
                            crate::types::FleetEventType::from(
                                ::aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            )
                        )
                        ?
                    )
                ;
                builder = builder.set_event_type(var_2);
            }
            ,
            s if s.matches("timestamp") /* Timestamp com.amazonaws.ec2#HistoryRecordEntry$Timestamp */ =>  {
                let var_3 =
                    Some(
                        ::aws_smithy_types::DateTime::from_str(
                            ::aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                            , ::aws_smithy_types::date_time::Format::DateTimeWithOffset
                        )
                        .map_err(|_|::aws_smithy_xml::decode::XmlDecodeError::custom("expected (timestamp: `com.amazonaws.ec2#DateTime`)"))
                        ?
                    )
                ;
                builder = builder.set_timestamp(var_3);
            }
            ,
            _ => {}
        }
    }
    Ok(builder.build())
}
