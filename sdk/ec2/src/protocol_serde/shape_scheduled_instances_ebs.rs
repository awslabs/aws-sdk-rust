// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(unused_mut)]
pub fn ser_scheduled_instances_ebs(mut writer: ::aws_smithy_query::QueryValueWriter, input: &crate::types::ScheduledInstancesEbs) -> Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope_1 = writer.prefix("DeleteOnTermination");
    if let Some(var_2) = &input.delete_on_termination {
        scope_1.boolean(*var_2);
    }
    #[allow(unused_mut)]
    let mut scope_3 = writer.prefix("Encrypted");
    if let Some(var_4) = &input.encrypted {
        scope_3.boolean(*var_4);
    }
    #[allow(unused_mut)]
    let mut scope_5 = writer.prefix("Iops");
    if let Some(var_6) = &input.iops {
        scope_5.number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_6).into()));
    }
    #[allow(unused_mut)]
    let mut scope_7 = writer.prefix("SnapshotId");
    if let Some(var_8) = &input.snapshot_id {
        scope_7.string(var_8);
    }
    #[allow(unused_mut)]
    let mut scope_9 = writer.prefix("VolumeSize");
    if let Some(var_10) = &input.volume_size {
        scope_9.number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_10).into()));
    }
    #[allow(unused_mut)]
    let mut scope_11 = writer.prefix("VolumeType");
    if let Some(var_12) = &input.volume_type {
        scope_11.string(var_12);
    }
    Ok(())
}

