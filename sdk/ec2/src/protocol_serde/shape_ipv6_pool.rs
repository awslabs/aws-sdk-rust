...
pub fn de_ipv6_pool(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::Ipv6Pool, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("poolCidrBlockSet") /* PoolCidrBlocks com.amazonaws.ec2#Ipv6Pool$PoolCidrBlocks */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_pool_cidr_blocks_set::de_pool_cidr_blocks_set(&mut tag, depth + 1)
                        ?
                    )
                ;
