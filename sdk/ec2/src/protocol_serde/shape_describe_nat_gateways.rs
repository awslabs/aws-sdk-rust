...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_nat_gateways::de_describe_nat_gateways(_response_body, output)
            .map_err(crate::operation::describe_nat_gateways::DescribeNatGatewaysError::unhandled)?;
...
) -> std::result::Result<crate::operation::describe_nat_gateways::builders::DescribeNatGatewaysOutputBuilder, ::aws_smithy_xml::decode::XmlDecodeError>
{
    ...
    if !(start_el.matches("DescribeNatGatewaysResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeNatGatewaysResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("natGatewaySet") /* NatGateways com.amazonaws.ec2.synthetic#DescribeNatGatewaysOutput$NatGateways */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_nat_gateway_list::de_nat_gateway_list(&mut tag, depth + 1)
                        ?
                    )
                ;
