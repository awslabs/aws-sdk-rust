...
pub fn de_target_reservation_value(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
    depth: u32,
) -> ::std::result::Result<crate::types::TargetReservationValue, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("targetConfiguration") /* TargetConfiguration com.amazonaws.ec2#TargetReservationValue$TargetConfiguration */ =>  {
                let var_2 =
                    Some(
                        crate::protocol_serde::shape_target_configuration::de_target_configuration(&mut tag, depth + 1)
                        ?
                    )
                ;
