...
pub fn de_ena_srd_specification_request(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::EnaSrdSpecificationRequest, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("EnaSrdUdpSpecification") /* EnaSrdUdpSpecification com.amazonaws.ec2#EnaSrdSpecificationRequest$EnaSrdUdpSpecification */ =>  {
                let var_6 =
                    Some(
                        crate::protocol_serde::shape_ena_srd_udp_specification_request::de_ena_srd_udp_specification_request(&mut tag, depth + 1)
                        ?
                    )
                ;
