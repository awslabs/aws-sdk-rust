...
> {
    ...
    if !(start_el.matches("ListImagesInRecycleBinResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected ListImagesInRecycleBinResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("imageSet") /* Images com.amazonaws.ec2.synthetic#ListImagesInRecycleBinOutput$Images */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_image_recycle_bin_info_list::de_image_recycle_bin_info_list(&mut tag, depth + 1)
                        ?
                    )
                ;
