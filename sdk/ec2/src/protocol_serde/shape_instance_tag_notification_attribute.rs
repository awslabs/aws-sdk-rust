...
pub fn de_instance_tag_notification_attribute(
    ...
    depth: u32,
) -> ::std::result::Result<crate::types::InstanceTagNotificationAttribute, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("instanceTagKeySet") /* InstanceTagKeys com.amazonaws.ec2#InstanceTagNotificationAttribute$InstanceTagKeys */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_instance_tag_key_set::de_instance_tag_key_set(&mut tag, depth + 1)
                        ?
                    )
                ;
