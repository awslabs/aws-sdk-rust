    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("ebsOptimizedInfo") /* EbsOptimizedInfo com.amazonaws.ec2#EbsInfo$EbsOptimizedInfo */ =>  {
                let var_3 =
                    Some(
                        crate::protocol_serde::shape_ebs_optimized_info::de_ebs_optimized_info(&mut tag, depth + 1)
                        ?
                    )
                ;
            ...
            s if s.matches("ebsCardSet") /* EbsCards com.amazonaws.ec2#EbsInfo$EbsCards */ =>  {
                let var_8 =
                    Some(
                        crate::protocol_serde::shape_ebs_card_info_list::de_ebs_card_info_list(&mut tag, depth + 1)
                        ?
                    )
                ;
