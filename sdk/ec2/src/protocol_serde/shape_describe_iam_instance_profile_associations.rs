...
pub fn de_describe_iam_instance_profile_associations_http_error(
...
> {
    ...
    let mut generic_builder = crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body)
        .map_err(crate::operation::describe_iam_instance_profile_associations::DescribeIamInstanceProfileAssociationsError::unhandled)?;
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_describe_iam_instance_profile_associations_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_iam_instance_profile_associations::de_describe_iam_instance_profile_associations(
            _response_body,
            output,
        )
        .map_err(crate::operation::describe_iam_instance_profile_associations::DescribeIamInstanceProfileAssociationsError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("DescribeIamInstanceProfileAssociationsResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected DescribeIamInstanceProfileAssociationsResponse got {start_el:?}"
        )));
    }
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("iamInstanceProfileAssociationSet") /* IamInstanceProfileAssociations com.amazonaws.ec2.synthetic#DescribeIamInstanceProfileAssociationsOutput$IamInstanceProfileAssociations */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_iam_instance_profile_association_set::de_iam_instance_profile_association_set(&mut tag, depth + 1)
                        ?
                    )
                ;
