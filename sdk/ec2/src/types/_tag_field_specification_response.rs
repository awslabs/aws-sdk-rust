// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>A single resource's tag configuration associated with the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct TagFieldSpecificationResponse {
    /// <p>The resource type for the tag keys associated with the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub resource_type: ::std::option::Option<crate::types::TaggableResourceType>,
    /// <p>The tag keys on your tagged resources to be displayed by the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub tag_keys: ::std::option::Option<::std::vec::Vec<::std::string::String>>,
}
impl TagFieldSpecificationResponse {
    /// <p>The resource type for the tag keys associated with the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub fn resource_type(&self) -> ::std::option::Option<&crate::types::TaggableResourceType> {
        self.resource_type.as_ref()
    }
    /// <p>The tag keys on your tagged resources to be displayed by the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    ///
    /// If no value was sent for this field, a default will be set. If you want to determine if no value was sent, use `.tag_keys.is_none()`.
    pub fn tag_keys(&self) -> &[::std::string::String] {
        self.tag_keys.as_deref().unwrap_or_default()
    }
}
impl TagFieldSpecificationResponse {
    /// Creates a new builder-style object to manufacture [`TagFieldSpecificationResponse`](crate::types::TagFieldSpecificationResponse).
    pub fn builder() -> crate::types::builders::TagFieldSpecificationResponseBuilder {
        crate::types::builders::TagFieldSpecificationResponseBuilder::default()
    }
}

/// A builder for [`TagFieldSpecificationResponse`](crate::types::TagFieldSpecificationResponse).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct TagFieldSpecificationResponseBuilder {
    pub(crate) resource_type: ::std::option::Option<crate::types::TaggableResourceType>,
    pub(crate) tag_keys: ::std::option::Option<::std::vec::Vec<::std::string::String>>,
}
impl TagFieldSpecificationResponseBuilder {
    /// <p>The resource type for the tag keys associated with the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub fn resource_type(mut self, input: crate::types::TaggableResourceType) -> Self {
        self.resource_type = ::std::option::Option::Some(input);
        self
    }
    /// <p>The resource type for the tag keys associated with the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub fn set_resource_type(mut self, input: ::std::option::Option<crate::types::TaggableResourceType>) -> Self {
        self.resource_type = input;
        self
    }
    /// <p>The resource type for the tag keys associated with the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub fn get_resource_type(&self) -> &::std::option::Option<crate::types::TaggableResourceType> {
        &self.resource_type
    }
    /// Appends an item to `tag_keys`.
    ///
    /// To override the contents of this collection use [`set_tag_keys`](Self::set_tag_keys).
    ///
    /// <p>The tag keys on your tagged resources to be displayed by the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub fn tag_keys(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        let mut v = self.tag_keys.unwrap_or_default();
        v.push(input.into());
        self.tag_keys = ::std::option::Option::Some(v);
        self
    }
    /// <p>The tag keys on your tagged resources to be displayed by the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub fn set_tag_keys(mut self, input: ::std::option::Option<::std::vec::Vec<::std::string::String>>) -> Self {
        self.tag_keys = input;
        self
    }
    /// <p>The tag keys on your tagged resources to be displayed by the Flow Logs Amazon EC2 Tags feature fields in your custom log format.</p>
    pub fn get_tag_keys(&self) -> &::std::option::Option<::std::vec::Vec<::std::string::String>> {
        &self.tag_keys
    }
    /// Consumes the builder and constructs a [`TagFieldSpecificationResponse`](crate::types::TagFieldSpecificationResponse).
    pub fn build(self) -> crate::types::TagFieldSpecificationResponse {
        crate::types::TagFieldSpecificationResponse {
            resource_type: self.resource_type,
            tag_keys: self.tag_keys,
        }
    }
}
